...
contract Growith is ERC721Enum, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public freeQuantity = 420;

    bool public saleStatus = true;
    mapping(address => uint256) public addressMintedBalance;

    constructor(string memory valueBaseURI) ERC721S("Growith", "Growith") {
        setBaseURI(valueBaseURI);
    }
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant {
        ...
        require(saleStatus == true, "Sales are not enabled");
        require(_mintAmount > 0, "Can't mint 0");
        require(_mintAmount <= maxMintAmount, "Can't mint more then 5");
        require(nftCount + _mintAmount <= maxSupply, "Can't go over supply");
        require(msg.value >= cost * _mintAmount || nftCount + _mintAmount <= freeQuantity, "Incorrect amount");
        for (uint256 i = 0; i < _mintAmount; ++i) {
            _safeMint(msg.sender, nftCount + i, "");
        }
        delete nftCount;
    }
    ...
    function setFreeQuantity(uint256 _newFreeQuantity) public onlyOwner {
        freeQuantity = _newFreeQuantity;
    }
