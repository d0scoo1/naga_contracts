    ...
    ReentrancyGuardUpgradeable
{
    ...
    function releasePartialFunds(uint256 amount) external onlyOwner nonReentrant {
        if (amount > address(this).balance) {
            revert NotEnoughFunds();
        }
        AddressUpgradeable.sendValue(payable(ownerWallet), amount);
    }
    ...
    function devMint(address[] memory _to, uint256[] memory _quantity) public onlyOwner {
    ...
    // V7
    function setBuybackContract(address _address) external onlyOwner {
        buybackContract = _address;
    }

    function claimMonolithBuyback(address to) external nonReentrant {
        if (msg.sender != buybackContract) {
            revert OnlyBuybackContract();
        }
        _safeMint(to, 1);
    }
}
