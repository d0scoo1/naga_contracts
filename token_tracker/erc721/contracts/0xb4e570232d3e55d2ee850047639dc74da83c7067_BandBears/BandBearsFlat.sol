...
contract BandBears is ERC721Enumerable {
    /*//////////////////////////////////////////////////////////////
                             STATE VARIABLES
    //////////////////////////////////////////////////////////////*/
    ...
    constructor() ERC721("Band Bears", "BANDB") {
        /* Setup */
        owner = msg.sender;

        /* Start the fairsale from above the rebase tokens */
        fairSaleTokenID =
            (NUM_BONG_BEARS + NUM_BOND_BEARS + NUM_BOO_BEARS + NUM_BABY_BEARS) +
            1;
    }
    ...
    function mint() external payable {
        require(mintOpen > 0, "BandBears: Minting not enabled");
        require(msg.value >= MINT_PRICE, "BandBears: Not Enough ETH");
        require(fairSaleTokenID < MAX_SUPPLY, "BandBears: Sold Out");
        unchecked {
    ...
    function claim() external {
        ...
        require(claimOpen > 0, "BandBears: Claiming not enabled");
        unchecked {
            bondOwned = ERC721(BOND_BEARS).balanceOf(msg.sender);
            for (uint256 j = 0; j < bondOwned; j += 1) {
                tokenID = ERC721Enumerable(BOND_BEARS).tokenOfOwnerByIndex(
                    msg.sender,
                    j
                );
            ...
            for (uint256 j = 0; j < booOwned; j += 1) {
                tokenID = ERC721Enumerable(BOO_BEARS).tokenOfOwnerByIndex(
                    msg.sender,
                    j
                );
                ...
                if (!hasClaimedRebase[_hash]) {
                    ...
                    _mint(
                        msg.sender,
                        tokenID + NUM_BONG_BEARS + NUM_BOND_BEARS
                    );
                    ++sum;
                }
            }
            ...
            for (uint256 j = 0; j < babyOwned; j += 1) {
                tokenID = ERC721Enumerable(BABY_BEARS).tokenOfOwnerByIndex(
                    msg.sender,
                    j
                );
            ...
            require(
                bondOwned + booOwned + babyOwned != 0 && sum > 0,
                "Band Bears: None to claim"
            );
        }
    }
    ...
    function eligibleForClaim(address user)
        external
        view
        returns (uint256 sum)
    {
    ...
    function setBaseURI(string memory newBaseURI) external requiresTrust {
        baseURI = newBaseURI;
    }

    function airdrop(address[] memory addrs) external requiresTrust {
        unchecked {
            uint256 temp = airdropTokenIDs;
            for (uint256 i = 0; i < addrs.length; ++i) {
                _mint(addrs[i], temp++);
            }
            // Ensure all bears eventually get airdropped, but not over.
            require(temp <= NUM_BONG_BEARS + 1, "Reee");
            airdropTokenIDs = temp;
        }
    }
    ...
    function setOwner(address newOwner) external requiresTrust {
        owner = newOwner;
    }

    modifier requiresTrust() {
        require(msg.sender == owner, "BandBears: Auth");
        _;
    }
}
