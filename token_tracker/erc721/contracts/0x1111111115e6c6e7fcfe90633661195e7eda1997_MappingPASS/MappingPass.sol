...
contract MappingPASS is ERC721Royalty, ERC721Enumerable, Ownable {
    ...
    function getMapping(uint256 tokenId_) public view returns (address nft, uint256 tokenId) {
    ...
    function tokenURI(uint256 tokenId_) public view override(ERC721) returns (string memory) {
        ...
        if (m.nft != address(0)) {
            ...
            if (m.owner == NFT.ownerOf(tokenId_)) {
    ...
    function teamMint(address[] calldata recipients_, uint8[] calldata amounts_) external onlyOwner {
        uint16 sum;
