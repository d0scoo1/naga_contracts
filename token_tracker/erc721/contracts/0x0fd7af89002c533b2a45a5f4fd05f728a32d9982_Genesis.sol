...
contract Genesis is ERC721Enumerable, Ownable {
  ...
  uint256 public PRESALE_PRICE = 0.45 ether;
  uint256 public MAX_PER_MINT = 3;
  uint256 public MAX_PER_MINT_WHITELIST = 3;
  ...
  constructor(string memory pBaseTokenURI, address pWithdrawAddress, bytes32 pMerkleRoot) ERC721("Genesis", "GENESIS") {
      _tokenIds.increment();
      baseTokenURI = pBaseTokenURI;
      withdrawAddress = pWithdrawAddress;
  ...
  function _withdrawAddress() internal view virtual returns (address) {
  ...
  function mintNFTWhitelist(bytes32[] calldata merkleProof, uint256 qty) public payable {
    uint256 totalMinted = _tokenIds.current();

    require(preSale, "Pre-Sale is not active");
    require(qty > 0 && qty <= MAX_PER_MINT_WHITELIST, "Mint quantity is not correct. Insert a number between 1 and 20");
    require(whitelistClaims[msg.sender] + qty <= MAX_PER_MINT_WHITELIST, "You've already claimed your NFTs");
    ...
    require(msg.value >= PRESALE_PRICE * qty, "Not enought ether");

    for (uint256 i = 0; i < qty; i++) {
      whitelistClaims[msg.sender]++;
      _mintNFT(msg.sender);
    }
  }
  ...
  function mintNFTs(uint256 qty) public payable {
    ...
    require(msg.value >= PRICE.mul(qty), "Not enought ether");
  ...
  function partialWithdraw(uint256 quantity) public onlyOwner {
    uint256 balance = address(this).balance;
    require(balance > 0, "No ether left to withdraw");
    require(balance > quantity, "Not enought balance ");
