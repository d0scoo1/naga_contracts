...
/**
 * @title Gutter Punks Boards contract.
 * @author The Gutter Punks team.
 *
 */
contract GutterPunksBoards is ERC721A, Ownable, IERC721Receiver {
    using Strings for uint256;

    event SetSaleIsActive(bool saleIsActive);
    event SetBurnIsActive(bool burnIsActive);
    event Finalized();
    ...
    uint256 public _currentPrice = 0.100 ether;

    /// @notice Controls whether minting is allowed via the regular mint function.
    bool public _saleIsActive = true;
    
    /// @notice Controls whether minting is allowed via the burn to mint function.
    bool public _burnIsActive = true;
    ...
    string internal _baseTokenURI = "https://boards.gutterpunks.xyz/metadata/";
    string internal _contractURI = "https://boards.gutterpunks.xyz/GutterPunksBoards.json";
    ...
    constructor(
    ) ERC721A("Gutter Punks Boards", "GPB") {
    }
    ...
    function setBurnIsActive(bool burnIsActive) external onlyOwner {
        _burnIsActive = burnIsActive;
        emit SetBurnIsActive(burnIsActive);
    }
    ...
    function setGutterPunksTargetContract(address contractAddress) external onlyOwner notFinalized { 
    ...
    function airdrop(address[] calldata recipients) external notFinalized {
        require(msg.sender == _allowedAirdropper || msg.sender == owner(), "Not authorized.");
		require(totalSupply() + recipients.length <= MAX_SUPPLY, "Cannot airdrop more than max supply.");
		
        for (uint256 i = 0; i < recipients.length; i++) {
            _mint(recipients[i], 1, '', true);
        }
    }
    ...
    function onERC721Received(address operator, address from, 
        uint256 tokenId, bytes calldata data) external override returns (bytes4) {
        require(msg.sender == _gpAddress, "TOKEN NOT GUTTER PUNK");
        require(totalSupply() + 1 <= MAX_SUPPLY, "WOULD EXCEED MAX BOARD SUPPLY");
