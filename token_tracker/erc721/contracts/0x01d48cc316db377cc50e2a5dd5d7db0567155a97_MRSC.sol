...
/**
  ...
  February 8th, 2022.
*/
contract MRSC is ERC165, IERC721, IERC721Metadata, Ownable{
    ...
    uint256 public MAX_CAP = 4444;

    uint8 public MAX_MINT_PER_TX = 20;

    uint256 private ETH_MAX_SUPPLY = 1500;

    uint256 private INCUBATOR_MAX_SUPPLY = 2944;
    // 2503 Pairs from breeding
    // 100 for community giveaways
    // 341 to be sold with CHIZ
  ...
  function symbol() external override view returns (string memory symbol_ret){
    ...
    */
    ...
    function mintPublic(uint256 _mintAmount) public payable {
      ...
      require(eth_supply + _mintAmount <= ETH_MAX_SUPPLY, "Exceeds total eth supply");
    ...
    function mintReserved(
      uint256 _mintAmount
        ...
        uint256 supply = totalSupply();
        require(!mintingPaused, "Contract is paused");
        require(_mintAmount > 0, "Mint amount must be greater than 0");
        require(_mintAmount <= MAX_MINT_PER_TX, "Mint amount exceeds max per transaction");
        require(supply + _mintAmount <= MAX_CAP, "Mint amount exceeds max supply");
        ...
        require(WlAddressToCap[msg.sender] >= _mintAmount, "can't mint that many");

        //Reduce number of allocation
        WlAddressToCap[msg.sender] -= _mintAmount;
        cheapMint(msg.sender, _mintAmount);
        incubator_supply = incubator_supply + _mintAmount;
    }
    ...
    */
    function addToWhitelistArray(address[] memory _whitelisted, uint256[] memory _allowedMintCnt) public onlyOwner {
      require(_whitelisted.length == _allowedMintCnt.length, "MRSC 400 - 2 arrays shall have the same length");
      for (uint256 i=0; i<_whitelisted.length; i++)
      {
          WlAddressToCap[_whitelisted[i]] = _allowedMintCnt[i];
      }
    }
