...
contract Loonies is ERC721A, ERC721AQueryable, Ownable {
    ...
    constructor(string memory baseURI) ERC721A("Loonies", "LOONIES") {
        setBaseURI(baseURI);
        saleEnabled = false;
        whitelistEnabled = false;
        currentPrice = 0 ether;
        currentMaxSale = 10000;
    ...
    modifier saleIsOpen() {
        require(totalSupply() <= maxTokenSupply, "Loonies: Sale ended");
        _;
    }
    ...
    function mint(
        ...
        require(total <= maxTokenSupply, "Loonies: Max limit");

        if(total >= limitUntilOne) {
            _count = 1;
        }

        require(total + _count <= maxTokenSupply, "Loonies: Max limit");
        require(total + _count <= currentMaxSale, "Loonies: Max sale limit");
        require(mintCount[msg.sender] + _count <= maxMintPerWallet, "Loonies: Max wallet limit");
        require(_count <= maxMintPerTx, "Loonies: Max mint for tx limit");
        require(saleEnabled, "Loonies: Sale is not active");
        require(msg.value >= getPrice(_count), "Loonies: Value below price");

        if (whitelistEnabled == true) {
            require(merkleRoot != 0x0, "Loonies: merkle root not set");

            // Verify if the account has already claimed
            require(
                !isPresaleListClaimed(msg.sender),
                "Loonies: account already claimed"
            );

            // Verify we cannot claim more than the max amount
            require(
                _count <= _presaleMaxAmount,
                "Loonies: can only claim less than or equal to the max amount"
            );

            // Verify the merkle proof.
            require(
                validClaim(msg.sender, _presaleMaxAmount, _merkleProof),
                "Loonies: invalid proof"
            );
    ...
    function _mintElements(address _to, uint256 _amount, bool increaseCount) private {
        ...
        require(totalSupply() <= maxTokenSupply, "Loonies: Limit reached");
    }
    ...
    function _payout(address _address, uint256 _amount) private {
        ...
        require(success, "Loonies: Transfer failed");
    }
    ...
    function mintReserve(uint256 _count, address _to) public onlyOwner {
        uint256 total = totalSupply();
        require(total <= maxTokenSupply, "Loonies: Sale ended");
        require(total + _count <= maxTokenSupply, "Loonies: Max limit");
    ...
    function setLimitUntilOne(uint256 limitUntilOne_) external onlyOwner {
        limitUntilOne = limitUntilOne_;
    }
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        require(balance > 0, "Loonies: Balance should be above 0");
