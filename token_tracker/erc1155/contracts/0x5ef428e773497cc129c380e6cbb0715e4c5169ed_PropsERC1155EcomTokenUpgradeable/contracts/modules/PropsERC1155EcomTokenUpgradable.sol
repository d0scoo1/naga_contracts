    ...
    ERC1155BurnableUpgradeable
{
  ...
  //////////////////////////////////////////////////////////////*/
        ...
        external
    {
        ...
        //if 0 and  0 doesn't exist
        if(_token.tokenId == 0 && keccak256(abi.encodePacked(tokens[_token.tokenId].name)) == keccak256("")){
           tokens[_token.tokenId] = _token;
           tokenIndex++;
        }
        //if updating and a token does exist at the index
        else if(_token.tokenId > 0 && keccak256(abi.encodePacked(tokens[_token.tokenId].name)) != keccak256("")){
            tokens[_token.tokenId] = _token;
        }
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function togglePause(bool pause) external {
      require(hasMinRole(MINTER_ROLE));
      if(pause){
        _pause();
      }else{
        _unpause();
