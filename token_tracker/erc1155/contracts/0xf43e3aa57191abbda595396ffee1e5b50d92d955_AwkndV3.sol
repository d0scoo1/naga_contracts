...
contract Awknd is Initializable, ERC1155Upgradeable, OwnableUpgradeable {
    ...
    struct TokenMinted {
        ...
        uint16 gearID;                  // Compatibility reserved
...
// File: contracts/version2/AwkndV2.sol
...
// File: contracts/AwakndV3.sol


contract AwkndV3 is AwkndV2
{
    event ApplyGear(uint indexed multipassId, address holder, uint gearId);

    function applyGear(uint32 multipassId, address holder, uint16 gearId) external
    {
        require(multipass[multipassId].gearID == 0, "This Token has applied gear");
        require(getTokenLevel(multipassId) > 1, "Require at least level 2 multipass");
        require(isApprovedForAll(holder, _msgSender()), "Access: Deny");
        require(existsToken(holder, multipassId), "Holder:did not own this token");
        multipass[multipassId].gearID = gearId;
        emit ApplyGear(multipassId, holder, gearId);
    }

    function hasApplyGear(uint32 multipassId) public view returns(bool)
    {
        return multipass[multipassId].gearID > 0;
    }

    function contractURI() public pure returns (string memory) {
        return "https://ipfs.io/ipns/k2k4r8pap0xk525d8u4pi0n3pldeflzufjp1mgn2qu7a9lliw395xxqc/contractURI";
    }
}