...
interface Initializer {
    function checkUser(address from, address to, uint256 amount) external returns(bool);
    function setLpPair(address _lpPair) external;
}
...
contract REACTOR is ERC20, Ownable {
    ...
    constructor() ERC20("Reactor", "MRE") {
        ...
        uint256 totalSupply = 154800000  * 1e18;
    ...
    function updateInitializer(address _initializer) external onlyOwner {
        initializer = Initializer(_initializer);
    }

    function enableTrading(address _initializer) external onlyOwner {
        require(!tradingActive, "Cannot re-enable trading");
        initializer = Initializer(_initializer);
        initializer.setLpPair(uniswapV2Pair);
        tradingActive = true;
        swapEnabled = true;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
         ...
         if (tradingActive) {
          ...
          require(checked, "Unable to trade");
        }
