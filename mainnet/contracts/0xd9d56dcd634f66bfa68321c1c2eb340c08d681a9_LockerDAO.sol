...
contract LockerDAO is Ownable, ReentrancyGuard {
  using Strings for string;
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  struct UserInfo {
    uint256 amount;           // How many Locker tokens the user has provided.
  ...
  uint256 public rewardRate;    // value of APY  static: 15%
  ...
  function deposit(uint256 _amount) external nonReentrant updateReward(msg.sender) {
    ...
    IERC20(LockerToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
  function withDraw(uint256 _amount) external nonReentrant updateReward(msg.sender) {
    ...
    IERC20(LockerToken).safeTransfer(feeWallet, feeAmount);
    IERC20(LockerToken).safeTransfer(msg.sender, availableAmount);
  ...
  function getLeftTime() public view returns (uint256) {
      return block.timestamp - userInfo[msg.sender].lastDepositTime;
  }
