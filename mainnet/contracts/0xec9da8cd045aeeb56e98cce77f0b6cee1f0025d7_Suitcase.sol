...
contract Suitcase is ERC721Enumerable, AccessControl {
    ...
    constructor(uint256 _maxSupply, string memory _contractURI) ERC721("aStable Suitcase", "SUITCASE") {
        require(_maxSupply > 0, "Suitcase: max supply should be greater than zero");
        require(bytes(_contractURI).length > 0, "Suitcase: Cannot set empty contractURI");
    ...
    modifier onlyArtist() {
        require(hasRole(ARTIST_ROLE, msg.sender), "Suitcase: only artist");
        _;
    }
    ...
    function updateContractURI(string memory _contractURI) public onlyArtist {
        require(bytes(_contractURI).length > 0, "Suitcase: Cannot set empty contractURI");
        contractURI = _contractURI;
    }

    function updateTokenURI(uint256 _tokenId, string memory _tokenURI) public onlyArtist {
        require(_exists(_tokenId), "Suitcase: Update URI query for nonexistent token");
        require(bytes(_tokenURI).length > 0, "Suitcase: Cannot set empty tokenURI");
