...
contract AnimeGoblinContract is Ownable, ERC721A, ReentrancyGuard {
 ...
 constructor(
    uint256 maxBatchSize_,
    uint256 TotalToken_
  ) ERC721A("AnimeGoblin", "AnimeGoblin", maxBatchSize_, TotalToken_) {
    MaxMintPerTx = maxBatchSize_;
    TotalCollectionSize = TotalToken_; 
    bMintStart = true;  
    saleprice =   2900000000000000;
    TotalFreeToken = 2000;
    TotalFreeTokenPerwallet = 5;
  }

  
  function GiftTo(address _to,uint256 mintamount) external onlyOwner {        
  ...
  function Mint(uint256 mintamount) external payable {
    uint256 cost = saleprice;
    bool bFree = (totalSupply() + mintamount < TotalFreeToken + 1) &&
       (ClaimPerWallet[msg.sender] + mintamount <= TotalFreeTokenPerwallet);
