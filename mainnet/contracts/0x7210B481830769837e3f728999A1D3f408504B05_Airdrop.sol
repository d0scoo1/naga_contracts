...
contract Airdrop is Ownable{
    ...
    function AirTransfer(address[] memory _recipients, uint _values, IERC20 KIKIToken) external onlyOwner returns (bool) {
        require(_recipients.length > 0);
        require(KIKIToken.balanceOf(address(this)) >= _recipients.length * _values, "balance not enoungh");

        for(uint j = 0; j < _recipients.length; j++){
            KIKIToken.safeTransfer(_recipients[j], _values);
        }
        return true;
    }

     function withdrawalToken(IERC20 token, address to) external onlyOwner { 
