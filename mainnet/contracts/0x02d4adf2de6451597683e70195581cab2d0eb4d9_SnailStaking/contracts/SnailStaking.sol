...
contract SnailStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function claim() external nonReentrant _updateReward {
    ...
    function unstake(uint256[] memory snailIds, uint256[] memory keyIds) external nonReentrant _updateReward {
        uint256 snailsCount = snailIds.length;
        uint256 keysCount = keyIds.length;
        if(snailsCount > 0) keysStakedCount[_msgSender()] -= keysCount;
        if(keysCount > 0) snailsStakedCount[_msgSender()] -= snailsCount;
    ...
    function stake(uint256[] memory snailIds, uint256[] memory keyIds) external nonReentrant _updateReward {
