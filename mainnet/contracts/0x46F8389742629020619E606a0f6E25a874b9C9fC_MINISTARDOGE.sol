...
contract MINISTARDOGE is Context, IERC20, Ownable {
    ...
    string private _name = "MINI STAR DOGE";
    string private _symbol = "STARDOGE";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 100000000 * 10**_decimals; 

    uint256 public _ratioMK = 23;
    uint256 public _ratioP = 1;

    address payable public MarketingWallet = payable(0x76D93708f204fBd8cb5f8372f719409637a9F193);

    uint256 public _MarketingBuyTax = 5;
    uint256 public _LPBuyTax = 1;
    uint256 public _TotalBuyTax = 0;

    uint256 public _MarketingSellTax = 5;
    uint256 public _LPSellTax = 1;
    ...
    uint256 public burnamount = (10000000000000 * 10 ** _decimals);
    uint256 public newburnLP = burnamount;  
    ...
    constructor () {
        ...
        _TotalBuyTax = _LPBuyTax.add(_MarketingBuyTax);
        _TotalSellTax = _LPSellTax.add(_MarketingSellTax);
        _totalDistributionShares = _ratioP.add(_ratioMK);
    ...
    function swapAndLiquify(uint256 tAmount) private swapIsLocked {
        uint256 tokensForLP = tAmount.mul(_ratioP).div(_totalDistributionShares).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;
        uint256 totalETHFee = _totalDistributionShares.sub(_ratioP.div(2));
        
        uint256 amountETHLiquidity = amountReceived.mul(_ratioP).div(totalETHFee).div(2);
        uint256 amountETHTeam = amountReceived.mul(_ratioMK).div(totalETHFee);
   ...
   function updatesellfee(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
        _LPSellTax = newLiquidTax;
        _MarketingSellTax = newMarketTax;
        _TotalSellTax = _LPSellTax.add(_MarketingSellTax);
    }

    function updatebuyfee(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
        _LPBuyTax = newLiquidTax;
        _MarketingBuyTax = newMarketTax;
        _TotalBuyTax = _LPBuyTax.add(_MarketingBuyTax);
    }

    function updatemaxtrx(uint256 newbuylimit, uint256 newwalletlimit) external onlyOwner() {
