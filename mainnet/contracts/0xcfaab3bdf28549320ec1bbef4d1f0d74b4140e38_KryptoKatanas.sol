...
contract KryptoKatanas is ERC721A, Ownable, IBitCitizenCollection {
    ...
    constructor(string memory provenanceHash, string memory baseTokenURI, string memory unrevealedMetaUri) ERC721A("KryptoKatanas", "KRYPTOKATANAS") {
        // reserve 1/1s for promotions
        reserveToken(INIT_RESERVED);
        // Set data
    ...
    function mintToken(uint numberOfTokens) public payable {
        ...
        require((totalSupply() + numberOfTokens) <= MAX_TOKEN, "Number of tokens requested are not available");
        require((tokenPrice * numberOfTokens) <= msg.value, "Not enough ether value sent");
