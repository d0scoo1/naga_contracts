...
contract MoverOlympusNFT is Initializable, ContextUpgradeable, AccessControlEnumerableUpgradeable, ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable, ERC721PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using AddressUpgradeable for address payable;

    function initialize(string memory name, string memory symbol, string memory baseTokenURI, uint256 _claimStart, uint256 _claimEnd, address _feeAddress, uint256 _feeAmount) public virtual initializer {
    ...
    function setClaimInterval(uint256 _startTime, uint256 _endTime) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "admin only");
        claimStart = _startTime;
        claimEnd = _endTime;
    }

    function claimNFT() public payable {
        require(block.timestamp >= claimStart, "claim not started");
        require(block.timestamp < claimEnd, "claim time ended");
