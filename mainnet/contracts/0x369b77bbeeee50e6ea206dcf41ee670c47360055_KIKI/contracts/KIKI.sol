...
contract KIKI is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[to] == false && to != uniswapV2Pair) {
            uint256 maxHoldingPerWallet = _tTotal.mul(330).div(10000);
            uint256 expectedBalance = balanceOf(to).add(amount);
            require(
                expectedBalance <= maxHoldingPerWallet,
                "Cant hold more than 3.3%"
            );
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 fromLiquidityFee = contractTokenBalance.div(_liquidityFee).mul(
            liquidityFee
        );
