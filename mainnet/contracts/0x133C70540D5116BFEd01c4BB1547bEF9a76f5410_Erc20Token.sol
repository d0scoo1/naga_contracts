...
contract Erc20Token is Context, Initializable, IERC20, IERC20Metadata {
    ...
    function getVersion() external pure returns (uint256) { return 10; }
    ...
    function mint(address to, uint256 amount) onlyAdmin external {
        require(_totalSupply + amount <= _maxTotalSupply, "exceeded max total supply");
        _mint(to, amount);
    }
    
    function mintMax(address to) onlyAdmin external {
        require(_totalSupply < _maxTotalSupply, "max total supply reached");
        _mint(to, _maxTotalSupply - _totalSupply);
    }
    
    function burn(uint256 amount, bool fromContract) onlyAdmin external {
        if (fromContract) {
