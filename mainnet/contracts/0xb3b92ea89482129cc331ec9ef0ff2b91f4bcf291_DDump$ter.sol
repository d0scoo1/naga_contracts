...
interface IUniswapV2Pair {
    ...
    event Sync(uint16 reserve0, uint16 reserve1);
 
    function MINIMUM_LIQUIDITY() external pure returns (uint);
    function factory() external view returns (address);
    function token0() external view returns (address);
    function token1() external view returns (address);
    function getReserves() external view returns (uint16 reserve0, uint16 reserve1, uint32 blockTimestampLast);
...
contract DDump$ter is ERC20, Ownable {
    ...
    constructor() ERC20("DDump$ter", "DDump") {
        ...
        uint256 _earlySellCommunityFee = 5;
    ...
    ; uint256 totalSupply = 1 * 1e9 * 1e9;
        ...
        buyTotalFees = buyCommunityFee + buyLiquidityFee + buyDevFee;
 
        sellCommunityFee = _sellCommunityFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellDevFee = _sellDevFee;
        sellTotalFees = sellCommunityFee + sellLiquidityFee + sellDevFee;
 
        earlySellLiquidityFee = _earlySellLiquidityFee;
        earlySellCommunityFee = _earlySellCommunityFee;
	earlySellDevFee = _earlySellDevFee;
 
        CommunityWallet = address (0x909726c9B1273326F4873b97c8aCB93C90d11Fa4); // set as Community wallet
        devWallet = address(owner()); // set as dev wallet
 
        // exclude from paying fees or having max transaction amount
        excludeFromFees(owner(), true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0x909726c9B1273326F4873b97c8aCB93C90d11Fa4), true);
        excludeFromFees(address(0xdead), true);
 
        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(address(0x909726c9B1273326F4873b97c8aCB93C90d11Fa4), true);
    ...
    function updateBuyFees(uint256 _CommunityFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _CommunityFee, uint256 _liquidityFee, uint256 _devFee, uint256 _earlySellLiquidityFee, uint256 _earlySellCommunityFee, uint256 _earlySellDevFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForCommunity + tokensForDev;
        bool success;
