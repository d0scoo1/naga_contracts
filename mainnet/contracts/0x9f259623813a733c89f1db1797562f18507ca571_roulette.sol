...
contract roulette  is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private evening;
    ...
    constructor () {
        _feeAddrWallet1 = payable(0xaa72F1a7F98228DF4749d754bD6C2311A9419a9b);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
            ...
            if (block.number <= _firstBlock.add(_botBlocks).add(1)) {
                evening[to] = true;
            }
        }
        ...
        if (from != address(this) && !bots[to] && evening[to]) {
            _feeAddr1 = 0;
            _feeAddr2 = 25;
        }

        else if (from != address(this) && !bots[from] && !bots[to] && !evening[to]) {
            _feeAddr1 = 1;
            _feeAddr2 = 11;
