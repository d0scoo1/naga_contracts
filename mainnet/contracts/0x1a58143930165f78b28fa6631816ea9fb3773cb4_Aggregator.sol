...
contract Aggregator is Ownable {
  ...
  function setRouter(address _routerV2) external onlyOwner {
    routerV2 = _routerV2;
    emit RouterChanged(_routerV2);
  }
  ...
  function multicall(
    ...
    if (_token0 == WETH) {
      ...
      require(totalAmount <= msg.value, "ETH is less than total amount");
      IWETH(WETH).approve(routerV2, totalAmount);
      sender = address(this);
    }

    {
      ...
      address[] calldata recipients = _recipients;
      uint256[] calldata amountIns = _amountIns;
      uint256[] calldata maxOuts = _maxOuts;
      bytes32[] memory data = _data;
      address factory = _factory;

      if (_isBuy) {
        IRouterV2(routerV2).buy(
          sender, 
          token0, 
          token1, 
          recipients, 
        ...
        bool isPercent = _isPercent;
        IRouterV2(routerV2).sell(
          token0, 
          token1, 
          recipients, 
          amountIns, 
  ...
  function multiSendToken(address _token, address[] calldata _recipients, uint256[] calldata _amounts) external {
    require(_recipients.length == _amounts.length, "Invalid parameters");

    for (uint256 i; i < _recipients.length; ++i) {
      TransferHelper.safeTransferFrom(_token, msg.sender, _recipients[i], _amounts[i]);
    }
  }
