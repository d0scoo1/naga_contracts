...
contract Staking0 is Initializable, ReentrancyGuardUpgradeable {
    ...
    function deposit(address tokenAddress, address wallet, uint256 amount) public nonReentrant {
        ...
        // Scoping this so we don't run out of stack space <sigh>
        {
            ...
            balances[wallet][tokenAddress] = balances[wallet][tokenAddress].add(amount);
        ...
        if (checkpoints.length == 0) {
            ...
            if (checkpoints[last].epochId < currentEpoch) {
                ...
                checkpoints.push(Checkpoint(currentEpoch + 1, BASE_MULTIPLIER, balances[wallet][tokenAddress], 0));
            }
            ...
            // the last action happened in the current epoch
                ...
                checkpoints[last].startBalance = balances[wallet][tokenAddress];
            }
        }
    ...
    function withdraw(address tokenAddress, address wallet, uint256 amount) public nonReentrant {
        require(isApproved[msg.sender], "Caller must be an approved");
        require(balances[wallet][tokenAddress] >= amount, "Staking: balance too small");

        balances[wallet][tokenAddress] = balances[wallet][tokenAddress].sub(amount);
        ...
        if (checkpoints[last].epochId < currentEpoch) {
            checkpoints.push(Checkpoint(currentEpoch, BASE_MULTIPLIER, balances[wallet][tokenAddress], 0));
