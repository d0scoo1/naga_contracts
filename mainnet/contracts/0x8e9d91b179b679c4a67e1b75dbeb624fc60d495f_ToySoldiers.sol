...
// File: contracts/ToySoldiers.sol
    ...
    PaymentSplitter
{
    ...
    uint256 _price = 200000000000000000; //0.20 ETH
    ...
    address[] private _team = [
        0x2EDa1897E3A727fb117319baC640dc4a24F1F305,
        0x962082Bcef9402a9EA362cfb0363D9D09Fc1c295
    ];

    constructor()
        ERC721("Toy Soldiers", "TOYSOLDIERS")
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        require(
            ...
            "Toy Soldiers: max total supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Toy Soldiers: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function whitelistMint(uint256 _amount, bytes32[] memory proof) external payable {
        require(whitelistMintState, "Toy Soldiers: Whitelist Mint is OFF");
        require(!paused, "Toy Soldiers: contract is paused");
        require(_amount > 0, "Toy Soldiers: zero amount");
        require(verifyWhitelistMint(msg.sender, proof), "Toy Soldiers: You are not whitelisted");

        require(
            totalNFT + _amount <= maxSupply,
            "Toy Soldiers: max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "Toy Soldiers: Ether value sent is not correct"
        );
        uint256 _newItemId;
    ...
    function publicMint(uint256 _amount) external payable {
        require(publicState, "Toy Soldiers: Public is OFF");
        require(_amount > 0, "Toy Soldiers: zero amount");
        require(
            totalNFT + _amount <= maxSupply,
            "Toy Soldiers: max supply exceeded"
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Toy Soldiers: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Toy Soldiers: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
