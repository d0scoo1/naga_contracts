...
interface IdevsBalStrategy {
    ...
    function claimGaugeReward2(address _gauge) external;
...
contract StrategyGeneralBoostBase is StrategyBase {
    ...
    address public devsBalStrategy;
        ...
        )
    {
        ...
        gauge = _gauge;
        devsBalStrategy = _devsBalStrategy;
        treasury = _treasury;
    }
    
    function balanceOfPool() public override view returns (uint256) {
        uint256 amount = IBalGauge(gauge).balanceOf(devsBalStrategy);
        return amount;
    }

    function getHarvestable(address _token) external view returns (uint256) {
        uint256 _pendingReward = IBalGauge(gauge).claimable_reward(devsBalStrategy, _token);
    ...
    function deposit() public override {
        ...
        if (_want > 0) {
            IERC20(want).safeTransfer(devsBalStrategy, _want);
            IdevsBalStrategy(devsBalStrategy).deposit(gauge, want);
        }
    }
    ...
    function _withdrawSome(uint256 _amount)
        ...
        returns (uint256)
    {
        IdevsBalStrategy(devsBalStrategy).withdraw(gauge, want, _amount);
        return _amount;
    }
    ...
    function harvest() public override onlyBenevolent {
        IdevsBalStrategy(devsBalStrategy).claimGaugeReward2(gauge);
