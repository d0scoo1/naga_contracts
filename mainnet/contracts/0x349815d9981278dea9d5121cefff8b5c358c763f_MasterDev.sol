...
contract MasterDev is Ownable, ReentrancyGuard {
    ...
    modifier nonDuplicated(address _lpToken) {
        require(poolExistence[IBEP20(_lpToken)] == false, "nonDuplicated: duplicated");
        _;
    }

    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint256 _allocPoint, address _lpToken, uint16 _depositFeeBP,  IStrategy _strategy,  bool _withUpdate) public onlyOwner nonDuplicated(_lpToken) {
        ...
        poolExistence[IBEP20(_lpToken)] = true;
