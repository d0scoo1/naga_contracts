...
contract CurveGaugeIncentives is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function claimable(address _user, address _gauge, address _token) external view returns (uint _amount) {
        ...
        if (block.timestamp > _checkpointedPeriod) {
            /* 
             * return indiv/total * (future + current)
             * start by collecting total slopes at the end of period
             */

            uint _functionalPeriod = _checkpointedPeriod+WEEK;
            uint _totalWeight = IGaugeController(gaugeControllerAddress).points_weight(_gauge, _functionalPeriod).bias;
            uint _blacklistedWeight = _calculateBlacklistedWeight(_gauge, _functionalPeriod);
            ...
            if (_totalWeight > 0 && _individualSlope.end > 0) {
                uint _individualWeight = (_individualSlope.end - _functionalPeriod) * _individualSlope.slope;
    ...
    function _voterEarnedRewards(uint _pricePerPercent, uint _gaugeWeight) internal pure returns (uint) {
        ...
        // this would cause an overflow, so we return uint_max
        if (_pricePerPercent > max_uint/(10**18)) {
            return 2**256 - 1;
        }
    ...
    function _updatePeriod(address _gauge, address _token) internal returns (uint _currentPeriod) {
        ...
        if (_currentTime > _checkpointedPeriod) {
            // Checkpoint the gauge
            IGaugeController(gaugeControllerAddress).checkpoint_gauge(_gauge);
            
            // Get NEW current period after gauge is checkpointed
            _currentPeriod = IGaugeController(gaugeControllerAddress).time_total();
    ...
    function updateLastUserClaim(address _user, address _gauge, address _token, uint _time) public onlyOwner {
        last_user_claim[_user][_gauge][_token] = _time;
    }
    /* ========== END OWNER FUNCTIONS ========== */
