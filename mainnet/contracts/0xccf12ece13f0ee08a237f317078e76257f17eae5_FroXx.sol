...
contract FroXx is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function mint() public payable {
        ...
        require(msg.value >= 0.03 ether, "Not enough funds in account!");
    ...
    function buildImage(uint256 _tokenId) internal view returns(string memory) {
        ...
        string memory encodedData = Base64.encode(bytes(string(abi.encodePacked('{"name": "FroXx #',currNOC.tokenID,'", "description": "FroXx is randomly generated art. Created on mint execution. Each FroXx is stored directly on the Blockchain.", "attributes": ', jsonPacket,', "image": "data:image/svg+xml;base64,', Base64.encode(bytes(svgimage)), '"}'))));
