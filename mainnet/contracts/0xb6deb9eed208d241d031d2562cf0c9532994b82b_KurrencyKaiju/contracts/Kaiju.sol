...
contract KurrencyKaiju is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public price = 0.02 * 10 ** 18;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI2).length > 0
            ? string(abi.encodePacked(baseURI2, tokenId.toString())) : "" ;
    }
    ...
    function mintTokens(address[] calldata recipients) public payable {
        ...
        require(totalMints + 1 <= totalCount, "max supply reached!");
        require(owner() == msg.sender ,"You are not the owner");
        for(uint i=0; i<recipients.length; i++){
            _mint(recipients[i], 1 + totalMints++);
        }
  }
