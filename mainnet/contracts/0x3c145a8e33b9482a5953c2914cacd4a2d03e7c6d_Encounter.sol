...
contract Encounter is ERC721Enumerable, ReentrancyGuard, Ownable {
        ...
        uint256 private uniqueId = (random("WORLDEATERUNIQUEID")%6666)+1;
        bool public uniqueMinted = false;

        uint256 public price = 20000000000000000; //0.02 ETH
        uint256 public characterOrLootPrice = 10000000000000000; //0.01 ETH

        //Loot Contract
        address public lootAddress = 0xFF9C1b15B16263C61d017ee9F65C50e4AE0113D7;
        AdventurerInterface public lootContract = AdventurerInterface(lootAddress);
        
        //Characters Contract
        address public characterAddress = 0x7403AC30DE7309a0bF019cdA8EeC034a5507cbB3;
        AdventurerInterface public characterContract = AdventurerInterface(characterAddress);
    ...
    function mint() public payable nonReentrant {
        require(currentId < 6666, "No more tokens to mint");
    ...
    function multiMint(uint256 number) public payable nonReentrant {
        ...
        require(currentId + number < 6666, "Mint would exceed maximum token amount");
    ...
    function mintWithCharacterOrLoot() public payable nonReentrant {
        require(lootContract.balanceOf(msg.sender) > 0 || characterContract.balanceOf(msg.sender) > 0, "You need either a Loot or Character NFT for discounted minting");
        require(currentId < 6666, "No more tokens to mint");
        require(characterOrLootPrice <= msg.value, "Ether value sent is not correct");
        _safeMint(_msgSender(), currentId);
        if(currentId == uniqueId) uniqueMinted = true;
        currentId = currentId + 1;
    }
    ...
    function multiMintWithCharacterOrLoot(uint256 number) public payable nonReentrant {
        require(lootContract.balanceOf(msg.sender) > 0 || characterContract.balanceOf(msg.sender) > 0, "You need either a Loot or Character NFT for discounted minting");
        require((characterOrLootPrice * number) <= msg.value, "Ether value sent is not correct");
        require(currentId + number < 6666, "Mint would exceed maximum token amount");
        require(number < 20, "Cannot mint more than 20 tokens at once");
        for (uint256 i = 0; i < number; i++) {
            _safeMint(msg.sender, currentId);
    ...
    function ownerClaim(uint256 tokenId) public nonReentrant onlyOwner {
        require(tokenId > 6666 && tokenId < 7001, "Token ID invalid");
