 ...
 * Many changes were made to handle payments for art purchases.
 */
...
contract PaymentSplitter is Context {
    ...
    mapping(address => uint256) public artShares;
    ...
    function _release(address payable account) internal virtual {
        uint256 totalReceived = address(this).balance + totalReleased() - artBalance;
        uint256 payment = _pendingPayment(account, totalReceived, released(account));
        uint256 totalPayment = payment + artShares[account]; //payment plus art payment
 
        //in case calculation is wrong then give remaining balance
        if (totalPayment > address(this).balance) {
            totalPayment = address(this).balance;
        }

        if (totalPayment > 0) {
            _released[account] += payment;
            _totalReleased += payment;
            artBalance -= artShares[account];
            artShares[account] = 0;
