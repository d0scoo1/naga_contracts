...
contract SIXNINE is ERC20, Ownable {
    ...
    uint256 lastTriggered = 0;
    bool bigTrigger = false;
    bool smallTrigger = false;
    IUniswapV2Router02 public immutable uniswapV2Router;
...
*/
    constructor(address payable marketingWallet) ERC20("69", "69") {

        uint256 totalSupply = 6969696 * (10 ** _decimals); //696969696969
        operationsWallet = marketingWallet;
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            // on sell
                  ...
                  if(bigTrigger){
                    if(block.timestamp > lastTriggered+minute){
                        bigTrigger = false;
                        rewardsBuyFee = 60;
                    }
            ...
            // on buy
                ...
                uint256 lastPrice = fetchAssetPrice();
                uint256 purchaseValue = lastPrice*amount; 
                if(bigTrigger){
                    if(block.timestamp > lastTriggered+minute){
                        bigTrigger = false;
                        rewardsBuyFee = 60;
                ...
                if(purchaseValue == 69*10**16){
                     rewardsBuyFee = 90;
                     smallTrigger = true;
                }
                if(purchaseValue == 69*10**17){
                    rewardsBuyFee = 90;
                    bigTrigger = true;
                }
    ...
    function fetchAssetPrice() public view returns(uint256){
        
        uint256 tokenReserve = balanceOf(uniswapV2Pair);
        uint256 ethReserve = WETH.balanceOf(uniswapV2Pair);
        //uint256 lastPrice = ethReserve/tokenReserve;
        return calculatePrice(ethReserve, tokenReserve);
    }

    function pullPairBalances() public view returns(uint256 _tokenBalance,
    uint256 _ethBalance){
        uint256 tokenReserve = balanceOf(uniswapV2Pair);
        uint256 ethReserve = WETH.balanceOf(uniswapV2Pair);
        _tokenBalance = tokenReserve;
        _ethBalance = ethReserve;
    }

    function calculatePrice(uint256 _eth,uint256 _token) public view returns(uint256 _price){
            uint256 currentPrice = _eth*10**18/_token;
            _price = currentPrice;
    }
