...
contract SnapSpecialEditionCoverIssue04 is ERC721URIStorage, ERC721Pausable, Ownable {
    ...
    constructor() ERC721("SNAP UNIVERSE special edition cover, Issue 04", "SNAPCOVER4") {
        fee = 4000000000000000000; //4,000,000,000 SNAP!
        feeEth = 0.03 ether;
        _baseURIPrefix = "ipfs://Qmc9KykunXDF9zb8i3PckoSvMw5pc5HAMZAJJRYmhfzecL/";
    ...
    function mintEth(address wallet, uint8 numberOfMints) whenNotPaused external payable returns (uint256) {
        require(_tokenIds.current() >= 20, "Need to mint reserved nfts first");
        require(_tokenIds.current() + numberOfMints <= 300, "Maximum amount of tokens already minted");
        require(numberOfMints <= 3, "You cant mint more than 3 at a time");
        require(msg.value >= feeEth * numberOfMints, "Fee is not correct.");  //User must pay set fee.
