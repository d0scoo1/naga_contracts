...
// File: contracts/TOADZONTAPES.sol
 ...
 */
 ...
 contract TOADZONTAPES is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    // Special thanks to Pagzi Tech for their Pagzi protocol to reduce gas fees. pagzi.ca
    constructor(
        string memory name,
     ...
     function MintTapePublic(uint256[] memory TOADZ_IDs) external payable nonReentrant {
        ...
        require(TOADZ_IDs.length + totalSupply() <= 6969, "Purchase would exceed max supply of Tapes.");
        require(MintedWallets[msg.sender] + TOADZ_IDs.length <= 3, "Each wallet can only mint up to 3 Tapes.");
        for (uint256 j=0; j<TOADZ_IDs.length;j++){
            require(TOADZ_IDs[j] <= 6969 && TOADZ_IDs[j] > 0, "That Tape doesnt exist."); 
            require(!TapesMinted[TOADZ_IDs[j]], "The Tape for that Toad has been claimed");
        }    
