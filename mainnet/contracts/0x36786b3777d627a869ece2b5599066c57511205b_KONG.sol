...
Twitter: https://mobile.twitter.com/Kong_ETH_Token
*/
...
contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xFA996Ffb4909a2223C00505a9d73932734940909;
...
contract KONG  is Ownable, IERC20, IERC20Metadata {
    ...
    uint256 private _totalSupply=1_000_000_000_000  *(10**decimals());
    
    bool public isFeeActive=true;

    address marketingWallet=0x0437a27BE96165aBaf57053AAA7D47219992aF7b;
    
    
    uint256 _feeDecimal = 2; // do not change this value...
    uint256 marketingFee=800; //marketing fee 8%
    uint256 burnFee=100; // burn fee 1%
    uint256 liquidityFee = 100; // means 1% add liquidity
    
    
    uint256 public maxTxAmount = _totalSupply; // no limit
    uint256 public _maxWalletToken = 10_000_000_000 *(10**decimals()); // 1% of total supply
    ...
    uint256 public minTokensBeforeSwap = 100_000_000  *(10**decimals());
    
    uint256 public minTokensBeforeSwapForMarketing = 100_000  *(10**decimals());
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!_maxWallet[recipient] && recipient != address(this)  && recipient != address(0) && recipient != marketingWallet){
        ...
        uint256 marketingTokenBalance = balanceOf(marketingWallet);
        bool overMinTokenBalanceForMarketing = marketingTokenBalance >= minTokensBeforeSwapForMarketing;

        if (!inSwapAndLiquify && overMinTokenBalance && sender != uniswapV2Pair && swapAndLiquifyEnabled) {
            swapAndLiquify(contractTokenBalance);
        }else if(!inSwapAndLiquify && overMinTokenBalanceForMarketing && sender != uniswapV2Pair && swapAndLiquifyEnabled){
            swapMarketingEth(marketingTokenBalance);
        }
    ...
    function collectBuyFee(address account, uint256 amount) private returns (uint256) {
        ...
        if(marketingFee != 0){
          ...
          //@dev burn fee
        if(burnFee != 0){
            uint256 _burnFee = amount.mul(burnFee).div(10**(_feeDecimal + 2));
            transferAmount = transferAmount.sub(_burnFee);
            _burn(account,amount);
            emit Transfer(account,address(0),_burnFee);
        }
    ...
    function swapMarketingEth(uint256 _balance) private lockTheSwap {
           
        uint256 initialBalance = address(this).balance;

        // swap tokens for ETH
        swapTokensForEth(_balance); 
    ...
    function setFee(uint256 _makretingFee, uint256 _liquidityFee) external onlyOwner {
    ...
    function setMinTokensBeforeSwap(uint256 amount,uint256 _marketingAmount) external onlyOwner {
        minTokensBeforeSwapForMarketing = _marketingAmount;
