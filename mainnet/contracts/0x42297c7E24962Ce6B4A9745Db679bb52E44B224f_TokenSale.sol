...
contract TokenSale is Ownable {
    ...
    uint256 public REF_PERCENT = 1500;
    uint256 public FUND_PERCENT = 8000;
    ...
    constructor(address USDTAddr, address TOKENAddr, address initialOwner, address initialFeeWallet, address initialFundWallet, address defaultReferrer, uint256 _rateMul, uint256 _rateDiv) public Ownable(initialOwner) {
    ...
    function buyToken(uint256 usdtAmount, address referrer) public {
        ...
        require(tokens >= 1e6, "Minimum purchase is 1 token");
    ...
    function getReferralInfo(address account, uint256 from, uint256 to) public view returns(address[] memory referrals, uint256[] memory bonuses) {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            bonuses[i] = users[referrals[from + i]].totalBonuses;
        }
    }
    ...
    function setRate(uint256 newRateMul, uint256 newRateDiv) public onlyOwner {
        require(newRateMul >= 1 && newRateDiv >= 1);
    ...
    function setWallets(address _fee, address _fund, address _defaultRef) public onlyOwner {
        require(_fee != address(0) && _fund != address(0) && _defaultRef != address(0));
