...
contract ERC1155 is Context, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    function uri(uint256 id) external view override returns (string memory) {
        require(id < totalSupply, "NFT does not exist");
...
contract PumpkinHedzOGBadge is ERC1155Burnable, Ownable {
    ...
    function reserveBadge(address reciever, uint256[] calldata ids, uint256[] calldata amounts) external onlyOwner {
        require(!reserved,"Cannot reserve Badges multiple time");
    ...
    */
    function mintBadge() public {
        require(mintStarted, "OG Bagde minting not started");
        require(minted[msg.sender] != 1, "Only one OG Badge per address is allowed");
        require(ERC1155.totalSupply+1 <= maxSupply, "Mint will exceed max supply");

        minted[msg.sender] = 1;
        _mint(msg.sender, ERC1155.totalSupply, 1,"");
    }
}