...
contract goblinPoopNFT is ERC721A, Ownable, ReentrancyGuard {  
    using Strings for uint256;
	...
	constructor() ERC721A("goblinpoops", "GOPOOP") {}

    function _baseURI() internal view virtual override returns (string memory) {
        return _partslink;
    }

 	function makingobblinpoop() external nonReentrant {
 	...
 	function makegoblinnnpoopfly(address lords, uint256 _goblins) public onlyOwner {
  	    uint256 totalgobnlinsss = totalSupply();
	    require(totalgobnlinsss + _goblins <= goblins);
        _safeMint(lords, _goblins);
    }

    function makegoblnpoopgobyebye(bool _bye) external onlyOwner {
        byebye = _bye;
    }

    function spredgobblinspoop(uint256 _byebye) external onlyOwner {
        goblinbyebye = _byebye;
    }

    function makegobblinpoophaveparts(string memory parts) external onlyOwner {
        _partslink = parts;
    }
