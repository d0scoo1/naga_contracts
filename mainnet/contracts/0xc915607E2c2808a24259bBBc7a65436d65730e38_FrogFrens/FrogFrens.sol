...
contract FrogFrens is ERC721URIStorage, Ownable{

    event mintfrogs (address indexed minter, uint256 startWith, uint256 times);
    ...
    constructor() ERC721("FrogFrens", "FF") {
    ...
    function mintFrog(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalfrogs + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit mintfrogs(_msgSender(), totalfrogs+1, _times);
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalfrogs + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit mintfrogs(_addr, totalfrogs+1, 1);
        _mint(_addr, 1 + totalfrogs++);
    }
