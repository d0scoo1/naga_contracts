...
// File: @openzeppelin/contracts@4.3.2/token/ERC721/extensions/ERC721Burnable.sol
...
// File: contracts/CryptoLabs v4.sol
...
/// @custom:security-contact marktwainsvapepen@gmail.com
contract CryptoLabs is ERC721, ERC721Enumerable, ReentrancyGuard, Pausable, Ownable, ERC721Burnable {
    using Strings for uint256;
     
    uint public mintPrice = 0.03 ether;
    uint public MAX_SUPPLY = 9100;
    uint public MAX_PER_TRANS = 100;
    ...
    function safeMint(uint256 amount) public payable nonReentrant {
    require(_isSaleActive, "Sale is currently not active");
    uint256 supply = totalSupply();
    require(supply + amount < MAX_SUPPLY+1, "Not enough thicc bois left :(");
    require(amount > 0 && amount < MAX_PER_TRANS + 1, "Look- we ALL want these thicc bois, but the max per transaction is 10");
    
    if (msg.sender != owner()) {
    require(msg.value >= mintPrice * amount, "Amount of ether sent not correct.");
    }
  ...
  function setmaxMAX_PER_TRANS(uint256 _newMAX_PER_TRANS) public onlyOwner {
    MAX_PER_TRANS = _newMAX_PER_TRANS;
  }
  ...
  function setBaseURI(string memory _newBaseURI) public onlyOwner {
    ...
    function toggleSaleActivation() external onlyOwner {
