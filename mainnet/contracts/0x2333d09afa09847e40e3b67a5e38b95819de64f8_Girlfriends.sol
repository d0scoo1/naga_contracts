...
contract Girlfriends is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string[] private interests = [
        ...
        "Community gardening",
        "Skateboarding",
        "Shilling NFTs on twitter",
        "Watching The Office",
        "Climbing",
        "Her pet turtle",
        "Star Signs & Tarot Cards",
        "Cosplaying",
        "Going to festivals",
        "Playing Runescape",
        "Skiing",
    ...
    string[] private sex = [
        ...
        "Sees partners as equals",
        "Will invite her friend",
        "Likes to be spanked",
    ...
    string[] private love = [
        "Loves you",
        "Pretends she doesn't love you (loves you)",
        "Loves your brother",
        "Loves the Earth",
        "Loves herself",
        "Will love you one day",
        "Is scared to love you",
        "Loves you",
        "Loves you",
        "Loves you"
    ];
    ...
    function getVibe(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "VIBE", vibe);
    }

    function getInterests(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "INTERESTS1", interests);
    }
    
    function getInterests2(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "INTERESTS2", interests);
    }
    ...
    function getLove(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "LOVE", love);
    }
    ...
    function pluck(
       ...
       // no greatness traits. all Girlfriends are great. 
        return output;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        parts[5] = getVibe(tokenId);

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = getInterests(tokenId);

        parts[8] = '</text><text x="10" y="100" class="base">';

        parts[9] = getInterests2(tokenId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Girlfriends #',
                        toString(tokenId),
                        '", "description": "Girlfriends are randomized generated and stored on chain. Images and other functionality are intentionally omitted for others to interpret. Feel free to use Girlfriends in any way you want. No, not like that, what is wrong with you. Inspired and compatible with Loot (for Adventurers)", "image": "data:image/svg+xml;base64,',
    ...
    constructor() ERC721("Girlfriends", "GFs") Ownable() {}
}
