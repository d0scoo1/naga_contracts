...
///
/// This contract is copied from Alchemix https://github.com/alchemix-finance/alchemix-protocol
contract StakingPools is ReentrancyGuard {
  ...
  /// @dev like _stakes but for vesting based staking
  mapping(address => mapping(uint256 => Stake.Data)) private _vestingStakes;
  ...
  modifier onlyVesting() {
    require(msg.sender == vesting, "StakingPools: only vesting");
    _;
  }
  ...
  /// @dev used to set the vesting contract address
  ...
  /// @dev like deposit but used to enbale staking of vested tokens
  /// @notice the vesting account must not be used for staking outside of the vesting contract saking
    ...
    returns (bool)
  {
    ...
    Stake.Data storage _stake = _vestingStakes[_account][_poolId];
    ...
    // we need to transfer the tokens from the vesting address to this contract
    _pool.token.safeTransferFrom(msg.sender, address(this), _depositAmount);

    emit TokensDeposited(_account, _poolId, _depositAmount);
    return true;
  }

  /// @dev allows the vesting contract to eithr withdraw, claim, or exit staking
  /// @dev if you want to claim provide `false` for both `_doWithdraw` and `_doExit`
  /// @dev if you want to completely exit provide `true` for both `_doWithdraw` and `_doExit`
  /// @dev for exiting you may supply a value of `0` for _withdrawAmount
  function withdrawOrClaimOrExitVesting(
    address _account,
    uint256 _poolId,
  ...
  // claim
    ...
    // make sure to mint to the vesting contract
    reward.mint(msg.sender, _claimAmount);

    emit TokensClaimed(_account, _poolId, _claimAmount);

    if (_doWithdraw) {
      // if exit withdraw all staked amount
      if (_doExit) {
        _withdrawAmount = _stake.totalDeposited;
      }
      // if the amount they are attempting to withdraw is more
      // than the amount they have staked, just withdraw total deposited
      if (_withdrawAmount > _stake.totalDeposited) {
        _withdrawAmount = _stake.totalDeposited;
      }
      ...
      // make sure to transfer to the vesting contract
      _pool.token.safeTransfer(msg.sender, _withdrawAmount);
      emit TokensWithdrawn(_account, _poolId, _withdrawAmount);
    }
    return (true, _claimAmount);
  }
  ...
  function deposit(uint256 _poolId, uint256 _depositAmount) external nonReentrant {
    ...
    _deposit(msg.sender, _poolId, _depositAmount);
  }
  ...
  function withdraw(uint256 _poolId, uint256 _withdrawAmount) external nonReentrant {
    ...
    _withdraw(msg.sender, _poolId, _withdrawAmount);
  }
  ...
  function exit(uint256 _poolId) external nonReentrant {
    ...
    _claim(msg.sender, _poolId);
    _withdraw(msg.sender,_poolId, _stake.totalDeposited);
  }
  ...
  function getStakeTotalDeposited(address _account, uint256 _poolId, bool _vesting) external view returns (uint256) {
    if (_vesting) {
      Stake.Data storage _stake = _vestingStakes[_account][_poolId];
  ...
  function getStakeTotalUnclaimed(address _account, uint256 _poolId, bool _vesting) external view returns (uint256) {
    if (_vesting) {
      Stake.Data storage _stake = _vestingStakes[_account][_poolId];
  ...
  /// @param _poolId        the pool to deposit tokens into.
  /// @param _depositAmount the amount of tokens to deposit.
  function _deposit(address _account, uint256 _poolId, uint256 _depositAmount) internal {
    Pool.Data storage _pool = _pools.get(_poolId);
    Stake.Data storage _stake = _stakes[_account][_poolId];
    ...
    _pool.token.safeTransferFrom(_account, address(this), _depositAmount);
  ...
  /// @param _poolId          The pool to withdraw staked tokens from.
  /// @param _withdrawAmount  The number of tokens to withdraw.
  function _withdraw(address _account, uint256 _poolId, uint256 _withdrawAmount) internal {
    Pool.Data storage _pool = _pools.get(_poolId);
    Stake.Data storage _stake = _stakes[_account][_poolId];
    ...
    _pool.token.safeTransfer(_account, _withdrawAmount);
  ...
  ///
  /// @notice use this function to claim the tokens from a corresponding pool by ID.
  function _claim(address _account, uint256 _poolId) internal {
    Stake.Data storage _stake = _stakes[_account][_poolId];

    uint256 _claimAmount = _stake.totalUnclaimed;
    _stake.totalUnclaimed = 0;

    reward.mint(_account, _claimAmount);
