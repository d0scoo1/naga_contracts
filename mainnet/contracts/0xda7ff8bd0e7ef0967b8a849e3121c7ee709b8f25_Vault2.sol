...
// File contracts/vault/IYieldAggregator.sol
...
// File contracts/yield/IEthAnchorConversionPool.sol
...
// File contracts/vault/VaultV2.sol
...
contract Vault2 is Vault {
    ...
    /**
     * @notice used to deposit USDC to a destination address using funds from smart contract
     * @param _account accout receiving the transference of USDC
     * @param _amount amount of USDC to be sent to _account
     */
        ...
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        require(
            _account != address(0),
            "Vault: Receiveing address cannot be 0."
        );

        require(
            usdcToken.balanceOf(address(this)) >= _amount,
            "Vault: There is not enough funds to transfer"
        );

        usdcToken.transfer(_account, _amount);
    }
