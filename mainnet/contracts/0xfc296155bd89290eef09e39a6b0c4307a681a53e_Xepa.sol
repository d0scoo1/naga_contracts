 ...
 */
 ...
 contract Xepa is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ERC721URIStorage, Ownable {
      ...
      * See {ERC721-tokenURI}.
      */
     constructor() ERC721("Xepa Gallery", "XEPA") {
      ...
      */
     function mint_by_id(address to, uint256 tokenId) internal virtual {
         require(hasRole(MINTER_ROLE, _msgSender()), "Xepa/minter-role-required");
         _mint(to, tokenId);
     }      
     /**
      * @dev Creates a new token for `to` with a `tokenId` and `tokenXEPAURI`. Its token ID will be automatically
      * assigned (and available on the emitted {IERC721-Transfer} event), and the token
      * URI autogenerated based on the base URI passed at construction.
      *
      * See {ERC721-_mint}.
     ...
     /**
      * @dev Creates a new token for `to` with a massive `tokenId` and `tokenXEPAURI`. Its token ID will be automatically
      * assigned (and available on the emitted {IERC721-Transfer} event), and the token
      * URI autogenerated based on the base URI passed at construction.
      *
      * See {ERC721-_mint}.
      ...
      * - the caller must have the `MINTER_ROLE`.
      */
     function bulk_xepa_minter(address[] memory to, uint256[] memory tokenId, string[] memory tokenXEPAURI) public virtual {
         require(hasRole(MINTER_ROLE, _msgSender()), "Xepa/minter-role-required");

         uint256 i = 0;
         while (i < tokenId.length) {
             _mint(to[i], tokenId[i]);
             _setTokenURI(tokenId[i], tokenXEPAURI[i]);
             i += 1;
         }
     }     
