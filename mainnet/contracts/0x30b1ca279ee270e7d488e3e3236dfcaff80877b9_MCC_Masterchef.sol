 ...
 * successful.
 * To use this library you can add a `using SafeERC20 for IERC20Upgradeable;` statement to your contract,
...
library SafeERC20 {
     ...
     * {IERC20Upgradeable-approve}, and its usage is discouraged.
     *
 ...
 *
 * MCCally, the non-standard {decreaseAllowance} and {increaseAllowance}
...
contract MCCToken is ERC20Upgradeable {
    ...
    */
     ...
     * @dev Block number must be a mccalized block or else this function will revert to prevent misinformation.
...
contract MCC_Masterchef is OwnableUpgradeable {
    ...
    // MCC tokens created per block.
    uint256 public MCCPerBlock;
    // Bonus muliplier for early MCC makers.
    ...
    // The block number when MCC mining starts.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 mccReward = multiplier.mul(MCCPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
       // mcc.mint(devaddr,mccReward.div(10));
       // mcc.mint(address(this),mccReward);
        pool.accMCCPerShare = pool.accMCCPerShare.add(mccReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for MCC allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                   // safeMCCTransfer(msg.sender, pending);
