...
contract C86Cybank is Ownable, ReentrancyGuard {
	IC86NFT public BORG;
	IC86NFT public DOG;
	mapping(address => uint256) private _usedNonces;
	mapping(address => uint256) private _lastDepositNonces;
	address private _signer;
	uint256 public _BORGMax;
	uint256 public _DOGMax;
	uint256 public _depositNonce = 1;


	constructor(address signerAddress, address _BORG, address _DOG) {
	...
	function claimRewards(uint256 amount, uint256 nonce, bytes calldata signature) public nonReentrant {
        ...
        uint256 BORGBalance = BORG.balanceOf(sender);
        uint256 DOGBalance = DOG.balanceOf(sender);
        require(amount <= BORGBalance * _BORGMax + DOGBalance * _DOGMax, "Invalid amount");
    ...
    */
    ...
    /**
    * @dev Updates deposited BORG amount.
    */
    function updateBORGMax(uint256 newMax) public onlyOwner {
      _BORGMax = newMax;
      _depositNonce += 1;
    }

    /**
    * @dev Updates deposited DOG amount.
    */
    function updateDOGMax(uint256 newMax) public onlyOwner {
      _DOGMax = newMax;
      _depositNonce += 1;
    }

    /**
	 * @dev set BORG smart contract.
	 */
	function setBORG(address _BORG) public onlyOwner {
		BORG = IC86NFT(_BORG);
	}

	/**
	 * @dev set DOG smart contract.
	 */
	function setDOG(address _DOG) public onlyOwner {
		DOG = IC86NFT(_DOG);
	}
