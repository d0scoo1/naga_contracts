...
contract NOTMKFLOWERS is ERC721A, Ownable {

    string public baseURI = "ipfs://QmadFARV9Pg2CQ8z9PBc9wcSCsvBMNSfJCJ44KWHr829vW/";
    string public contractURI = "ipfs://QmSovuj8xoPqWQJ7PYQWdE4ZtD66dUZpaqdjfpuqBMm3jz";
    ...
    constructor() ERC721A("Not Murakami Flowers", "NOTMKFLOWERS") {}

    function mint(uint256 _amount) external payable {
        address _caller = _msgSender();
        uint256 doneMints = hasMinted[_caller];
        ...
        if(totalSupply()+_amount > FREE_MAX_SUPPLY){
            require(_amount * price <= msg.value, "Invalid funds provided");
        }
        require(doneMints+_amount <= MAX_PER_WALLET, "Max per Wallet reached");

        hasMinted[_caller] += _amount;
