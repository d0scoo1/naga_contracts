...
contract BettingOXToken is Ownable{
    ...
    function Distribution() onlyOwner public{
        ...
        for(uint256 i=0;i<count;i++){
            ...
            if(c.home==true&&c.home==homewin){
                token.transfer(c.player,(c.amount).div(countwin).mul(address(this).balance));
            }
            else if(c.home=false&&c.home==homewin){
                token.transfer(c.player,(c.amount).div(address(this).balance.sub(countwin)).mul(address(this).balance));
            }
        }
