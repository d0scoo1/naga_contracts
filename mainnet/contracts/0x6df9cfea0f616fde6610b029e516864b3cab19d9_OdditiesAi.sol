...
contract OdditiesAi is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
    string public baseURI = "ipfs://QmVZaoUeuSEperbNXW4QWxo2hBXrokCjgnJm4ifJMBTVox/";
	uint256 public constant PRICE = 0.0055 ether;
	...
	function mint(uint256 amt) external payable validate(amt) nonReentrant {
		...
		require(numberMinted(msg.sender) + amt < maxPerWallet + 1, "Your wallet owns the max. amount of OdditiesAI.");
		require(amt < maxPerTx + 1, "You tried to mint too many OdditiesAI in one transaction.");

		_safeMint(msg.sender, amt);
	}

	function mintFree(uint256 amt) external validate(amt) nonReentrant {
		require(freeMinted + amt < totalFree + 1, "We've given out all of our free OdditiesAI.");
		require(freeMintedByAddress[msg.sender] + amt < maxFreePerWallet + 1, "You've already minted the max. amount of free OdditiesAI.");
