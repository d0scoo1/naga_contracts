...
// ERC721A Contracts v4.0.0
    ...
    contract BoredPeepNFT is Ownable, ERC721A {
    ...
    address constant internal TEAM_ADDRESS = 0xAbDbcA056769B05be9F2fFeeEB034e08518B0924;
    string public baseURI = "ipfs://QmY32U9yPSxAicTZTnBYTz9TzTbdnAAthRF82dUFWwq7Ur/";
    string public constant baseExtension = ".json";
    string public hiddenMetadataUri;
    mapping(address => uint256) public freeMintedAmount;
    mapping(address => uint256) public mintedAmount;
    constructor() ERC721A("BoredPeep NFT", "BoredPeep") {_safeMint(msg.sender, 200); setHiddenMetadataUri("ipfs://QmfJ1a5dJQ4XjtzxLfMriBxWBvAVvLTmUBCEuvFDCLrEuw/hidden.json");}
    
    modifier mintCompliance() {require(saleIsActive, "Sale is not active yet."); require(tx.origin == msg.sender, "Wrong Caller"); _;}
    function mint(uint256 _quantity) external payable mintCompliance() {require (msg.value >= price * _quantity, "BoredPeep Insufficient Fund.");
        require(maxSupply >= totalSupply() + _quantity, "BoredPeep Exceeds max supply."); uint256 _mintedAmount = mintedAmount[msg.sender];
        require(_mintedAmount + _quantity <= maxPerAddressDuringMint, "BoredPeep Exceeds max mints per address!");
    ...
    function freeMint(uint256 _quantity) external mintCompliance() {
        require(maxFreeSupply >= totalSupply() + _quantity, "BoredPeep Exceeds max supply."); uint256 _freeMintedAmount = freeMintedAmount[msg.sender];
        require(_freeMintedAmount + _quantity <= maxPerAddressDuringFreeMint, "BoredPeep Exceeds max free mints per address!"); freeMintedAmount[msg.sender] = _freeMintedAmount + _quantity; _safeMint(msg.sender, _quantity);}
