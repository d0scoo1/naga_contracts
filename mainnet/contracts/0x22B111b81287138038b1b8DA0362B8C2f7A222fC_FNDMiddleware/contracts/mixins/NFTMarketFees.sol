...
abstract contract NFTMarketFees is FoundationTreasuryNode, SendValueWithFallbackWithdraw {
  ...
  {
    try implementationAddress.getCreatorAndImmutableRoyalties(nftContract, tokenId) returns (
      address payable _creator,
      address payable[] memory _recipients,
    ...
    if (creatorRecipients.length == 0) {
      ...
      try implementationAddress.getMutableRoyalties(nftContract, tokenId) returns (
        address payable[] memory _recipients,
        uint256[] memory _splitPerRecipientInBasisPoints
      ) {
