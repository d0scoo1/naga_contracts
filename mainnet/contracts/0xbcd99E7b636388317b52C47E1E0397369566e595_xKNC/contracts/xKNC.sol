    ...
    ReentrancyGuardUpgradeSafe
{
        ...
        notLocked(msg.sender)
    {
        ...
        lock(msg.sender);
        uint256 fee = _administerEthFee(FeeTypes.MINT, msg.value);

        uint256 ethValueForKnc = msg.value.sub(fee);
        uint256 kncBalanceBefore = getFundKncBalanceTwei();

        uint256 availableKncBefore = getAvailableKncBalanceTwei();
        _swapEtherToKnc(ethValueForKnc, minRate);
        uint256 availableKncAfter = getAvailableKncBalanceTwei();

        // get knc deposit amount
        uint256 kncToDeposit = availableKncAfter.sub(availableKncBefore);
        _deposit(kncToDeposit);
        ...
        notLocked(msg.sender)
    {
        ...
        uint256 kncBalanceBefore = getFundKncBalanceTwei();
        uint256 fee = _administerKncFee(kncAmountTwei, FeeTypes.MINT);

        _deposit(kncAmountTwei.sub(fee));
    ...
    /**
     * Stakes any leftover KNC balance in contract
     */
    function adminStake() external onlyOwnerOrManager {
        _administerKncFee(getAvailableKncBalanceTwei(), FeeTypes.CLAIM);
        _deposit(getAvailableKncBalanceTwei());
    }
    ...
    /*
     * @notice Returns the amount of asset held
     */
    function getAmountOfAssetHeld() public view returns (uint256) {
        return getFundKncBalanceTwei();
    }
    ...
    /*
     * @notice Returns withdrawable fee amounts
     */
        ...
        returns (address[2] memory feeAssets, uint256[2] memory feeAmounts)
    {
        ...
        feeAssets[1] = address(knc);
        feeAmounts[0] = address(this).balance;
        feeAmounts[1] = withdrawableKncFees;
    }
