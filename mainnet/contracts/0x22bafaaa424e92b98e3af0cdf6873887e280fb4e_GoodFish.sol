...
contract GoodFish is ERC721Enumerable,Ownable,ReentrancyGuard{

    uint _tokenId;

    address game;

    uint MAX_METADATA = 4000;

    string base;

    constructor(address gameAddress,string memory initBase) ERC721("Good Fish","GFS"){
        game = gameAddress;
        base = initBase;
    }
    ...
    function mint(address receiver) external onlyGame nonReentrant{
        _tokenId++;
        _safeMint(receiver,_tokenId);
    } 

    function burn(uint tokenId) external onlyGame{
        _burn(tokenId);
    }

    function setMetaLength(uint newLength) external onlyOwner{
        MAX_METADATA = newLength;
    }
    ...
    function tokenURI(uint tokenId) public view override returns (string memory){
        ...
        return
        bytes(baseURI).length > 0
        ? string(abi.encodePacked(baseURI, Strings.toString(tokenId%MAX_METADATA)))
        : "";
    }
