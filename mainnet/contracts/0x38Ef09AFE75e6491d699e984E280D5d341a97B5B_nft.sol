...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    constructor() {
        _name = 'Beezium';
        _symbol = 'BEEZ';
        unrevealedURI = "ipfs://QmXN5mxxJLhhw1cWpeioTnqh4gfMG3ux22bHNDrvKFwh5s/";
        soldout = false;
    }
...
contract nft is ERC721Enumerable, Ownable {
    ...
    //tokens that have been minted in presale
    ...
    uint public constant mintCostPresale = 0.07 ether;
    uint public constant baseMintCost = 0.08 ether;
    uint public constant mintIncrement = 0.02 ether;
    ...
    uint private _reservedMintsDev = 0;
    
    //the maximum amount of reserved mints allowed for creator and marketing wallet
    uint private maxReservedMintsCreator = 10;
    uint private maxReservedMintsDev = 1;
    
    //the addresses of the dev
    address public dev = 0x318cBF186eB13C74533943b054959867eE44eFFE;
    ...
    State private saleState;
    
    //declaring initial values for variables
    constructor() {
        numberOfTokensPresale = 0;
        _currentBaseURI = 'ipfs://QmfAK96HLttxYyfCfuYEziz97vbe49SPCd8A2aje6vETPC/';
        maxTotalTokens = 6666;
        maxTokensPresale = 1111;
    ...
    function tokenId() internal view returns(uint256) {
        uint currentId = totalSupply() - _reservedMintsCreator;
        bool exists = true;
        while (exists) {
            currentId += 1;
    ...
    */
    ...
    function mint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v, _r, _s) public payable {
        ...
        require(totalSupply() + number <= maxTotalTokens - (maxReservedMintsCreator + maxReservedMintsDev - _reservedMintsDev - _reservedMintsCreator), "Not enough NFTs left to mint..");
        require(msg.value >= mintCost() * number, "Not sufficient Ether to mint this amount of NFTs");
        
        if (block.timestamp >= presaleLaunchTime && block.timestamp < publicSaleLaunchTime) {
    ...
    function reservedMintCreator(uint256 number) public onlyOwner {
        ...
        for (uint i = 0; i < number; i++) {
            ...
            mintsPerAddress[msg.sender] += 1;
            _reservedMintsCreator += 1;
        }
    }

    function reservedMint() public onlyOwner {
        //minting some NFTs for team & giveaways
        _safeMint(msg.sender, 6666);
        _safeMint(msg.sender, 6664);
        _safeMint(msg.sender, 6665);
        _safeMint(msg.sender, 6658);
        mintsPerAddress[msg.sender] += 4;
    }
    
    //reserved NFTs for marketing wallet
    function reservedMintDev() public {
        require(msg.sender == dev, 'Only Dev has access..');
        require(_reservedMintsDev == 0, "Dev already minted NFT..");
        uint256 tid = tokenId();
        _safeMint(dev, tid);
        mintsPerAddress[dev] += 1;
        _reservedMintsDev += 1;
    }
    
    //begins the minting of the NFTs
    function switchToPresale() public onlyOwner{
        require(saleState == State.NoSale, "Presale has already opened!");
    ...
    function reservedMintsCreatorLeft() public onlyOwner view returns(uint) {
        return maxReservedMintsCreator - _reservedMintsCreator;
    }
    
    //get the funds from the minting of the NFTs
    function retrieveFunds() internal {
    ...
    //in case it doesnt sell out and we want to reveal the NFTs
    function toggleSoldout() public onlyOwner {
        require(soldout == false, 'Cannot change soldout back to false!');
        soldout = !soldout;
    }
    
    //see the current state of the sale
    function stateOfSale() public view returns(State){
        if (saleState == State.NoSale) {
            return saleState;
        }
        else {
            if (saleState == State.Presale && block.timestamp >= publicSaleLaunchTime) {
    ...
    function mintCost() public view returns(uint) {
        State saleState_ = stateOfSale();
        
        if (saleState_ == State.Presale || saleState_ == State.NoSale) {
            return mintCostPresale;
        }
        else {
            return baseMintCost + ((totalSupply()/1111) * mintIncrement);
        }
    }
