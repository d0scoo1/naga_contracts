...
contract GOSHAX is ERC20 {
    ...
    GOSHAXDividendTracker public dividendTracker;

    address public liquidityWallet;
    address payable public devWallet = 0x97e93B3F96A6014685306C601f0978b9d34CD9FE;
    ...
    uint256 public immutable DevWalletFee;
    ...
    constructor() public ERC20("GOSHAX", "GOSHAX") {
        uint256 _ETHRewardsFee = 2;
        uint256 _liquidityFee = 2;
        uint256 _DevWalletFee = 5;

        ETHRewardsFee = _ETHRewardsFee;
        liquidityFee = _liquidityFee;
        DevWalletFee = _DevWalletFee;
        totalFees = _ETHRewardsFee.add(_liquidityFee).add(_DevWalletFee);


    	dividendTracker = new GOSHAXDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        GOSHAXDividendTracker newDividendTracker = GOSHAXDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the GOSHAX token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GOSHAX: Account is already the value of 'excluded'");
    ...
    function changedevWallet(address payable newAddress) external onlyOwner {
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "GOSHAX: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue != gasForProcessing, "GOSHAX: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(
            ...
            uint256 devWalletTokens = contractTokenBalance.mul(DevWalletFee).div(totalFees);
...
contract GOSHAXDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("GOSHAX_Dividend_Tracker", "GOSHAX_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "GOSHAX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "GOSHAX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GOSHAX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GOSHAX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "GOSHAX_Dividend_Tracker: Cannot update claimWait to same value");
