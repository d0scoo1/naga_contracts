 ...
 * 
 *    
 *   Accompaniments by Takens Theorem
 * 
...
/**
 * @title Accompaniments by Takens Theorem contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract ACTT1 is ERC721, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    uint256 private MAX_SUPPLY = 32;
    mapping (uint256 => string) private nftDescriptions;
    mapping (uint256 => string) private nftNames;
    mapping (uint256 => string) private ipfsCids;
    mapping (uint256 => address) private externalNftAddrs;
    mapping (uint256 => uint256) private externalTokenIds;
    ...
    function updateOwner(uint256 tokenId) public {
        require(_exists(tokenId), "Sidecar NFT: nonexistent token");

        emit Transfer(address(this), ownerOf(tokenId), tokenId);        
    }

    function renderDescription(uint256 tokenId) private view returns (string memory) {
        string memory extName = externalNft(externalNftAddrs[tokenId]).name();
        return string(abi.encodePacked("DO NOT BUY, SELL OR TRANSFER THIS NFT.\\n\\nThis is a sidecar NFT: It cannot be bought, sold or transferred directly. This NFT travels with a token of:\\n\\n- ",extName,"\\n- 0x",
            Strings.toAsciiString(externalNftAddrs[tokenId]),"\\n- Accompaniment to token #",Strings.toString(externalTokenIds[tokenId]),
            "\\n\\n",nftDescriptions[tokenId],
            "\\n\\n- IPFS CID:\\n- ",ipfsCids[tokenId],"\\n\\n- Owner:\\n- 0x",
            Strings.toAsciiString(ownerOf(tokenId)))); 
    }
    
    function mintNft(string memory name, address externalNftContract, uint256 externalTokenId, string memory description, string memory ipfsCid) public onlyOwner {
        ...
        ipfsCids[newItemId] = ipfsCid;
        externalNftAddrs[newItemId] = externalNftContract;
        externalTokenIds[newItemId] = externalTokenId;

        _mint(_msgSender(), newItemId);
    }    

    function sidecarNotice() private pure {
        revert('Sidecar NFT: cannot buy, sell or transfer');
    }
    ...
    function ownerOf(uint256 tokenId) public view override returns (address owner) {
        require(_exists(tokenId), "Sidecar NFT: nonexistent token");
        // summon owner of paired tokenId from root contract
        return externalNft(externalNftAddrs[tokenId]).ownerOf(externalTokenIds[tokenId]); 
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "ERC721URIStorage: URI query for nonexistent token");
        
        bytes memory json = abi.encodePacked('{"name":"',nftNames[tokenId],
                                            '", "description":"',renderDescription(tokenId),
                                            '", "created_by":"Takens Theorem", "image":"',ipfs,ipfsCids[tokenId],
                                        '"}');
    ...
    constructor() ERC721("ACTT1", "Accompaniments by Takens Theorem") {}    
}