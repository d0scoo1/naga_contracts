...
contract ApeMotorClub is ERC721A, Ownable {
  constructor() ERC721A("Ape Motor Club", "AMC", 10) {}
    ...
    uint256 public constant MAX_NFT_PRESALE = 700;
    uint256 public constant MAX_NFT = 6666;
    uint256 public publicSalePrice = 150000000000000000;  // 0.15 ETH
    ...
    uint256 private dropNum = 3333;
    address private fundWallet = 0xAe51CDE60e59995200852ff875Ab281F5ecF7724;

    mapping(address => uint256) public whiteListClaimed;
 
    bytes32 public merkleRootPre = 0xf170676f0e74aa3ef9357e05ea7d1324733c6d7864960abd1a66557cd48ad6ad;

    /*
     * Function to reveal all Ape Motor Club
    */
    function revealNow() 
        external 
        onlyOwner 
    ...
    */
     ...
     * It is payable. Amount is calculated as per (publicSalePrice.mul(_numOfTokens))
    */
    function mintNFT(
        ...
        payable
    {
        ...
        require(publicSalePrice.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");
        
        _safeMint(msg.sender, _numOfTokens);
        
        if(totalSupply() >= dropNum || totalSupply() == MAX_NFT) {
        ...
        payable
    {
        ...
        require(verify(_proof, merkleRootPre, keccak256(abi.encodePacked(msg.sender))), "Not whitelisted");
        require(totalSupply() < MAX_NFT, 'All public tokens have been minted');
        require(_numOfTokens <= WHITELIST_MAX_MINT, 'Cannot purchase this many tokens');
        require(totalSupply().add(_numOfTokens) <= MAX_NFT_PRESALE, 'Purchase would exceed max presale supply of NFTs');
        require(whiteListClaimed[msg.sender].add(_numOfTokens) <= WHITELIST_MAX_MINT, 'Purchase exceeds max whitelisted');
        require(preSalePrice.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");
        whiteListClaimed[msg.sender] += _numOfTokens;
        _safeMint(msg.sender, _numOfTokens);
    }

    function setDropNum(uint256 newNum) public onlyOwner {
        dropNum = newNum;
    }

    function setWallet(address addr) public onlyOwner {
        fundWallet = addr;
    }
    ...
    */
    function mintByOwner(
        ...
        onlyOwner
    {
        require(totalSupply()+1 <= MAX_NFT_PRESALE, "Tokens number to mint cannot exceed number of MAX tokens");
        _safeMint(_to, 1);
    }
    ...
    */
    ...
    function setMerkleRootPre(bytes32 _merkleProof) public onlyOwner {
        merkleRootPre = _merkleProof;
    }
