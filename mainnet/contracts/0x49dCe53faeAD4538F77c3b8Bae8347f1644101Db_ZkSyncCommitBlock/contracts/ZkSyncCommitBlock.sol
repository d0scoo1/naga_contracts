...
//        emit BlockVerification(_blockNumber);
    ...
    internal view returns (uint32[] memory blockSizes, uint256[] memory inputs) {
    ...
    /// @notice Verify proof -> checkWithdrawals (accrue balances from withdrawals) -> remove priority requests
    ...
    function emitDepositNFTCommitEvent(uint32 _blockNumber, Operations.DepositNFT memory depositNFTData) internal {
        emit DepositNFTCommit(_blockNumber, depositNFTData.accountId, depositNFTData.owner, depositNFTData.globalId);
    }

    function emitFullExitNFTCommitEvent(uint32 _blockNumber, Operations.FullExitNFT memory fullExitNFTData) internal {
        emit FullExitNFTCommit(_blockNumber, fullExitNFTData.accountId, fullExitNFTData.owner, fullExitNFTData.globalId);
    }
    ...
    function collectOnchainOps(uint32 _blockNumber, bytes memory _publicData, bytes memory _ethWitness, uint32[] memory _ethWitnessSizes)
        ...
        while (pubDataPtr < pubDataEndPtr) {
            ...
            if (opType == Operations.OpType.Transfer) {
            ...
            } else if (opType == Operations.OpType.MintNFT) {
                pubDataPtr += MINT_NFT_BYTES;
            } else if (opType == Operations.OpType.TransferNFT) {
                pubDataPtr += TRANSFER_NFT_BYTES;
            } else if (opType == Operations.OpType.TransferToNewNFT) {
                pubDataPtr += TRANSFER_TO_NEW_NFT_BYTES;
            } else if (opType == Operations.OpType.ApproveNFT) {
                pubDataPtr += APPROVE_NFT_BYTES;
            } else if (opType == Operations.OpType.ExchangeNFT) {
                pubDataPtr += EXCHANGE_NFT;
            } else {
                ...
                } else if (opType == Operations.OpType.DepositNFT) {
                    bytes memory pubData = Bytes.slice(_publicData, pubdataOffset + 1, DEPOSIT_NFT_BYTES - 1);
                    Operations.DepositNFT memory depositNFTData = Operations.readDepositNFTPubdata(pubData);

                    emitDepositNFTCommitEvent(_blockNumber, depositNFTData);

                    OnchainOperation memory onchainOp = OnchainOperation(
                        Operations.OpType.DepositNFT,
                        pubData
                    );
                    commitNextPriorityOperation(onchainOp, currentPriorityRequestId);
                    currentPriorityRequestId++;

                    pubDataPtr += DEPOSIT_NFT_BYTES;
                ...
                } else if (opType == Operations.OpType.PartialExitNFT) {
                    Operations.PartialExitNFT memory data = Operations.readPartialExitNFTPubdata(_publicData, pubdataOffset + 1);

                    bool addToPendingWithdrawalsQueue = true;
                    withdrawalsDataHash = keccak256(abi.encode(withdrawalsDataHash, addToPendingWithdrawalsQueue, data.owner, data.globalId));

                    pubDataPtr += PARTIAL_EXIT_NFT_BYTES;
                ...
                } else if (opType == Operations.OpType.FullExitNFT) {
                    bytes memory pubData = Bytes.slice(_publicData, pubdataOffset + 1, FULL_EXIT_NFT_BYTES - 1);

                    Operations.FullExitNFT memory fullExitNFTData = Operations.readFullExitNFTPubdata(pubData);
                    emitFullExitNFTCommitEvent(_blockNumber, fullExitNFTData);

                    bool addToPendingWithdrawalsQueue = false;
                    withdrawalsDataHash = keccak256(abi.encode(withdrawalsDataHash, addToPendingWithdrawalsQueue, fullExitNFTData.owner, fullExitNFTData.globalId));

                    OnchainOperation memory onchainOp = OnchainOperation(
                        Operations.OpType.FullExitNFT,
                        pubData
                    );
                    commitNextPriorityOperation(onchainOp, currentPriorityRequestId);
                    currentPriorityRequestId++;

                    pubDataPtr += FULL_EXIT_NFT_BYTES;
    ...
    function commitNextPriorityOperation(OnchainOperation memory _onchainOp, uint64 _priorityRequestId) internal view {
        ...
        } else if (_onchainOp.opType == Operations.OpType.DepositNFT) {
            require(Operations.depositNFTPubdataMatch(priorReqPubdata, _onchainOp.pubData), "vnp14");
        } else if (_onchainOp.opType == Operations.OpType.FullExit) {
            require(Operations.fullExitPubdataMatch(priorReqPubdata, _onchainOp.pubData), "vnp15");
        } else if (_onchainOp.opType == Operations.OpType.FullExitNFT) {
            require(Operations.fullExitNFTPubdataMatch(priorReqPubdata, _onchainOp.pubData), "vnp16");
        } else if (_onchainOp.opType == Operations.OpType.CreatePair) {
            require(Operations.createPairPubdataMatch(priorReqPubdata, _onchainOp.pubData), "vnp17");
        } else {
            revert("vnp18"); // invalid or non-priority operation
        }
    }
