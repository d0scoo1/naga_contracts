...
contract Sale is Ownable {
    ...
    constructor(address _tokenAddress, address _priceFeedAddress) {
        require(_tokenAddress != address(0), "tokenAddress is zero");
        require(_priceFeedAddress != address(0), "priceFeedAddress is zero");
    ...
    function getEthUsdPrice() public view returns (uint) {
        ...
        // chainlink result is 8 decimals, convert it to 18 decimals
        return uint(price) * 1e10; 
    }
    
    function toggleLock() public onlyOwner {
        contractLocked = !contractLocked;
        emit ContractLockUpdated(contractLocked);
    }
    ...
    function calcTokenAmount(uint256 _ethAmount) public view returns (uint) {
        return _ethAmount * getEthUsdPrice() / tokenUsdPrice;
    }

    function withdrawEth(uint256 _amount) public onlyOwner {
        require(_amount > 0, "cannot withdraw zero");
        require(_amount <= address(this).balance, "not enoough eth in contract");
    ...
    function changePrice(uint256 _tokenUsdPrice) public onlyOwner {
        require(tokenUsdPrice != _tokenUsdPrice, "new price equals current");
        require(tokenUsdPrice > 0, "new price is zero");
        tokenUsdPrice = _tokenUsdPrice;
        emit TokenPriceUpdated(_tokenUsdPrice);
    }
}