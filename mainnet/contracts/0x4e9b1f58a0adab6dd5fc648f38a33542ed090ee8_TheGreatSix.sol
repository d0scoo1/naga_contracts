...
contract TheGreatSix is ERC721Enumerable, Ownable {
    ...
    uint256 public _lastNFTsMinted=0;
    mapping (address => uint256) public VIPAmounts;
    string public baseTokenURI;
    constructor(string memory baseUR) ERC721("The Great Six", "TGS") {
        setBaseURI(baseUR);
    }
    ...
    function mintNFTs(uint256[] memory _tokennft) public payable {
        uint256 _count=_tokennft.length;
        ...
        for (uint256 i = 0; i < _count; i++) {            
            _mintSingleNFTCustom(_tokennft[i]);
            _lastNFTsMinted=_tokennft[i];
        }
    }
    function claimFreeNFTsVIP() public {
        ...
        _mintSingleNFTCustom(_count);
    }
    function getlastNFTsMinted() public view returns(uint256) {
        return _lastNFTsMinted;
    }
    ...
    function _mintSingleNFTCustom(uint256 newTokenID) private {        
