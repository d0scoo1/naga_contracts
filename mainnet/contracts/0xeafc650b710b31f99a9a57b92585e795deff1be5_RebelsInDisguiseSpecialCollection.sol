...
// File: contracts/erc721SpecialCollection.sol
...
contract RebelsInDisguiseSpecialCollection is ERC721, ERC721Enumerable, Ownable {
    ...
    // Mint function for only owner to mint other addresses.
    function mintToAddress(address receiverAddress, uint256 _count) public onlyOwner {
        require(isMintingLocked == false, "Minting has been locked!");

        for (uint256 index = 0; index < _count; index++) {
            _mint(receiverAddress, _tokenId);
            _tokenId ++;
        }
    }
