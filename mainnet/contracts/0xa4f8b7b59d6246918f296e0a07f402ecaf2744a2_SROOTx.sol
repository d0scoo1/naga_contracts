...
contract SROOTx is ERC20Upgradeable, Delegatable  {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // capture the contract's current WETH balance.
        // this is so that we can capture exactly the amount of WETH that the
        // swap creates, and not make the liquidity event include any WETH that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for WETH
        swapTokensForEth(halfOfLiquify); // <- this breaks the WETH -> TOKEN swap when swap+liquify is triggered
