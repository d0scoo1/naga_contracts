...
contract DwarfsGirls is ERC721Enumerable, Ownable {
    uint public  MAX_TOKEN = 2000;
    uint public  MAX_RESERV = 30;
    uint public  basePrice = 8*10**16; // 0.08 ETH
	string _baseTokenURI;
	bool public saleEnable = false;
	
	mapping (uint256 => bool) internal _whitelistDAW;

  
   DwarfsGirls internal DAW = DwarfsGirls(0x39239b6040207A4D3d657aD00554669A992cA451);  // 0x39239b6040207A4D3d657aD00554669A992cA451
    ...
    constructor(string memory baseURI) ERC721("Dwarfs Girls", "DWGIRL")  {
        setBaseURI(baseURI);
    }
    ...
    function mint(address _to, uint _count) public payable {
        ...
        require(totalSupply() + MAX_RESERV +_count <= MAX_TOKEN, "Exceeds limit");
    ...
    function mintReserve(address _to, uint _count) public onlyOwner {
        require(_count <= MAX_RESERV, "Exceeds limit");
        for(uint i = 0; i < _count; i++){
            _safeMint(_to, totalSupply());
            MAX_RESERV--;
        }
    ...
    function CompletedMintForDAW(uint256 _mespid) public view virtual returns (bool) {
    ...
    function TokenForMintAvailable(address _holder) public view virtual returns (uint256) {
        ...
        uint256[] memory tokensId = new uint256[](DAW.balanceOf(_holder));
        for(uint i = 0; i < DAW.balanceOf(_holder); i++){
            tokensId[i] = DAW.tokenOfOwnerByIndex(_holder, i);
            
            if (_whitelistDAW[tokensId[i]])
              {} else {
                   tokenAvailable++;
              }
        ...
        uint256[] memory tokensId = new uint256[](DAW.balanceOf(msg.sender));
        for(uint i = 0; i < DAW.balanceOf(msg.sender); i++){
            tokensId[i] = DAW.tokenOfOwnerByIndex(msg.sender, i);
