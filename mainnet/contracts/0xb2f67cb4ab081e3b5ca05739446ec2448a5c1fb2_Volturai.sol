...
contract Volturai  is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint8 private _decimals = 9;

    string private _name = "Volturai";                                          
    string private _symbol = "$VOLTURAI";                                           
    ...
    uint256 public _marketingFee = _buyFees;
    ...
    address payable public marketingWallet = payable(0x4D21AE3dEf4Ea346C52c513c556fBd9c0e500763);
    ...
    function setFees(address recipient) private {
        ...
        _marketingFee = _buyFees;
        if (recipient == uniswapV2Pair) { // sell
            _marketingFee = _sellFees;
        }
    }
    ...
    function setBuyersFee(uint256 buyFee) external onlyOwner() {
        _buyFees = buyFee;
    }

    function setSellersFee(uint256 sellfees) external onlyOwner() {
        _sellFees = sellfees;
    }
