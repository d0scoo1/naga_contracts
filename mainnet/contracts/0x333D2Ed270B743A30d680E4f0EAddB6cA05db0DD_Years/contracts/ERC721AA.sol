...
contract ERC721AA is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory _tokenURI = _calculateuri(tokenId);
    ...
    /**
    @dev tokenuri is calculate from tokenid
     */
    function _calculateuri(uint256 tokenId) internal view virtual returns (string memory) {
        unchecked {
            if (tokenId >= 4000) {
                tokenId = tokenId - 4000;
                return string(abi.encodePacked(tokenId.toString(),"_AD.json"));
            }
            tokenId = 4000 - tokenId;
            return string(abi.encodePacked(tokenId.toString(),"_BC.json"));
        }
    }
