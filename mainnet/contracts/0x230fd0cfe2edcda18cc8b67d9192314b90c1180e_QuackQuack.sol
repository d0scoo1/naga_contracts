...
library Strings {
    ...
    function toString(uint256 value) internal pure returns (string memory) {
        // Insred by OraclizeAPI's implementation - MIT licence
...
abstract contract QQ721 is Context, ERC165, IERC721, IERC721Metadata {
  ...
  function ownerOf(uint tokenId) public view override returns( address owner ){
    require(_exists(tokenId), "QQ721: query for nonexistent token");
  ...
  function approve(address to, uint tokenId) external override {
    ...
    require(to != owner, "QQ721: approval to current owner");

    require(
      _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
      "QQ721: caller is not owner nor approved for all"
    );
  ...
  function safeTransferFrom(address from, address to, uint tokenId, bytes memory _data) public override {
    require(_isApprovedOrOwner(_msgSender(), tokenId), "QQ721: caller is not owner nor approved");
  ...
  function _checkOnERC721Received(address from, address to, uint tokenId, bytes memory _data)private returns ( bool ){
    if (to.isContract()) {
      try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
        ...
        if (reason.length == 0) {
          revert("QQ721: transfer to non ERC721Receiver implementer");
        } else {
          assembly {
  ...
  function _safeTransfer(address from, address to, uint tokenId, bytes memory _data) internal{
    _transfer(from, to, tokenId);
    require(_checkOnERC721Received(from, to, tokenId, _data), "QQ721: transfer to non ERC721Receiver implementer");
  }
...
abstract contract QQ721Enumerable is QQ721, IERC721Enumerable {
  function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165, QQ721) returns( bool isSupported ){
  ...
  function tokenOfOwnerByIndex(address owner, uint index) external view override returns( uint tokenId ){
    ...
    revert( "QQ721Enumerable: owner index out of bounds" );
  }

  function tokenByIndex(uint index) external view override returns( uint tokenId ){
    require(index < tokens.length, "QQ721Enumerable: query for nonexistent token");
    return index;
  }
}
...
abstract contract QQ721Batch is QQ721Enumerable, IERC721Batch {
...
contract QuackQuack is QQ721Batch, Ownable {
  ...
  constructor() QQ721("Quack Quack", "QUACK") { }
  ...
  function mint( uint quantity) external payable {
    ...
    uint fee = PRICE * quantity;
    require(fee <= msg.value, "fee too low");
  ...
  function mintTo(address[] calldata accounts, uint[] calldata quantity ) external payable onlyOwner{
    require(quantity.length == accounts.length, "must provide equal quantities and accounts" );
  ...
  function resurrect( address[] calldata accounts, uint[] calldata tokenIds ) external onlyOwner{
    require(tokenIds.length == accounts.length,   "must provide equal tokenIds and accounts" );
  ...
  function _burn( address from, uint tokenId ) private {
    require( _exists( tokenId ), "query for nonexistent token" );
    require( from == tokens[ tokenId ].owner, "owner mismatch" );
