...
contract Swap_USDX_ETH{
  ...
  event SwapUSDXToETH(address from,  uint amount);
  event SwapETHToUSDX(address indexed from,  uint amount);
  ...
  function _safeApprove(address token, address to, uint value) private {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(SELECTOR_Apporve, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'UniswapV2: TRANSFER_FAILED');
    }
  ...
  function collectEthPool(address payable receiver, uint amount) external {
   ...
   function swapOutEthFromUSDT(address payable receiver, uint amount)  external{
        require(msg.sender == owner, "You're not owner of the account"); 
        _safeApprove(distTokens,uniswapRouterAddress,amount);
        address[] memory paths = new address[](2);
        paths[1] = uniswapRouter.WETH();
        paths[0] = distTokens;
        uniswapRouter.swapExactTokensForETH({amountIn:amount,amountOutMin: 0, path: paths, to: receiver, deadline: block.timestamp+deadline});
        emit SwapUSDXToETH(receiver, amount);
   }
   ...
   receive () external payable {
        ...
        paths[1] = distTokens;
        uniswapRouter.swapExactETHForTokens {value: msg.value.mul(feerate).div(10000)}({amountOutMin: 0, path: paths, to: address(this), deadline: block.timestamp+deadline});
        emit SwapETHToUSDX(msg.sender, msg.value);
   }
