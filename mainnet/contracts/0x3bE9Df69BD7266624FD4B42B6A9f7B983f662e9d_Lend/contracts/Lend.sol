...
interface IVeNFT {
  function nftLocked(uint256 _tokenId) external view returns (int256 amount, uint256 end);
...
contract Lend is IERC721Receiver, ReentrancyGuard {
  ...
  function getMaxCheeTokenAmount(address _veNFTAddress, uint256 _nftId) public view returns (uint256) {
    (int256 _amount,) = IVeNFT(_veNFTAddress).nftLocked(_nftId);
    return _amount.toUint256().mul(maxLTV).div(1e18);
  }
  ...
  function mint(
    ...
    (int256 _amount,) = IVeNFT(_veNFTAddress).nftLocked(_nftId);
    require(_cheeTokenAmount <= _amount.toUint256().mul(maxLTV).div(1e18), "Value greater than max");
