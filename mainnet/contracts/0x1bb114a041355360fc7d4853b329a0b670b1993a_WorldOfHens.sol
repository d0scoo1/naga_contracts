...
contract WorldOfHens is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public maxMintAmount = 2999;
    
    // Total reserved 200, 20 in presale and 180 in public sale
    uint256 public presaleReservedForTeam = 20;
    uint256 public reservedForTeam = 180;
    ...
    bool public presaleRevealed = false;
        ...
        payable
    {
        ...
        // Owner
        if (msg.sender == owner()) {
            require(presaleReservedForTeam >= _mintAmount);
            presaleReservedForTeam -= _mintAmount;
        }
        ...
        if (msg.sender != owner()) {
            ...
            require(
                tokenCount + _mintAmount <= presaleMintAmount,
                string(abi.encodePacked("Limit token ", tokenCount.toString()))
            );
            require(
                supply + _mintAmount <= presaleMaxSupply - presaleReservedForTeam,
                "Max Supply"
            );

            // Cost for whitelisted
            totalAmount = _mintAmount;
            if (isWhitelisted(msg.sender)) {
                totalAmount = totalAmount * whitelistCost;
            }
            else {
        ...
        returns (string memory)
    {
        ...
        if (presaleRevealed == false) {
            return notRevealedUri;
        }

        if (presaleRevealed == true && revealed == false && tokenId > presaleMaxSupply) {
            return notRevealedUri;
        }
    ...
    function setPresaleReserved(uint256 _presaleReserved) public onlyOwner {
        presaleReservedForTeam = _presaleReserved;
    }
    ...
    function setPresaleMaxMintAmount(uint256 _newPresaleMaxMintAmount) public onlyOwner {
        presaleMintAmount = _newPresaleMaxMintAmount;
    }
    ...
    function setPresaleRevealed(bool _presaleRevealed) public onlyOwner {
        presaleRevealed = _presaleRevealed;
    }
