...
contract SpaceChicks is ERC721A, Ownable {
  ...
  enum Status { SALE_NOT_LIVE, VIP_LIVE, PRESALE_LIVE, SALE_LIVE }

  uint256 public constant SUPPLY_MAX = 4444;
  uint256 public constant RESERVE_MAX = 100;
  uint256 public constant VIP_PRICE = 0.06 ether;
  ...
  constructor() ERC721A("SpaceChicks", "SC") {
  ...
  function mint(uint256 quantity) external payable {
    ...
    if(state == Status.VIP_LIVE) {
      require(whitelist[msg.sender], "SpaceChicks: Sender not in WhiteList");
      require(msg.value >= VIP_PRICE * quantity, "SpaceChicks: Insufficient ETH");
    } else if(state == Status.PRESALE_LIVE) {
      require(whitelist[msg.sender], "SpaceChicks: Sender not in WhiteList");
