/**
 *   Website: https://jujutsu.moe
 *   Twitter: https://twitter.com/JujutsuToken1
 *   Telegram: https://t.me/JujutsuEth
 *   
 *   All Social Medias: @JujutsuETH
 */
...
contract TRADABLE_ERC20 is Ownable, IERC20, IERC20Metadata {
...
contract Jujutsu is TRADABLE_ERC20 {
    using SafeMath for uint256;

    IUniSwapRouter private uniswapRouter;

    string private constant _name = "jujutsu Kaisen";
    string private constant _symbol = "JSU";
    uint8  private constant _decimals = 9;

    uint8 private feeOnBuy = 12;
    uint8 private feeOnSell = 12;

    uint256 private constant tokenSupply = 1000000000000000 * 10 ** _decimals;
    ...
    constructor() {
        ...
        marketingWallet = address(0xb4ab1178643b6Aa9c4DaAAc1a7F3fD24e99687d9);

        setLiftFromFee(owner(), true);
        setLiftFromFee(address(this), true);
        setLiftFromFee(deadAddress, true);
        setLiftFromFee(address(uniswapRouter), true);
        _approve(owner(), address(uniswapRouter), ~uint256(0));
    
        maxTransactionAmount = tokenSupply.mul(1).div(100);
        maxWalletSize = tokenSupply.mul(2).div(100);
        swapTokensAtAmount = tokenSupply.mul(20).div(10000);

        uint256 ownerToken = tokenSupply.mul(80).div(100);
        uint256 deadAddrToken = tokenSupply.mul(20).div(100);

        _balances[address(owner())] = ownerToken;
        _balances[address(deadAddress)] = deadAddrToken;

        emit Transfer(zeroAddress, owner(), ownerToken);
        emit Transfer(owner(), deadAddress, deadAddrToken);
    }
    ...
    function isHolder(address from, address to) private view returns(bool) {
        return 
         from != owner() &&
         to != owner() &&
         to != deadAddress &&
    ...
    function limitCheck() public view returns(bool) {
        return block.timestamp <= tradingTimeStamp + ( 120 seconds );
    }
    ...
    function setLiftFromFee(address _wallet, bool _val) public onlyOwner {
    ...
    function isTradingOpen() public view returns(string memory) {
        return tardingOpen ? "YES" : "NO";
    }
