...
contract StakingFacet {
    ...
    function initDaoStaking(address _stakeborgToken, address _rewards) public {
        require(_stakeborgToken != address(0), "Stakeborg Token address must not be 0x0");

        LibStakingStorage.Storage storage ds = LibStakingStorage.daoStakingStorage();

        require(!ds.initialized, "Dao Staking: already initialized");
        LibOwnership.enforceIsContractOwner();

        ds.initialized = true;

        ds.stakeborgToken = IERC20(_stakeborgToken);
    ...
    function deposit(uint256 amount) public {
        ...
        LibStakingStorage.Storage storage ds = LibStakingStorage.daoStakingStorage();
        uint256 allowance = ds.stakeborgToken.allowance(msg.sender, address(this));
        ...
        _updateLockedStakeborgToken(stakeborgTokenStakedAtTs(block.timestamp).add(amount));
        ...
        ds.stakeborgToken.transferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 amount) public {
        ...
        _updateLockedStakeborgToken(stakeborgTokenStakedAtTs(block.timestamp).sub(amount));
        ...
        ds.stakeborgToken.transfer(msg.sender, amount);
    ...
    function lock(uint256 timestamp) public {
        ...
        LibStakingStorage.Storage storage ds = LibStakingStorage.daoStakingStorage();
        LibStakingStorage.Stake[] storage checkpoints = ds.userStakeHistory[msg.sender];
        LibStakingStorage.Stake storage currentStake = checkpoints[checkpoints.length - 1];
    ...
    function balanceAtTs(address user, uint256 timestamp) public view returns (uint256) {
        LibStakingStorage.Stake memory stake = stakeAtTs(user, timestamp);

        return stake.amount;
    }

    // stakeAtTs returns the Stake object of the user that was valid at `timestamp`
    function stakeAtTs(address user, uint256 timestamp) public view returns (LibStakingStorage.Stake memory) {
        LibStakingStorage.Storage storage ds = LibStakingStorage.daoStakingStorage();
        LibStakingStorage.Stake[] storage stakeHistory = ds.userStakeHistory[user];

        if (stakeHistory.length == 0 || timestamp < stakeHistory[0].timestamp) {
            return LibStakingStorage.Stake(block.timestamp, 0, block.timestamp, address(0));
        }
    ...
    function stakeborgTokenStaked() public view returns (uint256) {
        return stakeborgTokenStakedAtTs(block.timestamp);
    }

    // stakeborgTokenStakedAtTs returns the total raw amount of stakeborg token users have deposited into the contract
    // it does not include any bonus
    function stakeborgTokenStakedAtTs(uint256 timestamp) public view returns (uint256) {
        return _checkpointsBinarySearch(LibStakingStorage.daoStakingStorage().stakeborgTokenStakedHistory, timestamp);
    }
    ...
    function delegatedPowerAtTs(address user, uint256 timestamp) public view returns (uint256) {
        return _checkpointsBinarySearch(LibStakingStorage.daoStakingStorage().delegatedPowerHistory[user], timestamp);
    }
    ...
    function userLockedUntil(address user) public view returns (uint256) {
        LibStakingStorage.Stake memory c = stakeAtTs(user, block.timestamp);
    ...
    function _checkpointsBinarySearch(LibStakingStorage.Checkpoint[] storage checkpoints, uint256 timestamp) internal view returns (uint256) {
    ...
    function _stakeMultiplier(LibStakingStorage.Stake memory stake, uint256 timestamp) internal view returns (uint256) {
    ...
    function _updateUserBalance(LibStakingStorage.Stake[] storage checkpoints, uint256 amount) internal {
        if (checkpoints.length == 0) {
            checkpoints.push(LibStakingStorage.Stake(block.timestamp, amount, block.timestamp, address(0)));
        } else {
            LibStakingStorage.Stake storage old = checkpoints[checkpoints.length - 1];

            if (old.timestamp == block.timestamp) {
                old.amount = amount;
            } else {
                checkpoints.push(LibStakingStorage.Stake(block.timestamp, amount, old.expiryTimestamp, old.delegatedTo));
            }
        }
    }
    ...
    function _updateUserLock(LibStakingStorage.Stake[] storage checkpoints, uint256 expiryTimestamp) internal {
        LibStakingStorage.Stake storage old = checkpoints[checkpoints.length - 1];

        if (old.timestamp < block.timestamp) {
            checkpoints.push(LibStakingStorage.Stake(block.timestamp, old.amount, expiryTimestamp, old.delegatedTo));
        } else {
    ...
    // then there must be at least 1 checkpoint
    function _updateUserDelegatedTo(LibStakingStorage.Stake[] storage checkpoints, address to) internal {
        LibStakingStorage.Stake storage old = checkpoints[checkpoints.length - 1];

        if (old.timestamp < block.timestamp) {
            checkpoints.push(LibStakingStorage.Stake(block.timestamp, old.amount, old.expiryTimestamp, to));
        } else {
            old.delegatedTo = to;
        }
    }
    ...
    function _updateDelegatedPower(LibStakingStorage.Checkpoint[] storage checkpoints, uint256 amount) internal {
        if (checkpoints.length == 0 || checkpoints[checkpoints.length - 1].timestamp < block.timestamp) {
            checkpoints.push(LibStakingStorage.Checkpoint(block.timestamp, amount));
        } else {
            LibStakingStorage.Checkpoint storage old = checkpoints[checkpoints.length - 1];
            old.amount = amount;
        }
    }

    // _updateLockedStakeborgToken stores the new `amount` into the stakeborg token locked history
    function _updateLockedStakeborgToken(uint256 amount) internal {
        LibStakingStorage.Storage storage ds = LibStakingStorage.daoStakingStorage();

        if (ds.stakeborgTokenStakedHistory.length == 0 || ds.stakeborgTokenStakedHistory[ds.stakeborgTokenStakedHistory.length - 1].timestamp < block.timestamp) {
            ds.stakeborgTokenStakedHistory.push(LibStakingStorage.Checkpoint(block.timestamp, amount));
        } else {
            LibStakingStorage.Checkpoint storage old = ds.stakeborgTokenStakedHistory[ds.stakeborgTokenStakedHistory.length - 1];
            old.amount = amount;
        }
    }
}
