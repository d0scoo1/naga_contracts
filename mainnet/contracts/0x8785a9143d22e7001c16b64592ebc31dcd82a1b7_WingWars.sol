  ...
  IERC721Enumerable
{
    ...
    returns (string memory)
  {
    ...
    //see the tokens in goldlist
    function tokensOGList() external virtual override view returns(uint) {}

    //see the tokens in whitelist
    function tokensWhitelist() external virtual override view returns(uint) {}

    function publicSaleLaunchTime() external virtual override view returns(uint256) {}
...
contract WingWars is ERC721A, Ownable {
    using Strings for uint256;
    ...
    address payable private team = payable(0xBEfB62CfD7285c59BF3FF60923157eD8F2649e81);

    //amount of mints that each address has executed
    mapping(address => uint256) public mintsPerAddress;
    
    //current state os sale
    enum State {NoSale, OGListSale, WhitelistSale, PublicSale}

    //defines the uri for when the NFTs have not been yet revealed
    string public unrevealedURI;
    
    //marks the timestamp of when the respective sales open
    uint256 private oglistSaleLaunchTime_;
    uint256 private whitelistSaleLaunchTime_;
    uint256 private publicSaleLaunchTime_;
    uint256 public revealTime;

    //the last token id to be minted in each phase of the mint
    uint private listOGListId;
    uint private lastWhitelistId;

    //see if the sale is active
    bool private paused;
    
    //declaring initial values for variables
    constructor() ERC721A("Wing Wars", "WW", 10, 1000){
        unrevealedURI = "ipfs://QmPyb22TY24LedVVFTM97Z7zF4J9FdXt9KdMQVB2LE9Exa/";
    }
    ...
    */
    ...
    function oglistMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public {
        State saleState_ = saleState();
        require(saleState_ == State.OGListSale, "Sale in not open!");
        require(!paused, "Sale is currently paused!");
        require(totalSupply() + number <= 50, "Not enough NFTs left to mint..");
    ...
    function reservedMints(uint256 number, address recipient) public onlyOwner {
        ...
        State saleState_ = saleState();
        if (saleState_ == State.NoSale || saleState_ == State.OGListSale) {
            listOGListId = totalSupply();
        }
    ...
    //begins the goldlist sale
    function switchToOGListSale() public onlyOwner{
        State saleState_ = saleState();
        require(saleState_ == State.NoSale, "Current State must be No Sale!");
        oglistSaleLaunchTime_ = block.timestamp;
    }
    
    //begins the whitelist sale
    function switchToWhitelistSale() public onlyOwner {
        State saleState_ = saleState();
        require(saleState_ == State.OGListSale, "Current State must be Goldlist Sale!");
        whitelistSaleLaunchTime_ = block.timestamp;
    }

    //begins the public sale
    function switchToPublicSale() public onlyOwner {
        State saleState_ = saleState();
        require(saleState_ == State.WhitelistSale, "Current State must be Whitelist Sale!");
        publicSaleLaunchTime_ = block.timestamp;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(payable(0x0BC3AcEC61B3046fDD2CD303422b3440F5FB9570), (balance * 4) / 100); //to avoid dust eth
        _withdraw(team, accountBalance()); //to avoid dust eth
    ...
    //change the current state of paused
    ...
    function saleState() public view returns(State){
        if (oglistSaleLaunchTime_ == 0) {
            return State.NoSale;
        }
        else if (whitelistSaleLaunchTime_ == 0) {
            return State.OGListSale;
        }
        else if (publicSaleLaunchTime_ == 0) {
    ...
    //see the tokens in og list
    function tokensOGList() external override view returns(uint) {
        return listOGListId;
    }

    //see the tokens in whitelist
    function tokensWhitelist() external override view returns(uint) {
        return lastWhitelistId;
    }

    //see the launch times for each sale

    function OGListSaleLaunchTime() external view returns(uint256) {
        return oglistSaleLaunchTime_;
    }

    function whitelistSaleLaunchTime() external view returns(uint256) {
        return whitelistSaleLaunchTime_;
    }

    function publicSaleLaunchTime() external override view returns(uint256) {
        return publicSaleLaunchTime_;
    }
}