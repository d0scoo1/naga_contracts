...
contract STARSEASNFT is BEP721,Ownable {
	...
	) public BEP721(name, symbol) {}
	
	function getNFTData(uint _tokenId) view public returns (
		address
        ,uint256
	) {		
	   return(
			nftData[_tokenId].owner
			,nftData[_tokenId].royalties
		);
    }
    ...
    function mint(
		...
		require(royalties <= 1200, "Royalties: Max 12%");
		...
		nftData[_tokenId].royalties = royalties;		
