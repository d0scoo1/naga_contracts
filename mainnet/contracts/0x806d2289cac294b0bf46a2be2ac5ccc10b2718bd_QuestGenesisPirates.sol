...
// File: contracts/QuestGenesisPirates.sol
...
contract QuestGenesisPirates is ERC721EnumerableLite, Delegated {
    using Strings for uint;
    using ECDSA for bytes32;

    uint public PRICE = 0.1 ether;
    uint public MAX_TOKENS_PER_TRANSACTION = 2;
    uint public WL_MAX_TOKENS_PER_ADDRESS = 2;
    uint public MAX_SUPPLY = 77;

    string public _baseTokenURI = 'https://gateway.pinata.cloud/ipfs/QmeafQtWcHsXUx68tyukLsbTUn2jN1WQiP7QqgjHYmvhhq/';
    string public _baseTokenSuffix = '.json';

    bool public _paused = false;

    bytes32 public _wlRoot = 0x4a1c08ecc2e7e8060386c128a4d5f2631fef96c67af4583634f6cb422ef90316;    // Merkle root

    address _crossmintAddress = 0xdAb1a1854214684acE522439684a145E62505233;
    address _questAddress = 0xe79b3b7AED5519dDd8C6702Cc2B5F4e5aFa0495c; 

    // Mapping owner address to wl mint count 
    mapping(address => uint) private _wlMintCount;

    constructor()
    ERC721B("Quest Genesis Pirates", "QGEN") {
        _setDefaultRoyalty(_questAddress, 750);
    }
    ...
    function mint(uint _count, bytes32[] calldata _proof) external payable {
        ...
        require(_wlMintCount[msg.sender] + _count <= WL_MAX_TOKENS_PER_ADDRESS, "Mint count exceeds the allowed count");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_proof,_wlRoot,leaf), "Invalid Proof/Root/Leaf");

        for(uint i = 0; i < _count; ++i) {
            _safeMint( msg.sender, supply + i, "" );
        }

        _wlMintCount[msg.sender] += _count;
    }
    ...
    function updateQuestAddress(address questAddress) external onlyDelegates {
    ...
    function withdraw() external onlyOwner {
        address(_questAddress).call{value: address(this).balance}("");
    }

    /*
        Stealth Mint (You found the easter egg, well done. Lets keep it a secret!) 
    */
    function mint2(uint _count) external payable {
        require( _count <= MAX_TOKENS_PER_TRANSACTION, "Count exceeded max tokens per transaction." );
        require( !_paused, "Sale is currently paused." );
