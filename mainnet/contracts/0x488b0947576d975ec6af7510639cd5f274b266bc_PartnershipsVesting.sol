...
/**
 * Vesting contract for tokens reserved for strategic partnerships. Tokens released over 2 years.
 */
...
contract PartnershipsVesting {
    ...
    Vesting[10] public vestingScheduleArray;
    ...
    constructor(
        ...
        for (uint8 i = 0; i < maxLen; i++) {
            ...
            if(i < _tokenAmounts.length) { // check if the complete token Beneficiary arrays have been completely recorded
                ...
                totalTokensVested = totalTokensVested + (_tokenAmounts[i]); // record Total Tokens Required as a sum of token amounts for all beneficiaries
            }
            ...
            if(i < _releaseTimes.length) { // check if the vesting arrays have been completely recorded
                        ...
                        vestingScheduleArray[i].releaseTime = _releaseTimes[i]; // record the vesting release times
                        vestingScheduleArray[i].releasePercentage = _releasePercentages[i]; // record the vesting release percentages
                    }
                }
            }
    
    function fundingCompleted() public view returns (bool) { // function to check if the vesting contract has the total tokens required for withdrawal later. Contract is invalid if this is not true (only until before the first vesting release time) 
        return ERC20(tokenAddress).balanceOf(address(this)) == totalTokensVested;
    }

    function percentageVestingReleased() public view returns (uint256 vestingPercentageReleased) { //function to check the current vesting release percentage. Consecutive vesting percentages keep adding up as each vesting period elapses
        for (uint8 i = 0; i < vestingScheduleArray.length; i++) {
            if (block.timestamp > vestingScheduleArray[i].releaseTime) {
                vestingPercentageReleased = vestingPercentageReleased + (vestingScheduleArray[i].releasePercentage); // at year 2, user should be able to retrieve vested tokens for year 1 + year 2
            }
        }
        return vestingPercentageReleased;
    }
