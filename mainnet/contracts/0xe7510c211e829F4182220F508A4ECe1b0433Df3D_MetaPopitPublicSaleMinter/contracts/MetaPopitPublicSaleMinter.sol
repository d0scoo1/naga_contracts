 ...
 */
contract MetaPopitPublicSaleMinter is NftMintingStation {
    ...
    function setPreSaleMintedIds(uint256[] calldata _cacheIds, uint256[] calldata _values) external onlyOwnerOrOperator {
        require(_cacheIds.length == _values.length, "Invalid data");
        for (uint256 i = 0; i < _cacheIds.length; i++) {
             _tokenIdsCache[_cacheIds[i]] = _values[i];
        }
    }
}
