...
→  WHY SPACE JAM?
Space Jam is a deflationary ERC20 token with one goal in mind - to create a playable game with a chance 
to win actual money, with SPACE JAM being the token used to facilitate funds transfers.

→  ARE FUNDS SAFE?
As referenced in our second pinned message, all funds are safe and liquidity lock link is provided below. 
The token itself will have renounced ownership but the actual game contract will be handled through a governance smart contract.
...
contract jAM is ERC20{
    ...
    uint256 private _totalSupply = 8888888 * 10 ** 18;

    constructor () ERC20('COME ON AND SLAM',unicode'🏀SPACEJAM ') {
