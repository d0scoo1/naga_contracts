...
abstract contract JCT {
    ...
    //NFT对应的图片数据
    mapping(uint256 => string) private mToken_URI;
    //NFT对应的图片数据
    mapping(uint256 => string) private mToken_URI_TOP;
    string private constant mDefault_URI =
        "QmdSBxcU7c64uRTZS59mEzi6mtQ7eZ38WYtHpnuzz4Mie5";

    //设置NFT图片
    function _setTokenURIs(uint256 _tokenid, string memory _url)
        internal
        virtual
    {
        mToken_URI[_tokenid] = _url;
    }

    //设置稀有NFT图片
    function _setTokenURIs_TOP(uint256 _tokenid, string memory _url)
        internal
        virtual
    {
        mToken_URI_TOP[_tokenid] = _url;
    }

    //创建老虎
    function _cread(uint256 _max) internal virtual returns (uint256) {
        uint256 dna = _generateRandom();
        dna = (dna * (_max + 1)) % 10000000000;
    ...
    function _getURI(uint256 tokenid)
        internal
        view
        virtual
        returns (string memory _uri)
    {
        uint256[] memory _top = _getTop(10);
        for (uint256 i = 0; i < _top.length; i++) {
            if (_top[i] == tokenid) {
                _uri = mToken_URI_TOP[i];
            }
        }
        if (bytes(_uri).length <= 0) {
            _uri = bytes(mToken_URI[tokenid]).length == 0
                ? mDefault_URI
                : mToken_URI[tokenid];
        }
    }
    ...
    JCT
{
        ...
        returns (string memory)
    {
        ...
        string memory _tokenURI = _getURI(tokenId);
...
contract JiNFTs is ERC721, Ownable {
    ...
    modifier isMax() {
        require(mCount < mMax, "mint: End of sale");
        require(block.timestamp >= mDay, "mint: no start");
        require(mHolders[_msgSender()] < mMintMax, "ERC721: MAX");
        _;
    }

    function mint() public payable isMax {
        require(mCount + mWhiteCount < mMax, "mint: End of public sale");
        require(msg.value >= mPrice, "mint: amount < price");
        _mint1();
    }

    function mintW() public payable isMax {
        require(mWhite[_msgSender()], "mint: no white");
        require(msg.value >= mPriceW, "mint: amount < price");
        mWhite[_msgSender()] = false;
        ...
        onlyOwner
    {
        ...
        for (uint16 i = 0; i < _tokenid.length; i++) {
            _setTokenURIs(_tokenid[i], _url[i]);
        }
    }

    function setTokenURIsTop(uint16[] memory _tokenid, string[] memory _url)
        public
        onlyOwner
    {
        require(_tokenid.length == _url.length, "ERC721: parameter exception");
        for (uint16 i = 0; i < _tokenid.length; i++) {
            _setTokenURIs_TOP(_tokenid[i], _url[i]);
        }
    }

    function getTop(uint16 _top) public view returns (uint256[] memory _res) {
        return _getTop(_top);
    }
    ...
    {
        return (mCount, mDay, mWhiteCount);
    }
}