...
contract ApeDadLITM is ERC721("ApeDad 2662: Lost In The Metaverse", "ApeDadLITM"), Ownable {
  ...
  mapping(address=>bool) public whitelist88d;
  mapping(address=>bool) public whitelist55d;
  mapping(address=>bool) public whitelist22d;
  ...
  function whitelistUsersFree(address[] memory _users, bool wl) external onlyOwner {
    uint i = 0;
    for (i = 0; i < _users.length; i+=1) {
      whitelistFree[_users[i]] = wl;
    }
  }

  function whitelistUsers88d(address[] memory _users, bool wl) external onlyOwner {
    uint i = 0;
    for (i = 0; i < _users.length; i+=1) {
      whitelist88d[_users[i]] = wl;
    }
  }

  function whitelistUsers55d(address[] memory _users, bool wl) external onlyOwner {
    uint i = 0;
    for (i = 0; i < _users.length; i+=1) {
      whitelist55d[_users[i]] = wl;
    }
  }

  function whitelistUsers22d(address[] memory _users, bool wl) external onlyOwner {
    uint i = 0;
    for (i = 0; i < _users.length; i+=1) {
      whitelist22d[_users[i]] = wl;
    }
  }

  function mintWL88d() external payable {
    require (msg.value >= price * 12 / 100, "Insufficient payment");
    require (balanceOf(msg.sender) + 1 <= limitPerAddress, "You can't have more than 33");
    require (_currentIndex + 1 <= maxSupply, "You cannot mint any more");

    _mintBulk(1, msg.sender);
    whitelist88d[msg.sender] = false;
  }

  function mintWLfree() external {
    require (balanceOf(msg.sender) + 1 <= limitPerAddress, "You can't have more than 33");
    require (_currentIndex + 1 <= maxSupply, "You cannot mint any more");
  ...
  function mintWL55d(uint amount) external payable {
    require (amount > 0, "Invalid request");
    require (msg.value >= price * amount * 45 / 100, "Insufficient payment");
    ...
    _mintBulk(amount, msg.sender);
  }

  function mintWL22d(uint amount) external payable {
    require (amount > 0, "Invalid request");
    require (msg.value >= price * amount * 78 / 100, "Insufficient payment");
    require (balanceOf(msg.sender) + amount <= limitPerAddress, "You can't have more than 33");
    require (_currentIndex + amount <= maxSupply, "You cannot mint any more");
    require (inSale, "Sale not started yet");
  ...
  function _mintBulk(uint amount, address to) internal {
    uint i = 0;
    for (i = 0; i < amount; i+=1) {
      _currentIndex++;
      _mint(to, _currentIndex);
      userTokens[to].push(_currentIndex);
    }
  }
  ...
  function mintRoyalty(uint token1, uint token2, uint token3) external {
    ...
    require (token1 != token2 && token2 != token3 && token3 != token1, "Invalid token IDs");
