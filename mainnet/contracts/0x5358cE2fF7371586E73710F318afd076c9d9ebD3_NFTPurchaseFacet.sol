...
contract NFTPurchaseFacet {
     ...
     * Note that `value` may be zero.
     */
    event NFTPurchase(address from, address collectionAddress, uint256 value, uint8 currency, uint16 tokenId);

    /** 
    @notice Purchase a NFT with ETH
    @param collectionAddress Collection's address of the purchased NFT.
    @param tokenId TokenId of the purchased NFT.
     */
    function purchaseNFTWithETH(address collectionAddress, uint256 tokenId) external payable {
        emit NFTPurchase(msg.sender, collectionAddress, msg.value, 0, uint16(tokenId));
    }

    /** 
    @notice Purchase a NFT with MNFT
    @param collectionAddress Collection's address of the purchased NFT.
    @param tokenId TokenId of the purchased NFT.
    @param mnftTokenValue $MNFT amount to transfer for the token purchase.
     */
    function purchaseNFTWithMNFT(address collectionAddress, uint256 tokenId, uint256 mnftTokenValue) external payable {
        ...
        emit NFTPurchase(msg.sender, collectionAddress, mnftTokenValue, 1, uint16(tokenId));
    }
