...
contract LunarCycles is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Lunar Cycles - The Contract is Paused");
        require(
            tokenCount + _amount <= maxSupply,
            "Lunar Cycles - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "Lunar Cycles - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Lunar Cycles - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Lunar Cycles- insufficient ethers"
            );
        }
