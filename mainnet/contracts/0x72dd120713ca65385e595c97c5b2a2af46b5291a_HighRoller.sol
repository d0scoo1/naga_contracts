...
contract HighRoller is ERC721, Ownable{
    ...
    uint256 constant BOUNTY_AMOUNT = 0.259 ether;
    ...
    bytes32 public patronListMerkleRoot;
    bytes32 public goldListMerkleRoot;
    bytes32 public silverListMerkleRoot;
    ...
    constructor() ERC721("HighRoller", "HighRoller") public{
    }
    ...
    function setGoldListMerkleRoot(bytes32 merkleRoot) public onlyOwner {
        goldListMerkleRoot = merkleRoot;
    }

    function setSilverListMerkleRoot(bytes32 merkleRoot) public onlyOwner {
        silverListMerkleRoot = merkleRoot;
    }

    function setPatronListMerkleRoot(bytes32 merkleRoot) public onlyOwner {
        patronListMerkleRoot = merkleRoot;
    }

    function setOwnerMerkleRoot(bytes32 merkleRoot) public onlyOwner {
        ownerMerkleRoot = merkleRoot;
    }

    function ownerSale(uint256 numberOfTokens) public onlyOwner {
    ...
    function goldListSale(bytes32[] memory merkleProof, uint256 numberOfTokens) public payable isValidMerkleProof(merkleProof, goldListMerkleRoot) {
    ...
    function silverListSale(bytes32[] memory merkleProof, uint256 numberOfTokens) public payable isValidMerkleProof(merkleProof, silverListMerkleRoot) {
        require(!MINTING_PAUSED, "Minting is not active");
        require(CURRENT_STAGE == STAGE.PRE_SALE, "Current stage should be PRE_SALE");
        require(totalSupply() < MAX_ELEMENTS, 'All tokens have been minted');
        require(totalSupply() + numberOfTokens < MAX_ELEMENTS, 'Purchase would exceed max supply');
    ...
    function patronListSale(bytes32[] memory merkleProof, uint256 numberOfTokens) public payable isValidMerkleProof(merkleProof, patronListMerkleRoot) {
        require(!MINTING_PAUSED, "Minting is not active");
        require(CURRENT_STAGE == STAGE.PRE_SALE, "Current stage should be PRE_SALE");
        require(totalSupply() < MAX_ELEMENTS, 'All tokens have been minted');
        require(totalSupply() + numberOfTokens < MAX_ELEMENTS, 'Purchase would exceed max supply');
    ...
    function openBounty() public onlyOwner {
        uint256 index = 0;
    ...
    function withdraw(bytes32[] memory merkleProof) public onlyowner(merkleProof) {
