...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(ComptrollerInterface comptroller_,
        ...
        require(err == uint(Error.NO_ERROR), "!interest rate model");
        ...
        require(err == uint(Error.NO_ERROR), "!admin fee");
    ...
    function totalBorrowsCurrent() external nonReentrant(false) returns (uint) {
        require(accrueInterest() == uint(Error.NO_ERROR), "!accrue interest");
        return totalBorrows;
    }
    ...
    function accrueInterest() public returns (uint) {
        ...
        require(borrowRateMantissa <= borrowRateMaxMantissa, "crazy borrow rate");

        /* Calculate the number of blocks elapsed since the last accrual */
        (MathError mathErr, uint blockDelta) = subUInt(currentBlockNumber, accrualBlockNumber);
        require(mathErr == MathError.NO_ERROR, "!block delta");
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        require(vars.mathErr == MathError.NO_ERROR, "!MINT_EXCHANGE_CALCULATION");
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        require(redeemTokensIn == 0 || redeemAmountIn == 0, "!redeemTokensIn or !redeemAmountIn");
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        require(vars.mathErr == MathError.NO_ERROR, "!REPAY_BORROW_NEW_ACCOUNT_BORROW_BALANCE_CALCULATION");

        (vars.mathErr, vars.totalBorrowsNew) = subUInt(totalBorrows, vars.actualRepayAmount);
        require(vars.mathErr == MathError.NO_ERROR, "!REPAY_BORROW_NEW_TOTAL_BALANCE_CALCULATION");
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, CTokenInterface cTokenCollateral) internal returns (uint, uint) {
        ...
        require(amountSeizeError == uint(Error.NO_ERROR), "!LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE");
        ...
        require(seizeError == uint(Error.NO_ERROR), "!token seizure");
    ...
    function _setComptroller(ComptrollerInterface newComptroller) internal returns (uint) {
        ...
        require(newComptroller.isComptroller(), "marker method = false");
    ...
    function _setInterestRateModelFresh(InterestRateModel newInterestRateModel) internal returns (uint) {
        ...
        require(newInterestRateModel.isInterestRateModel(), "marker method = false");
