...
contract TurtleShellIslandsSerum is ERC721Delegated, ReentrancyGuard {
  using Counters for Counters.Counter;

  constructor(address baseFactory, string memory customBaseURI_)
    ERC721Delegated(
      baseFactory,
      "Turtle Shell Islands: Serum",
      "TSIS",
      ConfigSettings({
        royaltyBps: 250,
  ...
  uint256 public constant PRICE = 11000000000000000;
  ...
  function mint(uint256 count) public payable nonReentrant {
    ...
    require(
      msg.value >= PRICE * count, "Insufficient payment, 0.011 ETH per item"
    );
