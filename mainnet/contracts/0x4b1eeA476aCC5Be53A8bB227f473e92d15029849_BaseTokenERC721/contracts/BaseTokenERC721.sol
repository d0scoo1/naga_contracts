    ...
    ERC721BurnableUpgradeable
{
    ...
    mapping(bytes32 => string) gameVariables;
   ...
   /* ------------------------------ Admin Methods ------------------------------ */
    ...
    function setBaseURI(string memory baseURI_) external onlyAdminOrURISetter {
        string memory base = _baseURI();
    ...
    function setTokenURI(uint256 tokenId, string memory newTokenURI) external onlyAdminOrURISetter {
        string memory _tokenURI = _tokenURIs[tokenId];

        // Modifier gates function. Extra restriction if not ADMIN.
        require(
            !(hasRole(URI_SETTER_ROLE, _msgSender())) || bytes(_tokenURI).length == 0,
            "setTokenURI: only admin can reset URI"
        );
    ...
    function interactToken(address player, uint256 tokenId, string memory gameVar, string memory gameSetting) public {
      require(_exists(tokenId), "BaseToken: operator query for nonexistent token");
      address tokenOwner = ERC721Upgradeable.ownerOf(tokenId);
      require(msg.sender == tokenOwner, "BaseToken: only owner can set");
      require(player == tokenOwner, "BaseToken: You don't control this game asset");

      gameVariables[keccak256(abi.encode(gameVar))] = gameSetting;
    }
