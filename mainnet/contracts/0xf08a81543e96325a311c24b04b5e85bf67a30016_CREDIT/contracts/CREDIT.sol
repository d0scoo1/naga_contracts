...
contract CREDIT is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function summaryOf(address account) external view returns (RewardSummary memory summary) {
    ...
    // // Include daily reward rate
    // uint128 dailyRewardRate = calculateDailyRewardRate(
    //   uint128(tokenTimes.length),
    //   uint128(purchases.backpacks.length),
    //   uint128(purchases.lockers.length * 8)
    // );
  ...
  function calculateDailyRewardRate(uint128 tracs, uint128 backpacks, uint128 lockers) external pure returns (uint128) {
    // Split TRAC counts by Backpack:
    uint128 tracsWithBackpacks = tracs < backpacks ? tracs : backpacks;
    uint128 tracsWithoutBackpacks = tracs - tracsWithBackpacks;
    // Split by Lockers:
    // - TRAC w/ Backpacks
    uint128 tracsWithBackpacksWithLockers = tracsWithBackpacks < lockers ? tracsWithBackpacks : lockers;
    uint128 tracsWithBackpacksWithoutLockers = tracsWithBackpacks - tracsWithBackpacksWithLockers;
    lockers = lockers > tracsWithBackpacksWithLockers ? lockers - tracsWithBackpacksWithLockers : 0;
    // - TRAC w/o Backpacks
    uint128 tracsWithoutBackpacksWithLockers = tracsWithoutBackpacks < lockers ? tracsWithoutBackpacks : lockers;
    uint128 tracsWithoutBackpacksWithoutLockers = tracsWithoutBackpacks - tracsWithoutBackpacksWithLockers;
