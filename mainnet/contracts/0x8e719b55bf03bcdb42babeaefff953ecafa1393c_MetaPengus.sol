...
contract MetaPengus is ERC721Enumerable, Ownable{
  ...
  bool public freemintAvail = true;
  ...
  function freemint() public payable {
    require(freemintAvail == true, "free minting is not available");
    require(hasFreeMint(msg.sender), "user does not have a free mint");
    uint256 supply = totalSupply();
    require(supply + 1 <= maxSupply, "max NFT limit exceeded");
    bool minted = addressFreeminted[msg.sender];
    require(minted == false, "1 free mint per winner");
  ...
  function hasFreeMint(address _user) public view returns (bool) {
  ...
  function setfreemintAvail(bool _state) public onlyOwner {
      freemintAvail = _state;
  }
