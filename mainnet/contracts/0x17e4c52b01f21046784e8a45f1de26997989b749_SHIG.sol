...
contract SHIG is Context, IERC20, Ownable {
    ...
    address payable public _devWalletAddress  = payable(0x214e7cBE7bc52fD5B56df1F4cd1784fd5107021f);
 ...
 //     uint256 public firstLiveBlock;
    ...
    uint256 public _onetwentyfive=25;
    uint256 public _BurnFee = _onetwentyfive.div(100);

    uint256 public _taxFee = _onetwentyfive.div(1000000);
    ...
    uint256 public _maxHoldings = _tTotal.div(100); // 1%
    ...
    function setMaxHoldings(uint256 maxHoldings) external onlyOwner() {
        _maxHoldings = maxHoldings * 10**_decimals;
    }
    function setMaxTXEnabled(bool enabled) external onlyOwner() {
        maxTXEnabled = enabled;
    }

    function setMaxHoldingsEnabled(bool statusHolding) external onlyOwner() {
        maxHoldingsEnabled = statusHolding;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(maxHoldingsEnabled){
            if(from == uniswapV2Pair && from != owner() && to != owner() && to != address(uniswapV2Router) && to != address(this)) {
                ...
                require(balance.add(amount) <= _maxHoldings, "Holding amount exceeds the max Holding value.");
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 liq=_liquidityFee;
        if (_liquidityFee == 0) {  liq =1;  }
        uint256 BNBbalance = address(this).balance;
        uint256 devTokens = contractTokenBalance.mul(_devFee).div(liq);
        uint256 liqTokens = contractTokenBalance.mul(_liqFee).div(liq);
        uint256 burnTokens = contractTokenBalance.mul(_BurnFee).div(liq);
  ...
  //      emit MinTokensBeforeSwapUpdated( numTokensSellToAddToLiquidity);
    ...
    function ResetTaxes () public onlyOwner  {
        ...
        _devFee = 10;
        _liqFee = 2;
        _BurnFee = _onetwentyfive.div(100);
        _taxFee = _taxFee = _onetwentyfive.div(100000);
        _previousTaxFee = _taxFee;
