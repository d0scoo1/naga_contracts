...
contract PeaPets is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "PeaPets - The Contract is Paused");
        require(
            totalSupply() + _amount <= maxSupply,
            "PeaPets - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _amount,
                "PeaPets - insufficient ethers"
            );
        }
