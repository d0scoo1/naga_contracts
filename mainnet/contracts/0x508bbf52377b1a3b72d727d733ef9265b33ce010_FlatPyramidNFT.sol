    ...
    IERC721Enumerable
{
    ...
    function setForSale(uint256 _tokenId, uint256 salePrice)
        external
        returns (uint256)
    {
        ...
        itemsForSale.push(
            AuctionItem(newItemId, _tokenId, salePrice, true, "$")
        );
        totalItems++;
    ...
    function buy(uint256 indexId) external payable {
        ...
        require(
            payedPrice >= itemsForSale[indexId].askingPrice,
            "Not enough amount to buy"
        );

        uint256 fee =
            (payedPrice.mul(4000000000000000000)).div(100000000000000000000);
...
contract FlatPyramidNFT is ERC721 {
    ...
    constructor(
        ...
        address _USDT,
        address signer
    ) ERC721("FlatPyramid 3D Models", "FP3DM", msg.sender) {
        FP3D = IERC20(_FP3D);
        USDC = IERC20(_USDC);
        USDT = IERC20(_USDT);
    ...
    function mintNFT(
        ...
        require(
            signatureContract ==
                ecrecover(
                    getSignedHash(keccak256(abi.encodePacked(this, tokenId))),
                    v,
                    r,
                    s
                ),
    ...
    function mintNftAgainstUSDC(
        ...
        require(
            USDC.allowance(_msgSender(), address(this)) >= amount,
    ...
    function mintNftAgainstUSDT(
        ...
        USDT.transferFrom(_msgSender(), ownerContract, amount);
        _mint(player, tokenId);
        _setTokenURI(tokenId, tokenURI);
        return tokenId;
    }

    function mintNftAgainstFP3D(
        address player,
        string memory tokenURI,
        uint256 amount,
        uint256 tokenId,
        ...
        require(
            FP3D.allowance(_msgSender(), address(this)) >= amount,
            "amount exceeds allowance"
        );
