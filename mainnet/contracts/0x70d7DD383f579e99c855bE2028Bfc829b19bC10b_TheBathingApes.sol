...
contract TheBathingApes is ERC721, Ownable, PaymentSplitter {
    ...
    //  Amount of Genesis NFTs
    uint256 public amntGenesisNfts;
    //  Amount of NON Genesis NFTs
    uint256 public amntNonGenesisNfts;
    //  Price per NFT
    uint256 public price;
    //  Price genesis per NFT
    uint256 public priceGenesis;
    //  URI control
    string newURI;
    //  Sale is active
    ...
    //  Genesis mint control
    bool public isGenesisActive;
    
    constructor(address[] memory _payees, uint256[] memory _shares) ERC721("The Bathing Apes", "TBA") PaymentSplitter(_payees, _shares) payable {
        //  Max amount of tokens
        maxAmntTokens = 10000;
        //  Max tokens per transaction
        maxTknPerTxs = 20;      
        //  Price genesis per NFT in wei
        priceGenesis = 100000000000000000 wei;
        //  Deactivate sale
        saleIsActive = false;
        //  Unlock URI
        URIlocked = false;
        ...
        amntGenesisNfts = 500;
        //  Amount of NON Genesis NFTs
        amntNonGenesisNfts = 9500;
        //  Genesis mint control
        isGenesisActive = true;
    }

    //  Set price for non genesis NFTs
    ...
    function mintNFT(uint256 amountTokens) public payable {
        ...
        //  If Genesis mint is active
        if (isGenesisActive) {
            require ((amountTokens.add(checkMintedTokens()) <= amntGenesisNfts) , "You are minting more Genesis NFTs than there are available, mint less tokens!");
            ...
            require (msg.value == (priceGenesis.mul(amountTokens)), "Amount of Ether incorrect, try again.");
        }
        
        //  If Genesis mint is not active
        else {
            ...
            require ((amountTokens.add(checkMintedTokens()) <= amntNonGenesisNfts.add(amntGenesisNfts)), "You are minting more NFTs than there are available, mint less tokens!");
            require (amountTokens <= maxTknPerTxs, "Sorry, the max amount of tokens per transaction is set to 20");
            //  Requires the correct amount of ETH
