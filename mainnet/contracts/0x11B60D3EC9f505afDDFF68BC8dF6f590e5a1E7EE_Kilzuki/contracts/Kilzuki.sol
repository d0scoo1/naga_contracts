...
contract Kilzuki is ERC721A, Ownable, ReentrancyGuard {
    ...
    function freeMint(uint256 _mintAmount) public mintCompliance(_mintAmount) {
        require(
            ...
            "Kilzuki: Free mint phase not open yet!"
        );

        require(
            totalSupply() + _mintAmount <= maxFreeMintSupply,
            "Kilzuki: Exceeds max free mint supply!"
        );
        require(
            _numberMinted(msg.sender) + _mintAmount <= maxFreeMintAmountPerAddr,
            "Kilzuki: Exceeds max free mint per wallet!"
        );
        ...
        mintPriceCompliance(_mintAmount)
    {
        require(
            ...
            "Kilzuki: Exceeds max mint per wallet!"
        );
