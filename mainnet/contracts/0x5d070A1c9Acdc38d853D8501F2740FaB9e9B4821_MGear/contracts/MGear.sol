...
contract MGear is ERC721Enumerable, Ownable {
  ...
  mapping(uint256 => uint256) public tokenDataTokenId;
  ...
  function encodeMgear(
    ...
    if (mwordIndex > 0) {
      ...
      require(!inscribed[mwordIndex], "alr_w");
    }
    ...
    uint256 tokenData = (mgear &
      0x000000000000000000000000000000007FFFFFFFFFFFFFFFFFFFFFFFFFFFFFF8) + rarity;

    require(tokenDataTokenId[tokenData] == 0, "dupe");

    tokenDataTokenId[tokenData] = tokenId;

    return tokenData + (uint256(mwordIndex) << 127) + (uint256(nameFormat) << 140);
  }
  ...
  function upgradeAndEncode(
    ...
    if (rarity < 3) {
      ...
      if (upgradeRoll < rarityToUpgradeRoll[rarity][0] && unrealUpgrades < 16) {
        ...
        return encodeMgear(mgear, UNREAL, mwordIndex, nameFormat, tokenId);
      }
      if (upgradeRoll < rarityToUpgradeRoll[rarity][1] && fabledUpgrades < 64) {
        fabledUpgrades = fabledUpgrades + 1;
        return encodeMgear(mgear, FABLED, mwordIndex, nameFormat, tokenId);
      }
      if (upgradeRoll < rarityToUpgradeRoll[rarity][2] && extraordinaryUpgrades < 128) {
        extraordinaryUpgrades = extraordinaryUpgrades + 1;
        return encodeMgear(mgear, EXTRAORDINARY, mwordIndex, nameFormat, tokenId);
      }
    }
    return encodeMgear(mgear, rarity, mwordIndex, nameFormat, tokenId);
  }
  ...
  function getTransmutationRarity(uint256 hashed, uint256 mpunk) public view returns (uint8) {
    ...
    //normal punk roll out of 64. shift 160 to make sure no collision with meaningful bits
    uint8 roll = uint8((hashed >> 160) & 0x3f);
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    require(ERC721._exists(tokenId), "not_found");
  ...
  function rename(
    ...
    require(ERC721.ownerOf(tokenId) == msg.sender, "own_g");
    if (mwordIndex > 0) {
      require(
        mineableWords.ownerOf(mineableWords.tokenByIndex(mwordIndex - 1)) == msg.sender,
        "own_w"
  ...
  function mint(
    ...
    tokenIdToMGear.push(upgradeAndEncode(hashed, rarity, mwordIndex, nameFormat, tokenId));
  ...
  function transmute(
    ...
    require(mineablePunks.ownerOf(mpunk) == msg.sender, "own_p");
    require(!transmuted[mpunk], "alr_p");
    ...
    uint256 tokenId = tokenIdToMGear.length;
    tokenIdToMGear.push(
      encodeMgear(hashed, getTransmutationRarity(hashed, mpunk), mwordIndex, nameFormat, tokenId)
    );
    ERC721._safeMint(msg.sender, tokenId);
