...
contract gObLiNbUrGeRwtf is ERC721A, Ownable {
	using Strings for uint256;
	...
	constructor(string memory initUnrevealedUriPrefix) ERC721A("gObLiNbUrGeR.wtf", "GBWTF") {
		paused = true;
		cost = 0.005 ether;
		maxSupply = 2_000;
		freeToMintSupply = 1_500;
		maxMintAmountPerAddress = 2;
	...
	function mint(uint256 amount) external payable {
		require(tx.origin == _msgSender(), "gObLiNbUrGeR.wtf: contract denied");
		require(!paused, "gObLiNbUrGeR.wtf: minting is paused");
		require(_totalMinted() + amount <= maxSupply, "gObLiNbUrGeR.wtf: max token supply exceeded");
		require(amount > 0 && _numberMinted(_msgSender()) + amount <= maxMintAmountPerAddress, "gObLiNbUrGeR.wtf: invalid mint amount");

		if (_totalMinted() + amount > freeToMintSupply || _claimedFreeMint(_msgSender())) {
			require(msg.value >= cost * amount, "gObLiNbUrGeR.wtf: insufficient ether");
		} else {
	...
	function airDrop(address[] calldata addresses, uint8[] calldata amounts) external onlyOwner {
		require(addresses.length == amounts.length && addresses.length > 0, "gObLiNbUrGeR.wtf: invalid function arguments");

		uint256 newSupply = _totalMinted();
		for (uint256 i = 0; i < amounts.length; i++) newSupply += amounts[i];

		require(newSupply <= maxSupply, "gObLiNbUrGeR.wtf: max token supply exceeded");
	...
	function withdraw() external onlyOwner {
		...
		require(success, "gObLiNbUrGeR.wtf: failed to transfer");
	}
