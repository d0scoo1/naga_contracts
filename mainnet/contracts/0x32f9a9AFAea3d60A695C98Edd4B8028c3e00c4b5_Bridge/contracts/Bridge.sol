...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    mapping(uint8 => uint256) public _depositFees;
    ...
    /**
        @notice Changes domain deposit fee.
        @notice Only callable by admin.
        @param chainID ID of chain deposit will be bridged to.
        @param newFee Value {_fee} will be updated to.
     */
    function adminChangeDomainFee(uint8 chainID, uint256 newFee) external onlyAdmin {
        require(_depositFees[chainID] != newFee, "Current fee is equal to new fee");
        _depositFees[chainID] = newFee;
    }
    ...
    function deposit(uint8 destinationChainID, bytes32 resourceID, bytes calldata data) external payable whenNotPaused {
        require(msg.value == (_depositFees[destinationChainID] != 0 ? _depositFees[destinationChainID] : _fee), "Incorrect fee supplied");
