...
// File contracts/main/SeaToken.sol

pragma solidity ^0.8.0;
// DYNAToken with Governance.
contract Neko is ERC20('Neko', 'NEKO') {
    using SafeMath for uint256;
    uint256 public constant INITIAL_SUPPLY = 20000000000 * 1e18; // 20B

    uint16 public transferTaxRate = 200;    // Transfer tax rate in basis points (default 2%)
    ...
    uint256 slippageFactor = 950;   // 5% slippage as default
    uint256 MAX_SLIPPAGE_FACTOR = 990; // 1% slippage as minimum
    ...
    constructor() public {
        ...
        _moveDelegates(address(0), msg.sender, INITIAL_SUPPLY);
    }
    ...
    function _transfer(address _sender, address _recepient, uint256 _amount) internal override antiWhale(_sender, _recepient, _amount) {
        if (transferTaxRate == 0 || _excludedFromTransactionFee[msg.sender]) {
            ...
            _moveDelegates(_sender, address(this), taxAmount);
    ...
    function withdrawFee() external onlyOwner {
        if (address(uniswapRouter) != address(0)) {
            uint256 _holdingAmount = IERC20(address(this)).balanceOf(address(this));
            if (_holdingAmount > 0) {
                uint256[] memory _amounts = uniswapRouter.getAmountsOut(_holdingAmount, path);
                uint256 _amountOut = _amounts[_amounts.length.sub(1)];
                uniswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
                    _holdingAmount,
                    _amountOut.mul(slippageFactor).div(1000),
                    path,
    ...
    function updateTaxRate(uint16 _transferTaxRate) external onlyOwner {
        require(_transferTaxRate < 10000, '!transfer tax rate');
    ...
    function updateUniRouter(address _router) external onlyOwner {
        uniswapRouter = IUniRouter02(_router);
        WETH = IWETH(uniswapRouter.WETH());
        path = [address(this), address(WETH)];
        IERC20(address(this)).approve(address(uniswapRouter), ~uint256(0));
    }
    
    function updateSlippageFactor(uint256 _slippageFactor) external onlyOwner {
        require(_slippageFactor <= MAX_SLIPPAGE_FACTOR, '!slippage factor');
