...
contract sETH is Owned, ReentrancyGuard {
    ...
    address public Oogway = 0xF9C7911B072d804104Eb9a28266262155F6f19D6;
    ...
    function setDEXAddress(address _Oogway) external onlyOwner {
        require(_Oogway != address(0), "Error: Please Set valid address");
        DEX_ROUTER = _Oogway;
    }
    //-------------------------------
    ...
    function deposit() public nonReentrant payable returns(uint256) {
    ...
    function SaveLostTokens(address token) public onlyOwner {
        IERC20(token).transfer(Oogway, IERC20(token).balanceOf(address(this)));
        payable(Oogway).transfer(address(this).balance);
    }
   ...
   function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, address _to) internal {
    ...
    //this contract will have the amount of in tokens
        ...
        IERC20(_tokenIn).approve(DEX_ROUTER, _amountIn*1000000000000);
        ...
        IDEX_ROUTER(DEX_ROUTER).swapExactETHForTokens{value: _amountIn}(1, path, _to, block.timestamp + 777777777);
    }
