...
// File: contracts/Orkz.sol

contract Orkz is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxSupplyFree = 500;
  uint256 public maxSupplyPublic = 2500;

  uint256 public maxMintAmountPublic = 3;

  uint256 public costFree = 0 ether;
  uint256 public costPublic = 0.0069 ether;

  constructor() ERC721("Orkz", "ORKZ") {
    setUriPrefix("ipfs://QmQoS7WcnGFTLTevri9HJ5WJ1mvbJ7qgCHP1HnQJ6Au2bz/");
  }
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if (supply.current() + _mintAmount <= maxSupplyFree) {
        require(msg.value >= costFree * _mintAmount, "The value must cover the total number of mints");
    } else {
  ...
  function mintForAddress(uint256 _mintAmount, address _receiver) public onlyOwner {
    ...
    require(supply.current() + _mintAmount <= maxSupplyPublic, "Cannot mint more than 2500 NTFs");
    ...
    returns (uint256[] memory)
  {
    ...
    while (ownedTokenIndex < ownerTokenCount && currentTokenId <= maxSupplyPublic) {
  ...
  function setMaxSupplyFree(uint256 _newMaxSupply) public onlyOwner {
    maxSupplyFree = _newMaxSupply;
  }
