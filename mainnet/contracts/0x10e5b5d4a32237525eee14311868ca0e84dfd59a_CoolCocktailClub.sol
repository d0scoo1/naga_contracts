...
contract CoolCocktailClub is ERC721Enumerable, Ownable {
    ...
    uint256 private _price = 5 * 10**16;  // 0.05 ETH per
    bool public _paused = true;
    uint public constant MAX_ENTRIES = 1000;

    constructor(string memory baseURI) ERC721("Cool Cocktail Club", "CCC")  {
        setBaseURI(baseURI);

        // team gets the first 50 for giveaways
        mint(msg.sender, 50);
    }
    ...
    function mint(address _to, uint256 num) public payable {
        ...
        if(msg.sender != owner()) {
          ...
          require( num < (_maxMint+1),"You can only mint 10 CCC at a time");
          require( msg.value >= _price * num,"Not enough ETH sent");
        }
