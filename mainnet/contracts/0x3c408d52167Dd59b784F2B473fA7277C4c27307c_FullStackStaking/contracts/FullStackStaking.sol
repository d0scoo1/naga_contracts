...
contract FullStackStaking is Ownable, IERC721Receiver, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.UintSet; 
    
    //addresses 
    address public stackerAddress;
    address public floorStakingAddress;
    ...
    constructor(
      address _stacker,
      address _floorStakingAddress,
      uint256 _rate,
      uint256 _expiration
    )
    {
        stackerAddress = _stacker;
        floorStakingAddress = _floorStakingAddress;
        rate = _rate;
...
/* STACKED FARM MECHANICS */
    ...
    function claimRewards(uint256[] calldata tokenIds) public {
      ...
      if (reward > 0) {
        IFloorStaking(floorStakingAddress).mint(msg.sender, reward);
      }
    }
