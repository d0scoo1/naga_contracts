...
contract NymphFeet is ERC721Enumerable, ERC721Burnable, Ownable, ReentrancyGuard, VRFConsumerBase {
  ...
  */
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    return string(abi.encodePacked(_uriTemplate, uint2str((tokenId+randomResult)%100), ".jpg"));
  }

  // mint related functions
  function mint(address[] calldata addresses) public payable nonReentrant {    
    ...
    for (uint8 i=0; i<addresses.length;i++) {
        ...
        _safeMint(addresses[i], nextId);
        emit Mint(addresses[i], nextId);
    }
  }
