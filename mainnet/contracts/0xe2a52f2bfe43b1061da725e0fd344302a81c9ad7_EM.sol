...
Dedicated to the meme king himself Elon Musk $EM
...
contract EM is Context, IERC20, Ownable {
    ...
    constructor (address payable marketing, address payable marketing2) {
        marketingAddress = marketing;
        _isExcludedFromFee[marketing] = true;
        _isExcludedFromFee[marketing2] = true;
    ...
    function setBuyFee(uint256 buyFee) external onlyOwner() {
        require (buyFee <= 10, "buy Fee can't exceed 10%");
        _buyFee = buyFee;
    }

    function setSellFee(uint256 sellFee) external onlyOwner() {
        require (sellFee <= 10, "sell fee can't exceed 10%");
        _sellFee = sellFee;
    }
