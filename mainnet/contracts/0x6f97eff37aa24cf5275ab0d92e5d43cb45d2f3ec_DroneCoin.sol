...
contract DroneCoin is ERC20Token {
    constructor() public
    ERC20Token("DroneCoin", "DRC", 18, 3300000000 * (10 ** 18)) {
    }
    ...
    function lockToken(address account, uint256 amount)
    public
    onlyOwner
    returns (bool) {
        _lock_token(account, amount);
        return true;
    }

    function lockTokenWhole(address account)
    public
    onlyOwner
    returns (bool) {
        _lock_token(account, _balances[account]);
        return true;
    }

    function unlockToken(address account, uint256 amount)
    public
    onlyOwner
    returns (bool) {
        _unlock_token(account, amount);
        return true;
    }

    function _lock_token(address account, uint256 amount) internal {
        require(amount > 0, "LockToken: the amount is empty");

        _balances[account] = _balances[account].sub(amount);
        _locked_balances[account] = _locked_balances[account].add(amount);
        emit TokenLocked(account, amount);
    }

    function _unlock_token(address account, uint256 amount) internal {
        require(amount > 0, "LockToken: the amount is empty");

        _locked_balances[account] = _locked_balances[account].sub(amount);
