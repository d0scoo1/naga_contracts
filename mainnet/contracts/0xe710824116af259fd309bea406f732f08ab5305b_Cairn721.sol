...
contract Cairn721 is Context, ERC721, Ownable{

    address payable public withdrawalAddress;
    address public rocksAddress;

    event MetadataUpdated(string newTokenUriBase);
    event CairnMinted(address sender, uint256 tokenId);
    event TokensBurnt(uint256 [] rocks, address pfpAddress, uint256 pfpId, string collectorsNote);

    constructor(
        string memory givenTokenURIBase,
        address givenRocksAddress
    ) ERC721("UNSHADED CAIRN", "CAIRN" ) {
        _setBaseURI(givenTokenURIBase);
        rocksAddress = givenRocksAddress;
    }
    ...
    function setRocksAddress(address givenRockAddress) public onlyOwner{
        rocksAddress = givenRockAddress;
    }

    //Withdrawal - there will be no price tho
    ...
    function mintCairn( address pfpAddress, uint256 pfpToBurn, uint256[] memory rocksToBurn, string memory collectorsNote) public { 
        require(rocksToBurn.length >= 11, "not enough rocks to burn");
        require(pfpAddress != rocksAddress, "pfp address must be different from rocks address");

        for (uint256 i = 0; i < 11; i++) {
                IERC721(rocksAddress).safeTransferFrom(msg.sender, address(0xdEaD), rocksToBurn[i]);
        }

        IERC721(pfpAddress).safeTransferFrom(msg.sender, address(0xdEaD), pfpToBurn);

        emit TokensBurnt(rocksToBurn, pfpAddress, pfpToBurn, collectorsNote);
        mint();
    }

    function mint() private returns (uint256) {
        emit CairnMinted(msg.sender, totalSupply());
