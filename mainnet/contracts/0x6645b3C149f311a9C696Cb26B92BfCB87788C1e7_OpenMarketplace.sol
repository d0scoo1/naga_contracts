...
contract OpenMarketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder, EIP712, AccessControl, ERC721Enumerable, ERC721URIStorage {
    ...
    mapping(bytes => Order) public orderByAssetId;

    // From ERC721 registry assetId to NewCollection (to avoid asset collision)
    mapping(bytes => NewCollection) public newCollectionByAssetId;

    // From ERC721 registry assetId to Bid (to avoid asset collision)lockedContent
    mapping(bytes => Bid) public bidByOrderId;
    ...
    //royality mapping to particular seller
    mapping(address => uint256) public sellerRoyality;
     ...
     * @param _uri - Duration of the order (in hours)
     */
    function createOrder(uint256 _priceInWei, uint256 _expiresAt, address _currency, string memory _uri, bytes memory _signature, uint256 _royalty, bool _isNewColl, string memory _name, string memory _symbol, string memory _baseURI, string memory _lockedContent) public whenNotPaused {
        _createOrder(_priceInWei, _expiresAt, _currency, _uri, _signature, _royalty, _isNewColl, _name, _symbol, _baseURI, _lockedContent);
    }
    ...
    function cancelOrder(Order memory _order) public whenNotPaused {
        ...
        if (bid.id != 0) {
            _cancelBid(_order, bid.id, bid.bidder, bid.price);
        }
     ...
     */
    function updateOrder(Order memory _order, uint256 _priceInWei, uint256 _expiresAt)
        public whenNotPaused {
        Order storage order = orderByAssetId[_order.signature];
        ...
        emit OrderUpdated(_order.signature, _priceInWei, _expiresAt);
    }
 
    /**
     * @dev Executes the sale for a published NFT
     */
    function safeExecuteOrder(Order calldata _order) public payable whenNotPaused {
        // Get the current valid order for the asset or fail

        Order memory order = orderByAssetId[_order.signature];

        require(order.price == _order.price, "Amount not correct");
        ...
        order.currency == MARKETPLACE_ETHER ? 
            ...
            :
            require(order.price == order.price, "Marketplace: invalid price");
        
        uint256 tokenId = totalSupply();

        if(!order.isNewColl) {
            _mint(msg.sender, tokenId);
            _setTokenURI(tokenId, order.uri);
            sellerRoyality[order.seller]= order.royalty;
            tokenCreator[tokenId] = order.seller;
            userLockedContent[tokenId] = order.lockedContent;
        }
        ...
        // Remove pending bid if any
        Bid memory bid = bidByOrderId[order.signature];
     ...
     */
    function safePlaceBid(Order memory _order, uint256 _priceInWei, uint256 _expiresAt)
        public payable whenNotPaused {

        Order memory order = orderByAssetId[_order.signature];

        order.currency == MARKETPLACE_ETHER ? 
            _createBid(_order, msg.value, _expiresAt)
            :
            _createBid(_order, _priceInWei, _expiresAt);
     ...
     *  can only be canceled by seller or the contract owner
     */
    function cancelBid(Order memory _order) public whenNotPaused {
        Bid memory bid = bidByOrderId[_order.signature];
     ...
     */
    function acceptBid(Order memory _order) public whenNotPaused {
        // check order validity
        // Order memory order = getValidOrder(order);
        ...
        require(order.seller == msg.sender, "Signature invalid or unauthorized");
       ...
       if(!order.isNewColl) {
        ...
        // remove bid
        delete bidByOrderId[order.signature];
     ...
     * @dev Executes the sale for a published NFT
     */
    function _executeOrder(Order memory _order) internal {
        // remove order
        delete orderByAssetId[_order.signature];
     ...
     */
    function _createOrder(uint256 _priceInWei, uint256 _expiresAt, address _currency, string memory _uri, bytes memory _signature, uint256 _royalty, bool _isNewColl, string memory _name, string memory _symbol, string memory _baseURI, string memory _lockedContent) internal {
        ...
        // create the orderId
        // bytes32 orderId = keccak256(abi.encodePacked(block.timestamp, msg.sender, _priceInWei, _uri));

        // save order
        orderByAssetId[_signature] = Order({
            // id: orderId,
            seller: msg.sender,
            ...
            royalty: _royalty,
            isNewColl: _isNewColl,
            lockedContent: _lockedContent
        });

        if(_isNewColl) {
            newCollectionByAssetId[_signature] = NewCollection({
                name: _name,
                symbol: _symbol,
                baseURI: _baseURI
            });
        ...
        emit OrderCreated(_signature, msg.sender, nftToken, _priceInWei, _expiresAt, _currency);
    }
     ...
     */
    function _createBid(Order memory _order, uint256 _priceInWei, uint256 _expiresAt) internal {
        ...
        if (_expiresAt > _order.expiresAt) {
            _expiresAt = _order.expiresAt;
        }
        ...
        // Create bid
        bytes32 bidId = keccak256(abi.encodePacked(block.timestamp, msg.sender, _order.signature, _priceInWei, _expiresAt));

        // Save Bid for this order
        bidByOrderId[_order.signature] = Bid({
            id: bidId,
        ...
        // currectBidCount = currectBidCount.add(1);

        emit BidCreated(bidId, nftToken, order.signature, msg.sender, _priceInWei, _expiresAt);
    }
     ...
     *  can only be canceled by seller or the contract owner
     */
    function _cancelOrder(Order memory _order) internal {
        delete orderByAssetId[_order.signature];
     ...
     */
    function _cancelBid(Order memory _order, bytes32 _bidId, address _bidder, uint256 _escrowAmount) internal {
        delete bidByOrderId[_order.signature];
    ...
    function _hash(Order memory _order) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
        keccak256("Order(address seller,address nftAddress,uint256 price,uint256 expiresAt,uint256 royalty,string uri)"),
    ...
    function _verify(Order memory _order) public view returns (address) {
        bytes32 digest = _hash(_order);
