...
contract ChainFacesHD is ERC721Enumerable, IERC721Receiver {
    ...
    /// @dev Reference to ChainFaces HD - Shrine contract
    Ownable public immutable shrine;
    ...
    // Only if called by shrine owner
    modifier onlyOwner() {
        if(msg.sender != shrine.owner()) {
            revert NotOwner();
        }
        _;
    }
    ...
    constructor(address _renderer, address _elixir, address _cfa, address _cf1, address _shrine) ERC721("ChainFaces HD", "(o_o)") {
        renderer = ChainFacesHDRendererInterface(_renderer);
        elixir = ChainFacesHDElixirInterface(_elixir);
        cfa = ERC721(_cfa);
        cf1 = ERC721(_cf1);
        shrine = Ownable(_shrine);
    }
