...
contract Resolver is Helpers {
    ...
    function getUnclaimedRewards(
        ...
        (reward, ) = staker.getRewardInfo(key, tokenId);
    ...
    function calculateMinAndMaxLiquidity(
        uint256 amount0,
        uint256 amount1,
        address pool
    ) public view returns (uint128 maxLiquidty, uint128 minLiquidty) {
        IUniswapV3Pool poolContract = IUniswapV3Pool(pool);
        (uint160 sqrtRatioX96, int tick,,,,,) = poolContract.slot0();
         {
            int256 Diff = 887272 - (tick > 0 ?  tick : - tick);
            int256 minTick = tick - Diff;
            int256 maxTick = tick + Diff;
            maxLiquidty = LiquidityAmounts.getLiquidityForAmounts(
                sqrtRatioX96,
                TickMath.getSqrtRatioAtTick(int24(minTick)),
                TickMath.getSqrtRatioAtTick(int24(maxTick)),
                amount0,
                amount1
            );
            ...
            // console.logInt(int256(minTick));
            // console.logInt(int256(maxTick));
            // console.log(amount0);
            // console.log(amount1);
        }

        // console.logInt(int256(tick - 60));
        // console.logInt(int256(tick + 60));

       

        minLiquidty = LiquidityAmounts.getLiquidityForAmounts(
            sqrtRatioX96,
            TickMath.getSqrtRatioAtTick(int24(tick - 60)),
            TickMath.getSqrtRatioAtTick(int24(tick + 60)),
            amount0,
            amount1
        );
        ...
        returns (PositionInformation[] memory)
    {
        ...
        for (uint256 i = 0; i < paramArray.length; i++) {
            ...
            positionInfo[i].rewardRate = getRewardsRate(paramArray[i].incentiveKey);
            positionInfo[i].userLiquidity = positions(paramArray[i].tokenId).liquidity;
