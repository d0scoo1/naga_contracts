...
contract ShkreliDao is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string public uriPrefix = "https://shkrelidao.s3.filebase.com/";
    ...
    function freeMint(uint256 _mintAmount) public mintCompliance(_mintAmount) {
        require(
            ...
            "ShkreliDAO: Free mint phase not open yet!"
        );

        require(
            totalSupply() + _mintAmount <= maxFreeMintSupply,
            "ShkreliDAO: Exceeds max free mint supply!"
        );
        require(
            _numberMinted(msg.sender) + _mintAmount <= maxFreeMintAmountPerAddr,
            "ShkreliDAO: Exceeds max free mint per wallet!"
        );
        ...
        mintPriceCompliance(_mintAmount)
    {
        require(
            ...
            "ShkreliDAO: Exceeds max mint per wallet!"
        );
