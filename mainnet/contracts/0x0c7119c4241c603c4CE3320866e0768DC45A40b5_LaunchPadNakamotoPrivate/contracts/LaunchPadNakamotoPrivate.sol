...
contract LaunchPadNakamotoPrivate is Ownable {
    ...
    uint256 public thirdVestingUnlockTimestamp; // December 29, 4h PM UTC
    ...
    function initVestingsTimestamp(
        ...
        require(
            _third > _second && _second > _first && _first > block.timestamp,
            "No good timestamp"
        );
    ...
    function _claimableAmount(address user) internal view returns (uint256) {
        ...
        if (block.timestamp > thirdVestingUnlockTimestamp) {
            ...
            if (_secondVestingAmount[user] > 0) {
                ...
                uint256 _toClaim = claimable[user].mul(multiplier).div(100);
                amount = _toClaim.mul(7000).div(10000);
            }
            ...
            if (_firstVestingAmount[user] > 0) {
                ...
                uint256 _toClaim = claimable[user].mul(multiplier).div(100);
                amount = _toClaim.mul(4000).div(10000);
            }
        }
        return amount;
    }
    ...
    function claim() external returns (bool) {
        ...
        if (!_initialClaimDone[msg.sender]) {
            ...
            _firstVestingAmount[msg.sender] = _toClaim.div(3); // 30% at first vesting (1month later)
            _secondVestingAmount[msg.sender] = _toClaim.div(3);
            _thirdVestingAmount[msg.sender] = _toClaim
                .sub(_firstVestingAmount[msg.sender])
                .sub(_secondVestingAmount[msg.sender]);
        }
        if (
            _thirdVestingAmount[msg.sender] > 0 &&
            block.timestamp >= thirdVestingUnlockTimestamp
