...
contract SinsTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSIN }
    ...
    address public sSIN;
    uint public sSINQueue; // Delays change to sSIN address
    ...
    constructor (
        ...
        isLiquidityToken[ _SINDAI ] = true;
        liquidityTokens.push( _SINDAI );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for SIN
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SIN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SIN ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SIN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISINERC20( SIN ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSIN ).balanceOf( msg.sender ); // Can only borrow against sSIN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SIN
        @param _amount uint
     */
    function repayDebtWithSIN( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISINERC20( SIN ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SIN, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SIN ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SIN ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SIN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SIN decimals
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSIN ) { // 9
            sSINQueue = block.number.add( blocksNeededForQueue );
        } else return false;
