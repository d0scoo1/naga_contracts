...
contract MoverNFTSWS is Initializable, ContextUpgradeable, AccessControlEnumerableUpgradeable, ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable, ERC721PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using AddressUpgradeable for address payable;

    function initialize(string memory name, string memory symbol, string memory baseTokenURI, address custodian, uint256 claimLimit) public virtual initializer {
        __MoverNFTSWS_init(name, symbol, baseTokenURI);
     ...
     * See {ERC721-tokenURI}.
     */
    function __MoverNFTSWS_init(string memory name, string memory symbol, string memory baseTokenURI) internal initializer {
        __Context_init_unchained();
        __ERC165_init_unchained();
        __AccessControl_init_unchained();
        __AccessControlEnumerable_init_unchained();
    ...
    function prefixed(bytes32 addrhash) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked("CLAIM NFT SWS ", addrhash));
    }
