...
contract xxxLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    address private lootAddress = 0x5FbDB2315678afecb367f032d93F642f64180aa3;
    ...
    function pluckWithSuffix(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray, string[] memory suffixArray) internal pure returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
        string memory output = sourceArray[rand % sourceArray.length];
    ...
    function pluckWithPrefix(uint256 tokenId, string memory keyPrefix, string[] memory sourceArray, string[] memory prefixArray) internal pure returns (string memory) {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, toString(tokenId))));
        string memory output = sourceArray[rand % sourceArray.length];
        uint256 greatness = rand % 21;
        if (greatness >= 19) {
