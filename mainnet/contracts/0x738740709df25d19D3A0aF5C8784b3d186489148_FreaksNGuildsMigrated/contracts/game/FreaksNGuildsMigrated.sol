...
contract FreaksNGuildsMigrated is Initializable, Controllable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable{
  ...
  //////////////////////////////////////////////////////////////*/ 
    ...
    /** 
        @notice migrate freaks and celestials then call castle to travel
        @param freakIds list of freak IDs to migrate then travel
        @param celestialIds list of celestial IDs to migrate then travel
        @param fbxAmount amount of FBX to travel
    */
    function migrateAndTravel(uint256[] calldata freakIds, uint256[] calldata celestialIds, uint256 fbxAmount) external {
        require(freakIds.length > 0 || celestialIds.length > 0, "no freaks or celestials to migrate");
        for(uint256 i = 0; i < freakIds.length; i++){
            require(fngOriginal.ownerOf(freakIds[i]) == msg.sender, "you don't own this ser");
            ...
            freakSupply++;
            _mint(address(castle), freakIds[i]);

        }
        for(uint256 i = 0; i < celestialIds.length; i++){
            require(fngOriginal.ownerOf(celestialIds[i]) == msg.sender, "you don't own this ser");
            ...
            celestialSupply++;
            _mint(address(castle), celestialIds[i]);
        }
        castle.travelFromMigration(freakIds, celestialIds, fbxAmount, msg.sender);
    }
    ...
    */
    function getSpecies(uint256 tokenId) external view returns (uint256 species) {
