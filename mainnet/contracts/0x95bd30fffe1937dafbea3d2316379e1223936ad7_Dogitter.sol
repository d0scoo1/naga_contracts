...
contract Dogitter is ERC721, Ownable, ERC721Enumerable {
    using Strings for uint256;
    ...
    uint256 public immutable maxWhitelistMint = 1000;
    uint256 public whitelistMinted;
    mapping(address => int256) public claimedWL;
    bool public canMintWL = false;

    mapping(address => int256) public claimedPL;
    bool public canMintPL = false;

    uint256 public immutable maxMint = 1999;
    ...
    */
    ...
    function mintWL(bytes32[] calldata merkleProof)
        ...
        canMint
    {
        require(canMintWL,"You cannt mint now");
        require(claimedWL[msg.sender] < 2, "Exceeds max per wallet");
        require(whitelistMinted < maxWhitelistMint, "Not enough wl tokens remaining to mint");


        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);
        claimedWL[msg.sender]++;
        whitelistMinted++;
    }

    function mint() public canMint {
        require(canMintPL,"You cannt mint now");
        require(claimedPL[msg.sender] < 1, "Exceeds max per wallet");

        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);
        claimedPL[msg.sender]++;
    }
    ...
    function setCanMintWL() public onlyOwner{
        canMintWL=!canMintWL;
    }

    function setCanMintPL() public onlyOwner{
        canMintPL=!canMintPL;
    }
}