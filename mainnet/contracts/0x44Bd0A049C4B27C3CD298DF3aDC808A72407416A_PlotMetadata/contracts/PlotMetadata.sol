...
contract PlotMetadata is IPlotMetadata, OwnableUpgradeable {
  ...
  */
  ...
  function _formatMetadata(
    ...
    return
      FormatMetadata.formatMetadata(
        ...
        imageURI(tokenId),
        attributes,
        FormatMetadata.formatAdditionalMetadata(
          "animation_url",
  ...
  function animationURI(uint256 tokenId) public view returns (string memory) {
    string memory baseAnimationURI = _baseAnimationURI;
    return
      bytes(baseAnimationURI).length > 0
        ? string(abi.encodePacked(baseAnimationURI, tokenId.toString()))
        : "";
  }
  ...
  */
  ...
  function setBaseAnimationURI(string calldata baseAnimationURI)
    external
    onlyOwner
  {
