...
contract FLUFF is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"Fluffette Rogan";
    string private constant _symbol = unicode"FLUFF";
    
    uint256 private minContractTokensToSwap = 1e9 * 10**9;
    uint8 private constant _decimals = 9;
    uint256 private _taxFee = 1;
    uint256 private _teamFee = 9;
    uint256 private _liquidityFeePercentage = 33;

    uint256 private _buyFee = 9;
    uint256 private _sellFee = 27;
    ...
    function setBuyFee(uint256 buy) external {
        require(_msgSender() == _FeeAddress);
        require(buy <= 7, "Buy fee must be less than 7");
        _buyFee = buy;
    }
    ...
    function setTaxFee(uint256 tax) external {
        require(_msgSender() == _FeeAddress);
        require(tax <= 3, "tax must be less than 3");
        _taxFee = tax;
    }
