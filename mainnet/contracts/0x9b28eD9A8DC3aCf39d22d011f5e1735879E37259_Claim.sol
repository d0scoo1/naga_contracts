...
contract Claim is AccessControl {
    ...
    function seedAllocations (
      ...
      onlyAdmins
    {
      emit AllocationsUpdated (allocations, newRoot);
      allocations = newRoot;
    }
    ...
    {
      require (
        _verify(claimer, total, proof),
        'Incorrect merkle proof'
      );
      require (
        toClaim <= (total - claimed[claimer]),
        'Not claimable'
      );
      _disburse (claimer, toClaim);
      claimed[claimer] += toClaim;
      emit Claimed (claimer, toClaim);
    }
        ...
        returns (uint256)
    {
        require(
            _verify(claimer, total, proof),
            'Not part of allocations'
        );
        return (total - claimed[claimer]);
    }
    ...
    {
      if (amount > 0) {
        IERC20 arbitraryToken = IERC20(tokenAddress);
        require(
          arbitraryToken.transfer(msg.sender, amount),
    ...
    function _verify(
        ...
        returns (bool valid)
    {
        bytes32 leaf = keccak256(abi.encodePacked(claimer, total));
        return MerkleProof.verify(proof, allocations, leaf);
    }
