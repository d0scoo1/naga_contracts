...
// File: contracts\PixelMankind.sol




contract PixelMankind is ERC721Enumerable, Ownable {
    
    using Strings for uint256;

    string private baseURI = "ipfs://QmVxumYi84uFipPX8uwA29eUimCvKP1rpHyD6RhaP54jUR/"; 
    ...
    constructor() ERC721("PixelMankind", "PXM") {
        setBaseURI(baseURI);
        mintPixelHuman(1);
    }
    ...
    function mintPixelHuman(uint256 _mintAmount) public payable {
        uint256 supply = totalSupply();
        require(!paused, "the contract is paused");
        require(_mintAmount > 0 && _mintAmount <= maxMintAmount, "invalid minting amount");
        require(supply + _mintAmount <= maxSupply, "not enough tokens are available");

        if (msg.sender != owner()) {
            require(msg.value >= cost * _mintAmount, "credit not sufficient");
        }
    ...
    function contractURI() public pure returns (string memory) {
        return "https://pixelmankind.io/contract-info.json";
    }
