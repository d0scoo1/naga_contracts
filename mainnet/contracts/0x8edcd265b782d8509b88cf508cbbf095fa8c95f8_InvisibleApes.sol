...
// File contracts/InvisibleApes.sol



pragma solidity ^0.8.0;
contract InvisibleApes is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public freeMintCount = 888;   

    string private _tokenBaseURI = "";

    address private constant devAddr = 0x57DFE7b20dBbf905322af0EC5DE887F1c802e3bA;
    ...
    constructor() ERC721A("InvisibleApes", "InvisibleApe") {
    }
    ...
    function mint(uint256 numberOfTokens) external payable onlyActive nonReentrant() {
        ...
        if(numberOfTokens >= freeMintCount) {
            freeMintCount = 0;
        } else {
    ...
    function costForMint(uint256 _numToMint) public view returns(uint256) {
        if(_numToMint <= freeMintCount) {
            ...
            return price.mul(_numToMint.sub(freeMintCount));
        }
    }
