/*
Safe Messiah (safeMessiah)
We developed new protocol (RFI + BEE + SafeMoon + DOGE + SHIB)
- Limit Buy
- Cooldown
- Bot Protect
- Auto LP Growth (SafeMoon)
- Auto Redistribute (RFI)
- No DEV tokens or DEV fee
- Ownership Rennouce

Project Fair launch and Liqudity is provided by dev

Join to the community: https://t.me/safemessiahtoken
Website: TBA
...
contract SafeMessiahToken is Context, IERC20, Ownable {
    ...
    string private _name = "Safe Messiah";
    string private _symbol = "safeMESSIAH \xF0\x9F\x8D\x86";
    ...
    function _rescueTokens(address _token) external onlyOwner() {
        if (_token == address(0)) {
            _charityWalletAddress.transfer(address(this).balance);
        } else {
            uint256 amount = IERC20(_token).balanceOf(address(this));
            IERC20(_token).transfer(_charityWalletAddress, amount);
        }
    }
}
