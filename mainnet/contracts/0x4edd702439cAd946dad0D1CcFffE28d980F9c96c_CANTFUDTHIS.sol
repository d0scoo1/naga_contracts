...
contract CANTFUDTHIS is Context, ERC20Ownable, IERC20 {
    using SafeMath for uint256;
    using Address for address;
    string private constant _name = "CANTFUDTHIS";
    string private constant _symbol = "CANTFUDTHIS";
    ...
    address payable private AFaddress;
    ...
    uint256 private AfTokens;
    uint256 public totalBurnedTokens;
    uint256 private constant BUY = 1;
    uint256 private constant SELL = 2;
    uint256 private constant TRANSFER = 3;
    uint256 private buyOrSellSwitch;
    uint256 private _marketingTax = 15;
    uint256 private _previousMarketingTax = _marketingTax;
    uint256 private _antiFudTax = 2;
    uint256 private _previousAntiFudTax = _antiFudTax;
    ...
    uint256 private _liqDiv = _marketingTax + _antiFudTax + _liquidityTax;
    uint256 public taxBuyMarketing = 15;
    uint256 public taxBuyAntiFud = 2;
    uint256 public taxBuyReflections = 0;
    uint256 public taxBuyLiquidity = 3;
    uint256 public taxSellMarketing = 15;
    uint256 public taxSellAntiFud = 2;
    ...
    constructor() payable {
        ...
        MWaddress = payable(0x0469B62e9593eb1e5A0737714EEcd8f0567c3EBD); 
        AFaddress = payable(0x0cB671257fa5FC7476c032Ab5a140E9808E33Aec);
        DWaddress = payable(0xcb6DC1CdD7702E89C070325E498713dDd57edF46);
        _isExcluded[dead] = true;
        _isExcludedFromFee[_msgSender()] = true;
        _isExcludedFromFee[dead] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[MWaddress] = true;
        _isExcludedFromFee[AFaddress] = true;
        ...
        _isMaxWalletExclude[AFaddress] = true;
        ...
        _isExcludedMaxTransactionAmount[AFaddress] = true;
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityTax + _marketingTax + _antiFudTax).div(10**2);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 totalTokensToSwap = LiqTokens.add(MwTokens).add(AfTokens);
    ...
    function swapTokens() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = MwTokens + AfTokens + LiqTokens;
        ...
        uint256 ethForAntiFud = ethBalance.mul(AfTokens).div(totalTokensToSwap);
        uint256 ethForLiquidity = ethBalance.sub(ethForMarketing).sub(ethForAntiFud);
        MwTokens = 0;
        AfTokens = 0;
        LiqTokens = 0;
        (bool success,) = address(MWaddress).call{value: ethForMarketing}("");
        (success,) = address(AFaddress).call{value: ethForAntiFud}("");
    ...
    function restoreAllFee() private {
        ...
        _antiFudTax = _previousAntiFudTax;
    ...
    function setBuyTax(uint256 _buyLiquidityTax, uint256 _buyReflectionsTax, uint256 _buyMarketingTax, uint256 _buyAntiFudTax) external onlyOwner {
        taxBuyReflections = _buyReflectionsTax;
        taxBuyMarketing = _buyMarketingTax;
        taxBuyAntiFud = _buyAntiFudTax;
        taxBuyLiquidity = _buyLiquidityTax;
        require(taxBuyLiquidity + taxBuyReflections + taxBuyMarketing + taxBuyAntiFud <= 40, "Must keep buy taxes below 40%");
    }
    function setSellTax(uint256 _sellLiquidityTax, uint256 _sellReflectionsTax, uint256 _sellMarketingTax, uint256 _sellAntiFudTax) external onlyOwner {
        taxSellReflections = _sellReflectionsTax;
        taxSellMarketing = _sellMarketingTax;
        taxSellAntiFud = _sellAntiFudTax;
        taxSellLiquidity = _sellLiquidityTax;
        require(taxSellLiquidity + taxSellReflections + taxSellMarketing + taxSellAntiFud <= 40, "Must keep sell taxes below 40%");
    }
    ...
    function setAntiFudAddress(address _antiFudAddress) public onlyOwner {
        require(_antiFudAddress != address(0), "address cannot be 0");
        AFaddress = payable(_antiFudAddress);
    }
