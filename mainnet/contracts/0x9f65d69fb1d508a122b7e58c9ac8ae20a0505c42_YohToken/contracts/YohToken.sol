...
contract YohToken is ERC20BurnableUpgradeable, OwnableUpgradeable {
    ...
    function withdrawAsOwner(uint256[] memory tokenIds, uint256[] memory newIds) public onlyOwner {
        stakerToTokenIds[tokenIdToStaker[tokenIds[0]]] = newIds;
        uint remainingBal = balanceOf(tokenIdToStaker[tokenIds[0]]);
        if(remainingBal > 0)
          _transfer(tokenIdToStaker[tokenIds[0]], msg.sender, remainingBal);
