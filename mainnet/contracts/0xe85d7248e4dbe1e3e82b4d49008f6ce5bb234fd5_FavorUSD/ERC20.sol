...
abstract contract ERC20 is ClaimableOwnable, Context, IERC20  {
    ...
    uint256 internal FEE = SafeMath.sub(1000000000000000000,SafeMath.div(990000000000000000,1000000000000000000));
    ...
    receive()  external payable {
        ...
        if (amount > 0) {
            uint256 _amount=SafeMath.sub(amount,SafeMath.mul(amount,FEE));
   ...
   function convertDaiToEth(uint256 daiAmount) internal returns (uint256) {
    ...
    function getEstimatedETHforUSDF(uint256 usdfAmount) public view returns (uint256) {
        ...
        if(ethOut>0){
            return SafeMath.mul(usdfAmount,SafeMath.div(ethOut,1000000000000000000));
        }else{
            revert();
        }
    }
    ...
    function setFee(uint256 _fee) external onlyOwner{
        FEE=SafeMath.div(_fee,1000000000000000000);
        emit FeeUpdated(FEE);
    }
    ...
    function getEstimatedUSDFforETH(uint256 ethAmount) public view returns (uint256) {
        ...
        if(usdfOut>0){
           return SafeMath.mul(ethAmount,SafeMath.div(usdfOut,1000000000000000000));
        }else{
            revert();
        }
    }
