...
// File contracts/konnect-kct-v1.sol

// contracts/Token.sol
pragma solidity ^0.8.11;
contract KonnectKCT_V1 is 
    Initializable,
    OwnableUpgradeable,
    ERC777Upgradeable,
    UUPSUpgradeable
{
    ...
    function setAccountLock(address account, bool isLock) public onlyOwner {
        lockedAccounts[account] = isLock;
    }

    function getAccountLock(address account) public view returns (bool isLock) {
    ...
    function _beforeTokenTransfer(address operator, address from, address to, uint256 amount) internal virtual override {
        require(!lockedAccounts[from], 'locked account send prohibited');
        require(!lockedAccounts[to], 'locked account receive prohibited');
