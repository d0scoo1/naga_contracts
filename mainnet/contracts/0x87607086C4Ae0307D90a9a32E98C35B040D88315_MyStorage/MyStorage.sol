...
contract MyStorage is ERC721, VRFConsumerBase, Ownable {
    ...
    // ERC20 Token
    TokenERC20 immutable erc20Token;
    ...
    constructor(
        ...
        uint256 _erc20initialSupply,
        string memory _erc20tokenName,
        string memory _erc20tokenSymbol
    )
    ...
    ERC721(_nftName, _symbol) payable
    {
        ...
        state = new State();
        erc20Token = new TokenERC20(_erc20initialSupply, _erc20tokenName, _erc20tokenSymbol);
    ...
    function ERC20transfer(address _to, uint256 _value) external returns (bool success) {
        return erc20Token.transfer(_to, _value);
    }

    function ERC20transferFrom(address _from, address _to, uint256 _value) external returns (bool success) {
        return erc20Token.transferFrom(_from, _to, _value);
    }

    function ERC20approve(address _spender, uint256 _value) external onlyOwner returns (bool success) {
        return erc20Token.approve(_spender, _value);
    }

    function ERC20approveAndCall(address _spender, uint256 _value, bytes memory _extraData) external onlyOwner
        returns (bool success) {
        return erc20Token.approveAndCall(_spender, _value, _extraData);
    }

    function ERC20burn(uint256 _value) external onlyOwner returns (bool success) {
        return erc20Token.burn(_value);
    }

    function burnFrom(address _from, uint256 _value) external onlyOwner returns (bool success) {
        return erc20Token.burnFrom(_from, _value);
    }
