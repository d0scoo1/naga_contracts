...
contract FapToken is ERC20Burnable, Ownable, IERC721Receiver, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.UintSet; 
    
    //addresses 
    address nullAddress = 0x0000000000000000000000000000000000000000;
    address public pocketWaifuAddress;

    //uint256's 
    uint256 public expiration; 
    //rate governs how often you receive $FAP
    uint256 public rate; 
    ...
    constructor(
      address _pocketWaifu,
      uint256 _rate,
      uint256 _expiration
    ) ERC20("Fap", "FAP") 
    {
        pocketWaifuAddress = _pocketWaifu; 
        rate = _rate;
...
/* Unstacked farm */
    ...
    function deposit(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            IERC721(pocketWaifuAddress).safeTransferFrom(
                msg.sender,
                address(this),
                tokenIds[i],
                ''
