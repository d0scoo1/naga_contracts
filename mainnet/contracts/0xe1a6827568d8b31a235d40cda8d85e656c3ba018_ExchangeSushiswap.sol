...
contract ExchangeSushiswap is Ownable{
    ...
    ISushi public Sushi;
    address ETH;
    constructor(address _Sushi, address _owner) Ownable(_owner) {
        Sushi = ISushi(_Sushi);
        ETH = Sushi.WETH();
    }
    ...
    function executeSwapping(uint orderType, address[] memory path, uint256 assetInOffered, uint256 assetOutExpected, address to, uint256 deadline) external payable returns (uint[] memory){
        ...
        if(OrderType(orderType) == OrderType.EthForTokens) {
             swapResult = Sushi.swapExactETHForTokens{value:assetInOffered}(assetOutExpected, path, to, block.timestamp + deadline);
        }
        else if (OrderType(orderType) == OrderType.TokensForEth) {
            TransferHelper.safeApprove(path[0], address(Sushi), assetInOffered);
            swapResult = Sushi.swapExactTokensForETH(assetInOffered, assetOutExpected, path, to, block.timestamp + deadline);
        }
        else if (OrderType(orderType) == OrderType.TokensForTokens) {
            TransferHelper.safeApprove(path[0], address(Sushi), assetInOffered);
            swapResult = Sushi.swapExactTokensForTokens(assetInOffered, assetOutExpected, path, to, block.timestamp + deadline);
        }
        return swapResult;
