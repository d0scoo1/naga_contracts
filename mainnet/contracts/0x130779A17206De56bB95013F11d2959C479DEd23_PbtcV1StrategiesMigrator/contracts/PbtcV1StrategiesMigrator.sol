...
contract PbtcV1StrategiesMigrator is Initializable, UUPSUpgradeable, OwnableUpgradeable {
     ...
     *
     */
    function migrateIntoNewCurveGaugeFromOldCurveGauge() external {
        (uint256 pbtcV1Amount, uint256 crvRenBtcWbtcSbtcAmount) = _migrateFromOldUnipoolAndMigratePbtcV1(msg.sender);
     ...
     *
     */
    function migrateIntoIdleAATrancheFromOldCurveGauge() external {
        (uint256 pbtcV1Amount, uint256 crvRenBtcWbtcSbtcAmount) = _migrateFromOldUnipoolAndMigratePbtcV1(msg.sender);
        uint256 pBTCV2SbtcAmount = _depositPbtcV2AndCrvRenBtcWbtcSbtcIntoNewCurveMetapool(pbtcV1Amount, crvRenBtcWbtcSbtcAmount);
     ...
     *
     */
    function migrateIntoIdleBBTrancheFromOldCurveGauge() external {
        (uint256 pbtcV1Amount, uint256 crvRenBtcWbtcSbtcAmount) = _migrateFromOldUnipoolAndMigratePbtcV1(msg.sender);
        uint256 pBTCV2SbtcAmount = _depositPbtcV2AndCrvRenBtcWbtcSbtcIntoNewCurveMetapool(pbtcV1Amount, crvRenBtcWbtcSbtcAmount);
    ...
    function _migrateFromOldUnipoolAndMigratePbtcV1(address _owner) internal returns (uint256, uint256) {
        uint256 amount = IGaugeDeposit(OLD_PBTC_SBTC_CRV_GAUGE_DEPOSIT).balanceOf(_owner);
        _removePbtcV1FromUnipoolAndCollectRewards(amount, _owner);
