...
contract MaximusToken is IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xb97873D332cC3dCFdcf3dB7522eEb8BD1649C0Eb),
        treasury: payable(0xE92425900c1A24e0b25AdaDb983693c2f3e5e026)
        });
    ...
    function intializeContract(address _antiSnipe) external onlyOwner {
        ...
        _approve(address(this), address(dexRouter), type(uint256).max);
        _transfer(_owner, DEAD, (_tTotal * 4) / 10);
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 reflect, uint16 liquidity, uint16 marketing, uint16 treasury) external onlyOwner {
        uint16 check = reflect + liquidity + marketing + treasury;
    ...
    function setTaxesSell(uint16 reflect, uint16 liquidity, uint16 marketing, uint16 treasury) external onlyOwner {
        uint16 check = reflect + liquidity + marketing + treasury;
        require(check <= maxBuyTaxes);
    ...
    function setTaxesTransfer(uint16 reflect, uint16 liquidity, uint16 marketing, uint16 treasury) external onlyOwner {
        uint16 check = reflect + liquidity + marketing + treasury;
        require(check <= maxBuyTaxes);
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 treasury) external onlyOwner {
    ...
    function setEthLimitsEnabled(bool enabled) external onlyOwner {
        maxETHTradesEnabled = enabled;
    }
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingBalance = amtBalance - treasuryBalance;
