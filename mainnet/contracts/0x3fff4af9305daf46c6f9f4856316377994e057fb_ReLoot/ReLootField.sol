...
contract ReLootField {
    ...
    function GetSuffix(uint256 tokenId, uint256 idx) external view returns(string memory) {
        int[5] memory vector = _reloot.getEquipmentVector(tokenId, idx);
        if (vector[3]<0){
            return "";
        }
        return _lootGenerator.getFieldSuffix(uint256(vector[3]));
    }
    ...
    function GetNamePrefix(uint256 tokenId, uint256 idx) external view returns(string memory){
        ...
        string memory namePrefix = _lootGenerator.getFieldNamePrefix(uint256(vector[0]));
        namePrefix = string(abi.encodePacked('"', namePrefix, ' ', _lootGenerator.getFieldNameSuffix(uint256(vector[1])), '"'));
        return namePrefix;
    }
