...
contract SVGLD is Context, Ownable, ERC20 {
    // bloot contract is available at https://etherscan.io/address/0xff9c1b15b16263c61d017ee9f65c50e4ae0113d7
    address public svblootContractAddress =
        0x471E9CfcfB031B71813F0F7530696f41c09c4a9c;
    IERC721Enumerable public svblootContract;

    // Give out 10,000 Based Gold for every bloot Bag that a user holds
    uint256 public SVGLDPerTokenId = 10000 * (10**decimals());
    ...
    */
    ...
    constructor() Ownable() ERC20("SV GOLD", "SVGLD") {
        ...
        svblootContract = IERC721Enumerable(svblootContractAddress);
    }
    ...
    function claimById(uint256 tokenId) external {
        ...
        require(
            _msgSender() == svblootContract.ownerOf(tokenId),
            "MUST_OWN_TOKEN_ID"
        );
    ...
    function claimAllForOwner() external {
        uint256 tokenBalanceOwner = svblootContract.balanceOf(_msgSender());
        ...
        for (uint256 i = 0; i < tokenBalanceOwner; i++) {
            ...
            // the _claim() function
            _claim(
                svblootContract.tokenOfOwnerByIndex(_msgSender(), i),
                _msgSender()
            );
        }
    }
    ...
    function _claim(uint256 tokenId, address tokenOwner) internal {
        ...
        _mint(tokenOwner, SVGLDPerTokenId);
    }
    ...
    function daoSetblootContractAddress(address svblootContractAddress_)
        external
        onlyOwner
    {
        svblootContractAddress = svblootContractAddress_;
        svblootContract = IERC721Enumerable(svblootContractAddress);
    }
    ...
    function daoSetSVGLDPerTokenId(uint256 SVGLDDisplayValue)
        public
        onlyOwner
    {
        SVGLDPerTokenId = SVGLDDisplayValue * (10**decimals());
    }

   
    function daoSetSeasonAndSVGLDPerTokenID(
        uint256 season_,
        uint256 SVGLDDisplayValue
    ) external onlyOwner {
        daoSetSeason(season_);
        daoSetSVGLDPerTokenId(SVGLDDisplayValue);
    }
}