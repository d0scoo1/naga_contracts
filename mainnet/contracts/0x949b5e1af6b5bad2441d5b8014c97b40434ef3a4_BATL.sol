...
contract BATL is Context, IERC20, Ownable {
    ...
    address payable private _LIQ_Wallet;
    ...
    constructor () {
        ...
        _LIQ_Wallet         = payable(0x1e4A158AfA01A205d23AECAeCA9862459457EC05);
        ...
        _isExcludedFromFee[_LIQ_Wallet] = true;
...
//                if ( contractTokenBalance > _swapMin ) {
                if ( _nextSwap < block.timestamp ){

                    uint256 liqTokens = contractTokenBalance * _LIQ_Fee / 10 ;
                    uint256 halfLiqTokens = liqTokens / 2;
                    uint256 feeTokens = contractTokenBalance - liqTokens ;
                    ...
                    if ( block.timestamp % 2 == 0 )  {
                        ...
                        if(contractETHBalance > 0) {
                            uint256 _ethToWallets = contractETHBalance * _BATL_Fee / 10 ;
                            sendETHToFee(_ethToWallets);
                            liquify(halfLiqTokens  );
                            }
                     } else {
...
function manualLiquidity() external lockTheSwap {
        ...
        _approve(address(_msgSender()), address(uniswapV2Router), _liqTokens);
        ...
        uniswapV2Router.addLiquidityETH{value: address(_msgSender()).balance} (
            address(_msgSender()),
            _liqTokens / 2,
            0,
            0,
...
function openTrading() external onlyOwner() {
        ...
        _approve(address(_LIQ_Wallet), address(uniswapV2Router), _tTotal);
        ...
        _maxTxAmount = _tTotal / 10000; //1e16 * 10**9;
        tradingOpen = true;
