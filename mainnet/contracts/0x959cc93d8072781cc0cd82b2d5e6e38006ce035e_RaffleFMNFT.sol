...
// File: contracts/RaffleFMNFT.sol

pragma solidity >=0.7.0 <0.9.0;

contract RaffleFMNFT is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxMintAmountSpecial = 100;
  ...
  constructor() ERC721("RaffleFMNFT", "RFMNFT") {
    setHiddenMetadataUri("ipfs://QmcYxdx9NhTzC4PVDNC2VVTpwpCWkLyEyt3TgY3TeQgbcU/notRevealed.json");
  }

  modifier mintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0, "You must mint at least 1 NFTs");
    require(supply.current() + _mintAmount <= maxSupplyPublic, "Cannot mint more than maxSupplyPublic NTFs");
    _;
  }
  ...
  function mint(address _yourwallet, uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if (openPublic) {
      require(ownerTokenCount + _mintAmount <= maxMintAmountPublic, "You cannot own more than maxMintAmountPublic NFTs");
    }
    else if (openPresale) {
      require(isWhitelisted(_yourwallet), "You are not in the whitelist, wait for the public sale");
      require(supplyNow + _mintAmount <= maxSupplyPresale, "Cannot mint more than maxSupplyPresale NTFs in the private sale");
      require(ownerTokenCount + _mintAmount <= maxMintAmountPresale, "You cannot own more than maxMintAmountPresale NFTs");
      cost = costPreSale;
    }
    else if (openSpecial) {
      require(isWhitelisted(_yourwallet), "You are not in the special whitelist, wait for the public sale");
      require(supplyNow + _mintAmount <= maxSupplySpecial, "Cannot mint more than maxSupplySpecial NTFs in the special sale");
      require(ownerTokenCount + _mintAmount <= maxMintAmountSpecial, "You cannot own more than maxMintAmountSpecial NFTs");
      cost = costSpecial;
    }
  ...
  function mintForAddress(uint256 _mintAmount, address _receiver) public onlyOwner {
    require(_mintAmount > 0, "You must mint at least 1 NFTs");
    require(supply.current() + _mintAmount <= maxSupply, "Cannot mint more than maxSupply NTFs");
