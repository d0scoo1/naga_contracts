...
contract Creeps is ERC721Enumerable, Ownable{
	...
	mapping (address => bool) private wihiteList;

    constructor() ERC721("Creepy.club", "Creepy.club")  {
    ...
    function mint(address _to, uint _count) public payable {
        ...
        if (!presalePaused){
            require(inWhiteList(msg.sender), "the Sender is not in the WhiteList");
        }
    ...
    function addInWhiteList(address[] memory _addreses) external onlyOwner {
        for(uint i = 0; i < _addreses.length; i++){
            wihiteList[_addreses[i]] = true;
        }
    }
    
    function deleteFromWhiteList(address[] memory _addreses) external onlyOwner {
        for(uint i = 0; i < _addreses.length; i++){
            wihiteList[_addreses[i]] = false;
        }
    }
