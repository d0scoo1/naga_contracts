...
// File: Tiny Frens2.sol


contract TinyFrens is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxMintPerPresaleAccount = 3;
  uint256 public maxMintPerTx = 3;

  bool public useLimit = true;
  ...
  constructor() ERC721("Tiny Frens", "Tiny Frens") {
  ...
  function mint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if(onlyWhitelisted==true){
      if(useLimit){
        require(whitelistClaimed[msg.sender] + _mintAmount <= maxMintPerPresaleAccount,"Address has already claimed");
      }
      require(balanceOf(msg.sender) + _mintAmount <= maxMintPerPresaleAccount, "Mint limit exceeded." );
      ...
      require(_mintAmount <= maxMintPerTx, "Mint limit exceeded." );
    }
  ...
  function setMaxMintPerPresaleAccount(uint256 _maxMintPerPresaleAccount) public onlyOwner {
    maxMintPerPresaleAccount = _maxMintPerPresaleAccount;
  }
  ...
  function setUseLimit(bool _useLimit) public onlyOwner {
    useLimit = _useLimit;
  }
