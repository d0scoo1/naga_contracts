...
contract PandasHavingFun is ERC721A, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    uint256 public constant maxSupply = 10000;
    uint256 public constant freeMints = 2500;
    uint256 public constant maxFreeMintsPerWallet = 5;
    uint256 public constant maxTotalMintsPerWallet = 10;
    uint256 public reservedAssets = 0;
    uint256 public maxAmountPerTX = 5;
    uint256 public _price = 0.007 ether;
    string public _baseTokenURI;
    string public _uriSuffix = ".json";
    bool public isSaleActive;
    address proxyRegistryAddress;
    address t2 = 0x357a95aa26797f3e82D41A409Fa0D44B5bd7a59f;

    mapping (uint256 => string) private _tokenURIs;
    mapping (address => uint256) private freeMintsWallet;
    mapping (address => uint256) private totalMintsWallet;

    constructor(string memory baseURI, address _proxyRegistryAddress) ERC721A("Pandas Having Fun", "PHFUNS") {
        setBaseURI(baseURI);
        isSaleActive = false;
    ...
    function mintNFT(uint256 numAssets) external payable {
        ...
        if(totalSupply().add(numAssets) > freeMints){
            ...
            if(freeMintsWallet[msg.sender].add(numAssets) > maxFreeMintsPerWallet){
                require(msg.value >= _price.mul(numAssets),
                "Not enough ETH for this purchase! Maybe you forgot to set the price?");
            }else{
                require(freeMintsWallet[msg.sender].add(numAssets) <= maxFreeMintsPerWallet,
                "You can only mint 10 pandas for free!");
    ...
    function withdrawAll() public onlyOwner {
      ...
      uint256 _each = balance / 4;
      require(payable(msg.sender).send(_each+_each+_each),
          "Withdraw SENDER did not work...");
      require(payable(t2).send(_each),
          "Withdraw T2 did not work...");
