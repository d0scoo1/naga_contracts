...
// File: contracts/SwissCryptoApes.sol
...
contract SwissCryptoApes is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 public constant MAX_APES = 1291;
    uint256 public constant MAX_PER_MINT = 20;
    uint256 public constant PRESALE_MAX_MINT = 3;
    uint256 public constant MAX_APES_MINT = 20;
    uint256 public constant RESERVED_APES = 20;    
    ...
    event PresaleMint(address minter, uint256 amountOfApes);
    ...
    constructor() ERC721("Swiss Crypto Apes", "SCA") {
        baseTokenURI = "ipfs://QmebfbM93TU5S1cYhTxyE2aFp8pwsB7Yj1DADYdLuwnCb9/";        
    }

    function claimReserved(address recipient, uint256 amount) external onlyOwner {
        require(reservedClaimed != RESERVED_APES, "Already have claimed all reserved apes");
        require(reservedClaimed + amount <= RESERVED_APES, "Minting would exceed max reserved apes");
        require(recipient != address(0), "Cannot add null address");
        require(totalSupply() < MAX_APES, "All tokens have been minted");
        require(totalSupply() + amount <= MAX_APES, "Minting would exceed max supply");

        uint256 _nextTokenId = numApesMinted + 1;

        for (uint256 i = 0; i < amount; i++) {
            _safeMint(recipient, _nextTokenId + i);
        }
        numApesMinted += amount;
    ...
    function mintPresale(uint256 amountOfApes) external payable whenPresaleStarted {
        require(_presaleEligible[msg.sender], "You are not eligible for the presale");
        require(totalSupply() < MAX_APES, "All tokens have been minted");
        require(amountOfApes <= PRESALE_MAX_MINT, "Cannot purchase this many tokens during presale");
        require(totalSupply() + amountOfApes <= MAX_APES, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender] + amountOfApes <= PRESALE_MAX_MINT, "Purchase exceeds max allowed");
        require(amountOfApes > 0, "Must mint at least one ape");
        require(PRICE_PRESALE * amountOfApes == msg.value, "Eth amount is incorrect");
        ...
        emit PresaleMint(msg.sender, amountOfApes);
    }
    ...
    function mint(uint256 amountOfApes) external payable whenPublicSaleStarted {
        ...
        require(totalSupply() + amountOfApes <= MAX_APES, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender] + amountOfApes <= MAX_APES_MINT, "Purchase exceeds max allowed per address");
        require(amountOfApes > 0, "Must mint at least one ape");
