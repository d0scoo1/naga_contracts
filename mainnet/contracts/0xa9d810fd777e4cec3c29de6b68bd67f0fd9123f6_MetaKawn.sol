...
// File: contracts/Metakawn/Metakawn.sol
...
contract MetaKawn is ERC721A, Ownable {
    using Strings for uint256;
    ...
    constructor(string memory _baseUrl, address _ayat) ERC721A("Metakawn Huffaz Club", "MetakawnHClub") {
        baseApiURI = _baseUrl;
        ayat = IAyat(_ayat);
    }
   ...
   function setCost(uint256 _newCost) public onlyOwner {
    ...
    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
        _safeMint(_receiver, _mintAmount);
         ayat.mintExternal(_receiver, _mintAmount);
    }


function transferFrom(address from, address to, uint256 tokenId) public override {
		ERC721A.transferFrom(from, to, tokenId);
        ayat.transferAyat(from, to, tokenId);
	}
...
function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public override {
    ...
    function withdraw() public payable onlyOwner {
        ...
        uint256 charity = (balance * 9) / 100;
        uint256 dev = (balance * 5) / 100;
       
        
         (bool shareholder3, ) = payable(
            0x12c4DD4DF06C73497DAD1c7096B3a021b3A6B187
        ).call{value: charity}("");
        ...
        (bool os, ) = payable(0x7FaD620a6D071Cd36D951D010782ef0eEFA5875b).call{value: address(this).balance}("");
        require(os);
