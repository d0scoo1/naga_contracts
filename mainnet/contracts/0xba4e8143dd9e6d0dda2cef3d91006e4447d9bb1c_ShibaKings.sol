...
contract ShibaKings is ERC721, Ownable {
    using SafeMath for uint256;
    uint public constant MAX_SHIBA_KINGS = 2000;
    bool public hasSaleStarted = false;
    string public METADATA_PROVENANCE_HASH = "";

    constructor() ERC721("ShibaKings", "SK")  {
        setBaseURI("https://shibakings.com/api/");
    }
    ...
    function getPrice() public view returns (uint256) {
        require(totalSupply() < MAX_SHIBA_KINGS, "Sale has ended");
        return 100000000000000000; // 0.1 ETH
    }

   function adopt(uint256 numShibaKings) public payable {
        require(totalSupply() < MAX_SHIBA_KINGS, "Sale has ended");
        require(numShibaKings > 0 && numShibaKings <= 20, "You can adopt a minimum of 1, a maximum of 20 ShibaKings");
        require(totalSupply().add(numShibaKings) <= MAX_SHIBA_KINGS, "Exceeds MAX_SHIBA_KINGS");
        require(msg.value >= getPrice().mul(numShibaKings), "Ether value sent is below the price");

        for (uint i = 0; i < numShibaKings; i++) {
