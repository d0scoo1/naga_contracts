...
contract Bonding is Ownable, ReentrancyGuard, Pausable {
    ...
    IUniswapV2Pair public simpWETHUniswapPair;
    ...
    constructor(
        ...
        simpWETHUniswapPair = IUniswapV2Pair(_simpWETHUniswapPair);
    ...
    function getSimpPrice(uint256 amount) public view returns (uint256){
        (uint Res0, uint Res1,) = simpWETHUniswapPair.getReserves();
        // amount * SIMP / wETH
        return ((amount * Res0) / Res1);
        // return amount of SIMP that can be bought with amount wETH
    }

    /* ========== MUTATIVE FUNCTIONS ========== */
    function bond(uint256 amount) external nonReentrant whenNotPaused {
        require(availableTokens > 0, "There are not enough tokens available for bonding");
        require(amount > 0, "Cannot stake 0");
        address bonder = msg.sender;

        //get the current wETH/SIMP price
        uint256 currentExchangeRate = getSimpPrice(10 ** 18);

        uint256 simpReceived = amount.mul(currentExchangeRate).mul(discountRateDenominator).div(discountRateNumerator).div(10 ** 18);

        require(simpReceived < availableTokens, "There are not enough tokens available to bond");

        availableTokens -= simpReceived;
        ...
        depositToken.transferFrom(bonder, address(this), amount);
    ...
    function withdraw(uint256 id) public nonReentrant {
        ...
        uint256 simpReceived = amount.mul(oldExchangeRate).mul(oldDenominator).div(oldNumerator).div(10 ** 18);

        _balances[msg.sender][id].isWithdrawn = true;

        bondToken.transfer(msg.sender, simpReceived);
        emit Withdrawn(msg.sender, simpReceived);
    }

    /* ========== RESTRICTED FUNCTIONS ========== */
    function addBondToken(uint256 amount) external onlyOwner {
