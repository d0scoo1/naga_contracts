...
library SplitWithdrawals {
    ...
    modifier onlyWhenInitialized(bool initialized) {
        require(initialized, 'SplitWithdrawals/not-initialized');
        _;
    }
    ...
    function initialize(Payout storage _payout) external {
        ...
        require(_payout.recipients.length > 0, 'SplitWithdrawals/recipients');
        require(_payout.recipients.length == _payout.splits.length, 'SplitWithdrawals/mismatch-length');

        uint16 _total = 0;
        for (uint8 i = 0; i < _payout.splits.length; i++) {
            _total += _payout.splits[i];
        }
        require(_total == _payout.BASE, 'SplitWithdrawals/invalid-total');
    ...
    function updateWithdrawalRecipient(Payout storage _payout, address _recipient)
        external
        onlyWhenInitialized(_payout.initialized)
    {
        require(_recipient != address(0), 'SplitWithdrawals/zero-recipient');
        require(_recipient != address(this), 'SplitWithdrawals/self-recipient');
        ...
        require(_found, 'SplitWithdrawals/invalid-sender');
    }
