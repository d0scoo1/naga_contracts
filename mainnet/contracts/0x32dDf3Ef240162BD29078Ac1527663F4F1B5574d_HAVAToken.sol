/*
 ...
 Join us on telegram at https://t.me/HavaToken
...
contract HAVAToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0xbA70B36c9E6D3D52180DEad10e02ECc908BfeC31); // Marketing Address
        
    address payable public developerAddress =
        payable(0x0F2637141AB4aFb85B3554640E24B2406F53Ffec); // developer Address
        
    address payable public liquidityAddress =
        payable(0xbA70B36c9E6D3D52180DEad10e02ECc908BfeC31); // Liquidity Address
    ...
    string private constant _name = "HAVA Token";
    string private constant _symbol = "$HAVA";
    ...
    uint256 private _developerTokensToSwap = 5000;
    ...
    event SetBuyFee(uint256 marketingFee, uint256 liquidityFee, uint256 reflectFee, uint256 buyDeveloperFee);
    
    event SetSellFee(uint256 marketingFee, uint256 liquidityFee, uint256 reflectFee, uint256 sellDeveloperFee);
    ...
    event UpdatedDeveloperAddress(address developerAddress);
    ...
    constructor() {
        ...
        maxTransactionAmount = _tTotal * 1 / 1000; // 0.5% maxTransactionAmountTxn
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _developerTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyDeveloperFee;
                buyOrSellSwitch = BUY;
            } 
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellDeveloperFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_developerTokensToSwap).add(_marketingTokensToSwap);
        ...
        uint256 bnbForDeveloper = bnbBalance.mul(_developerTokensToSwap).div(totalTokensToSwap);
        
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForDeveloper);

        _liquidityTokensToSwap = 0;
        _marketingTokensToSwap = 0;
        _developerTokensToSwap = 0;
        
        (bool success,) = address(developerAddress).call{value: bnbForDeveloper}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _developerTokensToSwap += tLiquidity * _buyDeveloperFee / _liquidityFee;
            ...
            _developerTokensToSwap += tLiquidity * _sellDeveloperFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyDeveloperFee)
        external
        onlyOwner
    {
        ...
        _buyDeveloperFee = buyDeveloperFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDeveloperFee <= 20, "Must keep taxes below 20%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buyDeveloperFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellDeveloperFee)
        external
        onlyOwner
    {
        ...
        _sellDeveloperFee = sellDeveloperFee;
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDeveloperFee <= 30, "Must keep taxes below 30%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellDeveloperFee);
    }
    ...
    function setDeveloperAddress(address _developerAddress) external onlyOwner {
        ...
        _isExcludedFromFee[developerAddress] = true;
        emit UpdatedDeveloperAddress(developerAddress);
    }
