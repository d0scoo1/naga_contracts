...
interface ICirculatingOTHR {
    function OTHRCirculatingSupply() external view returns ( uint );
}
...
contract OtherUSDCDepository is IBondDepo, Ownable {
    ...
    struct DepositInfo {
        uint value; // Value
        uint payoutRemaining; // OTHR remaining to be paid
    ...
    address public circulatingOTHRContract; // calculates circulating supply
    ...
    constructor ( 
        ...
        address DAOWallet_, 
        address circulatingOTHRContract_
    ) {
        USDC = USDC_;
        OTHR = OTHR_;
        treasury = treasury_;
        ...
        circulatingOTHRContract = circulatingOTHRContract_;
    }
    ...
    function depositWithPermit( 
        ...
        bytes32 s ) 
    external override returns ( bool ) {
        ERC20Permit( USDC ).permit( msg.sender, address(this), amount_, deadline, v, r, s );
        _deposit( amount_, maxPremium_, depositor_ ) ;
        return true;
    }
    ...
    internal returns ( bool ) {
        ...
        IERC20( USDC ).safeTransferFrom( msg.sender, address(this), amount_ );

        uint value_ = amount_.div( 1e9 );
        uint payout_ = calculateBondInterest( value_ );

        require( payout_ >= 10000000, "Bond too small" ); // must be > 0.01 OTHR
        require( payout_ <= getMaxPayoutAmount(), "Bond too large");

        totalDebt = totalDebt.add( value_ );

        // Deposit token to mint OTHR
        IERC20( USDC ).approve( address( treasury ), amount_ );
        ITreasury( treasury ).depositReserves( amount_ ); // Returns OTHR
        ...
        IERC20( OTHR ).safeTransfer( DAOWallet, profit_.sub( DAOProfit_ ) );
        IERC20( OTHR ).safeTransfer( DAOWallet, DAOProfit_ );
    ...
    function redeem() external override returns ( bool ) {
        ...
        if ( percentVested_ >= 10000 ) { // if fully vested
            ...
            IERC20( OTHR ).safeTransfer( msg.sender, payoutRemaining_ );
            totalDebt = totalDebt.sub( value_ );
            return true;
        }

        // calculate and send vested OTHR
        uint payout_ = payoutRemaining_.mul( percentVested_ ).div( 10000 );
        IERC20( OTHR ).safeTransfer( msg.sender, payout_ );
    ...
    function getMaxPayoutAmount() public view returns ( uint ) {
        uint circulatingOTHR = ICirculatingOTHR( circulatingOTHRContract ).OTHRCirculatingSupply();

        uint maxPayout = circulatingOTHR.mul( maxPayoutPercent ).div( 10000 );
    ...
    /**
        @notice calculate amount of OTHR available for claim by depositor
        @param depositor_ address
        @return uint
     */
    ...
    function _calcDebtRatio() internal view returns ( uint _debtRatio ) {   
        ...
        if( useCircForDebtRatio ) {
            supply = ICirculatingOTHR( circulatingOTHRContract ).OTHRCirculatingSupply();
        } else {
            supply = IERC20( OTHR ).totalSupply();
        }
