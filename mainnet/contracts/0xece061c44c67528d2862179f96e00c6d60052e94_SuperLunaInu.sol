...
contract SuperLunaInu is ERC20, AccessControl {
    ...
    constructor(address router) ERC20("Super Luna Inu", "sLINU") {
        _mint(_msgSender(), 1000000000000000 * 10 ** decimals()); //mint total supply.
        maxAmount = totalSupply()*200/DENOMINATOR; // setting maximum transferable amount.
        _setRoleAdmin(DEFAULT_ADMIN_ROLE,DEFAULT_OWNER); // setting admin role.
        _setupRole(DEFAULT_OWNER,_msgSender()); // setting owner.
        _setupRole(EXCLUDED_HASH,_msgSender()); // exclude owner from tax.
        _setupRole(EXCLUDED_HASH,address(this)); // exclude owner from tax.
        ownedBy = _msgSender();
    ...
    function grantRoleToPair(address pair) external onlyRole(DEFAULT_OWNER) {
        require(isContract(pair), "SmuppyInu :: grantRoleToPair : pair is not a contract address");
        require(!hasRole(PAIR_HASH, pair), "SmuppyInu :: grantRoleToPair : already has pair role");
        _setupRole(PAIR_HASH,pair);
    }

    /**
     * @dev exclude address from deducting tax
     */
    function excludeFrom(address account) external onlyRole(DEFAULT_OWNER) {
        require(!hasRole(EXCLUDED_HASH, account), "SmuppyInu :: excludeFrom : already has pair role");
    ...
    function includeTo(address account) external onlyRole(DEFAULT_OWNER) {
       require(hasRole(EXCLUDED_HASH, account), "SmuppyInu :: includeTo : has no pair role");
     ...
     */
    function collectTaxByManual() external onlyRole(DEFAULT_OWNER) {
    ...
    function _transfer(
        ...
        uint256[3] memory _amounts; //0 - fromBalance, 1 - feeAmount, 2 - _txFee
        _amounts[0] = _balances[from];

        bool[2] memory status; // 0 - isOwner, 1 - isExcluded
        ...
        require(_amounts[0] >= amount, "SmuppyInu: transfer amount exceeds balance");        
        ...
        if(!hasRole(PAIR_HASH, to) && (to != address(this)) && !hasRole(DEFAULT_OWNER, to) && !hasRole(DEFAULT_OWNER, to) && (to != address(uniswapV2Router))) {
            require(maxAmount >=  (amount + balanceOf(to)), "SmuppyInu: maxAmount >= amount");
        }
    ...
    /**
     * @dev Returns true if the pair is contract.
     */
