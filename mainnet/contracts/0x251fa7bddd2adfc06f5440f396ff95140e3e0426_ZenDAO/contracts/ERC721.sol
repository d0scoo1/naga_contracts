...
abstract contract ERC721 is Context, ERC165, IERC721 {
  ...
  function approve(address to, uint256 tokenId) public virtual override {
    ...
    if (
      (_msgSender() != owner) && (isApprovedForAll(owner, _msgSender()) != true)
  ...
  function _safeTransfer(
    ...
    _transfer(from, to, tokenId);
    if (_checkOnERC721Received(from, to, tokenId, _data) != true)
  ...
  function _transfer(
    ...
    if (ERC721.ownerOf(tokenId) != from) revert TokenNotOwned();
    if (to == address(0)) revert ZeroAddress();
