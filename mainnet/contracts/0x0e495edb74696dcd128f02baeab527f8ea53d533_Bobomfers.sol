...
/*                                                                                                   
         ...
         _____     _             ___             
        | __  |___| |_ ___ _____|  _|___ ___ ___ 
        | __ -| . | . | . |     |  _| -_|  _|_ -|
        |_____|___|___|___|_|_|_|_| |___|_| |___|
...
contract Bobomfers is ERC721A {
    ...
    address public constant ownerWallet = 0xcd21866158d7f2F1303C905476d0cEa09b849e11;
    address public constant devWallet = 0xFf7235657b8D083e18A1B59Ca49D1D4785466ce2;
    address public owner;
    string public contractMetadata;
    string public baseTokenURI;
    string erm = "Unauthorized";

    constructor(string memory _contractMetadata, string memory _baseTokenURI) ERC721A("Bobomfers", "BBMF") {
        owner = msg.sender;
    ...
    function mint(uint256 quantity) external payable {
        ...
        uint256 mintCost = (supply <= freeMints) ? 0 : cost;
        if (msg.sender == owner) {
            mintCost = 0;
        } else {
            require(!salePaused, "Sale paused");
            require(quantity <= maxMints, "Exceeded max mints per trx");
        }
    ...
    function getCost() public view returns (uint256) {
        uint256 currentCost = (totalSupply() <= freeMints) ? 0 : cost;
        return currentCost;
    }
    ...
    function setFreeMints(uint256 _newfreeMints) external {
        require(msg.sender == owner, erm);
    ...
    function withdraw() external {
        require(msg.sender == owner, erm);
        payable(devWallet).transfer(address(this).balance * 15 / 100);
