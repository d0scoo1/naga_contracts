...
Telegram: http://t.me/metaverseda0

Official Metaverse DAO token

*/

// Verified using https://dapp.tools

// hevm: flattened sources of src/MDAO.sol
...
contract MetaverseDAO is Ownable, IERC20 {
    ...
    constructor(){
        address _marketingWallet = address(0xe2FDAA18bCE2F26A2c199804e0bD349E76860f9F);
        address _liquidityWallet = address(0xe2FDAA18bCE2F26A2c199804e0bD349E76860f9F);
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "MDAO: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "MDAO: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "MDAO: Sender is blacklisted");
        require(!isBlacklisted[recipient], "MDAO: Recipient is blacklisted");

        require(sender != address(0), "MDAO: transfer from the zero address");
        require(recipient != address(0), "MDAO: transfer to the zero address");
        ...
        require(
            ...
            "MDAO: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "MDAO: approve from the zero address");
        require(spender != address(0), "MDAO: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "MDAO: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "MDAO: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "MDAO: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "MDAO: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "MDAO: DEX pair can not be removed");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "MDAO: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function compound() public {
        require(compoundingEnabled, "MDAO: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "MDAO_DividendTracker";
    string private _symbol = "MDAO_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "MDAO_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "MDAO_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "MDAO_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "MDAO_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("MDAO_DividendTracker: method not implemented");
    }
