...
contract Spells is ERC721Enumerable, Ownable, ReentrancyGuard {

    uint256 public mintPrice = 10000000000000000;  ///0.01 ETH 

    address public devAddress = 0xbEd7F034C2F5F353484c26415EC98CA1e3efB067;
    ...
    string[] private rareActions = [
        "Reanimate dead with Blood of the Fallen",
        "Strengthened Radience of Light",
        "Toxic Blight Monsoon ",
        "Summon Frozen Tundra",
        "Time dialation Repeated Distruction",
        "Metal Hail with Motlen Sundering"
    ];

    constructor() ERC721("Spells (for Adventurers)", "SPELLS") Ownable() {}
    ...
    function getSpellName(uint256 tokenId) public view returns (string memory) {
        require(tokenId < 10001, "Token ID invalid");
        string[3] memory parts;
        uint256 rand0 = random(string(abi.encodePacked(toString(tokenId), "0")));
        parts[0] = spellWord0[rand0 % spellWord0.length];
        uint256 rand1 = random(string(abi.encodePacked(toString(tokenId), "1")));
        parts[1] = spellWord1[rand1 % spellWord1.length];
        uint256 rand2 = random(string(abi.encodePacked(toString(tokenId), "2")));
        parts[2] = spellWord2[rand2 % spellWord2.length];
        return string(abi.encodePacked(parts[0], " ", parts[1], " ", " Tomb of ", parts[2]));
    }

    function getSpellActions(uint256 tokenId) public view returns (string[4] memory) {
        string[4] memory spellActions;

        uint256 rand1 = random(string(abi.encodePacked(toString(tokenId), "1")));
        if (rand1 % 21 == 0) {
            spellActions[0] = rareActions[rand1 % rareActions.length];
        } else {
            spellActions[0] = actions[rand1 % actions.length];
        }
        
        uint256 rand2 = random(string(abi.encodePacked(toString(tokenId), "2")));
        if (rand2 % 21 == 0) {
            spellActions[1] = rareActions[rand2 % rareActions.length];
        } else {
            spellActions[1] = actions[rand2 % actions.length];
        }
        
        uint256 rand3 = random(string(abi.encodePacked(toString(tokenId), "3")));
        if (rand3 % 21 == 0) {
            spellActions[2] = rareActions[rand3 % rareActions.length];
        } else {
            spellActions[2] = actions[rand3 % actions.length];
        }

        uint256 rand4 = random(string(abi.encodePacked(toString(tokenId), "4")));
        if (rand4 % 21 == 0) {
            spellActions[3] = rareActions[rand4 % rareActions.length];
        } else {
            spellActions[3] = actions[rand4 % actions.length];
        }
    ...
    function spellTokenUri(uint256 tokenId) internal view returns (string memory) {
        string[4] memory spellActions = getSpellActions(tokenId);
        ...
        parts[1] = getSpellName(tokenId);

        parts[2] = '</text><text x="10" y="40" class="base">';

        parts[3] = spellActions[0];

        parts[4] = '</text><text x="10" y="60" class="base">';

        parts[5] = spellActions[1];

        parts[6] = '</text><text x="10" y="80" class="base">';

        parts[7] = spellActions[2];
