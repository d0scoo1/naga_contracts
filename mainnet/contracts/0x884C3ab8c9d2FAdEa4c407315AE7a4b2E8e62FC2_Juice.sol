...
contract Juice is ERC20Burnable, Ownable {
    ...
    uint256 internal stakedCount = 0;
    ...
    /**
    * @dev Get Staked Count
    */
    function getStakedCount()
        public
        view
        returns (uint256)
    {
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(
            stakerToTokenIds[msg.sender].length + tokenIds.length <=
                MAX_WALLET_STAKED,
            "Must have less than 50 tokens staked!"
        );
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            totalRewards =
                totalRewards +
                ((block.timestamp - tokenIdToTimeStamp[tokenId]) * emissionRateByTokenId(tokenId));
    ...
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            totalRewards =
                totalRewards +
                ((block.timestamp - tokenIdToTimeStamp[tokenIds[i]]) * emissionRateByTokenId(tokenIds[i]));
        ...
        returns (uint256)
    {
        ...
        return secondsStaked * emissionRateByTokenId(tokenId);
    }
    ...
    function getStaker(uint256 tokenId) public view returns (address) {
        ...
        /**
     * @dev Returns the current emission rate by token id
     */
    function emissionRateByTokenId(uint256 tokenId) public view returns (uint256) {

        if(tokenId <= 4096)
            return 1 * EMISSIONS_RATE;
        if(tokenId > 4096 && tokenId <= 6144)
            return 2 * EMISSIONS_RATE;
        if(tokenId > 6144 && tokenId <= 7168)
            return 3 * EMISSIONS_RATE;
        if(tokenId > 7168 && tokenId <= 7680)
            return 4 * EMISSIONS_RATE;
        if(tokenId > 7680 && tokenId <= 7936)
            return 5 * EMISSIONS_RATE;
        if(tokenId > 7936 && tokenId <= 8064)
            return 6 * EMISSIONS_RATE;
        if(tokenId > 8064 && tokenId <= 8128)
            return 7 * EMISSIONS_RATE;
        if(tokenId > 8128 && tokenId <= 8160)
            return 8 * EMISSIONS_RATE;
        if(tokenId > 8160 && tokenId <= 8176)
            return 16 * EMISSIONS_RATE;
        if(tokenId > 8176 && tokenId <= 8184)
            return 32 * EMISSIONS_RATE;
        if(tokenId > 8184 && tokenId <= 8188)
            return 64 * EMISSIONS_RATE;
        if(tokenId > 8188 && tokenId <= 8190)
            return 128 * EMISSIONS_RATE;
        if(tokenId > 8190 && tokenId == 8191)
            return 256 * EMISSIONS_RATE;

        revert();
    }

    function mintForBurnedNft(address account, uint256 amount) public {
        require(nfTokenAddress == msg.sender, "You can't call this method");
