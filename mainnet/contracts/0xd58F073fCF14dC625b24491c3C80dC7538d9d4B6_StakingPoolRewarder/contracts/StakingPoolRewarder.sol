...
contract StakingPoolRewarder is OwnableUpgradeable, IStakingPoolRewarder {
    ...
    function onReward(
        ...
        (uint256 lastVestedAmount,uint256 newClaimTime, bool allVested) = _calculateWithdrawableFromVesting(user, poolId);
        ...
        if (newUnvestedAmount > 0) {
            uint256 lastUnvestedAmount = (uint256(vestingSchedules[user][poolId].amount)).sub(lastVestedAmount);
