...
pragma solidity ^0.8.0;
contract NFT is ERC721Enumerable, Ownable, INFT, INFTMetadata, ReentrancyGuard {
    using Strings for uint256;

    uint256 public giftSupply;
    ...
    constructor(
        ...
        maxSupply = giftSupply + publicSupply;
        _tokenBaseURI = _uri;
        _tokenRevealedBaseURI = _uri;
    }
    ...
    */
    ...
    function purchase(uint256 numberOfTokens) external override payable nonReentrant {
        ...
        */
        require(totalPublicSupply < publicSupply, 'Purchase would exceed PUBLIC');
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            */
            if (totalPublicSupply < publicSupply) {
                /**
    ...
    function purchaseAllowList(uint256 numberOfTokens) external override payable nonReentrant {
        require(isActive, 'Contract is not active');
        require(isAllowListActive, 'Allow List is not active');
        require(_allowList[msg.sender], 'You are not on the Allow List');
        require(totalSupply() < maxSupply, 'All tokens have been minted');
        require(numberOfTokens <= allowListMaxMint, 'Cannot purchase this many tokens');
        require(totalPublicSupply + numberOfTokens <= publicSupply, 'Purchase would exceed PUBLIC');
    ...
    function gift(address[] calldata to) external override onlyOwner nonReentrant{
        require(totalSupply() < maxSupply, 'All tokens have been minted');
        require(totalGiftSupply + to.length <= giftSupply, 'Not enough tokens left to gift');
