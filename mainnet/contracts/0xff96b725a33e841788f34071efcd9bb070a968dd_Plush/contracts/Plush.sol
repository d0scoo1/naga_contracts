...
// File erc721a/contracts/ERC721A.sol@v3.2.0
...
/**
 *
 * @title Plush NFT contract
 * @notice This contract implements the Plush NFT, which is the first animated movie funded by its community
 */
contract Plush is ERC721A, AccessControlEnumerable, Ownable {
    using Strings for uint256;
    ...
    //Role allowing to manage minter role
    bytes32 public constant GOVERNOR_ROLE = keccak256("GOVERNOR_ROLE");
    //Role allowing to perform mint on the contract
    ...
    //Uri used when the NFTs are still not revealed
    string public unrevealedBaseURI;

    //Indicating if the NFTs have been revealed
    bool public revealed = false;

    constructor() ERC721A("Plush", "PLUSH") {
        _safeMint(msg.sender, 1); // Mint one NFT to safely configure OpenSea
        _setRoleAdmin(MINTER_ROLE, GOVERNOR_ROLE); //Governor role can manage minter role
        _setRoleAdmin(GOVERNOR_ROLE, ADMIN_ROLE); // Admin role can manage governor role
        _grantRole(ADMIN_ROLE, msg.sender); //Will be updated after discussion
        setUnrevealedBaseURI(""); //Need to set the good ipfs uri
    }

    //Restrict function to admin role only
    modifier onlyAdmin() {
        if (!hasRole(ADMIN_ROLE, msg.sender)) revert NotAllowed();
        _;
    }

    //Restrict function to minter role only
    modifier onlyMinter() {
        if (!hasRole(MINTER_ROLE, msg.sender)) revert NotAllowed();
        _;
    }
    ...
    //Change the unrevealed URI
    function setUnrevealedBaseURI(string memory _unrevealedBaseURI)
        public
        onlyAdmin
    {
    ...
    //Change the baseURI before the reveal
     ...
     */
    function mintTo(address _to, uint256 _quantity) public onlyMinter {
