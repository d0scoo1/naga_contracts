...
contract KillPutinCoin is Context, IERC20, Ownable {
    ...
    address public _bountyWalletAddress;
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _BountyTax,address routerAddress,address feeaddress,address tokenOwner,address service) public payable {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
		...
		_bountyFee = _BountyTax;
		_previousDevFee = _bountyFee;
        _previousLiquidityFee = _lpFee;
        _antiWhale = (_tTotal * 5 / 1000) * 10 ** _decimals;
        numTokensSellToAddToLiquidity = (_tTotal * 5 / 10000) * 10 ** _decimals;
        _bountyWalletAddress = feeaddress;
    ...
    function setMaxTxPercent(uint256 maxTxPercent) public onlyOwner {
        _antiWhale = maxTxPercent  * 10 ** _decimals;
    }
    
    function setDevWalletAddress(address _addr) public onlyOwner {
        _bountyWalletAddress = _addr;
    }
    ...
    function _takeDev(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[_bountyWalletAddress] = _rOwned[_bountyWalletAddress].add(rDev);
        if(_isExcluded[_bountyWalletAddress])
            _tOwned[_bountyWalletAddress] = _tOwned[_bountyWalletAddress].add(tDev);
    }
    ...
    function _transfer(
        ...
        if(contractTokenBalance >= _antiWhale)
        {
            contractTokenBalance = _antiWhale;
        }
