...
contract CULTOKI is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;

    string private constant _names = "CULTOKI";
    string private constant _ticker = "CULTOKI";
    ...
    uint256 private _previousRewardsTax = _rewardsTax;
    ...
    constructor() payable {
        ...
        MarketingWallet = payable(0xD7A14af7724907860eb57B9BBE05fc8998E88B4B);
        RewardsWallet = payable(0xDA3a5d3b5DD5887506EBD09AD1907af1F08cC2B3);
        DevWallet = payable(0x5623F6262Cce666c2aF8556f33f92879Fb00bc7A);
        ...
        _isMaxWalletExclude[RewardsWallet] = true;
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityTax + _marketingTax + _rewardsTax).div(10**2);
    }
    ...
    function swapTokens() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = tokensForMarketing + tokensForLiquidity + tokensForRewards;
        ...
        uint256 ethForLiquidity = ethBalance.sub(ethForMarketing).sub(ethForRewards);
    ...
    function removeAllFee() private {
        if (_liquidityTax == 0 && _marketingTax == 0 && _rewardsTax == 0 && _reflectionsTax == 0) return;
        _previousLiquidityTax = _liquidityTax;
        _previousMarketingTax = _marketingTax;
        _previousRewardsTax = _rewardsTax;
    ...
    function restoreAllFee() private {
        ...
        _rewardsTax = _previousRewardsTax;
    ...
    function setBuyTax(uint256 _buyMarketingTax, uint256 _buyLiquidityTax, uint256 _buyRewardsTax, uint256 _buyReflectionsTax) external onlyOwner {
        taxBuyMarketing = _buyMarketingTax;
        taxBuyLiquidity = _buyLiquidityTax;
        taxBuyRewards = _buyRewardsTax;
        taxBuyReflections = _buyReflectionsTax;
    }
    function setSellTax(uint256 _sellMarketingTax, uint256 _sellLiquidityTax, uint256 _sellRewardsTax, uint256 _sellReflectionsTax) external onlyOwner {
        taxSellMarketing = _sellMarketingTax;
        taxSellLiquidity = _sellLiquidityTax;
        taxSellRewards = _sellRewardsTax;
