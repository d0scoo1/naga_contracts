...
// File contracts/AltPressCraniumCover.sol





contract AltPressCraniumCover is ERC721, Ownable {

    string public provenanceHash;
    uint public price = 69000000000000000;
    mapping(address => uint256) public reservations;
    bool public isSaleActive = false;
    bool public isReservationActive = false;
    string public baseURL;
    uint256 public totalReserved = 0;
    uint256 public currentMintIndex = 0;

    constructor() ERC721("AltPressCraniumCover", "APCC") {}
    ...
    function setProvenanceHash(string memory ph) public onlyOwner {
        provenanceHash = ph;
    }    
    ...
    function safeMint(address to) public onlyOwner {
        require(currentMintIndex < 489, "cannot mint more than the ax supply");
        _safeMint(to, currentMintIndex);
    ...
    function reserve(uint256 howmany) public payable {
        require(isReservationActive, "reservation must be active");
        require(reservations[msg.sender] == 0, "cannot do a double reservation");
        require(howmany <= 3 && howmany >= 1, "can only reserve between 1 and 3");
        require(totalReserved <= 420, "reservation space is full!");
        require(price * currentMintIndex <= msg.value, "ether value sent is not correct");
    ...
    function mintReserved() public {
        require(currentMintIndex < 489, "sale must be active to mint");
        ...
        for(uint i = 0; i < res; i++) {
            _safeMint(msg.sender, currentMintIndex + i);
        }
        
        currentMintIndex = (res + currentMintIndex);
