...
“When TG and Twitter is created with 300 holders, 
the dev will reveal a HUGE surprise to the community”
...
contract Juneteenth is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "Juneteenth";
    string private constant _symbol = "J19";
    ...
    address payable private _communityAddress = payable(0x416904eFc71cB252e9CDfdF30cbE2BBAD8C86E7C);
    ...
    uint256 public _maxWalletSize = 2e4 * 10**9;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
               if (to != address(this) && from != address(this) && to != _communityAddress && from != _communityAddress ) {
                    require(amount <= _maxTxAmount, "TOKEN: Max Transaction Limit");
                }
            }

            if (to != uniswapV2Pair && to != _communityAddress && to != address(this) && to != deadAddress) {
    ...
    function sendETHToFee(uint256 amount) private {
        _communityAddress.transfer(amount);
    }
