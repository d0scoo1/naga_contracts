...
/**
 * @title MyNFTToken contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract MyNFTToken is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint public constant maxTokenPurchase = 12;
    uint public constant wlMaxTokenPurchase = 12;

    uint256 public MAX_TOKENS;

    // It will be the responsibility of the contract operator not to have those flags true at the same time.
    // But even if they are, nothing really really bad will happen.
    // Front-ends are suggested to check in the order wlSaleIsActive, saleIsActive.
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply, uint256 saleStart) ERC721(name, symbol) {
        uint8 daysUntilEnd = 9;
        MAX_TOKENS = maxNftSupply;
        REVEAL_TIMESTAMP = saleStart + (86400 * daysUntilEnd);
    }
    ...
    /**
     * Change the REVEAL_TIMESTAMP if needs be
     */
    ...
    /*     
    * Set provenance when it is available
    */
    ...
    */
    ...
    /*
    * Pause whitelist sale if active, make active if paused
    */
    ...
    * Mint Tokens
    */
    function mintToken(uint numberOfTokens) external nonReentrant payable {
        require(saleIsActive, "Sale must be active to mint Token");
        require(numberOfTokens <= maxTokenPurchase, "You cannot mint so many tokens per transaction");
    ...
    /**
    * Mint Tokens In Whitelist Sale
    */
    function wlMintToken(uint numberOfTokens, bytes32[] calldata proofs) external nonReentrant payable {
        require(wlSaleIsActive, "Whitelist sale must be active to mint Token");
        require(numberOfTokens <= wlMaxTokenPurchase, "You cannot mint so many tokens per transaction");
        require(totalSupply().add(numberOfTokens) <= MAX_TOKENS, "Purchase would exceed max supply of Tokens");
        require(tokenPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        require(MerkleProof.verify(proofs, _merkleRoot, keccak256(abi.encodePacked(msg.sender))), "Invalid Merkle proof");
    ...
    /**
     * Set manually the startingIndexBlock. The startingIndexBlock must be set (either through code or manually) in order
     * for setStartingIndex to be available.
     */
