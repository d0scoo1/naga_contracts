...
contract MPWRStaking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize(
        ...
        rewardsPerSecForStaking = 31709791970000000;
    ...
    function AdminWithdraw() external onlyOwner nonReentrant {
        mpwrToken.safeTransferFrom(address(this), _msgSender(), totalSupply);
        emit MPRWWithdraw(_msgSender(), totalSupply);
    }
    ...
    function withdraw(uint256 amount) external whenNotPaused nonReentrant {
        require(
            (stakers[_msgSender()].amount >= amount) && (amount > 0),
            "Withdraw: Amount lower than user balance or > 0"
        );
        require(totalSupply > 0 && totalSupply > amount, "Withdraw: Insufficent reward supply found");
        ...
        // Adjust user information
        stakers[_msgSender()].amount = stakers[_msgSender()].amount - amount;
        stakers[_msgSender()].reward = calculatePendingRewards(_msgSender());
        stakers[_msgSender()].since = block.timestamp;
        // Adjust total amount staked
        totalAmountStaked = totalAmountStaked - (lastreward + amount);
        totalPendingReward = totalPendingReward - lastreward;
        totalSupply = totalSupply - stakers[_msgSender()].reward;
        // Update pool
        _updatePool(amount + lastreward);
        // Transfer mpwrToken tokens to the sender
        mpwrToken.safeTransfer(_msgSender(), amount);

        emit Withdraw(_msgSender(), amount, stakers[_msgSender()].reward);
    }
    ...
    function withdrawAll() external whenNotPaused nonReentrant {
        ...
        uint256 amountToTransfer = stakers[_msgSender()].amount + pendingRewards;
        ...
        emit Withdraw(_msgSender(), amountToTransfer, pendingRewards);
    }
