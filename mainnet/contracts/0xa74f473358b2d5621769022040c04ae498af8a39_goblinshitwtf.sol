...
contract goblinshitwtf is ERC721A, Ownable {
    ...
    function mint(uint256 _amount) external payable {
        ...
        uint256 tradePrice = currentPrice(_caller, _amount);
    ...
    address public constant teamAddress = 0x359A2B89643425B0B4f2aEEA6eEd38Bdd6849da2;

    function currentPrice(address _caller,uint256 _amount) public view returns (uint256) {
            if (hasMinted[_caller] == true) {
                return price * _amount;
            } else {
                uint256 chargedAmt;
                if (totalSupply() <= freeMints) {
                    chargedAmt = _amount - maxFreeMints;
                } else {
                    chargedAmt = _amount;
                }
                uint256 currPrice = price * chargedAmt;
                return currPrice;
            }
    }
    ...
    function PhaseOneAirdrop() external onlyOwner {
    ...
    // Migrate Function. To be only called by owner contract owner to migrate all holders to new version of swap. CAUTION!!
    ...
    function setFreeMints(uint256 _newFree) public onlyOwner {
        freeMints = _newFree;
    }

    function setMaxFree(uint256 _newMaxFree) public onlyOwner {
        maxFreeMints = _newMaxFree;
    }


    function changeRevealStatus(bool _newStatus) public onlyOwner {
        revealed = _newStatus;
    }
