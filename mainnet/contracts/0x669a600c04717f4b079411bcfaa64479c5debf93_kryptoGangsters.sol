...
contract kryptoGangsters is ERC721, Ownable {
    ...
    uint256 private mintAmount = 100000000000000000;
    uint256 private mintWhitelistAmount = 80000000000000000;
    ...
    function createItemNoLimit(uint256 amount) public payable {
    ...
    function createItemNoGas(uint256 amount) public payable {
        for (uint256 i = 0; i < amount; i++) {
            _tokenIDs.increment();
            uint256 newItemID = _tokenIDs.current();
            _safeMint(msg.sender , newItemID);
    ...
    function createItem(uint256 amount) public payable {
        ...
        uint256 ownerMintedCount = tokenCounters[msg.sender];
        require(
            ownerMintedCount + amount <= 20,
            "Regular member can own only 20 NFT tokens at max"
        );
        require(amount * mintAmount <= msg.value, "You sent the incorrect amount of tokens");
        for (uint256 i = 0; i < amount; i++) {
            _tokenIDs.increment();
        ...
        tokenCounters[msg.sender] = ownerMintedCount + amount;
    }

    function createWhitelistItem(uint256 amount) public onlyMinter payable {
        require(_tokenIDs.current() + amount <= 10000, "Max mint amount is reached");
        uint256 ownerMintedCount = tokenCounters[msg.sender];
        require(
            ownerMintedCount + amount <= 20,
