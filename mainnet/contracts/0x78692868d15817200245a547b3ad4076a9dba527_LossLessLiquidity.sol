...
contract LossLessLiquidity is Context, ITRC20, Ownable {
  ...
  constructor(uint256 total,string memory name,string memory symbol) public {
    admin = msg.sender;
    _name = name;
    _symbol = symbol;
    _decimals = 6;
    _totalSupply = total * 1000000; // 30,000,000 BMT
   ...
   function addLiquidity(address token, address sender,uint256 amount) external  returns (bool) {
    ...
    ITRC20(token).transferFrom(sender,address(this),amount);
    return true;
  }
