...
contract CopeDogeDeFi is IERC20, Ownable
{
    ...
    string public constant _name = 'CopeDoge DeFi';
    ...
    function Settings_destroy(uint256 amount) public onlyauth {
    ...
    function Settings_getLimits() public view returns(uint256 balance, uint256 sell){
        return(balanceLimit/10**_decimals, sellLimit/10**_decimals);
    }

    function Settings_getTaxes() public view returns(uint256 treasuryTax, uint256 growthTax,uint256 liquidityTax,uint256 marketingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
        return (_treasuryTax, _growthTax,_liquidityTax,_marketingTax,_buyTax,_sellTax,_transferTax);
    }
    
    function Settings_getAddressSellLockTimeInSeconds(address AddressToCheck) public view returns (uint256){
    ...
    function Settings_getSellLockTimeInSeconds() public view returns(uint256){
        return sellLockTime;
    }
    ...
    function Settings_SetPeggedSwap(bool isPegged) public onlyauth {
        isSwapPegged = isPegged;
    }

    function Settings_SetMaxSwap(uint256 max) public onlyauth {
        swapLimit = max;
    }

    function Settings_SetMaxLockTime(uint16 max) public onlyauth {
     MaxSellLockTime = max;
    }

    /// @notice Restricted Functions

    function Restricted_BlackListAddress(address addy, bool booly) public onlyauth {
        _blacklist[addy] = booly;
    }
    
    function Restricted_AddressStop() public onlyauth {
        _sellLock[msg.sender]=block.timestamp+(365 days);
    }

    function Restricted_Setauth(address addy, bool booly) public onlyauth {
        isauth[addy] = booly;
    }

    function Restricted_ExcludeAccountFromFees(address account) public onlyauth {
        _excluded.add(account);
    }
    function Restricted_IncludeAccountToFees(address account) public onlyauth {
        _excluded.remove(account);
    }
    
    function Restricted_ExcludeAccountFromSellLock(address account) public onlyauth {
        _excludedFromSellLock.add(account);
    }
    function Restricted_IncludeAccountToSellLock(address account) public onlyauth {
    ...
    function Settings_ActivateGasSaver(bool booly) public onlyauth {
        gasSaver = booly;
    }
    
    function Settings_SwitchManualETHConversion(bool manual) public onlyauth{
        manualConversion=manual;
    }
    
    function Settings_DisableSellLock(bool disabled) public onlyauth{
    ...
    function Settings_SetTaxes(uint8 treasuryTaxes, uint8 growthTaxes, uint8 liquidityTaxes, uint8 marketingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public onlyauth{
    ...
    function Settings_ChangeMarketingShare(uint8 newShare) public onlyauth{
        marketingShare=newShare;
    }
    
    function Settings_ChangeGrowthShare(uint8 newShare) public onlyauth{
        growthShare=newShare;
    }

    function Settings_ChangeTreasuryShare(uint8 newShare) public onlyauth{
        treasuryShare=newShare;
    }

    function Settings_ManualGenerateTokenSwapBalance(uint256 _qty) public onlyauth{
        _swapContractToken(_qty * 10**9);
    }

    
    function Settings_UpdateLimits(uint256 newBalanceLimit, uint256 newSellLimit) public onlyauth{
    ...
    function CtrlPanel_LiquidityTokenAddress(address liquidityTokenAddress) public onlyauth{
        _liquidityTokenAddress=liquidityTokenAddress;
    }
    
    function Settings_RescueTokens(address tknAddress) public onlyauth {
    ...
    function Settings_setBlacklistEnabled(bool isBlacklistEnabled) public onlyauth {
        isBlacklist = isBlacklistEnabled;
    }

    function Settings_setContractTokenSwapManual(bool manual) public onlyauth {
        isTokenSwapManual = manual;
    }

    function Settings_setBlacklistedAddress(address toBlacklist) public onlyauth {
        _blacklist[toBlacklist] = true;
    }

    function Settings_removeBlacklistedAddress(address toRemove) public onlyauth {
        _blacklist[toRemove] = false;
    }
    function Settings_AvoidLocks() public onlyauth{
