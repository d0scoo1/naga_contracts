...
contract SokuSwapSamurai is ERC721Enumerable, Ownable {
    
    using SafeMath for uint256;
    
    bool public mintingActive;
    uint256 public mintingPrice = .07 ether;
    uint256 public mintingStartTimestamp;
    mapping(address => uint256) public numPurchased;
    mapping(address => bool) public whitelisted;

    uint256 public maxSupply = 1000;
    address public creator = 0x45d46f51c468F81600aa3cbcB9C7608cbf46044F;
    
    string public baseURI;
    
    
    constructor(uint256 mintingStart, string memory _uri) ERC721("SokuSwap SamuraiNFTv1", "SokuSamuraiV1") {
        mintingActive = false;
    ...
    function editWhitelist(address[] memory array) public onlyOwner {
        for(uint256 i = 0; i < array.length; i++) {
            address addressElement = array[i];
            whitelisted[addressElement] = true;
        } 
    }
    ...
    function whitelistMint() public {
        ...
        require(totalSupply().add(1) <= maxSupply, "This tx would exceed max supply");
        require(whitelisted[msg.sender] == true, "Not whitelisted");
        uint256 tsupply = totalSupply();

        whitelisted[msg.sender] = false;

        _safeMint(msg.sender, tsupply + 1);
        numPurchased[msg.sender] += 1;
    }
    ...
    */
    ...
    function withdrawFunds(address recipient) public onlyOwner {
        uint toCreator = address(this).balance.div(40);
