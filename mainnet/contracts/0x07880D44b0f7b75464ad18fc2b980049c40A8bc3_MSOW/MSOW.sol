 ...
 */
contract MSOW is Context, Ownable, ERC165, IMSOW, IERC721Metadata {
    ...
    uint256 public constant SALE_START_TIMESTAMP = 1635634800; // Sunday, Oct 31st, 2021 0:00:00 UTC
    uint256 public constant MINT_PRICE = 0;
    
    uint256 public disableWhitelistTimestamp = SALE_START_TIMESTAMP + (86400 * 1); // whitelist disabled after 1 day
    uint256 public nftsPerAddress = 2;
    
    address public constant WHITELIST_SIGNER = 0xd8E80144882d414E7769e9eDA23294531b752C17;
     ...
     */
    constructor () {
        _name = "MadameSofiasOracleOfWisdom";
        _symbol = "MSOW";
    ...
    /* Special thanks to Johnny Shiitake NFT for the whitelist code */
    ...
    */
    ...
    /**
    * @dev Changes the number of NFTs a unique address can mint (Callable by owner only)
    */
    function changeNftsPerAddress(uint256 _nftsPerAddress) onlyOwner external {
        require(_nftsPerAddress >= 0, "NFTs per address must be valid");
        nftsPerAddress = _nftsPerAddress;
    }
    
    /**
    * @dev Changes the number of NFTs a unique address can mint (Callable by owner only)
    */
    function changeDisableWhitelistTimestamp(uint256 _disableWhitelistTimestamp) onlyOwner external {
        require(_disableWhitelistTimestamp >= 0, "Timestamp must be valid");
        disableWhitelistTimestamp = _disableWhitelistTimestamp;
    }
    
    mapping (address => uint256) private addressMinted;
    
    function getNftsMintedByAddress(address addr) public view returns (uint256) {
        return addressMinted[addr];
    }
    
    function whitelistMintingHasStarted() public view returns (bool) {
        return block.timestamp >= SALE_START_TIMESTAMP;
    }
    
    function publicMintingHasStarted() public view returns (bool) {
    ...
    * @dev Mints NFTs
    */
    function mintNFT(bytes memory signature) external {
        ...
        uint256 nftsMintedByAddress = addressMinted[msg.sender];
        
        require(nftsMintedByAddress < nftsPerAddress, "Address already minted max amount of NFTs");
        uint256 numberOfNfts = 0;
        numberOfNfts = nftsPerAddress.sub(nftsMintedByAddress);
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
            ...
            /* final supply check */
            require(mintIndex < MAX_NFT_SUPPLY, "All NFTs have been minted");
