    ...
    NFTOrders
{
        ...
        returns (bool[] memory successes)
    {
        ...
        if (revertIfIncomplete) {
            ...
            for (uint256 i = 0; i < length; i++) {
                // Delegatecall `_buyERC1155FromProxy` to catch swallow reverts while
                // preserving execution context.
                (successes[i], ) = _implementation.delegatecall(
                    abi.encodeWithSelector(
                        this._buyERC1155FromProxy.selector,
                        sellOrders[i],
                        signatures[i],
        ...
        returns (bool[] memory successes)
    {
        ...
        if (revertIfIncomplete) {
            ...
            for (uint256 i = 0; i < length; i++) {
                // Delegatecall `_buyERC1155ExFromProxy` to catch swallow reverts while
                // preserving execution context.
                (successes[i], ) = _implementation.delegatecall(
                    abi.encodeWithSelector(
                        this._buyERC1155ExFromProxy.selector,
                        sellOrders[i],
                        signatures[i],
                        BuyParams(
                            erc1155FillAmounts[i],
    ...
    // @Note `_buyERC1155ExFromProxy` is a external function, must call from an external Exchange Proxy,
    //        but should not be registered in the Exchange Proxy.
    function _buyERC1155ExFromProxy(
        LibNFTOrder.ERC1155SellOrder memory sellOrder,
        LibSignature.Signature memory signature,
