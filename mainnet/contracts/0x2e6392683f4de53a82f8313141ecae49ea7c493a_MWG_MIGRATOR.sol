...
contract MWG_MIGRATOR is Context, IERC20, IERC20Metadata {
    ...
    constructor() {
        _name = "METAWHALE GOLD MIGRATOR";
        _symbol = "MWGm";
        MWG = 0x45128CB743951121Fb70cb570c0784492732778A;
    ...
    // ex: MWG.approve(thisContractAddress, AmountToMigrate)
    function transport(uint256 _amount) public virtual returns (bool) {
        require (isPortalOpen == true, "Portal is Closed.");
        require(IERC20(MWG).balanceOf(_msgSender()) >= _amount, "Insufficient Balance");
        IERC20(MWG).transferFrom(_msgSender(), deployer, _amount);
