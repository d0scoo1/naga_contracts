...
contract ChainPots is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    uint public constant MAX_POTS = 1024;
    ...
    string public GENERATOR_ADDRESS = "https://chainpots.com/generator/";
    string public IPFS_GENERATOR_ADDRESS = "";

    constructor() ERC721("ChainPots","POT")  {
        setBaseURI("https://chainpots.com/api/token/");
   ...
   function mintPots(uint256 maxPOTS) public payable {
        require(totalSupply() < MAX_POTS, "Sale has already ended");
        require(maxPOTS > 0 && maxPOTS <= 20, "You can claim minimum 1, maximum 20 stairs");
        require(totalSupply().add(maxPOTS) <= MAX_POTS, "Exceeds MAX_POTS");
        require(msg.value >= calculatePrice().mul(maxPOTS), "Ether value sent is below the price");

        for (uint i = 0; i < maxPOTS; i++) {
