...
// File: packages/hardhat/contracts/Pionion721a.sol

//SPDX-License-Identifier: MIT
pragma solidity ^0.8.7;

//   __  __      _          ____  _             _
//  |  \/  | ___| |_ __ _  |  _ \(_) ___  _ __ (_) ___  _ __  ___
//  | |\/| |/ _ \ __/ _` | | |_) | |/ _ \| '_ \| |/ _ \| '_ \/ __|
//  | |  | |  __/ || (_| | |  __/| | (_) | | | | | (_) | | | \__ \
//  |_|  |_|\___|\__\__,_| |_|   |_|\___/|_| |_|_|\___/|_| |_|___/

contract Pionion is ERC721A, Pausable, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public maxWalletMint = 8;

    uint256 public preSaleStart = 1653220800; // Sun May 22 2022 20:00:00 GMT+0800
    uint256 public preSaleDuration = 1 days;
    uint256 public publicSaleStart = 1653307200; // Mon May 23 2022 20:00:00 GMT+0800

    event EventMint(uint256 quantity_, uint256 totalSupply_);

    address public constant devAddress =
        0xF04a2cB00A503F9bd389e52f8E1D5D4E0dB0DB00;
    ...
    function preSalePrice(uint256 count_) public view returns (uint256) {
        return presaleCost.mul(count_);
    }

    function publicSalePrice(uint256 count_) public view returns (uint256) {
        return cost.mul(count_);
    }
    ...
    function setPreSaleStart(uint256 start_) external onlyOwner {
        preSaleStart = start_;
    }

    function setPreSaleDuration(uint256 duration_) external onlyOwner {
        preSaleDuration = duration_;
    }

    function setPublicSaleStart(uint256 start_) external onlyOwner {
        publicSaleStart = start_;
    }
    ...
    function setPresaleCost(uint256 cost_) external onlyOwner {
        presaleCost = cost_;
    }
    ...
    function withdrawAll() external onlyOwner nonReentrant {
        ...
        require(balance > 0);
        _widthdraw(devAddress, balance.mul(1).div(100));
    ...
    modifier preSaleIsOpen() {
        require(totalSupply() < collectionSize, "Sale ended");
        require(isPreSaleOn(), "PreSale not open");
        _;
    }
    modifier publicSaleIsOpen() {
        require(totalSupply() < collectionSize, "Sale ended");
        require(isPublicSaleOn(), "PublicSale not open");
        _;
    }
    ...
    function presaleMint(uint256 count_, bytes32[] calldata merkleProof_)
        external
        payable
        callerIsUser
        whenNotPaused
        preSaleIsOpen
    {
        ...
        require(count_ >= 1, "Must mint at least 1 token");
        require(
            count_ <= maxBatchSize,
            "Cannot mint more than max mint per transaction"
        );
        require(
            balanceOf(wallet) + count_ <= maxWalletMint,
            "Wallet address is over the maximum allowed mints"
        );
        require(
            totalSupply_ + count_ <= collectionSize,
            "Cannot mint over supply cap of 4444"
        );
        require(
            msg.value >= preSalePrice(count_),
            "Value below required mint fee for amount"
        );
        _safeMint(wallet, count_);
        emit EventMint(count_, totalSupply());
    }

    function publicSaleMint(uint256 count_)
        external
        payable
        callerIsUser
        whenNotPaused
        publicSaleIsOpen
    {
        ...
        require(
            msg.value >= publicSalePrice(count_),
            "Value below required mint fee for amount"
        );
        _safeMint(wallet, count_);
        emit EventMint(count_, totalSupply());
