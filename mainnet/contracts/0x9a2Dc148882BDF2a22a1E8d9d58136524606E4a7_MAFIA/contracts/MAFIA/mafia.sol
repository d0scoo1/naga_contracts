...
contract MAFIA is Context, IERC20, Ownable {
    ...
    string private constant _name = "MAFIA";
    string private constant _symbol = "MAFIA";

    uint8 private constant _decimals = 18;

    uint256 private constant _tTotal = 1e7 * (10**_decimals);
    ...
    uint256 private _buyTreasuryFee = 0;
    ...
    uint256 private _sellTreasuryFee = 2;
    ...
    constructor (address marketingWallet, address liquidityWallet, address treasuryWallet) {
    ...
    function openTrading() external onlyOwner() {
        ...
        _maxBuyAmount = 5e4 * (10**_decimals);
        _maxSellAmount = 5e4 * (10**_decimals);
        _maxWalletAmount = 2e5 * (10**_decimals);
        swapTokensAtAmount = 5e3 * (10**_decimals);
        tradingOpen = true;
    ...
    function setMaxBuyAmount(uint256 maxBuy) public onlyOwner {
        require(maxBuy >= 1e3 * (10**_decimals), "Max buy amount cannot be lower than 0.01% total supply.");
        _maxBuyAmount = maxBuy;
    }

    function setMaxSellAmount(uint256 maxSell) public onlyOwner {
        require(maxSell >= 1e3 * (10**_decimals), "Max sell amount cannot be lower than 0.01% total supply.");
        _maxSellAmount = maxSell;
    }
    
    function setMaxWalletAmount(uint256 maxToken) public onlyOwner {
        require(maxToken >= 1e4 * (10**_decimals), "Max wallet amount cannot be lower than 0.1% total supply.");
        _maxWalletAmount = maxToken;
    }
    
    function setSwapTokensAtAmount(uint256 newAmount) public onlyOwner {
        require(newAmount >= 1e2 * (10**_decimals), "Swap amount cannot be lower than 0.001% total supply.");
        require(newAmount <= 5e4 * (10**_decimals), "Swap amount cannot be higher than 0.5% total supply.");
    ...
    function setBuyFee(uint256 buyMarketingFee, uint256 buyLiquidityFee, uint256 buyTreasuryFee) external onlyOwner {
        require(buyMarketingFee + buyLiquidityFee + buyTreasuryFee <= 20, "Must keep buy taxes below 20%");
        _buyMarketingFee = buyMarketingFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyTreasuryFee = buyTreasuryFee;
    }

    function setSellFee(uint256 sellMarketingFee, uint256 sellLiquidityFee, uint256 sellTreasuryFee) external onlyOwner {
        require(sellMarketingFee + sellLiquidityFee + sellTreasuryFee <= 30, "Must keep sell taxes below 30%");
    ...
    function removeAllFee() private {
        if(_buyMarketingFee == 0 && _buyLiquidityFee == 0 && _buyTreasuryFee == 0 && _sellMarketingFee == 0 && _sellLiquidityFee == 0 && _sellTreasuryFee == 0) return;
    ...
    function _getTotalFees(bool isSell) private view returns(uint256) {
        if (isSell) {
            return _sellMarketingFee + _sellLiquidityFee + _sellTreasuryFee;
        }
        return _buyMarketingFee + _buyLiquidityFee + _buyTreasuryFee;
    }
