...
contract MoonBase is Ownable {
    ...
    uint256 private lastId = 1419;
    mapping(uint256 => bool) private claims;

    event UpdatedLastId(uint256 last);
    ...
    function updateLastId(uint256 id) external onlyOwner {
        lastId = id;

        emit UpdatedLastId(id);
    }
    ...
    function withdrawNFT(uint256 id) external onlyOwner {
        IERC721(mlz).transferFrom(address(this), msg.sender, id);
    }
    ...
    function claim(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokensNb;) {
            ...
            if (tokenId > lastId || claims[tokenId])
                revert CannotClaim();
