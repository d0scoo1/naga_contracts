...
contract Dickheads is ERC721, Ownable {
    event Mint(address indexed from, uint256 indexed tokenId);

    modifier mintingStarted() {
        require(mintingEnabled, "You are too early");
        _;
    }
    ...
    uint16 private totalTokens = 6969;
    uint256 private maxTokensPerWallet = 150;
    uint256 private maxTokensPerTransaction = 10;
    bool private mintingEnabled = false;
    // Price per NFT: 0.03 ETH
    ...
    uint256 public nextTokenToMint;

    constructor(string memory _initialBaseURI) ERC721("Dickheads", "PEN15") {
    ...
    /**
     * @dev Sets maxTokensPerTransaction
     */
    ...
    /**
     * @dev Sets the mint price for each token
     */
    ...
    {
        require(
            msg.value >= tokenPrice * amount,
            "Not enough Ether to claim the tokens"
        );

        require(amount <= maxTokensPerTransaction, "Unable to mint this many");

        require(
            claimedTokensPerWallet[msg.sender] + amount <= maxTokensPerWallet,
            "You cannot claim more tokens"
        );

        require(
            totalTokens - (nextTokenToMint - 1) > amount,
     ...
     */
    function getMintingEnabled() external view returns (bool) {
    ...
    function getTokenToBeClaimed() private returns (uint256) {
        uint256 tokenId = nextTokenToMint;
        nextTokenToMint++;
        return tokenId;
    }
