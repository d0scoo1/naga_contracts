        ...
        if (launchPool != address(0)) {
            if (tradingTime == 0) {
                ...
                if (sender == launchPool) {
                    ...
                    uint256 ethBal = IERC20Upgradeable(pairingToken).balanceOf(launchPool);
                    ...
                    if ( curPriceMultiple < (unlockMultiple * 1000) ) {
                        ...
                        lockTime[recipient] = block.timestamp + maxLock - maxLock * curPriceMultiple / (unlockMultiple * 1000);
