...
contract RoboPlus is ERC721URIStorage,Ownable {
    uint16 public tokenCounter=0;
    uint32 MAX_TOKEN_SUPPLY = 5432;
    uint16  public MAX_MINT_PER_TIME = 20;
    uint public nftCost = 6*10**16;
    uint256 public saleStartsOn;
    mapping(address=>bool) public whitelisted;

    constructor (uint256 _saleStartsOn) 
        ERC721 ("RoboPlus", "ROBO")
    {
        saleStartsOn = _saleStartsOn;
    }
    modifier onlyAfterPreSaleDuration(){
        require(block.timestamp>saleStartsOn);
        _; 
    }
    modifier onlyDuringPreSaleDuration(){
        require(block.timestamp<saleStartsOn);
        _; 
    }
    ...
    function mint(string[] memory _tokenURIs) onlyAfterPreSaleDuration payable external 
    {
    ...
    function presaleMint(string[] memory _tokenURIs) onlyWhitelisted onlyDuringPreSaleDuration payable external 
    {
        require(_tokenURIs.length <= MAX_MINT_PER_TIME, "minting limit exceeds");
        require(msg.value >= (nftCost * _tokenURIs.length), "Low Price");
    ...
    function whitelistUser(address _newAddr)public onlyOwner{
        whitelisted[_newAddr] = true;
    }
