...
contract RoboChickens is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;


  uint256 public MAX_PER_Transtion = 10; // maximam amount that user can mint per transaction
  uint256 public MAX_PER_Address = 200; // maximam amount that user can mint

  uint256 public  PRICE = 0.025 ether; //0.020 ether during pre sale & 0.025 ether during public sale
  ...
  uint public status = 0; // 0 - pause, 1 - pre sale & 2 - public Sale

  mapping(address => bool) private whitelistedAddresses;

  constructor() ERC721A("RoboChickens","RoboChickens", MaxMintPerBatch_, TotalCollectionSize_) {
    _uriBeforeRevel = "https://gateway.pinata.cloud/ipfs/QmR9LuViuRAyUemjiggusZegpEGWAYH2nCWUE44ckWVeov/";
    _baseTokenURI="https://gateway.pinata.cloud/ipfs/Qmcpbw1B2dVdpFqpmf7QA12M5XtekFtXjc19X4iqdkyqrj/";
  }
  ...
  function mint(uint256 quantity) external payable callerIsUser {
    ...
    require(totalSupply() + quantity <= Presale_Supply, "Reached max Pre Sale Supply");
    require(numberMinted(msg.sender) < 1  , "You can mint Upto 1 NFTs in Pre Sale" );
    require(quantity == 1,"Max can mint/hold 1 NFTs in Pre Sale Round. Wait for Public Sale to Mint more.");
    ...
    require(numberMinted(msg.sender) + quantity <= 200  , "Max can hold 200 NFTs" );
    require(quantity <= 10,"Max can mint 10 NFTs per transaction in a Public Sale");
    require(msg.value >= PRICE * quantity, "Not enough ETH in your wallet");
    _safeMint(msg.sender, quantity);    
    }
  }
