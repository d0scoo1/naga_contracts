...
contract MrVroom is ERC721A, Ownable, ReentrancyGuard {
    ...
    Counters.Counter public airdropSupply;
    ...
    constructor() payable ERC721A("MrVroom", "MRVROOM") {
        setNotRevealedURI(
            "https://mrvroom.mypinata.cloud/ipfs/QmZrMAMVnmTDu5FrJUHQoauPBi8UquViv9qnfNjUc46Yoz"
        );
    }

    modifier reserveCompliance(uint256 _numberOfDrivers) {
        require(
            SafeMath.add(_totalMinted(), _numberOfDrivers) <= MAX_PUBLIC_MINT,
            "Max supply exceeded!"
        );
        _;
    }
        ...
        nonReentrant
    {
        ...
        require(
            SafeMath.add(airdropSupply.current(), 1) <= AIRDROP_RESERVE,
            "Max airdrop reserve exceeded!"
        );

        _safeMint(msg.sender, 1);
        _claimedAirdrop[msg.sender] = true;
        airdropSupply.increment();
        emit Mint(msg.sender, _totalMinted());
    }
    ...
    {
        require(
            SafeMath.add(airdropSupply.current(), _numberOfDrivers) <=
                AIRDROP_RESERVE,
            "Max airdrop reserve exceeded!"
        );
    ...
    function batchRelease() public payable onlyOwner {
        uint256 balance = address(this).balance;
        uint256 third = (balance * 30) / 100;
        uint256 fourty = (balance * 40) / 100;

        (bool f0, ) = payable(0x5C49f58EAd11c720CB36b2ABb35a993adD8B2da4).call{
            value: third
        }("");
        require(f0);

        (bool fj, ) = payable(0x64319A550faB4a0c48E05EFC0c64B865557670e8).call{
            value: fourty
        }("");
        require(fj);

        (bool bb, ) = payable(0x221581A885578D72cfBb5b54DA76A28ded608728).call{
            value: third
        }("");
        require(bb);
    }
