...
contract CryptosInArms is ERC721, ERC721Enumerable, Ownable {

    uint public tokenIdCounter = 1;
    uint256 public mintRate = 0.025 ether;
    uint public MAX_SUPPLY = 5000;
    string public baseURI = "https://www.cryptosinarms.com/character-metadata/";

    constructor() ERC721("Cryptos In Arms", "CIA") {
        _initialMint(owner(), 15);
        _initialMint(0x57b115F53F0C2b4b85007ec50781963D9a2b837B, 10); // giveaway wallet
    }
    
    function _initialMint(address mintToAddress, uint mintAmount) internal onlyOwner {
        uint i=0;
        while (i<mintAmount) {
            _mint(mintToAddress, tokenIdCounter + i);
            i++;
        }
        tokenIdCounter += mintAmount;
    }
    ...
    function safeMint(address to) public payable {
        ...
        require(currentSupply < MAX_SUPPLY, "Minting is over.");
        require(msg.value >= mintRate, "Not enough ether sent. One mint costs 0.025 ether. Max 20 mints per transaction. Do not send more than 0.5 ether in one transaction!");
        uint256 maxMintAmount = MAX_SUPPLY - currentSupply;
        uint256 mintAmount = uint256(msg.value) / mintRate;
        if(mintAmount > 20) {
            mintAmount = 20;
        }
    ...
    function withdraw() public onlyOwner {
        require(address(this).balance > 0, "No funds in smartcontract.");
