...
contract Unemployables is ERC721A, Ownable {
  ...
  uint256 public constant maxSupply = 50001;
    ...
    returns (uint256)
  {
    ...
    require(block.timestamp < publicSaleEpoch, "pixelist sale over");
    require(
      MerkleProof.verify(
        merkleProof,
        pixelistMerkleRoot,
    ...
    require(count < maxSupply, "Soldout!");
  ...
  function publicMint(uint256 count) public payable returns (uint256) {
    require(saleActive, "Sale inactive");
    require(block.timestamp >= publicSaleEpoch, "Too soon");
    require(owner() == msg.sender || msg.value >= PUBLIC_SALE_PRICE * count,
  ...
  */
  ...
  function reveal() public onlyOwner {
    require(block.timestamp >= revealEpoch, "Too soon");
    revealed = true;
  }
  ...
  function setRevealEpoch(uint256 _epoch) external onlyOwner {
    revealEpoch = _epoch;
  }
