...
contract RedlionGazette is Context, AccessControlEnumerable, Ownable, ERC2981, ERC721, ERC721Enumerable {
    ...
    function claim(uint issue, bytes32[] calldata proof, uint amount) public {
        require(!_claimedAirdrop[msg.sender].get(issue), "CLAIM:ALREADY CLAIMED FOR ISSUE");
        _claimedAirdrop[msg.sender].set(issue);
        require(MerkleProof.verify(proof, issueToMerkleRoot[issue], keccak256(abi.encodePacked(msg.sender, amount))), "CLAIM:INVALID PROOF");
