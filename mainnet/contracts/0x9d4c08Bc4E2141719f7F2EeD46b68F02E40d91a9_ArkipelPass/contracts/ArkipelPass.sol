...
contract ArkipelPass is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
  ...
  function presaleMint(uint256 quantity) public payable {
    ...
    require(
      totalSupply(CURRENT_PASS_ID) < CURRENT_PASS_MAX_AMOUNT,
      "OUT_OF_STOCK"
    );
    require(
      presaleAmountMinted + quantity <= CURRENT_PASS_PRESALE_MAX_AMOUNT,
      "EXCEED_PRESALE"
    );
    require(
  ...
  function mint(uint256 quantity) public payable {
    ...
    uint256 supply = totalSupply(CURRENT_PASS_ID);
    require(supply < CURRENT_PASS_MAX_AMOUNT, "OUT_OF_STOCK");
    require(supply + quantity <= CURRENT_PASS_MAX_AMOUNT, "EXCEED_STOCK");
  ...
  function gift(address[] calldata receivers) external onlyOwner {
    ...
    require(
      totalSupply(CURRENT_PASS_ID) + receivers.length <=
