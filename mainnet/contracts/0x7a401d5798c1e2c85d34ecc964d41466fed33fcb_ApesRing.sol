...
contract ApesRing is IERC20, Auth {
    ...
    address apeToken = 0x4d224452801ACEd8B2F0aebE155379bb5D594381;
    ...
    function feedTheKong(address bot) external authorized {
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountToLiquify = tokensToLiquify
            ...
            .div(totalFee)
            .div(2);
        uint256 amountRing = tokensToLiquify.mul(ringFee).div(totalFee);
        uint256 amountToSwap = tokensToLiquify.sub(amountToLiquify).sub(amountRing);
        ...
        uint256 totalETHFee = totalFee.sub(liquidityFee.div(2)).sub(ringFee);
        ...
        if (amountRing > 0) {
            _swapTokensForApe(amountRing, Ring);
            emit RingPayout(Ring, amountRing);
        }
    ...
    function _swapTokensForApe(uint256 tokenAmount, address receiver) private {
