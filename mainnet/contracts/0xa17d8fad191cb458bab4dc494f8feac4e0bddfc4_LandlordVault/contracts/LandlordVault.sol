    ...
    ReentrancyGuardUpgradeable
{
    ...
    function RentAccumulated(address landlord) public view returns (uint256 rent) {
        uint256 balance = landlords.balanceOf(landlord);
        uint256 id;
        uint256 i = 0;
        rent = 0;
        for (i == 0; i < balance; i++) {
            id = landlords.tokenOfOwnerByIndex(landlord, i);
    ...
    function RoyaltiesAccumulated(address landlord) public view returns (uint256 royalties) {
        uint256 balance = landlords.balanceOf(landlord);
        uint256 id;
        uint256 i = 0;
        royalties = 0;
