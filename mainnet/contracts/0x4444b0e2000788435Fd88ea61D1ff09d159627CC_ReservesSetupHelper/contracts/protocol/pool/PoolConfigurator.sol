 ...
 *
 * @dev Implements the configuration methods for the Omni protocol
 **/
contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
    ...
    function _checkNoSuppliers(address asset) internal view {
        uint256 totalOTokens = IPoolDataProvider(
            _addressesProvider.getPoolDataProvider()
        ).getOTokenTotalSupply(asset);
        require(totalOTokens == 0, Errors.RESERVE_LIQUIDITY_NOT_ZERO);
    }
