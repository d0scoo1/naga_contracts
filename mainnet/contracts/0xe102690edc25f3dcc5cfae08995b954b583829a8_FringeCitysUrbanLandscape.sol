...
// File: contracts/FringeCityUrbanLandscape.sol
...
contract FringeCitysUrbanLandscape is ERC721Enumerable, Ownable, RoyaltiesV2Impl {
  using Strings for uint256;
  ...
  ) ERC721("FringeCitys Urban Landscape", "FRCU") {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= 10, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");

    if (msg.sender != owner()) {
        require(msg.value >= 0.05 ether * _mintAmount, "insufficient funds");
    }
