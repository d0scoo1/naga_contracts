...
contract ElonGoat is Context, IERC20, Ownable {
    ...
    address payable public devAddress = payable(0xe52B6f3bBA6251B6aAdba84C62371024c81DbC76); // Dev Address
    address payable public buyBackAddress = payable(0x310E8A394dc33f736AeC21783454D34E08943dc9); // BuyBack Address
    ...
    uint256 public _totalFee = 80;  // marketingDivisor + devDivisor + buyBackDivisor + autoLpDivisor
    ...
    event FeesUpdated(uint256 autoLpDivisor, uint256 devDivisor, uint256 marketingDivisor, uint256 buyBackDivisor);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(devAddress, transferredBalance.mul(devDivisor).div(totalBNBFee));
        transferToAddressETH(buyBackAddress, transferredBalance.mul(buyBackDivisor).div(totalBNBFee));
    ...
    function updateFeeDivisor(uint256 newAutoLpDivisor, uint256 newDevDivisor, uint256 newMarketingDivisor, uint256 newBuyBackDivisor) external onlyOwner {
        uint256 newTotalFee = newAutoLpDivisor.add(newDevDivisor).add(newMarketingDivisor).add(newBuyBackDivisor);
        ...
        buyBackDivisor = newBuyBackDivisor;

        _previousTotalFee = _totalFee;
        _totalFee = newTotalFee;

        emit FeesUpdated(newAutoLpDivisor, newDevDivisor, newMarketingDivisor, newBuyBackDivisor);
    }
