...
// File: contracts/EtheretroWatchCollection.sol

pragma solidity ^0.6.0;

contract EtheretroWatchCollection is ERC721Burnable,Ownable{
    ...
    constructor()  public  ERC721("Etheretro Watch Collection","ETRO1") {}
    ...
    function whiteListMint(address _account,uint256 _count) public onlyMinter {
        require(nextTokenId + _count <= maxSupply + 1,"max supply exceeded");
        
        for(uint256 i=0;i< _count;i++){
            _safeMint(_account, nextTokenId+i);
            _setTokenURI(nextTokenId, nextTokenId.toString());
        }
      
        nextTokenId += _count;
        emit SaleMint(_account, _count);
    }
