...
contract Crowdsale is ReentrancyGuard {
  ...
  struct _DateTime {
    ...
    /**
     * @param P_rate Number of token units a buyer gets per wei
     * @dev The rate is the conversion between wei and the smallest and indivisible
     * token unit. So, if you are using a rate of 1 with a ERC20Detailed token
     * with 3 decimals called TOK, 1 wei will give you 1 unit, or 0.001 TOK.
     * @param P_wallet Address where collected funds will be forwarded to
     * @param P_token Address of the token being sold
     */
    constructor (uint256 P_rate, address payable P_wallet, IERC20 P_token) {
        require(P_rate > 0, "Crowdsale: rate is 0");
        require(P_wallet != address(0), "Crowdsale: wallet is the zero address");
        require(address(P_token) != address(0), "Crowdsale: token is the zero address");
