...
contract EtherKoiPond is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    uint public constant MAX_PONDS = 8888;
    ...
    string public GENERATOR_ADDRESS = "http://ether-koi.com/";
    string public IPFS_GENERATOR_ADDRESS = "";

    constructor() ERC721("EtherKoiPond","KOI")  {
        setBaseURI("http://ether-koi.com/api/token/");
    ...
    function calculatePrice() public pure returns (uint256) {
        return 3000000000000000; // 0.03
    }
    
   function mint(uint256 maxPONDS) public payable {
        require(totalSupply() < MAX_PONDS, "Sale has already ended");
        require(maxPONDS > 0 && maxPONDS <= 10, "You can purchase minimum 1, maximum 10");
        require(totalSupply().add(maxPONDS) <= MAX_PONDS, "Exceeds maximum");
        require(msg.value >= calculatePrice().mul(maxPONDS), "Ether value sent is below the price");

        for (uint i = 0; i < maxPONDS; i++) {
