...
contract DaycareStaking is IERC721Receiver, Ownable, ReentrancyGuard, VestingWallet {
        ...
        override external daycareOpen isSkvllbaby(msg.sender) returns(bytes4) { 
            ...
            skvllbabiez[skvllbabyId] = 
                Skvllbaby(
                    ...
                    from, 
                    block.timestamp, 
                    skvllbabiez[skvllbabyId].accruedBalance > 0 ? skvllbabiez[skvllbabyId].accruedBalance : 0, 
                    true); 
            emit SkvllbabyCheckedIn( from, skvllbabyId );
            return IERC721Receiver.onERC721Received.selector;
    }
    
    function checkIn(uint256[] memory skvllbabyIds) external {
        for (uint256 i; i < skvllbabyIds.length; i++){
            SkvllpvnkzDaycare.safeTransferFrom( msg.sender, address(this), skvllbabyIds[i]);
        }
    }

    function checkOut(uint256[] memory skvllbabyIds) public daycareOpen {
    ...
    function collectRewards(uint256[] memory skvllbabyIds) public daycareOpen {
        uint256 rewardTimestamp = block.timestamp;
    ...
    function _releasePayment(uint256 rewardAmount) internal {
        ...
        release(address(SkvllpvnkzTreasury));
