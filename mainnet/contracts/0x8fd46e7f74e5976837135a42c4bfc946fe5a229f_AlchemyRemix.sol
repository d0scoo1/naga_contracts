...
// File contracts/AlchemyRemix.sol
...
contract AlchemyRemixData {
    ...
    // Mapping from token ID to commitmentId
    mapping (uint256 => RemixCommit) public _tokenCommitmentIds;
}

// Context removed, because OwnableUpgradeable contains ContextUpgradeable
contract AlchemyRemix is ERC721Upgradeable, IAlchemyRemix, UUPSUpgradeable, OwnableUpgradeable, AlchemyRemixData {
    // replaced Address lib with custom MyAddress to avoid delegateCall (its not allowed for Proxied contract)
    using AddressUtil for address;
    using Strings for uint256;

    // Token data
    string constant _name = "Alchemy Remix Token";
    ...
    function initialize(address alchemyRetort, address verifyAttestation) public initializer
    {
    ...
    function contractURI() public pure returns (string memory) {
        return "https://alchemynft.io/contracts/amx.json";
    }
    
    function mintUsingSequentialTokenId(address to, RemixCommit memory commitmentEntry) public override returns (uint256 tokenId) {
        require(_alchemyRetort == msg.sender, "Minting not permitted");  //uncomment to restrict minting
        tokenId = _tokenIdIndex;
        _mint(to, tokenId);
        _tokenCommitmentIds[tokenId] = commitmentEntry;
        _tokenIdIndex++;
    }
    ...
    function burn(uint256 tokenId) public {
        ...
        IAlchemyRetort alchemyRetort = IAlchemyRetort(_alchemyRetort);
        alchemyRetort.unwrapToken(_tokenCommitmentIds[tokenId], ownerOf(tokenId));
        _burn(tokenId);
        delete _tokenCommitmentIds[tokenId];
    }
    
    function getCommitmentId(uint256 tokenId) public view returns (uint256 commitmentId) {
        return _tokenCommitmentIds[tokenId].commitmentId;
    }

    function getCommitmentIndex(uint256 tokenId) public view returns (uint256 commitmentId) {
        return _tokenCommitmentIds[tokenId].commitmentIndex;
    }
