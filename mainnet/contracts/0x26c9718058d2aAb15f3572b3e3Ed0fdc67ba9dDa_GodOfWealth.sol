...
Caishen, Wade-Giles romanization Ts’ai Shen, also called Cai Boxing Jun, in Chinese religion, the popular god (or gods) of wealth,
 widely believed to bestow on his devotees the riches carried about by his attendants.

$CAISHEN is a hyper-deflationary reward token that was catered to be one of the fastest yield generating token.
...
Website: https://caishenERC.com
Telegram: @GodOfWarCaishen
*/
...
contract GodOfWealth is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("God Of Wealth", "CAISHEN") {

        uint256 totalSupply = 100 * 1e6 * 1e18;
        
        maxTransactionAmount = totalSupply * 1000 / 1000; // 0.5% maxTransactionAmountTxn
        swapTokensAtAmount = totalSupply * 50 / 10000; // 0.05% swap tokens amount
    ...
    function _transfer(
        ...
        if(takeFee){
            if(tradingActiveBlock + 0 >= block.number && (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from])){
