...
contract TDJSyntheticNft is ERC721Enumerable, Ownable {
    ...
    mapping(address => bool) public onceFreeMap;
    uint256 public nftPrice;
    uint256 public constant MAX_SUPPLY = 8888;
    Counters.Counter private _tokenIdTracker;

    event CreateNft(address indexed owner, uint256 indexed id);
    event SetExecutorAddressEvent(address indexed executorAddress);
    event SetNewPriceEvent(uint256 price);
    event PauseEvent(bool pause);
    constructor(address _executorAddress, uint256 _price) ERC721("TopDJ Tickets NFT", "TopDJ Tickets NFT"){
    ...
    function setExecutorAddress(address _executorAddress) public onlyOwner {
        executorAddress = _executorAddress;
        emit SetExecutorAddressEvent(_executorAddress);
    }
    function setPrice(uint256 _price) public onlyOwner {
        nftPrice = _price;
        emit SetNewPriceEvent(_price);
    }
   function adminMint() public onlyOwner {
        ...
        _mintAnNFT(msg.sender, _tokensId);
    }

    function mintByWhitelist(address to, uint256 _timestamp, uint256 code, bytes memory _signature) public nftIsOpen{
        uint256 _tokensId = getNextTokenID();
        address signerOwner = signatureWallet(to, code,_timestamp,_signature);
        require(totalSupply() + 1 <= MAX_SUPPLY, "Max supply reached");
        require(signerOwner == executorAddress, "Not authorized to mint");
        require(rawOwnerOf(_tokensId) == address(0) && _tokensId > 0, "Token already minted");
        require(_codeMap[code] == false, "code already exists");
        require(onceFreeMap[to] == false, "You have already mint a free NFT");
        _codeMap[code] = true;
        onceFreeMap[to] = true;
        _mintAnNFT(to, _tokensId);
    }

    function mint() payable public nftIsOpen{
        uint256 _tokensId = getNextTokenID();
        require(totalSupply() + 1 <= MAX_SUPPLY, "Max supply reached");
        require(nftPrice == msg.value, "value error");
        require(rawOwnerOf(_tokensId) == address(0) && _tokensId > 0, "Token already minted");
        _mintAnNFT(msg.sender, _tokensId);
    }

    function _mintAnNFT(address _to, uint256 _tokenId) private {
        _tokenIdTracker.increment();
        _mint(_to, _tokenId);
        emit CreateNft(_to, _tokenId);
    }
    ...
    function signatureWallet(address to, uint256 code, uint256 _timestamp, bytes memory _signature) public pure returns (address){
        return ECDSA.recover(keccak256(abi.encode(to, code, _timestamp)), _signature);
    }
