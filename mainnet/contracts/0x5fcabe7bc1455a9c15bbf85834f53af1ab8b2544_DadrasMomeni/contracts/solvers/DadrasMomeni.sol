 ...
 * Dadras-Momeni system of differential equations.
 ...
 */
contract DadrasMomeni is AttractorSolver3D {
    /**
     ...
     */
    int256 private constant ALPHA = 3 * 2**96;
    int256 private constant BETA = (27 * 2**96 - 2) / 10;
    int256 private constant GAMMA = (17 * 2**96 - 2) / 10;
    int256 private constant DELTA = 2 * 2**96;
    int256 private constant EPSILON = 9 * 2**96;
        ...
        returns (StartingPoint memory startingPoint)
    {
        ...
        (randomSeed, randNumber) = _random(randomSeed, range);
        startingPoint.startingPoint[1] = 5 * ONE + randNumber;

        (randomSeed, randNumber) = _random(randomSeed, range);
        startingPoint.startingPoint[2] = -4 * ONE + randNumber;
    }
     ...
     * considered systems. Before storing the results, points are projected to
     * 2D.
     ...
     * @return tangents Attention! This array is left empty for the Dadras 
     * Momeni system because we are going to use point locations exclusively.
     * See also `PointRenderer`.
     */
    function _solve(
        ...
        returns (bytes memory points, bytes memory tangents)
    {
        ...
        // We are not going to compute tangents for this one.
        tangents = new bytes(0);
                        ...
                        // RK4 intermediate step 0
                        {
                            ...
                            dxdt := add(
                                ...
                                sar(
                                    PRECISION,
                                    sub(
                                        mul(y, sar(PRECISION, mul(BETA, z))),
                                        mul(ALPHA, x)
                                    )
                                )
                            )
                            ...
                            dydt := add(
                                z,
                                sar(PRECISION, sub(mul(GAMMA, y), mul(x, z)))
                            )
                            ...
                            dzdt := sar(
                                PRECISION,
                                sub(
                                    mul(DELTA, sar(PRECISION, mul(x, y))),
                                    mul(EPSILON, z)
                                )
                            )
