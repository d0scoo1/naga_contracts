...
contract MyUniverse is Ownable, ERC721A {
  ...
  uint public MAX_SUPPLY          = 7983;
  uint public PRICE_PUBLIC        = 0.06 ether;
  uint public MAX_WHITELIST_BATCH = 3;

  mapping(address => uint8) private whitelistBuyList;

  constructor() ERC721A("The My Universe Official Collection", "My Universe") {
    revealed = false;
    open = false;
  }
  ...
  function _Mint(uint8 quantity) external payable callerIsUser{
    require(open == true, "Sale doesn't open");
    require(totalSupply() < MAX_SUPPLY, "PUBLIC_SALE: Please go to the Opensea to buy The My Universe.");
