...
contract TimelessNumber is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x5ea8097d601aDB560fa49a3425F8ceFF793D67a6);
    address payable private _DevAddress;

    uint256 public TMNUM_MAX = 7250;
    uint256 public PRESALE_PRICE = 40_000_000_000_000_000; // 0.04 ETH
    uint256 public PUBLIC_PRICE = 50_000_000_000_000_000; // 0.05 ETH
    uint256 private DEV_FEE = 0;
    uint256 private VIP_START_HOURS = 24 hours; // Start 24hrs before 
    uint256 private PRESALE_START_HOURS = 12 hours; // Start 12hrs before 
    
    uint256 public VIP_MINT_LIMIT = 1;
    uint256 public PRESALE_MINT_LIMIT = 3;
    uint256 public PUBLIC_MINT_LIMIT = 7;
    
    
    mapping(address => bool) _mappingVIPList;
    mapping(address => bool) _mappingWhiteList;
    mapping(address => uint256) _mappingMintCount;

    uint256 private _revealDateTime = 1640304000; // Date and time (GMT): Friday, December 24, 2021 12:00:00 AM
    uint256 private _activeDateTime = 1639526400; // Date and time (GMT): Wednesday, December 15, 2021 12:00:00 AM
    
    string private _tokenBaseURI = "";

    Counters.Counter private _publicTMNUM;

    constructor() ERC721("Timeless Number", "TMNUM") {
    ...
    function setPresaleHours (uint256 VIPStartHours, uint256 presaleStartHours) external onlyOwner {
        VIP_START_HOURS = VIPStartHours;
    ...
    function setVIPList(address[] memory VIPListAddress, bool bEnable) external onlyOwner {
        for (uint256 i = 0; i < VIPListAddress.length; i++) {
            _mappingVIPList[VIPListAddress[i]] = bEnable;
        }
    }
    ...
    function PRICE(address addr) public view returns (uint256) {
        if (block.timestamp < _activeDateTime - PRESALE_START_HOURS && block.timestamp > _activeDateTime - VIP_START_HOURS) {
            require(_mappingVIPList[addr] == true, "Only VIP member can mint now");
            return 0;
        } else if (block.timestamp < _activeDateTime && block.timestamp < _activeDateTime) {
            require(_mappingVIPList[addr] == true || _mappingWhiteList[addr] == true , "Only VIP and Whitelist member can mint now");
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicTMNUM.current();

            if (_publicTMNUM.current() < TMNUM_MAX) {
                _publicTMNUM.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        require(_publicTMNUM.current() < TMNUM_MAX,"Purchase would exceed TMNUM_MAX");
        ...
        if (msg.sender != owner()) {
            ...
            if (_mappingVIPList[msg.sender] == true) {
                require(block.timestamp > _activeDateTime - VIP_START_HOURS,"Contract is not active for VIP");

                if (block.timestamp < _activeDateTime - PRESALE_START_HOURS) {
                    require(_mappingMintCount[msg.sender] <= VIP_MINT_LIMIT,"Overflow for PRESALE_MINT_LIMIT");
                } else if (block.timestamp < _activeDateTime) {
                    require(_mappingMintCount[msg.sender] <= VIP_MINT_LIMIT + PRESALE_MINT_LIMIT ,"Overflow for PRESALE_MINT_LIMIT");
                } else {
                    require(_mappingMintCount[msg.sender] <= VIP_MINT_LIMIT + PRESALE_MINT_LIMIT + PUBLIC_MINT_LIMIT,"Overflow for PUBLIC_MINT_LIMIT");
                }
            } else if (_mappingWhiteList[msg.sender] == true) {
                require(block.timestamp > _activeDateTime - PRESALE_START_HOURS,"Contract is not active for presale");
                ...
                if (block.timestamp < _activeDateTime) {
                    ...
                    require(_mappingMintCount[msg.sender] <= PRESALE_MINT_LIMIT + PUBLIC_MINT_LIMIT,"Overflow for PUBLIC_MINT_LIMIT");
                }
            } else {
