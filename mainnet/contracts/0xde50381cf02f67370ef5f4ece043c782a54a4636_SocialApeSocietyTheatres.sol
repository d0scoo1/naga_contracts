...
contract SocialApeSocietyTheatres is AccessControl, ERC721Enumerable {
    using Strings for uint256;
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    
    uint256 public constant TIX_LIMIT = 1;
    uint256 public constant PRICE = 14000000000000000;
    ...
    Counters.Counter private _publicSAST;
    
    
    


    constructor(address owner_, string memory uri_) ERC721("Social Ape Society Theatres", "SAST")  {
...
function setContractURI(string memory URI) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function purchase(uint256 numberOfTokens)  public payable {
       if(msg.sender != owner()){
        ...
        require(numberOfTokens <= TIX_LIMIT, 'Can only mint up to 1 Tickets');
