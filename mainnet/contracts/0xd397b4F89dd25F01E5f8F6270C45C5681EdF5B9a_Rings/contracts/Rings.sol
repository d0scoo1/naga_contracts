...
contract Rings is ERC721URIStorage, Ownable{
 
    event Mintrings (address indexed minter, uint256 startWith, uint256 times);
    
    
    uint256 public totalrings;
    uint256 public totalCount = 7723; //bruhTotal
    uint256 public maxBatch = 10; // bruhBatch
    uint256 public price = 100000000000000000; // 0.06 eth
    ...
    function mintring(uint256 _times) payable public mintEnabled {
        require(_times >0 && _times <= maxBatch, "mint wrong number");
        require(totalrings + _times <= totalCount, "too much");
        require(msg.value == _times * price, "value error");
        payable(owner()).transfer(msg.value);
        emit Mintrings(_msgSender(), totalrings+1, _times);
        for(uint256 i=0; i< _times; i++){
            _mint(_msgSender(), 1 + totalrings++);
        }
    } 
    ...
    function whitelistMint(uint _times) payable public canWhitelistMint {
        ...
        require(totalrings + _times <= totalCount, "Mint amount will exceed total collection amount.");
    ...
    function adminMintGiveaways(address _addr) public onlyOwner {
        require(totalrings + 1 <= totalCount, "Mint amount will exceed total collection amount.");
        emit Mintrings(_addr, totalrings+1, 1);
        _mint(_addr, 1 + totalrings++);
    } 
