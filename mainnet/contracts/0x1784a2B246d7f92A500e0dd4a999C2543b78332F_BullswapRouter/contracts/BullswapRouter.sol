...
contract BullswapRouter {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = BullswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = BullswapLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "Bullswap: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = BullswapLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "Bullswap: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        (uint256 amount0, uint256 amount1) = IBullswapPair(pair).burn(to);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? BullswapLibrary.pairFor(factory, output, path[i + 2])
                : _to;
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            BullswapLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = BullswapLibrary.sortTokens(input, output);
            IBullswapPair pair = IBullswapPair(
                BullswapLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = BullswapLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
