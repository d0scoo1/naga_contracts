...
contract InscrutableApes is ERC721A, ERC721Holder, Ownable, ReentrancyGuard{
    ...
    uint256 public maxPreorder = 3;
    uint256 public maxPublicTx = 10;
    uint256 public maxPreorderReserved = 5000;

    bool public _isActive = false;
    bool public _presaleActive = false;
    bool public _preorderActive = true;
    ...
    function premint(uint256 _count, bytes32[] calldata _merkleProof) public payable nonReentrant {
        require(_presaleActive, "Pre mint is not active");
        require(totalSupply() + _count <= MAX_SUPPLY - preordersQty, "Purchase would exceed max supply of Tokens");
        require(_count <= maxPresale - presales[_msgSender()], "Insufficient mints left");
        require(_verify(_merkleProof, _msgSender()), "Invalid MerkleProof");
    ...
    function preorder(uint _count) public payable nonReentrant {
        ...
        require(preordersQty < maxPreorderReserved, "Exceeds Pre-Orders limit");
        require(_preorderActive, "Preorders is not active");
        require(totalSupply() + _count <= MAX_SUPPLY - preordersQty, "Exceeds MAX limit");
        require(preorders[msg.sender] + _count <= maxPreorder, "Exceeded max available to preorders");
        require(msg.value == _count * PRICE_PREORDER, "Incorrect funds");
    ...
    function cancelPreorder() public {
        ...
        preordersQty -= preorders[msg.sender];
        preorders[msg.sender] = 0;
    }

    function mintPreorder() public nonReentrant{
        require(totalSupply() < MAX_SUPPLY, "Sale end");
        require(!_preorderActive, "Preorder mint closed");
        require(preorders[msg.sender] > 0, "No preorders");
