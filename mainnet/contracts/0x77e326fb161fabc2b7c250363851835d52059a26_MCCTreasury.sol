 ...
 */
contract MCCTreasury is Ownable {
    using SafeMath for uint256;

    address public stables = 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48;
    address public rewards = 0x4Fd61669334F6feDf5741Bfb56FE673bD53a730F;
    address payable public treasury = 0xFBf335f8224a102e22abE78D78CC52dc95e074Fa;
    ...
    */
    ...
    /**
    * @notice Sweep Treasury Tokens
    */
    function sweepTreasuryMCC() public {
        require(msg.sender == tx.origin, "EOA Only");
        uint256 actualWithdrawAmount = BEP20Interface(mcc).balanceOf(address(this));

        // Transfer Token to withdrawAddress
        BEP20Interface(mcc).transfer(rewards, actualWithdrawAmount);

        emit WithdrawTreasury(mcc, actualWithdrawAmount, rewards);
    }

    /**
    * @notice Sweep Treasury Tokens
    */
    function sweepTreasuryStables() public {
        require(msg.sender == tx.origin, "EOA Only");
        uint256 actualWithdrawAmount = BEP20Interface(stables).balanceOf(address(this));

        // Transfer Token to withdrawAddress
        BEP20Interface(stables).transfer(treasury, actualWithdrawAmount);

        emit WithdrawTreasury(stables, actualWithdrawAmount, treasury);
    }

    /**
    * @notice Sweep Treasury ETH
    */
    function sweepTreasuryETH() public {
        require(msg.sender == tx.origin, "EOA Only");
        uint256 actualWithdrawAmount = address(this).balance;

        // Transfer to withdrawAddress
        treasury.transfer(actualWithdrawAmount);

        emit WithdrawTreasuryETH(actualWithdrawAmount, treasury);
    }
    ...
    */
    ...
    function setStables(address _stables) external onlyOwner {
    stables = _stables;
  }
