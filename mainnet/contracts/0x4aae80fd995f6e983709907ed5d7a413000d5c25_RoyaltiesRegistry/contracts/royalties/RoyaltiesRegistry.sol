...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
    ...
    function getRoyalties(address token, uint tokenId) override external returns (LibPart.Part[] memory) {
        ...
        //case royaltiesType = 2, royalties  v2
        if (royaltiesType == 2) {
            return getRoyaltiesV2(token,tokenId);
        }

        //case royaltiesType = 3, royalties  v1
        if (royaltiesType == 3) {
            return getRoyaltiesV1(token, tokenId);
        }
    ...
    /// @dev tries to get royalties -v2 for token and tokenId
    function getRoyaltiesV2(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
        try RoyaltiesV2(token).getV2Royalties(tokenId) returns (LibPart.Part[] memory result) {
            return result;
        } catch {
            return new LibPart.Part[](0);
        }
    ...
    /// @dev tries to get royalties -v1 for token and tokenId
    function getRoyaltiesV1(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
