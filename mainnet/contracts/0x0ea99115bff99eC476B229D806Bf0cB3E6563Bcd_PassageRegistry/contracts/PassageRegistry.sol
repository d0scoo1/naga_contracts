...
///                                  \___/'
///
/// @title Passage Registry
/// @notice The registry facilitates creation of Passports and Loyalty Ledgers
///

contract PassageRegistry is
    Initializable,
    ...
    IPassageRegistry
{
    ...
    function createPassport(
        ...
        bytes memory data = abi.encodeWithSignature(
            "initialize(address,string,string,bool,uint256)",
            _msgSender(),
            _tokenName,
            _tokenSymbol,
            _transferEnabled,
    ...
    function upgradePassport(uint256 version, address passportAddress) external returns (uint256 newVersion) {
        IPassport passport = IPassport(passportAddress);
    ...
    function createLoyalty() external returns (address loyaltyAddress) {
        bytes memory data = abi.encodeWithSignature("initialize(address)", _msgSender());
    ...
    function upgradeLoyalty(uint256 version, address loyaltyAddress) external returns (uint256 newVersion) {
        ILoyaltyLedger ll = ILoyaltyLedger(loyaltyAddress);
