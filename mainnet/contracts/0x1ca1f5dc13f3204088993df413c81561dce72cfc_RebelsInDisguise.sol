...
contract RebelsInDisguise is ERC721, ERC721Enumerable, Ownable {
    constructor(string memory name, string memory symbol) ERC721(name, symbol) 
    {}
    
    address public rebelsCoinAddress;
    string public _baseUri;
    uint256 public _tokenId = 0;
    bool public isMintingLocked = false;

    function mintTransfer(address to, uint256 _count) public {
        require(msg.sender == rebelsCoinAddress, "Not authorized");
        require(isMintingLocked == false, "Minting has been locked!");
    ...
    // Change the Rebels Coin address contract
    function setRebelsCoinAddress(address newAddress) public onlyOwner { 
        rebelsCoinAddress = newAddress;
    }
    ...
    function mintToAddress(address contractAddress, uint256 _count) public onlyOwner {
        ...
        for (uint256 index = 0; index < _count; index++) {
            _mint(contractAddress, _tokenId);
            _tokenId ++;
        }
    }
