...
contract ElfVille is ERC721A, Ownable {
	using Strings for uint256;
	...
	constructor(string memory initUnrevealedUriPrefix) ERC721A("Elf Ville", "ELF") {
		paused = true;
		cost = 0.004 ether;
		maxSupply = 2_000;
	...
	function mint(uint256 amount) external payable {
		require(tx.origin == _msgSender(), "ElfVille: contract denied");
		require(!paused, "ElfVille: minting is paused");
		require(_totalMinted() + amount <= maxSupply, "ElfVille: max token supply exceeded");
		require(amount > 0 && _numberMinted(_msgSender()) + amount <= maxMintAmountPerAddress, "ElfVille: invalid mint amount");

		if (_totalMinted() + amount > freeToMintSupply || _claimedFreeMint(_msgSender())) {
			require(msg.value >= cost * amount, "ElfVille: insufficient ether");
		} else {
	...
	function airDrop(address[] calldata addresses, uint8[] calldata amounts) external onlyOwner {
		require(addresses.length == amounts.length && addresses.length > 0, "ElfVille: invalid function arguments");

		uint256 newSupply = _totalMinted();
		for (uint256 i = 0; i < amounts.length; i++) newSupply += amounts[i];

		require(newSupply <= maxSupply, "ElfVille: max token supply exceeded");
	...
	function withdraw() external onlyOwner {
		...
		require(success, "ElfVille: failed to transfer");
	}
