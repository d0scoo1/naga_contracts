...
contract RoyaltyOwnersClub is Ownable, ERC721, Pausable {
    using Strings for uint256;

    uint256 public constant TOTAL_QTY = 3333;
    uint256 public constant FREE_MINT_MAX_QTY = 0;
    uint256 public constant TEAM_RESERVE_QTY = 3;
    ...
    constructor(string memory tokenBaseURI) ERC721("Royalty Owners Club", "ROC") {
    ...
    function reserve() external onlyOwner {
        for (uint256 i = 1; i <= TEAM_RESERVE_QTY; ++i) {
            _mint(msg.sender, mintedQty_ + i);
        }
