...
contract KKON is Context, IERC20, ERC20Ownable {
    ...
    uint256 private tokensForTreasuryToSwap;

    string private constant _name = "KKONS RESERVE";
    string private constant _symbol = "KKON";
    uint8 private constant _decimal = 18;

    //DEFAULT PLACE HOLDERS | OVERRIDDEN EACH TXN
    uint private _marTax = 7; // tax for marketing
    uint private _previousMarTax = _marTax;
    uint private _treasuryTax = 3; // tax for GiveAways
    uint private _previousTreasuryTax = _treasuryTax;
    ...
    uint private _liqDiv = _marTax + _treasuryTax + _liqTax;

    //MAIN TAX VALUES FOR BUY | CHANGEBLE WITH functon setBuyF()
    uint private _buyMarTax = 7;
    uint private _preBuyMarTax = _buyMarTax;
    uint private _buyTreasuryTax = 3;
    uint private _preBuyTreasuryTax = _buyTreasuryTax;
    ...
    uint private _sellMarTax = 7;
    uint private _preSellMarTax = _sellMarTax;
    uint private _sellTreasuryTax = 3;
    uint private _preSellTreasuryTax = _sellTreasuryTax;
    ...
    constructor() {
        ...
        marketingAddress = payable(0x6f3403d1f8abB81d61bDB78660410e31af00208E);
        treasuryAddress = payable(0xeE640dfA14e9adCf1dA878f2E9FD87ecCD9C331F);
        minTokensBeforeSwap = _tTotal.mul(5).div(10000);
        _isExcludedFromFee[_msgSender()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[address(marketingAddress)] = true;
        _isExcludedFromFee[address(treasuryAddress)] = true;
        ...
        _isMaxWalletExclude[address(treasuryAddress)] = true;
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        tokensForTreasuryToSwap += tLiquidity * _treasuryTax / _liqDiv;
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_marTax + _treasuryTax + _liqTax).div(10**2);
    }
    function removeAllFee() private {
        if (_refTax == 0 && _liqTax == 0 && _marTax == 0 && _treasuryTax == 0) return;
    ...
    function restoreAllFee() private {
        ...
        _treasuryTax = _preBuyTreasuryTax;
    }
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (_buyLimits == true && from == uniV2Pair) {
			require(amount <= 10000000000 * 10**18, "Limits are in place, please lower buying amount");
		}
        if(_initLaunch == true) {
        ...
        if (to == uniV2Pair) {
            _marTax = _sellMarTax;
            _treasuryTax = _sellTreasuryTax;
            _liqTax = _sellLiqTax;
            _refTax = _sellRefTax;
        } else if (from == uniV2Pair) {
            _marTax = _buyMarTax;
            _treasuryTax = _buyTreasuryTax;
            _liqTax = _buyLiqTax;
            _refTax = _buyRefTax;
        }
	...
	function swapTokens() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = tokensForLiquidityToSwap + tokensForMarketingToSwap + tokensForTreasuryToSwap;
        ...
        uint256 ethForTreasury = ethBalance.mul(tokensForTreasuryToSwap).div(totalTokensToSwap);
    ...
    function setBuyF(uint buyMarTax, uint buyTreasuryTax, uint buyLiqTax, uint buyRefTax) external onlyOwner {
        _buyMarTax = buyMarTax;
        _buyTreasuryTax = buyTreasuryTax;
        _buyLiqTax = buyLiqTax;
        _buyRefTax = buyRefTax;
    }
    function setSellF(uint sellMarTax, uint sellTreasuryTax, uint sellLiqTax, uint sellRefTax) external onlyOwner {
