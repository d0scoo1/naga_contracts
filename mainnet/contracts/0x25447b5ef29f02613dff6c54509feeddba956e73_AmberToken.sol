...
contract AmberToken is ERC20, Ownable {
    ...
    function mintForUser(address user) external {
        ...
        for (uint256 i = 0; i < total; i++) {
            uint256 id = DRAGONS.tokenOfOwnerByIndex(user, i);
    ...
    function mintForIds(uint256[] calldata ids) external {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            uint256 id = ids[i];
            address owner = DRAGONS.ownerOf(id);
            uint256 totalAmber_PerSecond_PerDragon = getTotalAmberPerSecondPerDragon(owner);
            uint256 claimed = lastClaim(id);
            uint256 time = min(block.timestamp, END);
            uint256 owed = (time - claimed) * totalAmber_PerSecond_PerDragon;
            _mint(owner, owed);
            last[id] = time;
        }
    }
