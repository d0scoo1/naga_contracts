...
contract yVaren is IyVaren, ERC20, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_varen != address(0) && _treasury != address(0), "yVaren: ZERO_ADDRESS");
    ...
    function stake(uint256 amount) external override nonReentrant {
        ...
        uint256 shares = totalSupply() == 0 ? amount : (amount.mul(totalSupply())).div(VAREN.balanceOf(address(this)));
    ...
    function withdraw(uint256 shares) external override nonReentrant {
        ...
        uint256 varenAmount = (VAREN.balanceOf(address(this))).mul(shares).div(totalSupply());
        _burn(msg.sender, shares);
        if (block.number < earlyWithdrawalFeeExpiry[msg.sender]) {
            uint256 feeAmount = varenAmount.mul(earlyWithdrawalFeePercent) / 1000000;
            VAREN.safeTransfer(treasury, feeAmount.mul(treasuryEarlyWithdrawalFeeShare) / 1000000);
    ...
    function getStakeVarenValue(address staker) external view override returns (uint256) {
        return (VAREN.balanceOf(address(this)).mul(balanceOf(staker))).div(totalSupply());
    }
    ...
    function propose(
        ...
        require(
            (VAREN.balanceOf(address(this)).mul(balanceOf(msg.sender))).div(totalSupply()) >= minVarenForProposal,
        ...
        proposals[id] = Proposal({
            ...
            totalForVotes: 0,
            totalAgainstVotes: 0,
            quorumVotes: VAREN.balanceOf(address(this)).mul(quorumPercent) / 1000000,
            executed: false
        });
    ...
    function vote(
        ...
        require(voteAmount <= balanceOf(msg.sender), "yVaren: INSUFFICIENT_BALANCE");
        _updateVoteExpiry();
        require(voteAmount >= voteLockAmount[msg.sender], "yVaren: SMALLER_VOTE");
        if (
    ...
    function executeProposal(uint256 id) external payable override nonReentrant {
        ...
        require(!proposal.executed, "yVaren: PROPOSAL_ALREADY_EXECUTED");
        {
            ...
            require(proposal.proposer != address(0), "yVaren: INVALID_PROPOSAL_ID");
            require(block.number >= proposal.endBlock, "yVaren: PROPOSAL_IN_VOTING");
        ...
        for (uint256 i = 0; i < proposal.targets.length; i++) {
            if (proposal.values[i] > 0) {
                require(remainingValue >= proposal.values[i], "yVaren: INSUFFICIENT_ETH");
    ...
    function setMinVarenForProposal(uint256 _minVarenForProposal) external override onlyThis {
        // min 0.01 Varen, max 520 Varen (1% of total supply)
        require(_minVarenForProposal >= 1e16 && _minVarenForProposal <= 520 * (1e18));
