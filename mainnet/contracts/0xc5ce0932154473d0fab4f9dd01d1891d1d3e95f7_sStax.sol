...
// File: interfaces/IgStax.sol


pragma solidity >=0.7.5;


interface IgStax is IERC20 {
  ...
  function migrate( address _staking, address _sStax ) external;
}
...
contract sStax is IsStax, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgStax public gStax; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked Stax", "sStax", 9) ERC20Permit("Staked Stax") {
    ...
    function setgStax(address _gStax) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gStax) == address(0), "gStax:  gStax already set");
        require(_gStax != address(0), "gStax:  gStax is not a valid contract");
        gStax = IgStax(_gStax);
    }
    ...
    /**
        @notice increases rStax supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sStax of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sStax
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sStax: insufficient balance");
    }
    ...
    // toG converts an sStax balance to gStax terms. gStax is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gStax.balanceTo(amount);
    }

    // fromG converts a gStax balance to sStax terms. sStax is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gStax.balanceFrom(amount);
    }

    // Staking contract holds excess sStax
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gStax.balanceFrom(IERC20(address(gStax)).totalSupply())).add(
