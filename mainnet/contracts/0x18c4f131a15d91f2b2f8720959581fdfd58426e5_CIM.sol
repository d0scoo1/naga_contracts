...
contract CIM is ERC721A, Ownable {
    uint256 public  maxSupply = 10000;
    mapping (address=>int) userAdr;

    constructor() ERC721A("Crypto Invisible Mfers", "CIM") {}

    uint256 public mintPrice = 0.00365 ether;
    uint256 public maxAmountPerMint = 10;
    function addWhite(address[] calldata user) external onlyOwner{
        for(uint i = 0;i<user.length;i++){
            userAdr[user[i]] = 1;
        }
    }
    function isWhite(address adr) external view returns(uint64){
        if(userAdr[adr] > 0){
            return 1;
        }else{
            return 0;
    ...
    function setMaxSupply(uint256 max) external onlyOwner {
        require( max >= totalSupply(), "max must > totalSupply");
        maxSupply = max;
    }
     ...
     * metadata URI
     */
    string private _baseURIExtended = "https://formfers.mypinata.cloud/ipfs/QmaS7tfSfxfEuCRonnyW4GzfZ23gXaikayTJkKTpQEEmQa/";
    ...
    function publicMint(uint amount) external payable {
        require(totalSupply() + amount <= maxSupply, "current phase minting was ended");
        require(amount <= maxAmountPerMint, "Exceeded max token purchase");
 
        uint256 totalMintCost = amount * mintPrice;
    ...
    function gift() public{
        require(userAdr[msg.sender] > 0, "Whitelist only");
        userAdr[msg.sender] = 0;
