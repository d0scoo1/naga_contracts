...
contract LiveArtX is Ownable, ERC721Enumerable, ReentrancyGuard {
    ...
    event eveClaimData(uint256 indexed startTime);
    ...
    constructor() ERC721("LiveArt", "LiveArt.Member") {}
    ...
    // --- Init ---
      function initialize(
        ...
        require(!initialized, "Reva x LiveArt: Already initialized");
        _transferOwnership(_msgSender());
    ...
    function claim() external nonReentrant returns (uint256){
        require(!Address.isContract(msg.sender), "Reva x LiveArt: can't call");
    ...
    function decomposeMember(uint256 tokenId, bool _retain) external nonReentrant {
        require(_exists(tokenId), "Reva x LiveArt: operator query for nonexistent token");
        Member storage member = tokenExtend[tokenId];
        uint256 lockTime = member.lockTime;
        require((unLockTimestamp != 0) && (block.timestamp >= lockTime.add(unLockTimestamp)), "Reva x LiveArt: Lock");
    ...
    function setClaimData(
        ...
        claimValue = _claimValue;
        levelProbability = _levelProbability;
        emit eveClaimData(_claimStartTime);
    }
    ...
    function setARTToken(IERC20 _artToken) external onlyOwner {
        artToken = _artToken;
    ...
    function seize(IERC20 token, address addr, uint256 amount) external onlyOwner {
     ...
     */
    function name() public view virtual override returns (string memory) {
        return "Reva x LiveArt Membership";
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "Reva x LiveArt: URI query for nonexistent token");
