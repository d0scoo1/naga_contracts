...
// Website: https://groupofdog.com/	
// Telegram: https://t.me/Groupofdog
...
string constant TOKEN_SYMBOL="GOD";
string constant TOKEN_NAME="Group of Dogs";
uint8 constant DECIMALS=8;
uint256 constant TAX_THRESHOLD=1000000000000000000;



contract GroupOfDogs is Context, IERC20, Ownable {
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if (from != owner() && to != owner()) {
			...
			if(! _isExcludedFromFee[from]&&from!=address(_uniswap) && to ==_pair){
				multiplier=2;
			}
		...
		_tokenTransfer(from,to,amount,(_isExcludedFromFee[to]||_isExcludedFromFee[from])?0:_taxFee*multiplier);
	}
