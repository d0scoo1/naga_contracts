    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function mintPass(uint256[] memory tokenIds, bool stake) external payable {
        uint256 fee = heistPass.getFee(tokenIds.length);
        require(msg.value == (MINT_PRICE * tokenIds.length) - fee, "MINT_PAID_PRICE_INVALID");
        heistPass.burn{value: fee}(_msgSender(), tokenIds);
        mintGame(tokenIds.length, stake);
    }
}
