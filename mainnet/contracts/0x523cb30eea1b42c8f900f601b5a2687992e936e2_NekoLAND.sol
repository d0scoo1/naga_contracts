...
contract NekoLAND is Context, IERC20 {
    ...
    uint256 public _reflectionsRatio = 3;
    ...
    address payable private _marketingWallet = payable(0x69Ac1Bd112F498F17Ab157DFd4960D98E85eF931);
    address payable private _buybackWallet = payable(0x69Ac1Bd112F498F17Ab157DFd4960D98E85eF931);
...
//===============================================================================================================
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_liquidityRatio + _marketingRatio + _reflectionsRatio == 0)
            return;
        uint256 toLiquify = ((contractTokenBalance * _liquidityRatio) / (_liquidityRatio + _marketingRatio + _reflectionsRatio)) / 2;

        uint256 toSwapForEth = contractTokenBalance - toLiquify;
        swapTokensForEth(toSwapForEth);

        uint256 currentBalance = address(this).balance;
        uint256 liquidityBalance = ((currentBalance * _liquidityRatio) / (_liquidityRatio + _marketingRatio + _reflectionsRatio)) / 2;
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _marketingWallet.transfer(((currentBalance - liquidityBalance) * _marketingRatio) / (_marketingRatio + _reflectionsRatio));
