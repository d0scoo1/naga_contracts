...
// File: contracts/KWWDataBoats.sol
...
contract KWWDataBoats is Ownable, IKWWDataBoats { 
    address gameManager;
    KWWBoats boatsContract;

    mapping(uint16 => BoatDetails) public boatsData;
    mapping(uint8 => uint8) public routeToDuration;

    uint8[] public mintPossibleStates = [1];
    uint8 public capacity = 10;

    /*
       EXECUTABLE FUNCTIONS
    */

    function initBoat(uint16 tokenId) override public onlyBoatsContract{
        uint8 state = randomState();
        boatsData[tokenId] = BoatDetails(new uint32[](0), 0, 0, state, 0);
    }

    //add kangaroo to the list of onboard kangaroos
    function addKangaroo(uint16 tokenId, uint32 kangarooId) public onlyGameManager {
        require(boatsData[tokenId].boatState != 0, "Token not exists");
        require(boatsData[tokenId].kangaroos.length < capacity, "Boat is full!");
        boatsData[tokenId].kangaroos.push(kangarooId);
    }

    //remove All Kangaroos on board - boat will be empty
    function removeAllKangaroos(uint16 tokenId) public onlyGameManager {
        require(boatsData[tokenId].boatState != 0, "Token not exists");
        delete boatsData[tokenId].kangaroos;
    }

    function updateStatus(uint16 tokenId, uint8 status) public onlyGameManager {
        require(boatsData[tokenId].boatState != 0, "Token not exists");
        boatsData[tokenId].status = status;
    }

    function updateRoute(uint16 tokenId, uint8 route) public onlyGameManager {
        require(boatsData[tokenId].boatState != 0, "Token not exists");
        boatsData[tokenId].route = route;
    }

    function updateBoatState(uint16 tokenId, uint8 boatState) public onlyGameManager {
        require(boatsData[tokenId].boatState != 0, "Token not exists");
        boatsData[tokenId].boatState = boatState;
    }

    function updateStartSailTime(uint16 tokenId, uint64 _startSailTime) public onlyGameManager{
        require(boatsData[tokenId].boatState != 0, "Token not exists");
        boatsData[tokenId].startSailTime = _startSailTime;
    }
    ...
    */
    ...
    function getBoatData(uint16 tokenId) public view returns(BoatDetails memory){
        require(boatsData[tokenId].boatState != 0, "Token not exists");
    ...
    function getStartSailTime(uint16 tokenId) public view returns(uint64){
      ...
      return boatsData[tokenId].startSailTime;
        ...
        function getStatus(uint16 tokenId) public view returns(uint8){
      require(boatsData[tokenId].boatState != 0, "Token not exists");
      return boatsData[tokenId].status;
    }
    ...
    function randomState() internal view returns(uint8){
      ...
      uint256 randomNum =  KWWUtils.random(boatsContract.totalSupply());
    ...
    */
    ...
    modifier onlyBoatsContract() {
        require(address(boatsContract) != address(0), "Boats Contract not exists");
        require(msg.sender == owner() || msg.sender == address(boatsContract), "caller is not the Boats Contract");
        _;
    }  
    ...
    */
    ...
    function setBoatsContract(address _addr) public onlyOwner{
      boatsContract = KWWBoats(_addr);
    }

    function setCapacity(uint8 _capacity) public onlyOwner{
      capacity = _capacity;
    }
