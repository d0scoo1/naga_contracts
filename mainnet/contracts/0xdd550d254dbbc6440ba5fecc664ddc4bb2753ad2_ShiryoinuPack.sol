...
interface IShiryoinuPack {
    function isFounder(uint256 tokenId) external returns (bool);
    function burn(uint256 tokenId) external;
}


contract ShiryoinuPack is IShiryoinuPack,ERC721Enumerable, Ownable{
    ...
    uint256 public minimumFounderAmount = 25000000000000;
    ...
    constructor(IERC20 _founderToken, address _walletA, address _walletB) ERC721("ShiryoinuPack", "ShiryoinuPack") {
        _baseTokenURI = "https://app.shiryoinu.com/packs/";
        walletA = _walletA;
        walletB = _walletB;
    ...
    function isFounder(uint256 tokenId) view public  override returns (bool){
       return packs[tokenId].founder;
    }
    ...
    function mint_pack(uint256 _amount) public payable returns (uint256[] memory minted) {
        require(_amount>0 &&  mintAmountToPrice[_amount]>0, "NFT: Invalid amount to mint.");
        require(msg.value>0 && msg.value==mintAmountToPrice[_amount] , "NFT: Invalid value.");
