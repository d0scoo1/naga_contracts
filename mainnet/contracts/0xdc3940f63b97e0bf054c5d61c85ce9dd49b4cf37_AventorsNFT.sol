...
// File: contracts/AventorsNFT.sol
...
contract AventorsNFT is Ownable, ERC721A, ReentrancyGuard, PaymentSplitter {
    uint256 public maxSupply = 4000;

    uint256 public icPrice = 0.1 ether;
    uint256 public prePrice = 0.15 ether;
    ...
    constructor(string memory name, string memory symbol, address[] memory _payees, uint256[] memory _shares) ERC721A(name, symbol) PaymentSplitter(_payees, _shares) payable {}
    ...
    function mintIC(uint256 quantity) external payable callerIsUser nonReentrant {
        require(_presaleActive, "Pre-sale is not active");
        require(quantity > 0, "Must mint more than 0 tokens");
        require(icPrice * quantity == msg.value, "Incorrect funds");
    ...
    function mintPresale(uint256 quantity) external payable callerIsUser nonReentrant {
        require(_presaleActive, "Pre-sale is not active");
        require(quantity > 0, "Must mint more than 0 tokens");
        require(prePrice * quantity == msg.value, "Incorrect funds");
        require(totalSupply() + quantity <= maxSupply, "Purchase would exceed max supply of Tokens");
