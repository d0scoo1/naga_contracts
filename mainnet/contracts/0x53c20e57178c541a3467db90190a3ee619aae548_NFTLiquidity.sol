...
// File contracts/NFT_Liquidity.sol
...
contract NFTLiquidity is ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => uint256) public lockedTokenIds;
    ...
    function setDeterminedPercentages(uint256 _determinedPercentages)
        public
        onlyOwner
    {
        determinedPercentages = _determinedPercentages;
    ...
    function lowerFloorSell(uint256 _tokenId) external {
        ...
        lockedTokenIds[tokenIdIndex++] = _tokenId;
    }

    function upperFloorBuy(uint256 _tokenId) external payable nonReentrant {
        require(tokenIdIndex > 0, "There are no locked tokens.");
        require(
            msg.value >=
                address(this)
                    .balance
                    .div(divideNumber)
                    .mul(determinedPercentages.add(100))
                    .div(100),
            "Insufficient funds."
        );
