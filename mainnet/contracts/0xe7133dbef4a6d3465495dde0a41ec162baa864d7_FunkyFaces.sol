...
contract FunkyFaces is ERC721, PaymentSplitter {
    ...
    constructor(address[] memory _payees, uint256[] memory _shares)
        ERC721("Funky Faces", "FUNKFACE")
        PaymentSplitter(_payees, _shares)
    {}

    function mint() external payable {
        if (totalSupply + 1 > 21) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "ipfs://QmcdSFKZgpW8Af5KNVDftZhp5wcN1pFALK7sXBxJoFX14C/",
