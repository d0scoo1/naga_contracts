...
contract MMINEToken is IERC20, Roles {
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "MMINE: funds are frozen."
        );
        _;
    }

    /**
     * @notice MMINEToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Mariana Mining Security Token";
        _symbol = "MMINE";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "MMINE: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "MMINE: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "MMINE: _x must not be equal to _y");
        require(_x > 0, "MMINE: _x must be greater than 0");
        require(_y > 0, "MMINE: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "MMINE: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "MMINE: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "MMINE: transfer from the zero address");
        require(_recipient != address(0), "MMINE: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "MMINE: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "MMINE: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "MMINE: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "MMINE: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "MMINE: approve from the zero address");
        require(_spender != address(0), "MMINE: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "MMINE: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "MMINE: transfer exceeds your transaction count limit");
        }
    }
}
