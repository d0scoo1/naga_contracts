...
contract DAIDAIERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    event AskNew(address indexed collection, address indexed seller, uint256 indexed tokenId, uint256 askPrice, address token);

    // Ask order is updated
    event AskUpdate(address indexed collection, address indexed seller, uint256 indexed tokenId, uint256 askPrice, address token);
    ...
    /**
     * @notice Buy NFT with token by matching the price of an existing ask order
    ...
    function buyNFT(
        ...
        if(_token != address(0)){
            IERC20(_token).safeTransferFrom(address(msg.sender), address(this), _price);
        } else {
            IWETH(WBNB).deposit{value: msg.value}();
        }
        _buyTokenWithToken(_collection, _tokenId, _price, _token);
    }
    ...
    function createAskOrder(
        ...
        // Emit event
        emit AskNew(_collection, msg.sender, _tokenId, _askPrice, _token);
    }
    ...
    //  * @param _token: token address 
    //  */
    // function createAskOrderByToken(
    //     address _collection,
    //     uint256 _tokenId,
    //     uint256 _askPrice,
    //     address _token
    ...
    //     IERC20(_token).safeTransferFrom(address(msg.sender), address(this), _askPrice);
    ...
    function modifyAskOrder(
        ...
        _askDetails[_collection][_tokenId].token = _token;

        // Emit event
        emit AskUpdate(_collection, msg.sender, _tokenId, _newPrice, _token);
    }
    ...
    //  * @param _newPrice: new price for listing (in wei)
    //  */
    // function modifyAskOrderByToken(
    //     address _collection,
    //     uint256 _tokenId,
    //     uint256 _newPrice, 
    //     address _token
     ...
     * @param _token: whether the token is bought with BNB (true) or WBNB (false)
     */
    function _buyTokenWithToken(
        address _collection,
        uint256 _tokenId,
        uint256 _price,
        address _token
        ...
        require(_token == askOrder.token, "Buy: Incorrect token");
        ...
        if(askOrder.token != address(0)){
            IERC20(askOrder.token).safeTransfer(askOrder.seller, netPrice);
        }else{
        ...
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, askOrder.token);
    }
