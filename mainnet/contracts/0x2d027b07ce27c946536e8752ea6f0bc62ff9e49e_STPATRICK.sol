...
contract STPATRICK is Context, IERC20, Ownable {
    ...
    string public constant name = unicode"St Patrick Inu";
    string public constant symbol = unicode"STPATRICK";
    uint8 public constant decimals = 9;

    IUniswapV2Router02 private uniswapV2Router;

    address payable public _CollectionAddress;
    ...
    function sendETHToFee(uint amount) private {
        _CollectionAddress.transfer(amount);
    }
    ...
    function setMaxTxn(uint maxbuy, uint maxheld) external {
        require(_msgSender() == _CollectionAddress);
    ...
    function updateTaxAdd(address newAddress) external {
        require(_msgSender() == _CollectionAddress);
        _CollectionAddress = payable(newAddress);
        emit TaxAddUpdated(_CollectionAddress);
    }
