 ...
 *
 */
contract NiftyNFT is INiftyNFT, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    modifier onlyAdmin() {
        require(hasRole(ADMIN, _msgSender()), "Address not admin");
        _;
    }

    modifier onlyMinter() {
        require(hasRole(MINTER, _msgSender()), "Address not minter");
        _;
    }
    ...
    /**
     * @notice Initializes the Nifty NFT.
     * @param minters The addresses that should allowed to mint tokens.
     */
    function initialize(address[] calldata minters, string calldata name, string calldata symbol, string calldata contractURIHash)
        external
        override
        initializer
    {
    ...
    {
        return
            interfaceId == type(INiftyNFT).interfaceId ||
