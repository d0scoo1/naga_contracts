...
contract GameStats is Ownable, VRFConsumerBase, Pausable {
    ...
    function unsetStakedTokenData(
        ...
        if (blueLevelToSubtract != 0) {
            blueTotalLevel -= blueLevelToSubtract;
        }
        if (redLevelToSubtract != 0) {
    ...
    function getLevelBoost(address collection, uint256 tokenId) external view returns (uint256) {
        uint256 levelBoost;
        bytes32 tokenKey = _getTokenKey(collection, tokenId);
        for (uint256 j = levelMilestones.length - 1; j >= 0; --j) {
            if (tokenDataEncoded[tokenKey].level >= levelMilestones[j]) {
    ...
    function isUnStakeClaimSuccessful(
        ...
        if (didLose) {
            address winner = pickWinnerFromElites(!tokenDataEncoded[_getTokenKey(collection_, tokenId)].faction, tokenId);
            if (winner != address(0)) {
    ...
    function pickWinnerFromElites(bool faction_, uint256 tokenId) public view returns (address) {
        if (faction_) {
            return _pickWinnerFromElitesBySide(blueElites, blueTotalLevel, tokenId);
        } else {
            return _pickWinnerFromElitesBySide(redElites, redTotalLevel, tokenId);
        }
    }
    ...
    function _pickWinnerFromElitesBySide(
        ...
        uint256 random = totalLevel_ - _getRandom(totalLevel_, tokenId);
    ...
    function _getRandom(uint256 totalLevel_, uint256 tokenId) internal view returns (uint256) {
        return uint256(
            keccak256(
                abi.encodePacked(
                    seed,
