...
contract Kevin is ERC721, ERC721Enumerable, Ownable {

    string public PROVENANCE;
    uint256 public constant earlyPrice = 0.02 ether;
    uint256 public constant latePrice = 0.069 ether;
    ...
    constructor() ERC721("Just Kevin", "JK") {
    }
    ...
    function mintToken(uint numberOfTokens) public payable {
        ...
        uint256 cost = getCost(supply, numberOfTokens);
        require(cost <= msg.value, "Ether value sent is not enough");
        if (cost < msg.value) {
    ...
    function getCost(uint256 currSupply, uint256 numberOfTokens) internal pure returns (uint256) {
        uint256 cost = 0;
        uint256 newAmount = numberOfTokens + currSupply;
        if (newAmount > 1000 && newAmount <= 5969) {
            if (currSupply < 1000) {
                cost = (newAmount - 1000) * earlyPrice;
            } else {
                cost = numberOfTokens * earlyPrice;
            }
        } else if (newAmount > 5969) {
            if (currSupply < 5969) {
                cost = ((5969 - currSupply) * earlyPrice) + ((newAmount - 5969) * latePrice);
            } else {
                cost = numberOfTokens * latePrice;
            }
        }
