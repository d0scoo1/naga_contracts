...
contract BlocksRewarder is Ownable, ReentrancyGuard {
    ...
    mapping(address => User) public totalPayoutsFor;
    ...
    function allocateRewards(address[] calldata _beneficiaries, uint256[] calldata _earnings) external onlyOwner {
        ...
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            ...
            uint256 _totalReceived = totalPayoutsFor[_beneficiary].allocatedBalance;

            totalPayoutsFor[_beneficiary].allocatedBalance = _totalEarned + _totalReceived;
            totalPayoutsFor[_beneficiary].stakeTime = block.timestamp;
    ...
    function _fetchUserReward() internal view returns (uint256 rewards){
        uint256 _timeStaked = block.timestamp - totalPayoutsFor[msg.sender].stakeTime;
        uint256 _userStakeFactor = _timeStaked * totalPayoutsFor[msg.sender].allocatedBalance;
        uint256 _stakeReward = _userStakeFactor / stakeRewardFactor;
        return _stakeReward;
    }

    function getAccumulatedRewards() external view returns (uint256 rewards){
        return _fetchUserReward();
    }

     function _contractBalance() internal view returns (uint256 balance) {
        uint256 blocksBalance = token.balanceOf(address(this));
    ...
    function claim() external nonReentrant {
        require(totalPayoutsFor[msg.sender].allocatedBalance > 0, "Balance must be > 0");

        uint256 _withdrawalAmount = totalPayoutsFor[msg.sender].allocatedBalance + _fetchUserReward();
        require(_withdrawalAmount <= _contractBalance(), "Contract balance too low.");

        // Update allocation
        increaseClaimed(_withdrawalAmount);
        totalAllocated -= totalPayoutsFor[msg.sender].allocatedBalance;

        // Update user allocation and stake time
        totalPayoutsFor[msg.sender].stakeTime = block.timestamp;
        totalPayoutsFor[msg.sender].allocatedBalance = 0;

        token.transfer(msg.sender, _withdrawalAmount);
    }
