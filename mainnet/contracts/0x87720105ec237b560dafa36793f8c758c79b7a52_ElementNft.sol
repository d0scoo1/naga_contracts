...
abstract contract ERC721Tradable is ERC721, ContextMixin, NativeMetaTransaction, Ownable {
    ...
    uint private totalLimit = 2500; //limit for total mint
    ...
    address private mintAddr;//mintAddr for system
    ...
    function mintTo(address _to) public {
        require(owner() == _msgSender() || msg.sender == mintAddr , "Ownable: caller is not the owner");
...
//limit
    ...
    function setSingleLimit(uint256 _limit)public onlyOwner returns(bool){
        require(_limit > 0," limit must be a positive number");
...
// mint
    function mintNft(uint num) public payable {
        require(!getPaused(),"mint nft was paused");
        require(num > 0,"mint num must be positive number");
        require(totalSupply()+num <= totalLimit,"sell out");
        if(!mintPublic){
            require(mintWhiteList[msg.sender],"your address not in white list");
        }
        require(mintCount[msg.sender]+num <= singleAddrLimit,"reach the minting limit");
        require(msg.value >= price*num,"ETH amount is not enough for minting");
    ...
    /**
        burn
     */
    function burn(uint256[] memory tokens) public returns(bool){
 ...
 */
contract ElementNft is ERC721Tradable {
