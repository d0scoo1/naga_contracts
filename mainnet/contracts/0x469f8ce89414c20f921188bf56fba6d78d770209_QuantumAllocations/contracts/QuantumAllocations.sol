    ...
    SignatureVerifier_V2
{
    ...
    function addInvestmentToProject(
        ...
        if(tokenId <=2000)
        require(projectsLedger[projectName].openForHolders, "project not open for non-whales");
        else require(projectsLedger[projectName].openForWhales, "project not open for whales");

        require(Quantum.ownerOf(tokenId) == msg.sender, "you are not the owner");
        ...
        */
        ...
        if (tetherOrCoin) USDT.transferFrom(msg.sender, address(this), amount);
    ...
    function addInvestmentForNonHolder(
        bytes32 projectName,
        bytes memory sig,
        uint256 amount,
        bool tetherOrCoin
        ...
        // checks if the project exist
        require(projectExist[projectName], "No such project");
        require(projectsLedger[projectName].openForPublic, "public not allowed");

        require(Quantum.balanceOf(msg.sender) == 0, "only non-holders");
 ...
 function toggleProjectOpenForHolders(
        bytes32 projectName
    ) external onlyOwner {
        require(projectExist[projectName], "No such project");
        projectsLedger[projectName].openForHolders = !projectsLedger[projectName].openForHolders;
    }

     function toggleProjectOpenForWhales(
        bytes32 projectName
    ) external onlyOwner {
        require(projectExist[projectName], "No such project");
        projectsLedger[projectName].openForWhales =  !projectsLedger[projectName].openForWhales;
    }

   function toggleProjectOpenForPublic(
        bytes32 projectName
    ) external onlyOwner {
        require(projectExist[projectName], "No such project");
        projectsLedger[projectName].openForPublic = !projectsLedger[projectName].openForPublic;
    }
    ...
    function editMaxAllocationsPerNonHolder(bytes32 projectName, uint256 _amount)
        external
        onlyOwner
    {
        require(projectExist[projectName], "No such project");
