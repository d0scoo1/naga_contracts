...
// File: contracts\categories\NFT.sol
...
contract NFT is Ownable, ERC721 {
        ...
        internal returns (uint256 tokenId)
    {
        ...
        royaltyAmount[tokenId] = _royaltyAmount;
...
contract OpenMarketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder {
     ...
     */
    function createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei,uint256 _royality, uint256 _expiresAt, address _currency) external whenNotPaused {
        _createOrder(_nftAddress, _assetId, _priceInWei, _royality, _expiresAt, _currency);
    }
    ...
    function safeExecuteOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) external payable whenNotPaused {
        ...
        if (order.royalty != 0) {
            ...
            if(NFT(order.nftAddress).getTokenMinter(_assetId) != msg.sender)
            {
               // Transfer royalty percentage for marketplace Owner
                order.currency == MARKETPLACE_ETHER ? 
                payable(address(NFT(order.nftAddress).getTokenMinter(_assetId))).transfer(royaltyAmount)
                :
                IERC20(order.currency).safeTransferFrom(msg.sender, address(NFT(order.nftAddress).getTokenMinter(_assetId)), royaltyAmount);
    ...
    function acceptBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public whenNotPaused {
        ...
        if (order.royalty != 0) {
            ...
            if(NFT(order.nftAddress).getTokenMinter(_assetId) != msg.sender)
            {
                ...
                payable(address(NFT(order.nftAddress).getTokenMinter(_assetId))).transfer(royaltyAmount)
                :
                IERC20(order.currency).safeTransfer(address(NFT(order.nftAddress).getTokenMinter(_assetId)), royaltyAmount);
