 ...
 */
contract REAPFarm is BaseFallback {
    /**
...
contract REAPFarms is Ownable {
    ...
    uint256 public constant POOL_UNSUPPORT = 3;
	...
	mapping (uint256 => mapping (address => UserInfoNFTSet)) private userInfoNFTSet;
    ...
    event DepositNFT(address indexed user, uint256 indexed pid, uint256 amount, uint256 liquidity);
    ...
    function startFarming(uint256 _sushiPerBlock) public onlyOwner {
        require(!mintStarted, "have started");
        mintStarted = true;
        sushiPerBlock = _sushiPerBlock;
        startBlock = block.number;
        bonusEndBlock = startBlock + 28800 * 20;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate, uint256 _type,bool _isNFT) public {
	...
	function depositNFT(uint256 _pid, uint256 _amount) public payable {
	    PoolInfo storage pool = poolInfo[_pid];
	    UserInfo storage user = userInfo[_pid][msg.sender];
		UserInfoNFTSet storage userset = userInfoNFTSet[_pid][msg.sender];
		
	    require(pool.isNFT, "NFT Pool");
	    updatePool(_pid);
	    ...
	    user.rewardDebt = user.amount.mul(pool.accSushiPerShare).div(1e12);
	    emit DepositNFT(msg.sender, _pid, _amount, 0);
