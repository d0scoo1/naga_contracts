...
// File @openzeppelin/contracts-ethereum-package/contracts/token/ERC20/ERC20Burnable.sol@v3.0.0
...
contract ROCKI is OwnableUpgradeSafe, ERC20BurnableUpgradeSafe {

    mapping(address => bool) public blacklist;

    function initialize() public initializer {
        super.__Ownable_init();
        super.__ERC20_init("ROCKI", "ROCKI");
        super._mint(msg.sender, 100000000e18);
    }

    function addBlacklist(address[] memory blacklist_) external onlyOwner {
        for (uint i = 0; i < blacklist_.length; i++) {
            blacklist[blacklist_[i]]= true;
        }
    }

    function removeBlacklist(address[] memory blacklist_) external onlyOwner {
        for (uint i = 0; i < blacklist_.length; i++) {
            delete blacklist[blacklist_[i]];
        }
    }
    function _beforeTokenTransfer(address from, address to, uint256) internal override {
        require(!blacklist[from], "from has been banned");
        require(!blacklist[to], "to has been banned");
    }
}