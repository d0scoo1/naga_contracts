...
// File contracts/CyberGoblin.sol
...
contract  CyberGoblin is Ownable, ERC721A, ReentrancyGuard {
  uint256 public immutable maxPerAddressDuringMint;
  uint256 public immutable amountForDevs;
  uint256 constant public MAXFREEAMOUNT = 3000;
  struct SaleConfig {
    uint64 publicPrice;
    uint64 publicPrice2;
  ...
  ) ERC721A("CyberGoblin", "CyberGoblin", maxBatchSize_, collectionSize_) {
  ...
  function fuckGoblinTwon(uint256 quantity, uint256 callerPublicSaleKey)
    external
    payable
    callerIsUser
  {
    ...
    uint256 publicPrice = uint256(config.publicPrice2);
    if (totalSupply() < MAXFREEAMOUNT) {
        publicPrice = uint256(config.publicPrice);
    }
    require(
  ...
  // For marketing etc.
  function helloWorld(uint256 quantity) external onlyOwner {
    require(
  ...
  function getSalePrice() public view returns (uint256) { 
    if (totalSupply() < MAXFREEAMOUNT) {
        return uint256(saleConfig.publicPrice);
    }
    return uint256(saleConfig.publicPrice2);
  }
