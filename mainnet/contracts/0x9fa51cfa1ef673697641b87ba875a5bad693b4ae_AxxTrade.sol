...
contract AxxTrade is Ownable, IERC721Receiver {
    ...
    function proposeTrade(uint256 proposerNft, uint256 accepterNft) public {
        require(msg.sender == nft.ownerOf(proposerNft), "cannot trade a token you dont own");

        Escrow memory accepterEscrow = escrow[accepterNft];
        uint256 accepterEscrowNft = accepterEscrow.accepterNft;
        address accepter = nft.ownerOf(accepterNft);

        if (accepterEscrowNft == proposerNft) {
            _finalizeTrade(accepterNft, msg.sender);
        } else {
            require(msg.sender != accepter, "cannot trade with yourself");
            nft.transferFrom(msg.sender, address(this), proposerNft);
            escrow[proposerNft] = Escrow({
                proposerNft: uint24(proposerNft),
                accepterNft: uint24(accepterNft),
                proposer: msg.sender,
    ...
    function _finalizeTrade(uint256 proposerNft, address accepter) internal {
        Escrow memory escrowed = escrow[proposerNft];
        uint256 accepterNft = escrowed.accepterNft;
        address proposer = escrowed.proposer;
        delete escrow[proposerNft];
        activeTrades -= 1;
        token.mint(accepter, 250000000000000000000);
        token.mint(proposer, 250000000000000000000);
        nft.transferFrom(accepter, proposer, accepterNft);
        nft.transferFrom(address(this), accepter, proposerNft);
    }

    function cancelTrade(uint256 proposerNft) public {
        Escrow memory escrowed = escrow[proposerNft];
        address proposer = escrowed.proposer;
        require(msg.sender == proposer || msg.sender == contractOwner, "not approved to make that decision");
        delete escrow[proposerNft];
        activeTrades -= 1;
        nft.transferFrom(address(this), proposer, proposerNft);
    }

    function getOutgoingProposals(address account) public view returns (uint256[] memory ownerTokens) {
        uint256 supply = nft.totalSupply();
        uint256[] memory tmp = new uint256[](supply);
        uint256 index = 0;
        for(uint tokenId = 1; tokenId <= supply; tokenId++) {
            if (escrow[tokenId].proposer == account) {
                tmp[index] = escrow[tokenId].proposerNft;
                index +=1;
            }
        }
    ...
    function getIncomingProposals(uint256 tokenId) public view returns (uint256[] memory proposedTokens) {
        uint256 supply = nft.totalSupply();
        uint256[] memory tmp = new uint256[](supply);
        uint256 index = 0;
        for(uint tokenIndex = 1; tokenIndex <= supply; tokenIndex++) {
            if (escrow[tokenIndex].accepterNft == tokenId) {
                tmp[index] = escrow[tokenIndex].proposerNft;
                index +=1;
            }
        }
        uint256[] memory tokens = new uint256[](index);
    ...
    function adminCancelTrades(uint256[] calldata tokenIds) public onlyOwner {
        for (uint i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            Escrow memory escrowed = escrow[tokenId];
            address proposer = escrowed.proposer;
            delete escrow[tokenId];
            activeTrades -= 1;
            nft.transferFrom(address(this), proposer, tokenId);
        }
    }
