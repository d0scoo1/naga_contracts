...
contract NABUTO is Context, IERC20 {
    ...
    uint256 public _developmentRatio = 5;
    ...
    address payable private _marketingWallet = payable(0xD5EA63a548B34d48f7D35434F9E8cbCcF6948D89);
    address payable private _buybackWallet = payable(0xE4A06e4203665e2F9f12c19849d37b0947FFB2B9);
...
//===============================================================================================================
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_developmentRatio + _marketingRatio + _liquidityRatio == 0)
            return;
        uint256 toLiquify = ((contractTokenBalance * _developmentRatio) / (_developmentRatio + _marketingRatio + _liquidityRatio)) / 2;

        uint256 toSwapForEth = contractTokenBalance - toLiquify;
        swapTokensForEth(toSwapForEth);

        uint256 currentBalance = address(this).balance;
        uint256 liquidityBalance = ((currentBalance * _developmentRatio) / (_developmentRatio + _marketingRatio + _liquidityRatio)) / 2;
        ...
        if (contractTokenBalance - toLiquify > 0) {
            _marketingWallet.transfer(((currentBalance - liquidityBalance) * _marketingRatio) / (_marketingRatio + _liquidityRatio));
