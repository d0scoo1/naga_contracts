...
contract AdminModule is Helpers {
    ...
    function updateRebalancer(address rebalancer_, bool isRebalancer_) external onlyAuth {
    ...
    function updateRatios(uint16[] memory ratios_) external onlyAuth {
        ...
        emit updateRatiosLog(ratios_[0], ratios_[1], ratios_[2], uint128(ratios_[3]) * 1e23);
    }

    function changeStatus(uint status_) external onlyAuth {
        _status = status_;
