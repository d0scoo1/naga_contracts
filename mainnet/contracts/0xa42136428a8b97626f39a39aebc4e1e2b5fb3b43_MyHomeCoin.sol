...
contract MyHomeCoin is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 97 * 10**8 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "MyHomeCoin";
    string private _symbol = "MYH";
    ...
    uint256 public _maxTxAmount = 97 * 10**7 * 10**9;	
    ...
    constructor ( address _developmentAddress, address _marketingWallet, address _charityAddress)  {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 charityAmt;
        uint256 developmentAmt;
        
        //Calculate marketing and other amount
        if(_liquidityFee == 0){
            marketingAmt = 0;
            charityAmt = 0;
            developmentAmt = 0;
        }else{
            marketingAmt = amount.mul(_marketingFee).div(1000);
            charityAmt = amount.mul(_charityFee).div(1000);
            developmentAmt = amount.mul(_developmentFee).div(1000);
        }
        

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(marketingAmt).sub(charityAmt).sub(developmentAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(marketingAmt).sub(charityAmt).sub(developmentAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(marketingAmt).sub(charityAmt).sub(developmentAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(marketingAmt).sub(charityAmt).sub(developmentAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(marketingAmt).sub(charityAmt).sub(developmentAmt)));
        }
        ...
        _transferStandard(sender, developmentAddress, developmentAmt);
