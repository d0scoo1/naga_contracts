...
contract ExchangeMain is ReentrancyGuarded, Ownable {
    ...
    //     if (callMint) {
    //         require(
    //             IVindergoodStore(order.target).owner() == order.maker ||
    //                 order.target == defaultCollection,
    //             "VindergoodExchange::INVALID_COLLECTION"
    //         );
    ...
    function atomicMatch(
        ...
        (bool result, bytes memory returnData) = proxy.proxy(
            sell.target,
            sell.howToCall,
            sell.callData
        );
