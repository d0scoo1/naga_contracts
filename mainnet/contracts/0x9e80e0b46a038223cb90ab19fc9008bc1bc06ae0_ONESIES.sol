...
contract ONESIES is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
    uint256 public SALE_NFT = 7527;
	uint256 public EARLYSALE_NFT = 250;
	
	uint256 public MAX_MINT_EARLYSALE = 5;
	uint256 public MAX_MINT_PRESALE = 3;
	uint256 public MAX_MINT_SALE = 3;
	
	uint256 public MAX_BY_MINT_IN_TRANSACTION_EARLYSALE = 5;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_PRESALE = 3;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_SALE = 3;
	
	uint256 public EARLYSALE_PRICE = 5 * 10**16;
	uint256 public PRESALE_PRICE = 6 * 10**16;
	uint256 public SALE_PRICE = 6 * 10**16;
	
	uint256 public LOCKED_FUND;
    ...
    constructor() ERC721P('Onesies', 'ONESIES') {}
	...
	function mintEarlySaleNFT(uint256 _count, bytes32[] calldata merkleProof) public payable{
		...
		users[msg.sender].earlysalemint = users[msg.sender].earlysalemint + _count;
    }
	...
	function updateEarlySalePrice(uint256 newPrice) external onlyOwner {
        EARLYSALE_PRICE = newPrice;
    }
	...
	function setEarlySaleStatus(bool status) public onlyOwner {
	   require(earlysaleEnable != status, "Incorrect value");
	...
	function updateEarlySaleMintLimit(uint256 newLimit) external onlyOwner {
	    require(EARLYSALE_NFT >= newLimit, "Incorrect value");
        MAX_MINT_EARLYSALE = newLimit;
    }
	...
	function updateEarlySaleSupply(uint256 newSupply) external onlyOwner {
	    require(newSupply >= EARLYSALE_MINTED, "Incorrect value");
        EARLYSALE_NFT = newSupply;
    }
	...
	function updateMintLimitPerTransectionEarlySale(uint256 newLimit) external onlyOwner {
	    require(EARLYSALE_NFT >= newLimit, "Incorrect value");
        MAX_BY_MINT_IN_TRANSACTION_EARLYSALE = newLimit;
    }
	...
	function updateEarlySaleMerkleRoot(bytes32 newRoot) external onlyOwner {
	   merkleRootEarlySale = newRoot;
	}
	
	function lockFund(uint256 ethValue) external onlyOwner {
	   require(address(this).balance >= ethValue, "Incorrect value");
	   LOCKED_FUND = ethValue;
	}
	
	function unlockFund(uint256 ethValue) external onlyOwner {
	   require(LOCKED_FUND >= ethValue, "Incorrect value");
	   LOCKED_FUND = LOCKED_FUND - ethValue;
	}
}