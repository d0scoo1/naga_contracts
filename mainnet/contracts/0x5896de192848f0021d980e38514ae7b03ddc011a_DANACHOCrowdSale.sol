...
library SafeERC20 {
    ...
    function safeTransfer(DANACHO token, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(DANACHO token, address from, address to, uint256 value) internal {
        _callOptionalReturn(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value));
    }

    function safeApprove(DANACHO token, address spender, uint256 value) internal {
    ...
    function safeIncreaseAllowance(DANACHO token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).add(value);
        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(DANACHO token, address spender, uint256 value) internal {
     ...
     */
    function _callOptionalReturn(DANACHO token, bytes memory data) private {
...
contract CROWDSALE is ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for DANACHO;


    // The token being sold
    DANACHO private _token;
     ...
     */
    constructor ( uint256 rate, address payable wallet, DANACHO token) public  {
     ...
     */
    function token_() public view returns (DANACHO) {
        return _token;
    }
...
contract AllowanceCrowdsale is CROWDSALE {
     ...
     */
    constructor (address tokenWallet,uint256 rate, address payable wallet, DANACHO token) CROWDSALE(rate,wallet,token) public  {
...
contract DANACHOCrowdSale is AllowanceCrowdsale,Ownable {
    using SafeMath for uint256;
    ...
    constructor(address tokenWallet,uint256 rate, address payable wallet, DANACHO token,uint256 minimumContribution,uint256 maximumContribution,uint256 openingTime, uint256 closingTime) AllowanceCrowdsale(tokenWallet,rate,wallet,token) public  {
  ...
  function _preValidatePurchase (address beneficiary,uint256 weiAmount) internal view override onlyWhileOpen {
      ...
      require(weiAmount <= _maximumContribution);
      super._preValidatePurchase(beneficiary,weiAmount);
  }
