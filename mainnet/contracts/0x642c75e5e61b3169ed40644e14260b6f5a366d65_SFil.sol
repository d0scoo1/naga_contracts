...
contract SFil is Context, IERC777, IERC20,IERC777Recipient {
  ...
  mapping(address => uint256) public AddrToStakingID;
  uint256[] public ValidStakingID;
  uint256 public ValidStakingLen;
  event ApplyStakingEvent(uint256 indexed sid, address indexed addr, string info);
  event IssueStakingEvent(uint256 indexed sid, address indexed addr, uint256 sfilNum);
  event DeleteStakingEvent(uint256 indexed sid, address indexed addr);
  ...
  //////////////////// for staking
  function IssueStaking(uint256 sid, uint256 sfilNum)
    public
    IsAdmin(){

    require(sid > 0, "sid error");
    
    StakingInfo storage info = StakingInfoMap[sid];
    require(info.SID > 0, "not valid sid by issue");
    ...
    _mint(addr, sfilNum, "", "");
    emit IssueStakingEvent(sid, addr, sfilNum);
  }

  function DeleteStakingByAdmin(uint256 sid)
    public
    IsAdmin(){
    deleteStaking(sid);
  }
  
  function ApplyStaking(string memory data)
    public{

    uint256 id = AddrToStakingID[msg.sender];
    require(id == 0, "one addr only apply once, maybe wait for admin to allow or delete previous apply");

    if(ValidStakingLen == ValidStakingID.length){
      ValidStakingID.push(StakingID);
    } else{
      ValidStakingID[ValidStakingLen] = StakingID;
    }

    StakingInfoMap[StakingID].Addr = msg.sender;
    StakingInfoMap[StakingID].Info = data;
    StakingInfoMap[StakingID].validIdx = ValidStakingLen;
    StakingInfoMap[StakingID].SID = StakingID;
    AddrToStakingID[msg.sender] = StakingID;

    emit ApplyStakingEvent(StakingID, msg.sender, data);
    StakingID++;
    ValidStakingLen++;
  }

  function DeleteStakingApply()
    public{

    uint256 sid = AddrToStakingID[msg.sender];
    require(sid != 0, "no apply can delete");

    deleteStaking(sid);
  }

  function GetNowStakingApply()
    public
    view
    returns(StakingInfo[] memory results){

    results = new StakingInfo[](ValidStakingLen);

    for(uint256 vid=0; vid < ValidStakingLen; vid++){
      results[vid] = StakingInfoMap[ValidStakingID[vid]];
    }
    return results;
  }

  function deleteStaking(uint256 sid)
    internal{
    StakingInfo storage info = StakingInfoMap[sid];
    require(info.SID > 0, "not valid sid");
    uint256 vid = info.validIdx;
    address addr = info.Addr;
    
    delete StakingInfoMap[sid];
    delete AddrToStakingID[addr];
    
    uint256 lastSID = ValidStakingID[ValidStakingLen-1];
    if(lastSID == sid){
      ValidStakingLen--;
      return;
    }

    ValidStakingID[vid] = lastSID;
    StakingInfoMap[lastSID].validIdx = vid;
    ValidStakingLen--;

    emit DeleteStakingEvent(sid, addr);
  }
