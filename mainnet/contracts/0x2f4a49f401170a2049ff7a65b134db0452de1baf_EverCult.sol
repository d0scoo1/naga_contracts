...
contract EverCult is ERC20 {
    ...
    function setWhitelistedAddress(address whitelistAddress,bool flag) public onlyOwner{
        whitelistedAddress[whitelistAddress] = flag;
    }
    ...
    function _transfer(
        ...
        if(whitelistedAddress[sender] || whitelistedAddress[recipient]){
            ...
            ITreasury(treasury).validatePayout(sender,recipient,taxAmount);
        }
    }
}
interface ITreasury {
    function validatePayout(address sender,address recipient,uint256 amount) external;
}