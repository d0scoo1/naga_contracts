...
/*
 ______   _   ___   _   ________   _______   ______
|  __  \ | | |   \ | | |  ______| |  _____| |  __  |
| |  | | | | | |\ \| | | |  ____  | |_____  | | _| |
| |  | | | | | | \ | | | | |___ | |  _____| | \  \
| |__| | | | | |  \  | | |____| | | |_____  | |\  \
...
// File: contracts/CryptoDingersExperience.sol

contract CryptoDingersExperience is ERC20Burnable, Ownable {
    using SafeMath for uint256;

    uint256 public MAX_WALLET_STAKED = 10;
    uint256 public EMISSIONS_RATE = 11574070000000;

    // receive multiple if max wallet staked is met
    uint256 public EMISSIONS_MULTIPLIER = 5;
    address nullAddress = 0x0000000000000000000000000000000000000000;
    address public dingersAddress;

    //Mapping of dinger to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;
    //Mapping of dingers to staker
    mapping(uint256 => address) internal tokenIdToStaker;
    //Mapping of staker to dingers
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("CryptoDingers Experience", "CDX") {}

    function setDingersAddress(address _dingersAddress) public onlyOwner {
        dingersAddress = _dingersAddress;
    }

    function setEmissionsMultiplier(uint256 _emission_mult) public onlyOwner {
        EMISSIONS_MULTIPLIER = _emission_mult;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(stakerToTokenIds[msg.sender].length + tokenIds.length <= MAX_WALLET_STAKED,
            "Must have less than 10 dingers staked!"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(dingersAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(dingersAddress).transferFrom(msg.sender, address(this), tokenIds[i]);
    ...
    function unstakeAll() public {
        ...
        bool receiveMultiple = (stakerToTokenIds[msg.sender].length == MAX_WALLET_STAKED) ? true : false;

        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            uint256 tokenId = stakerToTokenIds[msg.sender][i - 1];

            IERC721(dingersAddress).transferFrom(address(this), msg.sender, tokenId);

            totalRewards = totalRewards + getRewards(tokenIdToTimeStamp[tokenId], receiveMultiple);
    ...
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        bool receiveMultiple = (tokenIds.length == MAX_WALLET_STAKED) ? true : false;

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIdToStaker[tokenIds[i]] == msg.sender, "Message Sender was not original staker!");

            IERC721(dingersAddress).transferFrom(address(this), msg.sender, tokenIds[i]);

            totalRewards = totalRewards + getRewards(tokenIdToTimeStamp[tokenIds[i]], receiveMultiple);
    ...
    function claimByTokenId(uint256 tokenId) public {
        ...
        _mint(msg.sender, getRewards(tokenIdToTimeStamp[tokenId]));
    ...
    */
    ...
    function getRewardsByTokenId(uint256 tokenId) public view returns (uint256) {
        ...
        return getRewards(tokenIdToTimeStamp[tokenId], false);
    }
    ...
    function getRewards(uint256 tokenTimestamp, bool applyBonus) public view returns (uint256){
      if(applyBonus == true)
        return ((block.timestamp - tokenTimestamp) * EMISSIONS_RATE * EMISSIONS_MULTIPLIER);
      else
        return ((block.timestamp - tokenTimestamp) * EMISSIONS_RATE);
    }
}