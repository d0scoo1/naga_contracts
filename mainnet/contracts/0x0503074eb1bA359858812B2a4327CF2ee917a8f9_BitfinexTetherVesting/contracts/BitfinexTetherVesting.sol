...
contract BitfinexTetherVesting is Initializable, OwnableUpgradeable {
	...
	enum VestingSchedule { BITFINEX, TETHER }
	...
	function initialize() public initializer {
		...
		token = IERC20(0x725C263e32c72dDC3A19bEa12C5a0479a81eE688);
		...
		_createVesting(
			0xDeeF1A5C7331EBe6114A23C6017E10A957a55c2B,
			ONE_MILLION_TOKENS,
			VestingSchedule.TETHER,
			true,
			0
	...
	function _addLinearVestingSchedule(
		VestingSchedule _type,
		LinearVestingSchedule memory _schedule
	) internal {
	...
	function _forceCancelVesting(uint256 _vestingId, Vesting storage _vesting)
		internal
	{
	...
	function _withdrawFromVesting(Vesting storage _vesting, uint256 _vestingId)
		internal
	{
	...
	function getWithdrawableAmount(uint256 _vestingId)
		external
		view
		returns (uint256)
	{
	...
	function _getWithdrawableAmount(Vesting storage _vesting)
		internal
		view
		returns (uint256)
	{
	...
	function calculateAvailableAmount(Vesting storage _vesting)
		internal
		view
		returns (uint256)
	{
		LinearVestingSchedule[] memory vestingSchedule =
			vestingSchedules[_vesting.vestingSchedule];
		...
		for (uint256 i = 0; i < vestingSchedule.length; i++) {
			...
			uint256 amountThisLinearSchedule =
				calculateLinearVestingAvailableAmount(
					linearSchedule,
					_vesting.amount
				);
	...
	function calculateLinearVestingAvailableAmount(
		LinearVestingSchedule memory _linearVesting,
		uint256 _amount
		...
		uint256 amountThisVestingSchedule =
			_amount.mul(_linearVesting.portionOfTotal).div(
				PORTION_OF_TOTAL_PRECISION
			);

		uint256 amountPerPeriod =
			amountThisVestingSchedule.mul(_linearVesting.portionPerPeriod).div(
				PORTION_PER_PERIOD_PRECISION
			);

		return
			amountPerPeriod.mul(elapsedPeriods).min(amountThisVestingSchedule);
	}
	...
	function calculateElapsedPeriods(
		...
		return
			block.timestamp.sub(_linearVesting.startDate).div(
				_linearVesting.periodInSeconds
			);
	}
