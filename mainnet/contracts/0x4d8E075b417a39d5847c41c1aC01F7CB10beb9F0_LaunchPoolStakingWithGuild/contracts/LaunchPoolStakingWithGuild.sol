...
contract LaunchPoolStakingWithGuild is Ownable {
    ...
    /// @dev Number of reward tokens distributed per block, across all pools.
    uint256 public rewardPerBlock;

    /// @dev The total amount of reward token available for farming across all pools between start and end block.
    ...
    /// @dev The block number when rewards starts across all pools.
    uint256 public startBlock;

    /// @dev The block number when rewards ends.
    uint256 public endBlock;

    /// @dev Tracks ERC20 tokens added by owner
    ...
    /// @dev Create a new reward pool by whitelisting a new ERC20 token.
    ...
    /// @dev Update a pool's allocation point to increase or decrease its share of contract-level rewards
    /// @dev Can also update the max amount that can be staked per user
    ...
    /// @dev View function to see pending and unclaimed reward tokens for a given user
    ...
    /// @dev Cycles through the pools to update all of the rewards accrued
    ...
    /// @dev Updates a specific pool to track all of the rewards accrued up to the TX block
    ...
    /// @dev Where any user can stake their ERC20 tokens into a pool in order to farm rewards
    /// @param _pid ID of the pool
    ...
    /// @dev Allows a user to withdraw any ERC20 tokens staked in a pool
    ...
    /// @dev Emergency only. Should the rewards issuance mechanism fail, people can still withdraw their stake
    /// @param _pid Pool ID
