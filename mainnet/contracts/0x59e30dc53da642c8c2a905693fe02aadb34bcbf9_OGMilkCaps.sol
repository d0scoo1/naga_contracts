...
interface ICaps {
    function sendCaps(address _to, uint256 _amount) external;
}
...
contract CapsToken is ERC20("CapsToken", "CAPS"), Ownable {
    using SafeMath for uint256;
    uint256 constant public LP_TOTAL = 2000000 ether;
    mapping(address => bool) public canSendCaps;
    mapping(address => bool) public canBurnCaps;

    function sendCaps(address _to, uint256 _amount) external {
        require(canSendCaps[msg.sender], "Nope!");
        _mint(_to, _amount);
    }

    function burnCaps(address _from, uint256 _amount) external {
        require(canBurnCaps[msg.sender], "Nope!");
        _burn(_from, _amount);
    }

    function addCanBurn(address _canBurn) external onlyOwner {
        canBurnCaps[_canBurn] = true;
    }

    function removeCanBurn(address _canBurn) external onlyOwner {
        canBurnCaps[_canBurn] = false;
    }

    function addCanSendCaps(address _canBurn) external onlyOwner {
        canSendCaps[_canBurn] = true;
    }

    function removeCanSendCaps(address _canBurn) external onlyOwner {
        canSendCaps[_canBurn] = false;
    }

    constructor() public {
        _mint(msg.sender, LP_TOTAL);
    }
}
...
contract OGMilkCaps is ERC721, Ownable {
    ...
    uint256 public MILKCAPS_PRICE = 0.07394 ether;
    uint256 public RESERVED_FOR_TEAM = 394;
    uint256 public TEAM_COUNT = 0;


    uint256 public constant MAX_MILKCAPS = 7394;
    uint256 constant public BASE_RATE = 11 ether;

    uint256 constant public INITIAL_ISSUANCE_TIER_1 = 1000 ether;
    uint256 constant public INITIAL_ISSUANCE_TIER_2 = 500 ether;

    uint256 constant public TIER_1_MAX = 1395; //1395
    uint256 constant public TIER_2_MAX = 3395; //3395

    bool public isLive = true;

    // 12/31/2024 @11:59:59 EST
    uint256 constant public END = 1735707599000;
    ...
    function mintMilkCaps(uint numberOfTokens) external payable {
        require(isLive, "Can not mint Comics before the sale has started");
        require(totalSupply().add(numberOfTokens) <= MAX_MILKCAPS, "Purchase would exceed max supply of MilkCaps");
        require(MILKCAPS_PRICE.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        ...
        // ie:
        // RESERVED_FOR_TEAM = 100 (0-99)
        // 10 have been publicly minted
        // 10 have been minted by the team

        // what is the next index to be publicly minted?
        // 100 + 20 - 10 = 110

        for (uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = RESERVED_FOR_TEAM.add(totalSupply()).sub(TEAM_COUNT);
            _safeMint(msg.sender, mintIndex);
            updateRewardOnMint(msg.sender, mintIndex);
        }
    }

    function updateRewardOnMint(address _user, uint256 _tokenId) private {
        uint256 time = min(block.timestamp, END);
        uint256 timerUser = lastUpdate[_user];
        if (timerUser > 0){
            if (_tokenId < TIER_1_MAX) {
                rewards[_user] = rewards[_user].add(userBalance[_user].mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
                .add(INITIAL_ISSUANCE_TIER_1));
            }else if (_tokenId >= TIER_1_MAX && _tokenId < TIER_2_MAX) {
                rewards[_user] = rewards[_user].add(userBalance[_user].mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
                .add(INITIAL_ISSUANCE_TIER_2));
            }
        }else{
            if (_tokenId < TIER_1_MAX) {
                rewards[_user] = rewards[_user].add(INITIAL_ISSUANCE_TIER_1);
            }else if (_tokenId >= TIER_1_MAX && _tokenId < TIER_2_MAX) {
                rewards[_user] = rewards[_user].add(INITIAL_ISSUANCE_TIER_2);
            }
        }
    ...
    function getReward() external {
        ...
        if (rewards[_user] > 0) {
            rewards[_user] = 0;
            capstoken.sendCaps(_user, reward);
        }
    }
    ...
    function reserveForTeam(uint _amount, address _to) external {
        ...
        require(TEAM_COUNT.add(_amount) <= RESERVED_FOR_TEAM, "Can not mint more than 394 tokens for the team");
        uint256 timerUser = lastUpdate[_to];
        uint256 time = min(block.timestamp, END);

        for (uint i = 0; i < _amount; i++) {
            _safeMint(_to, TEAM_COUNT);
            rewards[_to] = rewards[_to].add(userBalance[_to].mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
                .add(INITIAL_ISSUANCE_TIER_1));
            TEAM_COUNT++;
        }
    ...
    constructor(address _caps) ERC721("OG Milk Caps", "OGMILKCAPS") {
        capstoken = ICaps(_caps);
        SaleManager manager = new SaleManager(owner(), address(this));
