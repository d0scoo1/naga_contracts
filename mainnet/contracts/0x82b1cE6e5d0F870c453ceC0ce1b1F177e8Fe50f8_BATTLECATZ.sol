...
contract BATTLECATZ is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, Mintable  {
   ...
   event Deposit(address indexed _from,uint256 _value,uint256 _nfcount);
    ...
    constructor(address _owner, address _imx,address payable fundsTo_, uint256 maxSupply_, uint256 maxPerTx_, uint256 pricePer_, uint256 pricePerPre_) ERC721("BATTLECATZ", "BATTLECATZ") Mintable(_owner, _imx){
        imx = _imx;
        require(_owner != address(0), "Owner must not be empty");
        transferOwnership(_owner);
        fundsTo = fundsTo_;
