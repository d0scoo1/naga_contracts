...
// File: contracts/SnakeLads.sol


pragma solidity ^0.8.7;

contract SnakeLads is Ownable, ERC721 {
    using SafeMath for uint256;

  uint public constant supplyLimit = 1888;
  uint256 public totalSupply;
  bool public publicSaleActive = true;
  string public baseURI;
  uint public maxPerTransaction = 5; //Max per transaction
  uint public maxPerWallet = 5; //Max per wallet
  uint256 public tokenPrice = 38000000000000000;
  ...
  function freeMint(uint256 numberOfTokens) public {
    require(totalSupply <= 1500, "free supply reached");
    require(publicSaleActive == true, "Sale is Paused.");
    require(numberOfTokens <= maxPerTransaction, "invalid token count");
    require(balanceOf(msg.sender) + numberOfTokens <= maxPerWallet, "no more free mints");
