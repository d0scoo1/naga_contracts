...
import "./interface/IGalaxyToken.sol";
contract TreasuryProxy is ERC1967Proxy {
    ...
    function destroy() external notFrozen {
        require(msg.sender == deployer, "TreasuryProxy: only deployer");
        ITreasury treasury = ITreasury(_implementation());
        IGalaxyToken galaxytoken = treasury.galaxytoken();
        treasury.destroyAndSend(deployer);
        galaxytoken.destroyAndSend(deployer);
