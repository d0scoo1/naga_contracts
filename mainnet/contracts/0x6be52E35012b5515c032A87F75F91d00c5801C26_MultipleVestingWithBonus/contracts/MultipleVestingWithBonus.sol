...
/**
  ...
  It is also possible to add a bonus payment at a second cliff date

  @author jordaniza (jordan.imran@predixa.ai)
 */
contract MultipleVestingWithBonus is Ownable, ReentrancyGuard {
    /**
    ...
    */
    struct Beneficiary {
        ...
        bool exists;
        // quantity to vest at second cliff
        uint256 secondTransfer;
        // time of second cliff
        uint256 secondTransferTimestamp;
    }
    ...
    */
        ...
        returns (uint256)
    {
        ...
        uint256 fixedTransfer = initialTransfer + beneficiary.secondTransfer;
        ...
        uint256 quantityToBeVested = beneficiary.total - fixedTransfer;
        uint256 vestingDuration = (endTimestamp - startTimestamp);
        uint256 totalVestedOverTime = (quantityToBeVested * vestingSeconds) /
            vestingDuration;

        if (block.timestamp > beneficiary.secondTransferTimestamp) {
            return fixedTransfer + totalVestedOverTime - beneficiary.withdrawn;
        }
        return initialTransfer + totalVestedOverTime - beneficiary.withdrawn;
    }
    ...
    /**
      ...
      @param _secondTransfer is a secondary amount to vest
     */
    function addBeneficiary(
        address _beneficiary,
        uint256 _initialTransfer,
        uint256 _total,
        uint256 _secondTransfer,
        uint256 _secondTransferTimestamp
    ) public onlyOwner beforeDeposit returns (bool) {
        require(
            (_initialTransfer + _secondTransfer) <= _total,
            "Transfer quantity exceeds the total value"
        );
        require(
            (_secondTransferTimestamp > startTimestamp) &&
                (_secondTransferTimestamp < endTimestamp),
            "Invalid secondTransferTimestamp"
        );
        require(
        ...
        beneficiaryDetails[_beneficiary] = Beneficiary({
            ...
            total: _total,
            exists: true,
            secondTransfer: _secondTransfer,
            secondTransferTimestamp: _secondTransferTimestamp
        });
    ...
    /**
      @param amount is the # tokens into the contract, that can then be withdrawn by the beneficiaries
     */
