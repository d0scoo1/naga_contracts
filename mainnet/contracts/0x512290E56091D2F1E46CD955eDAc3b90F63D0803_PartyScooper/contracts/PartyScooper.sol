...
contract PartyScooper is Ownable, FlashLoanReceiverBase {
    ...
    /// @notice The percentage of the max bid to request for flash loan
    /// @dev 952 = 95.2%
	uint256 public getMaxBidPercentage = 952;
        ...
        returns (bool)
    {
        ...
        console.log("min auction bid after flash bid", getMinAuctionBid(partyBid));
        ...
        console.log("min auction bid after party bid", getMinAuctionBid(partyBid));
    ...
    function raisePartyBid(address partyBid) external payable {
    ...
    function getFlashLoadBidAmount(address partyBid) internal view returns (uint256) {
        ...
        return Math.max((maximumPossiblePartyBid * getMaxBidPercentage) / 1000, getMinAuctionBid(partyBid)+1);
    }
    ...
    /// @notice Update the max bid percentage to get in a flash loan
	/// @param maxBidPercentage The new max bid percentage
	function setMaxBidPercentage(uint256 maxBidPercentage) public onlyOwner {
		getMaxBidPercentage = maxBidPercentage;
	}
