/**
 *To be submitted for verification on etherscan
 * Thisviolent
*/
...
// File: contracts/TheFallenAngels.sol

/**
 * @title TheFallenAngels contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract TheFallenAngels is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public MAX_ANGELS_SUPPLY;
    ...
    constructor() ERC721("The Fallen Angels", "ANGELS") {
        MAX_ANGELS_SUPPLY = 10000;
        REVEAL_TIMESTAMP = block.timestamp + (86400 * 7);
        mintPrice = 0; // 0.00 ETH
        maxToMint = 1;
        saleIsActive = false;
        wallet1 = 0x786EE4b323b6F11A33314f16BEcF8B37B025435e;
        wallet2 = 0x786EE4b323b6F11A33314f16BEcF8B37B025435e;
    }
    ...
    /**
     * Mint Angels by owner
     */
    function reserveAngels(address _to, uint256 _numberOfTokens) external onlyOwner {
    ...
    * Mints tokens
    */
    function mintAngels(uint256 numberOfTokens) external payable {
        require(saleIsActive, "Sale must be active to mint");
        require(numberOfTokens <= maxToMint, "Invalid amount to mint per once");
        require(totalSupply().add(numberOfTokens) <= MAX_ANGELS_SUPPLY, "Purchase would exceed max supply");
        require(mintPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        for(uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < MAX_ANGELS_SUPPLY) {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == MAX_ANGELS_SUPPLY || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() external {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_ANGELS_SUPPLY;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint256(blockhash(block.number - 1)) % MAX_ANGELS_SUPPLY;
        }
