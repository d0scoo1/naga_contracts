...
contract Marketplace {
    ...
    // Stores all active sell offers and maps them to their respective token ids
    mapping(uint256 => SellOffer) public activeSellOffers;

    // Stores all active buy offers and maps them to their respective token ids
    ...
    event SellOfferCancelled(uint256 tokenId, address seller);
    event BuyOfferCancelled(uint256 tokenId, address buyer);
    ...
    constructor(address _tokenContractAddress) {
        ...
        token = TiptoePunks(_tokenContractAddress);
    }
    ...
    modifier isApproved(uint256 tokenId) {
        ...
        require(token.isApprovedForAll(tokenOwner, address(this)) == true, "Not approved");
        _;
    }
    ...
    /// @notice Transfers royalties to the rights owner if applicable
    ...
    function _deduceRoyalties(uint256 tokenId, uint256 grossSaleValue) internal returns (uint256 netSaleAmount) {
        (address royaltiesReceiver, uint256 royaltiesAmount) = token.royaltyInfo(tokenId, grossSaleValue);
    ...
    function createSellOffer(uint256 tokenId, uint256 minPrice) external isApproved(tokenId) onlyTokenOwner(tokenId) {
        activeSellOffers[tokenId] = SellOffer({seller: msg.sender, minPrice: minPrice});

        emit NewSellOffer(tokenId, msg.sender, minPrice);
    }

    /// @notice Cancel a sell offer
    /// @param tokenId - id of the token whose sell order needs to be cancelled
    function cancelSellOffer(uint256 tokenId) external isApproved(tokenId) {
        ...
        emit SellOfferCancelled(tokenId, msg.sender);
    }

    /// @notice Purchases a token and transfers royalties if applicable
    /// @param tokenId - id of the token to sell
    function purchase(uint256 tokenId) external payable notTokenOwner(tokenId) {
        address seller = activeSellOffers[tokenId].seller;
        require(seller != address(0), "No active sell offer");

        // If for some reason the token is not approved anymore (transfers reset approvals),
        // we remove the sell order and revert
        address tokenOwner = token.ownerOf(tokenId);
        bool isMarketplaceApproved = token.isApprovedForAll(tokenOwner, address(this));
        if (isMarketplaceApproved == false) {
            delete (activeSellOffers[tokenId]);
            emit SellOfferCancelled(tokenId, seller);
    ...
    function makeBuyOffer(uint256 tokenId) external payable notTokenOwner(tokenId) {
        // Reject the offer if item is already available for purchase at a lower or identical price
        ...
        require(msg.value > activeBuyOffers[tokenId].price, "Previous buy offer higher or not expired");

        address previousBuyOfferOwner = activeBuyOffers[tokenId].buyer;
        uint256 refundAmount = buyOffersEscrow[previousBuyOfferOwner][tokenId];

        // Refund the owner of the previous buy offer
        buyOffersEscrow[previousBuyOfferOwner][tokenId] = 0;
        if (refundAmount > 0) {
            (bool success, ) = payable(previousBuyOfferOwner).call{value: refundAmount}("");
    ...
    /// @notice Cancels a buy offer and withdraws eth from escrow
    /// @param tokenId - id of the token whose buy order to remove
    function cancelBuyOffer(uint256 tokenId) external {
        require(activeBuyOffers[tokenId].buyer == msg.sender, "Not buyer");

        uint256 refundAmount = buyOffersEscrow[msg.sender][tokenId];
        ...
        emit BuyOfferCancelled(tokenId, msg.sender);
    }

    /// @notice Allows a token owner to accept the current buy offer (even without a sell offer)
    /// @param tokenId - id of the token whose buy order to accept
    function acceptBuyOffer(uint256 tokenId) external isApproved(tokenId) onlyTokenOwner(tokenId) {
        address buyer = activeBuyOffers[tokenId].buyer;
        require(buyer != address(0), "No buy offer");
        ...
        // Broadcast the sale
        emit Sale(tokenId, msg.sender, buyer, saleValue);
    }
}