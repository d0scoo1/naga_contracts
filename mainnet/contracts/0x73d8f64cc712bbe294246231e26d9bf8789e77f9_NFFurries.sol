...
contract NFFurries is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant FURRIES_SPECIAL = 6;
    uint256 public constant FURRIES_PUBLIC = 500;
    uint256 public constant FURRIES_MAX = FURRIES_SPECIAL + FURRIES_PUBLIC;
    uint256 public constant PURCHASE_LIMIT = 5;
    uint256 public constant PRICE = 140_000_000_000_000_000; // 0.14 ETH

    string private _contractURI = "";
    string private _tokenBaseURI = "";

    Counters.Counter private _specialNFFurries;
    Counters.Counter private _publicNFFurries;

    constructor() ERC721("Non-Fungible Furries", "NFF") {
        // _mintSpecials();
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicNFFurries.current() < FURRIES_PUBLIC, "Purchase would exceed FURRIES_PUBLIC");
        require(PRICE * numberOfTokens <= msg.value, "ETH amount is not sufficient");

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = FURRIES_SPECIAL + _publicNFFurries.current();

            if (_publicNFFurries.current() < FURRIES_PUBLIC) {
                _publicNFFurries.increment();
    ...
    function _mintSpecials() external onlyOwner {
        require(totalSupply() < FURRIES_SPECIAL, "All special tokens have been minted");

        for (uint256 i = 0; i < FURRIES_SPECIAL; i++) {
