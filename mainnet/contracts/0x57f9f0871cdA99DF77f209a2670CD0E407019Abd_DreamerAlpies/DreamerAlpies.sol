...
contract DreamerAlpies is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  /// @notice Libraries
  ...
  /// @dev states for whitelist claiming
  bytes32 public merkleRoot;
  uint256 public claimableAlpies;
  ...
  function initialize(
    ...
    claimableAlpies = _claimableAlpies;
  }
  ...
  function claimAlpies(
    ...
    require(MerkleProofUpgradeable.verify(_merkleProof, merkleRoot, _node), "invalid proof");
    ...
    claimableAlpies = claimableAlpies.sub(_amount);

    // Mark the _index as claim.
    _setClaimed(_index);
  ...
  function stopSale(bytes32 _freebieMerkleRoot, uint256 _revealBlock) external onlyOwner {
    require(totalSupply() < maxAlpies(), "sale done");
  ...
  function maximumPurchasable(address _buyer) public view returns (uint256) {
    ...
    // 1.1 Per window
    // 1.2 Per address
    // 1.3 maxAlpies - claimableAplies - totalSupply
    uint256 _supplyLeft = maxAlpies().sub(claimableAlpies).sub(totalSupply());
