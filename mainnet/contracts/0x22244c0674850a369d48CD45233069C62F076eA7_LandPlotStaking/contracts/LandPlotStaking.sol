...
contract LandPlotStaking is Ownable, ERC1155Receiver {
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < collectionIds.length; i++) {
            if (amounts[i] != 0) {
                rewardTokens[collectionIds[i]].transfer(account, rewardAmounts[i]);
                rewardTokens[collectionIds[i]].transfer(royaltyClaimer, royaltyAmounts[i]);
    ...
    function withdraw(uint256[] memory collectionIds) public {
        ...
        for (uint256 i = 0; i < collectionIds.length; i++) {
            if (amounts[collectionIds[i]] != 0) {
                rewardTokens[collectionIds[i]].transfer(msg.sender, rewardAmounts[i]);
                rewardTokens[collectionIds[i]].transfer(royaltyClaimer, royaltyAmounts[i]);
