...
// File: contracts/BlindBunnies.sol
    ...
    ReentrancyGuard
{
    ...
    uint256 public maxSupply = 3737;
    uint256 public giftSupply = 37;
    ...
    uint256[] private _teamShares = [91 , 9];
    address[] private _team = [
        0xE13fFE5e4Cd3A5D3EA23a8df25DDfc0438ced5f9,
        0x11bEff531De9b5F781553B691629bE98B36d2b8F
    ];

    constructor()
        ERC721("Blind Bunnies NFT", "BB")
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        require(
            ...
            "Blind Bunnies NFTy: max total supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Blind Bunnies NFT: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function presaleMint(uint256 _amount) external payable nonReentrant{
        require(presaleM, "Blind Bunnies NFT: Presale is OFF");
        require(!paused, "Blind Bunnies NFT: contract is paused");
        require(
            _amount <= presaleAmountLimit,
            "Blind Bunnies NFT: You can't mint so much tokens"
        );
        require(
        ...
        require(
            ...
            "Blind Bunnies NFT: max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "Blind Bunnies NFT: Ether value sent is not correct"
        );
        uint256 _newItemId;
    ...
    function publicMint(uint256 _amount) external payable nonReentrant{
        require(publicM, "Blind Bunnies NFT: Public is OFF");
        require(_amount > 0, "Blind Bunnies NFT: zero amount");
        require(
            totalMint + _amount <= maxSupply - giftSupply,
            "Blind Bunnies NFT: max supply exceeded"
        );
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Blind Bunnies NFT: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Blind Bunnies NFT: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
