...
contract METALYNX is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public constant uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);

    uint256 public autoLiquidityFee = 1;
    ...
    constructor() public ERC20("METALYNX", "METALYNX") {

        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());

        totalFees = devFee.add(autoLiquidityFee);
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(
            ...
            uint256 amtToSwap = swapAtAmount.mul(devFee).div(totalFees);
            _swapTokensForETH(amtToSwap);
            payable(owner()).transfer(address(this).balance);

            amtToSwap = (swapAtAmount.sub(amtToSwap)).div(2);
            _swapTokensForETH(amtToSwap);
            _addAutoLiquidity(amtToSwap, address(this).balance);
    ...
    function _addAutoLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
