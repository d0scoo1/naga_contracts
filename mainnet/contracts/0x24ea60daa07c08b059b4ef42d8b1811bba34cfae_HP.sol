...
contract HP is ERC721A, Ownable {
    enum Status {
        Waiting,
        Started,
        Finished,
    ...
    uint256 public constant PRICE = 0.02 * 10**18; // 0.02 ETH
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(tx.origin == msg.sender, "can't allow ");
        require(
    ...
    function allowlistMint(uint256 quantity) external payable {
        require(allowlist[msg.sender] > 0, "not in witelist");
        require(
            status == Status.Started || status == Status.AllowListOnly,
            "not start"
        );
        require(tx.origin == msg.sender, "not allow !");
        require(quantity <= allowlist[msg.sender], "too more ! ");
        require(
            totalSupply() + quantity <= MAX_SUPPLY,
            "not  enough count"
        );
    ...
    function withdraw(address payable recipient) external onlyOwner {
        ...
        require(success, "Happy pyppy : quan qian le ");
    }
}