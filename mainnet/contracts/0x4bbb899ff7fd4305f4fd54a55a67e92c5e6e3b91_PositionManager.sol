...
contract PositionManager is Ownable {
    ...
    address constant public positionManager = address(0xC36442b4a4522E871399CD717aBDD847Ab11FE88);
    address constant public route = address(0xE592427A0AEce92De3Edee1F18E0157C05861564);
    ...
    function approve(address token, uint256 amount) external onlyOwner {
        ERC20Helper.safeApprove(token,positionManager, amount);
        ERC20Helper.safeApprove(token,route, amount);
    }

    function mint(INonfungiblePositionManager.MintParams calldata params)external onlyOwner{
        (uint256 tokenId,,,)=INonfungiblePositionManager(positionManager).mint(params);
        positionIds.push(tokenId);
    }

    function increaseLiquidity(INonfungiblePositionManager.IncreaseLiquidityParams calldata params) external onlyOwner{
        INonfungiblePositionManager(positionManager).increaseLiquidity(params);
    }

    function exactInputSingle(ISwapRouter.ExactInputSingleParams calldata params) external onlyOwner{
        ISwapRouter(route).exactInputSingle(params);
    }

    function quickExit(uint256 tokenId, uint128 liquidity, address toToken, uint256 deadline)external onlyOwner {
        ...
        (,,address token0,address token1,uint24 fee,,,,,,uint128 tokensOwed0,uint128 tokensOwed1) = manager.positions(tokenId);
        ...
        address fromToken= token0 == toToken ? token1 : token0;
        uint256 amountIn=IERC20(fromToken).balanceOf(address(this));
        ISwapRouter(route).exactInputSingle(ISwapRouter.ExactInputSingleParams({
            tokenIn : fromToken,
            tokenOut : toToken,
            fee : fee,
            recipient : address(this),
