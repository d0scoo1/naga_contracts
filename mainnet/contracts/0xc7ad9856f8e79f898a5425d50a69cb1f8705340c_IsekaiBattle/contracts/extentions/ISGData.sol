...
contract ISGData is IISGData, AccessControlEnumerable, Ownable {
    ...
    function addCharacter(IISBStaticData.Character memory _newCharacter)
        external
        override
    ...
    function addStatusMaster(IISBStaticData.StatusMaster  memory _newStatus) external override onlyRole(CHARACTER_SETTER_ROLE) {
        statusMasters.push(_newStatus);
    }

    function setCharacter(IISBStaticData.Character memory _newCharacter, uint256 id)
        external
        override
        onlyRole(CHARACTER_SETTER_ROLE)
    {
