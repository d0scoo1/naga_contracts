...
contract HypexlaBiennale2022 is ERC1155, Ownable, EIP712 {
    using SafeERC20 for IERC20;
    
    mapping (address => bool) public allowedTradingErc20Tokens;
    
    constructor (string memory _uri, address tradingToken,
    string memory eip712Name, string memory eip712Version) ERC1155(_uri) EIP712(eip712Name, eip712Version) {
        
        require(tradingToken != address(0) , "Hype x la Biennale 2022: Trading Token zero address");
        
        allowedTradingErc20Tokens[tradingToken] = true;
    ...
    function _verifySig(ForwardData memory data, bytes memory sig) internal view {
        ...
        require(ECDSA.recover(digest, sig) == data.from, "Hype x la Biennale 2022: signature mismatch");
    }
        ...
        external 
    {
        ...
        require(allowedTradingErc20Tokens[tradingTokenAddress], "Hype x la Biennale 2022: trading token not allowed");
        ...
        require(msg.sender == data.to, "Hype x la Biennale 2022: not allowed to sell");
        require(block.timestamp <= data.deadline, "Hype x la Biennale 2022: signature is out-of-date");
        ...
        external 
    {
        ...
        require(data.to != address(0));
        require(msg.sender == data.to, "Hype x la Biennale 2022: not allowed to buy");
        require(block.timestamp <= data.deadline, "Hype x la Biennale 2022: signature is out-of-date");
