...
contract RoyaltiesDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    Royalties public immutable token;
    ...
    */
    ...
    constructor(address payable owner, address pair, address weth) DividendPayingToken("RoyaltiesDividendTracker", "$RLTS_DIVS") {
        token = Royalties(owner);
    ...
    function _transfer(address, address, uint256) internal override {
        silenceWarning = true;
        require(false, "RoyaltiesDividendTracker: No transfers allowed");
    }
    ...
    function updateVestingDuration(uint256 newVestingDuration) public onlyOwner {
        require(newVestingDuration <= 2592000, "RoyaltiesDividendTracker: max vesting duration is 30 days");
    ...
    function updateUnvestedDividendsMarketingFee(uint256 newUnvestedDividendsMarketingFee) public onlyOwner {
        require(newUnvestedDividendsMarketingFee <= 30, "RoyaltiesDividendTracker: max marketing fee is 30%");
