...
contract Munks is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public constant reserveMunks = 50;

    uint256 public constant _tokenPrice = 33300000000000000;

    // SHA256 of concatenated hash string containing all the hashes of all tokens
    string public FinalProofHash;

    // IPFS hash of a json containing the metadata of all the tokens, will be uploaded when all are sold
    ...
    constructor() ERC721("CryptoMunks", "MUNK") {}
    ...
    function buyTokens(uint256 numberOfTokens) public payable {
      require(saleIsActive, "Sale must be active to mint Munks");
      require(numberOfTokens > 0 && numberOfTokens <= maxPurchase, "Can only mint 20 tokens at a time");
      require((totalSupply() + numberOfTokens) <= MAX_NFT_SUPPLY, "Purchase would exceed max supply of Munks");
    ...
    /* receive 50 RANDOM Munks for the team and marketing purposes, then start the sale
    this function will be called 2 times due to the gas limit (25 in each transaction) */
    function getReserves() public onlyOwner {
        require(saleIsActive == false, "CryptoMunks: Reserves taken and sale started already!");
        for (uint256 x = 0; x < 25; x++) {
            uint256 randHash = _getRandomId();
            _safeMint(msg.sender, randHash);
        }
        if (totalSupply() == reserveMunks) {
            saleIsActive = true;
        }
    }
}