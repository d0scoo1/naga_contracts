...
contract NoLand is ERC721A, Ownable {
	using Strings for uint;

	uint public constant MINT_PRICE = 0.004 ether;
	uint public constant MAX_NFT_PER_TRAN = 20;
	uint public constant MAX_FREE = 1;
    uint public constant TOTAL_FREE = 5000;
	address private immutable SPLITTER_ADDRESS;
	uint public maxSupply = 10000;

	bool public isPaused;
    bool public isMetadataFinal;
    string private _baseURL = "ipfs://Qmdmw979A9KVTLzCwHP37Jaeb7qgoMuKHYz9CASaUuGfe8/";
	...
	function mint(uint count) external payable {
		require(!isPaused, 'Sales are off');
         ...
         bool isFree = ((totalSupply() + count < TOTAL_FREE + 1) &&
            (_walletMintedCount[msg.sender] < MAX_FREE));

        uint256 cost = MINT_PRICE;
        if (isFree) {
            require(msg.value >= (count * cost) - cost, "INVALID_ETH");
            require(totalSupply() + count <= maxSupply, "No more");
            require(count <= MAX_NFT_PER_TRAN, "Max per TX reached.");
