...
contract Rocket is ERC721A, Ownable {
  ...
  function setPrivateLimit(address[] calldata addresses, uint256[] calldata num) external onlyOwner {
    if (addresses.length != num.length) revert ArrayLengthMismatch();
  ...
  function mint(uint256 num) external payable {
    if (tx.origin != msg.sender) revert CalledFromContract();
    if (msg.value != mintPrice * num) revert WrongPaymentAmount();
    if (totalSupply() + num > maxSupply) revert MaxSupplyReached();
    ...
    if (num > remaining) revert MintLimitExceeded();
