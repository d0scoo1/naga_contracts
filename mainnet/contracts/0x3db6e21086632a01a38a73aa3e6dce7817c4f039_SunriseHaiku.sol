...
// File contracts/SunriseHaiku.sol

// @Title Sunrise Haiku NFT contract
// @Author Slurperino (@slurperino)

pragma solidity ^0.8.0;


contract SunriseHaiku is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function preMint() public payable {
    ...
    preMintWhitelist[msg.sender] = false;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(!paused, "Error: Mint is currently paused.");
    require(tokenCount + _mintAmount <= maxMintAmount, "Error: You can mint a maximum of 3 Sunrise Haikus.");
    require(supply + _mintAmount <= maxSupply, "Error: not enough Sunrise Haikus available to mint.");

    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Error: the Ether sent is less than the required amount.");
    }
  ...
  function isPreMintAllowed(address account) public view returns (bool) {
    return preMintWhitelist[account];
  }
  
  function whitelistAccount(address account) public onlyOwner {
    preMintWhitelist[account] = true;
  }
 
  function removeWhitelistAccount(address account) public onlyOwner {
    preMintWhitelist[account] = false;
  }

  function batchWhitelistAccounts(address[] calldata _addresses) external onlyOwner {
    for(uint256 i; i < _addresses.length; i++){
      preMintWhitelist[_addresses[i]] = true;
    }
  }
  ...
  function withdraw() public payable onlyOwner {
    require(payable(payment_splitter).send(address(this).balance));
  }
}