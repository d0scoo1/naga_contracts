...
contract Blingy is ERC20Burnable, Ownable {
	address public bitchAddress;
	address nullAddress = address(0);
	uint256 public dailyReward1 = 10 * 10 ** 18; // 10 BLINGYs per day
	uint256 public dailyReward2 = 8 * 10 ** 18; // 8 BLINGYs per day
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
            	IERC721Enumerable(bitchAddress).ownerOf(tokenIds[i]) == msg.sender && tokenIdToStaker[tokenIds[i]] == nullAddress,
            	"Token must be stakable by you!"
            );
            IERC721Enumerable(bitchAddress).transferFrom(msg.sender, address(this), tokenIds[i]);
            stakerToTokenIds[msg.sender].push(tokenIds[i]);
            tokenIdToTimeStamp[tokenIds[i]] = block.timestamp;
            tokenIdToStaker[tokenIds[i]] = msg.sender;
        }
    }
    function _getEmissionRate(uint256 tokenId) internal view returns(uint256) {
    ...
    function unstakeAll() public {
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            IERC721Enumerable(bitchAddress).transferFrom(address(this), msg.sender, tokenId);
    ...
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            IERC721Enumerable(bitchAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
