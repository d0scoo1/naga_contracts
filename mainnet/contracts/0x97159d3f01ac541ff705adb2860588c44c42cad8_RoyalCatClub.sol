...
contract RoyalCatClub is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress;

    uint256 public ROYCC_MAX = 10100;
    uint256 public PRESALE_PRICE = 0.0444 ether;
    uint256 public PUBLIC_PRICE = 0.0444 ether;
    uint256 private REVEAL_DELAY = 0 hours;
    uint256 private PRESALE_HOURS = 0 hours;
    uint256 public PURCHASE_LIMIT = 20;
    uint256 public PRESALE_MINT_LIMIT = ROYCC_MAX;
    uint256 public PUBLIC_MINT_LIMIT = ROYCC_MAX;
    ...
    Counters.Counter private _publicROYCC;

    constructor() ERC721("Royal Cat Club", "ROYCC") {}
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicROYCC.current();

            if (_publicROYCC.current() < ROYCC_MAX) {
                _publicROYCC.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicROYCC.current() < ROYCC_MAX,"Purchase would exceed ROYCC_MAX");
