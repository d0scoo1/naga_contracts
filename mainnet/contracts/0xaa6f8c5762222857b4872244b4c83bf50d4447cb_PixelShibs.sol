...
contract PixelShibs is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x8B71093a446881a96cEe3a682d83C1c6A58a7034);
    address payable private _DevAddress;

    uint256 public PIXELSHIB_MAX = 10000;
    uint256 public PRESALE_PRICE = 48_000_000_000_000_000; // 0.048 ETH
    uint256 public PUBLIC_PRICE = 60_000_000_000_000_000; // 0.06 ETH
    ...
    uint256 private _activeDateTime = 1639612800; // Date and time (GMT): Thursday, December 16, 2021 12:00:00 AM
    ...
    Counters.Counter private _publicPIXELSHIB;

    constructor() ERC721("PixelShibs", "PIXELSHIB") {
    ...
    function airdrop(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicPIXELSHIB.current();

            if (_publicPIXELSHIB.current() < PIXELSHIB_MAX) {
                _publicPIXELSHIB.increment();
                _safeMint(to, tokenId);
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicPIXELSHIB.current() < PIXELSHIB_MAX,"Purchase would exceed PIXELSHIB_MAX");
