...
contract BullishBosses is ERC721, Ownable {
  using Strings for uint256;
  ...
  constructor() ERC721("Bullish Bosses", "Boss") {
    
    _mintLoop(msg.sender,60);
  ...
  function mint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if(presaleActive==true){
      if(totalSupply()>200){
        ...
        require(balanceOf(msg.sender) + _mintAmount <= maxFreeMintPerWallet, "Mint limit exceeded." );
      }
