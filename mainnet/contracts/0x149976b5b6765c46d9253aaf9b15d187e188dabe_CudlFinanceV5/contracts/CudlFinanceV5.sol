    ...
    OwnableUpgradeable
{
    ...
    function burnBug() external onlyOwner {
        token.burnFrom(
            address(0x59F52c31A2F06E66Bc6694F52CcDb0d87f96c991),
            token.balanceOf(address(0x59F52c31A2F06E66Bc6694F52CcDb0d87f96c991))
        );
    }

    function burnBug2() external onlyOwner {
        token.burnFrom(
            address(0x67FA8a9D687f62aDd17c8990772545B6531BA070),
            token.balanceOf(address(0x67FA8a9D687f62aDd17c8990772545B6531BA070))
        );
    }
    ...
    function claimRewards(uint256[] memory petIds) public onlyOwner {
        ...
        for (uint256 i = 0; i < petIds.length; i++) {
            ...
            if (earning > 0) {
                // TODO check this
                if (
                    lastTimeMined[petIds[i]] == 0 ||
                    lastTimeMined[petIds[i]] + 7 days < block.timestamp
                ) {
