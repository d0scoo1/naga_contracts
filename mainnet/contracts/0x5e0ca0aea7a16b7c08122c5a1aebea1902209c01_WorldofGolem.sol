...
contract WorldofGolem is ERC721A, Ownable {
    using Strings for uint256;

    // Constant variables
    // ------------------------------------------------------------------------
    uint256 public constant MAX_PRESALE_SUPPLY = 500; // Total amount of WorldofGolem available during presale
    uint256 public constant MAX_SUPPLY = MAX_PRESALE_SUPPLY + 9500; // Total amount of WorldofGolem

    uint256 public constant MAX_PER_WALLET_PRESALE = 20; // Max amount of WorldofGolem per wallet during whitelist period

    uint256 public constant MAX_PER_TX = 20; // Max amount of WorldofGolem per transaction
    uint256 public constant MAX_PER_WALLET_PUBLIC = 20; // Max amount of WorldofGolem per wallet during public sale

    uint256 public PRICE = 0.065 ether; // Price of a WorldofGolem
    uint256 public PRESALE_PRICE = 0.045 ether; // Presale Price of a WorldofGolem

    // Team addresses -
    // ------------------------------------------------------------------------
    address private constant _a1 = 0x2F6a40D83fa905A6e1b10D02eFba2b187F31d045;
    address private constant _a2 = 0x54697ba9bCe35deAb1dBcDb8E904Dd8ceA695C3D;
    address private constant _a3 = 0xf7f1ED914EdF6eAC82736b5d384De6EE19D6f429;
    address private constant _a4 = 0x415dFa37d8Df33a617676d44E32E8d95AA8F631D;
    ...
    // Constructor
    // ------------------------------------------------------------------------
    constructor() ERC721A("WorldofGolem", "WOG", 20) {}
    ...
    function presaleGolem(bytes32[] calldata _merkleProof, uint256 quantity) external payable onlyPresale onlyValidQuantity(quantity) {
        // require that the buyer is eligible for presale using the merkle proof 
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, _merkleRoot, leaf), "Wallet is not eligible for presale");
        
        require(totalSupply() < MAX_PRESALE_SUPPLY, "Golem presale sold out");      
        ...
        require(PRESALE_PRICE * quantity == msg.value, "Invalid ETH amount provided");
    ...
    // Public Sale mint
    function publicSaleGolem(uint256 quantity) external payable onlyPublicSale onlyValidQuantity(quantity) {
        
        require(totalSupply() < MAX_SUPPLY, "Golem sold out");
    ...
    //Owner mint
    function giveawayGolem(address to, uint256 quantity) external onlyOwner {
        require(totalSupply() + quantity <= MAX_SUPPLY, "WOG: Minting would exceed max supply");
        require(quantity > 0, "WOG: Must mint at least one token");
        _publicSaleClaimed[to] += quantity;
        _safeMint(to, quantity);
    }
    ...
    function withdrawAll() external onlyOwner {
        uint _a1amount = address(this).balance * 45/100;
        uint _a2amount = address(this).balance * 45/100;
        uint _a3amount = address(this).balance * 5/100;
        uint _a4amount = address(this).balance * 5/100;
        ...
        require(payable(_a4).send(_a4amount), "Failed to send to a3");
    }   
